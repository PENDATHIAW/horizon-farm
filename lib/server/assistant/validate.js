...
const buildDecisionTrace = ({ intent, draft, fields, userId }) => ({ module: 'tracabilite', action: 'create_business_event', table: 'business_events', payload: { id: makeId('DTRACE'), event_type: 'decision_trace', source: 'centre_decisionnel', source_module: 'centre_ia', module_source: 'centre_ia', intent, decision_intent: intent, title: `Trace décisionnelle - ${fields.title || fields.name || intent}`, description: fields.description || fields.notes || draft?.ui?.subtitle || 'Validation demandée depuis le Centre décisionnel.', actor_id: userId || null, occurred_at: now(), event_date: today(), status: 'validation_requested', recommendation_id: fields.recommendation_id || fields.source_recommendation_id || draft?.recommendation_id || null, decision_key: fields.decision_key || draft?.decision_key || null, target_module: fields.target_module || fields.module || draft?.target_module || null, target_id: fields.target_id || fields.source_id || draft?.target_id || null, metadata: { intent, draft_type: draft?.type || draft?.intent || null, fields, impacted_modules: draft?.impacted_modules || [], ui: draft?.ui || null } } });
...
function buildActionsForDraft(draft = {}, userId = null) {
  ...
  const actions = [buildDecisionTrace({ intent, draft, fields, userId })];
  if (intent === 'business_plan') return [...actions, ...businessPlanActions(fields, userId)];
...
export default async function handler(req, res) {
  ...
  if (execute && !farmId) return json(res, 422, { ok: false, status: 'farm_required', message: 'Choisissez une ferme active avant l’enregistrement.' });
  const execution = execute ? await executeAssistantActions(req, actions, { userId, intent, farmId }) : { executed: false, mode: 'dry_run', results: actions.map((action) => ({ ...action, executed: false, status: 'dry_run' })) };
