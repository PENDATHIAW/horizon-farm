...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!alertId) return json(res, 400, { ok: false, error: 'L’alerte est manquante.' });

    const supabase = await getSupabaseAdmin();
    if (!supabase) return json(res, 503, { ok: false, dispatched: false, error: 'Le service de notifications n’est pas disponible.' });
    ...
    if (alertError) return json(res, 502, { ok: false, dispatched: false, error: 'L’alerte n’a pas pu être lue.' });
    if (!alertRow) return json(res, 404, { ok: false, dispatched: false, error: 'L’alerte est introuvable.' });
    ...
    const now = new Date();
    const retryBeforeIso = new Date(now.getTime() - 10 * 60 * 1000).toISOString();
    ...
    const { data: claimed, error: claimErr } = await supabase
      ...
      .or(`push_status.is.null,push_status.neq.sending,last_push_attempt_at.lt.${retryBeforeIso}`)
      .select('*')
      .maybeSingle();

    if (claimErr) return json(res, 502, { ok: false, dispatched: false, error: 'L’envoi n’a pas pu être préparé.' });
    ...
    if (sendResult?.simulated || (sendResult?.sent || 0) <= 0) {
      ...
      return json(res, sendResult?.simulated ? 503 : 502, {
        ok: false,
        dispatched: false,
        error: sendResult?.simulated
    ...
    return json(res, 500, { ok: false, dispatched: false, error: 'La notification n’a pas pu être envoyée.' });
  }
}
