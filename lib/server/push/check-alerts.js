...
async function claimAndSend({ supabase, alert, nowIso, now }) {
  ...
  const { data: claimed } = await supabase
    .from('alertes_center')
    .update({ push_status: 'sending', last_push_attempt_at: nowIso })
    .eq('id', alert.id)
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!supabase) return json(res, 503, { ok: false, sent: 0, error: 'Le service de notifications n’est pas disponible.' });
    ...
    if (error || !Array.isArray(data)) return json(res, 502, { ok: false, sent: 0, error: 'Les alertes n’ont pas pu être lues.' });
    ...
    if (failed > 0) {
      return json(res, 502, {
        ...
        error: 'Certaines notifications n’ont pas pu être envoyées.',
      });
    }
    return json(res, 200, { ok: true, sent, failed, total_checked: candidates.length });
  } catch (error) {
    console.error('check-alerts', error);
    return json(res, 500, { ok: false, sent: 0, error: 'Les alertes n’ont pas pu être envoyées.' });
  }
}
