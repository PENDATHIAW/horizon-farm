...
test('Cultures — 7 onglets cible dans horizonVision', () => {
  ...
  assert.deepEqual(tabs, ['Parcelles', 'Campagnes', 'Irrigation', 'Intrants & fertilisation', 'Récoltes', 'Coûts & marge', 'Historique']);
  assert.ok(MODULE_AUDIT_ORDER.includes('cultures'));
});

test('Cultures — navigation historique mappée', () => {
  assert.equal(resolveCulturesTab('Résumé'), 'Parcelles cultures');
