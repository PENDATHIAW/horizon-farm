...
| 2 | Investissements affiche deux boutons “Synchroniser BP”. | Corrigé partiellement / à vérifier visuellement | InvestissementsV8 recentré sur concrétisation, ancien portefeuille avancé remplacé par BaseInvestissements. À vérifier en UI pour confirmer qu’aucun doublon résiduel n’apparaît. |
...
| 9 | Accueil : fusionner “Pilotage ferme / L’essentiel à suivre” et “Objectif du mois / Situation actuelle”. | Corrigé | `DashboardV2` affiche un seul bloc “Pilotage ferme · objectif du mois”. |
...
| 14 | Impact & Valeur ERP : onglet Dossier Banque/Partenaire à relier au générateur. | Corrigé | `ImpactBusiness.jsx` réutilise `FinancingDossierGenerator`. |
...
| 16 | Équipements contient RH, équipements, dépenses, preuves : garder seulement équipements. | Corrigé | `EquipementsV2` recentré sur `Equipements`. À vérifier visuellement. |
| 17 | Ventes : BP/objectifs trop en haut ; créer/modifier/suivre vente doit être en haut. | Corrigé | Ventes routé vers `VentesV5` puis `VentesTerrainV2`. |
...
| 19 | Interconnexion vente → finances/comptabilité doit être automatique. | Corrigé côté flux vente terrain | `VentesTerrainV2` crée paiement, transaction finance, facture, document, impact source. Comptabilité reste à renforcer côté écritures formelles si modèle comptable détaillé requis. |
...
| 21 | Investissements : concrétiser un investissement doit exister. | Corrigé | `InvestissementsV8` ajoute “Concrétiser un investissement” : création lot/animal/culture depuis dépense BP effective. |
...
| 28 | Revoir structure de chaque module pour mettre le cœur du module en haut. | En cours / plusieurs modules corrigés | Ventes, Finances, Comptabilité, Accueil, Rapports, Impact, Investissements, Tâches/Alertes corrigés. Reste une passe visuelle globale. |
...
2. Vérifier visuellement Investissements pour confirmer absence totale de doublon “Synchroniser BP”.
3. Vérifier visuellement Équipements pour confirmer absence RH/dépenses/preuves.
4. Vérifier que `VentesTerrainV2` compile correctement avec tous les callbacks transmis.
