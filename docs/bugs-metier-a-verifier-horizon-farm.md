...
## 2. Avicole — audit complet des lots, effectifs, statuts et ventes

**Statut : [À revoir] critique**

### Observation utilisateur confirmée

Dans le module Avicole, des incohérences sont visibles :

```txt
- un lot avec effectif affiché à 0 reste visible dans la liste des lots actifs ;
- la fiche de modification calcule un effectif actuel différent du tableau ;
- exemple observé : initial 500, morts 18, volés 2, vendus 419, réformés 1, autres sorties 1, malades 3 ;
- la fiche calcule 59 mais le tableau affiche 0 ;
- statut affiché : vendu_partiellement alors que la décision vente indique Non prêt ;
- le module ne distingue pas assez clairement actif, vendu partiellement, vendu totalement, clôturé, à réformer.
```

### Règle métier attendue

Les malades restent présents dans le lot. Ils ne doivent pas être soustraits de l’effectif actuel sauf s’ils sont aussi morts, vendus, réformés, volés ou sortis.

Formule recommandée :

```txt
effectif_actuel_calcule = max(
  0,
  effectif_initial
  - morts
  - voles
...
Le champ `malades` doit rester un indicateur sanitaire :

```txt
morts / malades = 18 / 3
```

Mais `malades` ne doit pas diminuer l’effectif actuel.

### Cause probable à vérifier dans le code

Dans `AvicoleBase.jsx`, `activeCount(lot)` peut partir de `current_count` ou `effectif_actuel` si ce champ existe déjà, puis soustraire à nouveau morts/vendus/réformes/sorties.

Risque : si `current_count` est déjà un champ calculé, le tableau fait une double soustraction.

Exemple de problème :

```txt
current_count déjà calculé = 59
puis activeCount soustrait encore morts/vendus/réformes/sorties
→ résultat incorrect, parfois 0
```
...
```txt
1. champs sources saisis : initial_count, mortality, voles, vendus, reformes, sorties/autres_sorties ;
2. champ calculé affiché : effectif_actuel_calcule ;
3. champ sanitaire : malades ;
4. statut métier : actif / vendu_partiellement / vendu_totalement / cloture / a_reformer.
```

Règle technique :

```txt
Ne jamais recalculer un effectif actuel en repartant d’un effectif déjà calculé.
Toujours recalculer depuis l’effectif initial et les sorties source.
```

### Lots à 0 : affichage attendu

Si `effectif_actuel_calcule <= 0`, le lot ne doit pas rester affiché comme lot actif exploitable.

À décider et appliquer :

```txt
- soit le lot est masqué de la vue active ;
- soit il reste visible uniquement dans une vue Historique / Clôturés ;
- soit il est affiché avec statut vendu_totalement ou cloture ;
- il ne doit pas être proposé dans les workflows santé, vente, alimentation ou production.
```

### Décision vente attendue

La décision vente doit tenir compte de l’effectif actuel et du statut :

```txt
Si effectif_actuel_calcule <= 0
→ décision vente = Indisponible / Lot clôturé
→ aucune opportunité active
→ aucune source vente proposée.

Si vendu_partiellement et effectif_actuel_calcule > 0
→ décision vente peut rester Non prêt / Prêt / Réforme possible selon type, âge, poids, phase.

Si vendu_totalement ou cloture
→ décision vente = Indisponible.
```

### À vérifier / corriger

- [ ] même formule d’effectif dans le tableau, la fiche détail, la fiche modification, les KPI et les exports ;
- [ ] ne pas mélanger `current_count`, `effectif_actuel`, `initial_count`, `effectif_initial` sans normalisation ;
- [ ] ne pas soustraire deux fois morts/vendus/réformes/sorties ;
- [ ] ajouter `voles` dans la formule si le champ existe ;
- [ ] clarifier `sorties`, `autres_sorties`, `reformes`, `vendus` pour éviter les doublons de catégories ;
- [ ] `malades` reste dans l’effectif, sauf si aussi déclaré comme sortie réelle ;
- [ ] lot à 0 exclu des workflows actifs : vente, santé, alimentation, ponte, biosécurité ;
- [ ] lot à 0 visible seulement dans historique/clôturé ;
- [ ] si un lot est vendu totalement, fermer l’opportunité de vente liée ;
- [ ] si un lot est vendu partiellement, ne proposer en vente que le solde disponible ;
- [ ] mettre à jour les KPI Avicole avec la même formule que le tableau ;
- [ ] ajouter un test e2e : modification effectif/sorties → sauvegarde → retour fiche → tableau et fiche affichent le même effectif ;
- [ ] ajouter un test e2e : lot à 0 → absent des workflows actifs ;
- [ ] ajouter un test e2e : lot vendu partiellement → solde disponible cohérent dans Ventes.

### Workflow attendu

```txt
Modification lot avicole
→ recalcul unique depuis les champs sources
→ sauvegarde
→ refresh
→ tableau, fiche, KPI et exports affichent le même effectif
→ lot à 0 bascule hors vue active
→ workflows liés utilisent uniquement les lots avec effectif > 0
→ trace/audit si changement important
```

---

## 3. Animaux / Avicole — `prêt à la vente` ne persiste pas
...
## 4. Animaux / Avicole → opportunité de vente non visible
...
## 5. Ventes — workflow d’encaissement encore visible alors que la commande est soldée
