...
const buildActionsForDraft = (draft = {}, userId = null) => {
  ...
  if (intent === 'purchase_stock') {
    actions.push({ module: 'stock', action: 'create_stock_entry', table: 'stock', payload: { produit: fields.product_name, categorie: fields.category || 'aliment', quantite: Number(fields.quantity || 0), unite: fields.unit || 'unité', poids_unitaire_kg: fields.unit_weight_kg || null, poids_total_kg: fields.total_weight_kg || null, fournisseur_id: fields.supplier_id || null, fournisseur_nom: fields.supplier_name || null, type_mouvement: 'entree', source: 'horizon_assistant', date: fields.date || now().slice(0, 10), notes: fields.notes || '' } });
    if (fields.payment_status && fields.payment_status !== 'unknown') actions.push({ module: 'finances', action: fields.payment_status === 'paid' ? 'create_paid_expense' : 'create_supplier_payable', table: 'finances', payload: { type: 'sortie', categorie: 'achat_stock', libelle: `Achat ${fields.product_name || 'stock'} via Horizon`, montant: fields.payment_amount || null, statut: fields.payment_status === 'paid' ? 'paye' : fields.payment_status, fournisseur_id: fields.supplier_id || null, date: fields.date || now().slice(0, 10), source: 'horizon_assistant' } });
    if (!fields.supplier_id && fields.supplier_name) actions.push({ module: 'fournisseurs', action: 'prepare_supplier_creation', table: 'fournisseurs', requires_additional_validation: true, payload: { nom: fields.supplier_name, type: 'aliment', source: 'horizon_assistant' } });
  }

  if (intent === 'sale_record') {
    actions.push({ module: 'ventes', action: 'create_sales_order', table: 'sales_orders', payload: { client_id: fields.client_id || null, client_nom: fields.client_name || null, product_name: fields.product_name, quantity: Number(fields.quantity || 0), unit_price: fields.unit_price || null, total_amount: fields.total_amount || null, payment_status: fields.payment_status || 'unknown', date: fields.date || now().slice(0, 10), source: 'horizon_assistant' } });
    actions.push({ module: 'stock', action: 'prepare_stock_exit', table: 'stock', payload: { produit: fields.product_name, quantite: Number(fields.quantity || 0), type_mouvement: 'sortie', source: 'horizon_assistant' } });
  }
  ...
  if (intent === 'poultry_lot_creation') actions.push({ module: 'avicole', action: 'create_poultry_lot', table: 'avicole', payload: { type: fields.type, name: fields.name || fields.nom || null, initial_count: Number(fields.initial_count || fields.quantity || 0), age_weeks: fields.age_weeks || null, date_entree: fields.date || now().slice(0, 10), statut: 'actif', source: 'horizon_assistant' } });
  ...
  if (intent === 'finance_entry') actions.push({ module: 'finances', action: 'create_finance_transaction', table: 'finances', payload: { type: fields.transaction_type, montant: Number(fields.amount || 0), categorie: fields.category || 'non_categorise', libelle: fields.label || fields.notes || 'Transaction Horizon', date: fields.date || now().slice(0, 10), source: 'horizon_assistant' } });
...
export default async function handler(req, res) {
  ...
  const execution = execute
    ? await executeAssistantActions(req, actions, { userId, intent })
    : { executed: false, mode: 'dry_run', results: actions.map((action) => ({ ...action, executed: false, status: 'dry_run' })) };

  return json(res, 200, {
    ok: !execution.has_error,
    status: execute ? (execution.executed ? 'executed' : 'execution_attempted') : 'dry_run',
    intent,
    confirmed: true,
    executed: Boolean(execution.executed),
    execution_mode: execution.mode,
    execution_note: execute ? 'Exécution demandée avec le token utilisateur. Les règles Supabase/RLS restent appliquées.' : 'Simulation de validation. Aucune donnée écrite.',
    ...
    message: execution.executed
      ? `Nécessaire fait. Modules mis à jour: ${impactedModules.map((module) => moduleLabels[module] || module).join(', ')}.`
      : `Nécessaire préparé. Modules concernés: ${impactedModules.map((module) => moduleLabels[module] || module).join(', ')}.`,
    timestamp: now(),
  });
}
