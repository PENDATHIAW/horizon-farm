-- Corrige le trigger : supprime le filtre de mots-clés sensible aux accents
...
begin
  ...
  -- Anti-doublon : si déjà notifiée, on n'envoie rien
  if new.push_notified_at is not null then
    return new;
  end if;

  -- Priorité : critique/urgence ou familles prioritaires par module
...
-- Recrée le trigger (après insert uniquement)
