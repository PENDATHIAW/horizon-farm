-- =============================================================================
-- Horizon Farm — 6 migrations 202606 (à exéuter UNE SEULE FOIS dans Supabase)
-- Dashboard → SQL Editor → New query → coller ce fichier → Run
--
-- Contenu (ordre d'application) :
--   1. Champs push sur alertes_center
--   2. Trigger push critique (version finale, sans filtre mots-clés)
--   3. Colonnes issue_key / source / related sur tables métier
--   4. Table stock_movements + RLS
--   5. Table module_role_permissions + RBAC
--
-- Prérequis : tables alertes_center, system_settings, companies, stock, etc.
--             fonctions can_read_erp(), can_write_erp(), can_admin_erp(),
--             current_company_id(), current_profile_role()
--             extension pg_net (créée ci-dessous si absente)
-- =============================================================================

begin;

-- ---------------------------------------------------------------------------
-- 1/6 — 20260601000000_add_push_fields_to_alertes_center.sql
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2/6 — 20260601002000_update_push_trigger_remove_keyword_filter.sql
--       (version finale ; remplace la migration 20260601001000)
-- ---------------------------------------------------------------------------
...
begin
  ...
  if alert_severity in ('urgence', 'critique') then
    null;
  elsif alert_module in ('stock', 'avicole', 'animaux', 'sante', 'smartfarm', 'equipements', 'finances', 'clients', 'documents', 'documents_rapports', 'taches', 'ventes') then
    null;
  else
    return new;
  end if;
...
-- ---------------------------------------------------------------------------
-- 3/6 — 20260601010000_normalize_issue_link_fields.sql
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4/6 — 20260601100000_create_stock_movements.sql
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 5/6 — 20260601101000_module_role_permissions.sql
-- ---------------------------------------------------------------------------
...
-- Vérification rapide (optionnel, exécuter après le bloc ci-dessus) :
-- select column_name from information_schema.columns where table_name = 'alertes_center' and column_name like 'push%';
-- select to_regclass('public.stock_movements'), to_regclass('public.module_role_permissions');
