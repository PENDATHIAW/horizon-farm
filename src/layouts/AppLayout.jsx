import { AlertTriangle, Bell, Bot, CheckCircle, Droplets, LogOut, MapPin, Menu, Search, Settings, Thermometer, Wifi, WifiOff, Wind, X } from 'lucide-react';
...
export default function AppLayout({
  ...
  const farmLocation = currentWeather.location || currentWeather.localisation || currentWeather.city || currentWeather.place || currentWeather.nom_ferme || 'Ferme principale';
    ...
    <div className="flex h-full overflow-hidden">
      ...
      <main className="flex-1 flex flex-col overflow-hidden min-w-0">
        <header className="min-h-[72px] bg-[#ffffff] border-b border-[#dcfce7] flex items-center px-3 md:px-6 gap-2 md:gap-3 shrink-0 relative">
          ...
          <div className="min-w-0 flex-1"><p className="truncate text-sm font-black text-[#052e16] sm:text-base">Bonjour {displayUser}</p><p className="truncate text-[11px] font-medium capitalize text-[#6b8a6b] sm:text-xs">{currentDateTime}</p></div>
          <div className="hidden xl:flex items-center gap-2"><HeaderPill icon={MapPin}>{farmLocation}</HeaderPill><HeaderPill icon={Thermometer}>{weatherLoading ? 'Météo...' : `${currentWeather.temp}°C`}</HeaderPill><HeaderPill icon={Droplets}>{currentWeather.humidity}%</HeaderPill><HeaderPill icon={Wind}>{currentWeather.wind === '-' ? currentWeather.condition : `${currentWeather.wind} km/h`}</HeaderPill><HeaderPill icon={online ? Wifi : WifiOff}>{online ? 'En ligne' : 'Hors ligne'}</HeaderPill></div>
          <div className="hidden lg:block relative w-full max-w-sm"><VoiceSearch value={globalSearch} onChange={setGlobalSearch} placeholder="Recherche" />{results.length > 0 ? <div className="absolute top-12 left-0 right-0 z-40 bg-white border border-[#d1e5d1] rounded-xl shadow-xl overflow-hidden">{results.map((result) => <button key={`${result.moduleKey}-${result.id}`} type="button" onClick={() => { navigate(result.moduleKey); setGlobalSearch(''); }} className="w-full text-left px-3 py-2 hover:bg-[#ffffff] border-b border-[#d1e5d1]/60 last:border-b-0"><div className="text-sm font-semibold text-[#052e16]">{result.title}</div><div className="text-xs text-[#6b8a6b]">{result.moduleKey} · {result.subtitle}</div></button>)}</div> : null}</div>
