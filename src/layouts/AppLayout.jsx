...
export default function AppLayout({
  ...
  const alerts = useMemo(() => {
    const map = dataMap || {};
    const stocksCritiques = (map.stock || [])
    ...
    const animauxMalades = (map.animaux || [])
    ...
    const culturesRisque = (map.cultures || [])
    ...
    const financesRisque = (map.finances || [])
    ...
    const fournisseursRisque = (map.fournisseurs || [])
    ...
    const equipementsRisque = (map.equipements || [])
    ...
    const meteoRisque = currentWeather?.riskLevel && currentWeather.riskLevel !== 'stable' ? [{ id: 'meteo-risk', type: 'Météo / terrain', text: currentWeather.impact || 'Vérifier abreuvement, ventilation et parcelles.', moduleKey: 'dashboard', severity: 'amber' }] : [];
      ...
      <div className="flex h-full overflow-hidden">
        ...
        <main className="flex-1 flex flex-col overflow-hidden min-w-0">
          <header className="h-14 bg-[#fffdf8] border-b border-[#e7d9be] flex items-center px-3 md:px-6 gap-2 md:gap-3 shrink-0 relative">
            ...
            <div className="ml-auto flex items-center gap-1 md:gap-3"><button type="button" aria-label="Rechercher dans l’ERP" onClick={openMobileSearch} className="sm:hidden min-h-[44px] min-w-[44px] inline-flex items-center justify-center rounded-lg hover:bg-[#e7d9be] transition-colors text-[#8a7456] hover:text-[#2f2415] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#c9a96a]/30"><Search size={18} aria-hidden="true" /></button><div className={`hidden lg:flex items-center gap-1.5 rounded-lg border px-2.5 py-1.5 text-xs font-black ${simulatedMode ? 'border-amber-300 bg-amber-50 text-amber-700' : 'border-emerald-200 bg-emerald-50 text-emerald-700'}`}>{simulatedMode ? 'Données simulées' : 'Données réelles'} · {uiSettings.complexity === 'expert' ? 'Détaillé' : 'Simple'}</div><div className="hidden lg:flex items-center gap-1.5 text-xs bg-amber-500/10 border border-amber-500/30 rounded-lg px-3 py-1.5 text-amber-500"><Thermometer size={12} aria-hidden="true" />{weatherLoading ? 'Météo...' : `${currentWeather.temp}°C ress. ${currentWeather.apparentTemp}°C - ${currentWeather.condition} - ${currentWeather.humidite}%`}</div><button type="button" aria-label="Ouvrir les notifications" onClick={() => { setNotificationsOpen((value) => !value); setSettingsOpen(false); setMobileSearchOpen(false); }} className="relative min-h-[44px] min-w-[44px] inline-flex items-center justify-center rounded-lg hover:bg-[#e7d9be] transition-colors text-[#8a7456] hover:text-[#2f2415] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#c9a96a]/30" title="Notifications"><Bell size={18} aria-hidden="true" />{notifs > 0 ? <span className="absolute -top-0.5 -right-0.5 min-w-4 h-4 px-1 bg-red-500 rounded-full text-[9px] font-bold flex items-center justify-center text-white">{notifs > 99 ? '99+' : notifs}</span> : null}</button><button type="button" aria-label="Ouvrir l’assistant ERP" onClick={onOpenAssistant} className="min-h-[44px] min-w-[44px] inline-flex items-center justify-center rounded-lg hover:bg-[#e7d9be] transition-colors text-[#8a7456] hover:text-emerald-500 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-emerald-500/25" title="Assistant ERP"><Bot size={18} aria-hidden="true" /></button><button type="button" aria-label="Ouvrir les paramètres" onClick={() => { setSettingsOpen((value) => !value); setNotificationsOpen(false); setMobileSearchOpen(false); }} className="min-h-[44px] min-w-[44px] inline-flex items-center justify-center rounded-lg hover:bg-[#e7d9be] transition-colors text-[#8a7456] hover:text-[#2f2415] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#c9a96a]/30" title="Paramètres"><Settings size={18} aria-hidden="true" /></button></div>
            ...
            <SettingsPanel open={settingsOpen} onClose={() => setSettingsOpen(false)} user={user} displayUser={displayUser} online={online} meteo={currentWeather} weatherSource={weatherSource} sidebarOpen={sidebarOpen} setSidebarOpen={setSidebarOpen} setActive={setActive} onSignOut={onSignOut} />
          </header>
