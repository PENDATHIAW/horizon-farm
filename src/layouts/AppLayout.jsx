  ...
  const alerts = useMemo(() => {
    const stocksCritiques = (dataMap.stock || [])
      ...
      .map((item) => ({
        id: `stock-${item.id}`,
        type: 'Stock critique',
        text: `${item.produit}: ${item.quantite}/${item.seuil} ${item.unite || ''}`.trim(),
        moduleKey: 'stock',
        severity: 'danger',
      }));
    ...
    const vaccinsRetard = (dataMap.sante || [])
      ...
      .map((item) => ({
        id: `vaccin-${item.id}`,
        type: 'Vaccin en retard',
        text: `${item.nom} pour ${item.animal || 'animal non precise'} - prevu le ${item.prevue || 'date non renseignee'}`,
        moduleKey: 'sante',
        severity: 'danger',
      }));
    ...
    const animauxMalades = (dataMap.animaux || [])
      ...
      .map((item) => ({
        id: `animal-${item.id}`,
        type: 'Animal malade',
        text: `${item.name || item.id} (${item.type || 'type inconnu'}) a surveiller`,
        moduleKey: 'animaux',
        severity: 'danger',
      }));
    ...
    const culturesRisque = (dataMap.cultures || [])
      ...
      .map((item) => ({
        id: `culture-${item.id}`,
        type: 'Culture a risque',
        text: `${item.nom || item.id}: score sante ${item.score_sante || 0}%`,
        moduleKey: 'cultures',
        severity: 'amber',
      }));
    ...
    const lotsRisque = (dataMap.avicole || [])
      ...
      .map((item) => ({
        ...
        text: `${item.name || item.id}: mortalite ${item.mortality || 0}, score sante ${item.scoresSante || 0}%`,
        moduleKey: 'avicole',
        severity: 'danger',
      }));
    ...
    const financesRisque = (dataMap.finances || [])
      ...
      .map((item) => ({
        id: `finance-${item.id}`,
        type: item.type === 'entree' ? 'Client impaye' : 'Paiement partiel',
        text: `${item.libelle}: ${item.montant || 0} FCFA (${item.statut})`,
        moduleKey: 'finances',
        severity: 'amber',
      }));
    ...
    const fournisseursRisque = (dataMap.fournisseurs || [])
      ...
      .map((item) => ({
        ...
        type: 'Fournisseur a surveiller',
        text: `${item.nom || item.id}: dette ${item.dettes || 0} FCFA, statut ${item.statut || 'actif'}`,
    ...
    const equipementsRisque = (dataMap.equipements || [])
      ...
      .map((item) => ({
        ...
        type: 'Equipement a traiter',
        text: `${item.name || item.id}: ${item.status}`,
    ...
    const tachesRisque = (dataMap.taches || [])
      ...
      .map((item) => ({
        id: `tache-${item.id}`,
        type: 'Tache prioritaire',
        text: `${item.title || item.id}: ${item.priority || item.status}`,
        moduleKey: 'taches',
        severity: 'danger',
      }));
    ...
    const meteoRisque = meteo?.riskLevel && meteo.riskLevel !== 'stable'
      ? [{
          id: 'meteo-risk',
          type: 'Meteo / terrain',
          text: meteo.impact || 'Verifier abreuvement, ventilation et parcelles.',
          moduleKey: 'dashboard',
          severity: 'amber',
        }]
      : [];
        ...
        <header className="h-14 bg-[#fffdf8] border-b border-[#e7d9be] flex items-center px-3 md:px-6 gap-3 shrink-0 relative">
          ...
          <div className="ml-auto flex items-center gap-2 md:gap-3">
            ...
            >
              <Bell size={18} />
              {notifs > 0 ? (
                <span className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-red-500 rounded-full text-[9px] font-bold flex items-center justify-center text-[#2f2415]">
                  {notifs}
                </span>
              ) : null}
            </button>
          ...
          {settingsOpen ? (
            <div className="absolute right-3 top-14 z-50 w-[min(92vw,380px)] bg-[#ffffff] border border-[#d6c3a0] rounded-2xl shadow-2xl p-4">
              <div className="flex items-center justify-between mb-3">
                <div>
                  <p className="text-sm font-bold text-[#2f2415]">Parametres rapides</p>
                  <p className="text-xs text-[#8a7456]">Compte, affichage, connexion et meteo</p>
                </div>
              ...
              <div className="space-y-3">
                <div className="rounded-xl bg-[#fffdf8] border border-[#e7d9be] p-3">
                  <p className="text-xs text-[#8a7456]">Utilisateur connecte</p>
                  <p className="text-sm font-bold text-[#2f2415]">{displayUser}</p>
                  <p className="text-xs text-[#8a7456] mt-1">Role: {user?.user_metadata?.role || 'admin'}</p>
                </div>
                ...
                >
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-semibold text-[#2f2415]">Sidebar compacte</p>
                      <p className="text-xs text-[#8a7456]">Basculer entre menu large et menu compact</p>
                    </div>
                ...
                <div className="rounded-xl bg-[#fffdf8] border border-[#e7d9be] p-3">
                  ...
                  <p className={`text-xs mt-1 ${online ? 'text-emerald-600' : 'text-red-500'}`}>{online ? 'Connecte - synchronisation Supabase active' : 'Hors ligne - sync en attente'}</p>
                </div>

                <div className="rounded-xl bg-[#fffdf8] border border-[#e7d9be] p-3">
                  <p className="text-sm font-semibold text-[#2f2415]">Meteo</p>
                  <p className="text-xs text-[#8a7456] mt-1">
                    Source: {weatherSource === 'live' ? 'position actuelle au Senegal' : weatherSource === 'senegal-default' ? 'Dakar/Senegal par defaut' : 'donnees demo/fallback'} - {meteo.condition}, vent {meteo.windLabel}, pluie {meteo.precipitationProbability || 0}%
                  </p>
                </div>
