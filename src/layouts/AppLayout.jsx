...
const NAV_GROUPS = [
  { key: 'pilotage', label: 'Pilotage', ids: ['dashboard', 'assistant_erp', 'centre_ia', 'objectifs_croissance', 'impact_business', 'rapports'] },
...
export default function AppLayout({
  ...
  return <div className="h-screen bg-[#f8f5ef] text-[#2f2415] overflow-hidden" style={{ fontFamily: "'DM Sans', 'Segoe UI', sans-serif" }}>
    ...
    <div className="flex h-full overflow-hidden">
      <aside className={`${sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'} ${sidebarOpen ? 'md:w-64' : 'md:w-16'} fixed md:relative inset-y-0 left-0 z-40 w-[82vw] max-w-80 shrink-0 bg-[#fffdf8] border-r border-[#e7d9be] flex flex-col transition-all duration-300 overflow-hidden shadow-2xl md:shadow-none`}>
        ...
        <nav className="flex-1 py-4 overflow-y-auto space-y-4 px-2" aria-label="Navigation principale">
          {groupedNavItems.map((group) => <div key={group.key} className="space-y-1">
            {sidebarOpen ? <p className="px-3 pt-1 pb-1 text-[10px] font-black uppercase tracking-[0.2em] text-[#b39b78]">{group.label}</p> : <div className="mx-2 my-2 border-t border-[#eadcc2]" />}
        ...
        <div className="p-3 border-t border-[#e7d9be] space-y-2">
          ...
          {sidebarOpen ? <button type="button" onClick={signOutAction} className="w-full flex items-center gap-2 px-3 py-2 rounded-lg bg-[#e7d9be] cursor-pointer hover:bg-[#d6c3a0] transition-colors text-left"><div className="w-6 h-6 rounded-full bg-emerald-500/30 flex items-center justify-center text-emerald-600 font-bold text-xs">A</div><div className="flex-1 min-w-0"><div className="text-xs text-[#2f2415] font-semibold truncate">{displayUser}</div><div className="text-[10px] text-[#b39b78]">Exploitant principal</div></div><LogOut size={12} className="text-[#b39b78]" /></button> : null}
        </div>
      </aside>
      <main className="flex-1 flex flex-col overflow-hidden min-w-0">
        <header className="h-14 bg-[#fffdf8] border-b border-[#e7d9be] flex items-center px-3 md:px-6 gap-2 md:gap-3 shrink-0 relative">
          <button type="button" aria-label="Ouvrir le menu" onClick={() => setSidebarOpen?.(true)} className="md:hidden min-h-[44px] min-w-[44px] inline-flex items-center justify-center rounded-lg hover:bg-[#e7d9be] text-[#8a7456]"><Menu size={20} /></button>
          <div className="hidden sm:block flex-1 relative max-w-md"><VoiceSearch value={globalSearch} onChange={setGlobalSearch} placeholder="Recherche globale ERP..." />{results.length > 0 ? <div className="absolute top-12 left-0 right-0 z-40 bg-white border border-[#d6c3a0] rounded-xl shadow-xl overflow-hidden">{results.map((result) => <button key={`${result.moduleKey}-${result.id}`} type="button" onClick={() => { navigate(result.moduleKey); setGlobalSearch(''); }} className="w-full text-left px-3 py-2 hover:bg-[#fffdf8] border-b border-[#d6c3a0]/60 last:border-b-0"><div className="text-sm font-semibold text-[#2f2415]">{result.title}</div><div className="text-xs text-[#8a7456]">{result.moduleKey} - {result.subtitle}</div></button>)}</div> : null}</div>
          ...
          <div className="ml-auto flex items-center gap-1 md:gap-3">
            <button type="button" aria-label="Rechercher dans l’ERP" onClick={() => { setMobileSearchOpen(true); setNotificationsOpen(false); setSettingsOpen(false); }} className="sm:hidden min-h-[44px] min-w-[44px] inline-flex items-center justify-center rounded-lg hover:bg-[#e7d9be] text-[#8a7456]"><Search size={18} /></button>
            ...
            <button type="button" aria-label="Ouvrir les notifications" onClick={() => { setNotificationsOpen((value) => !value); setSettingsOpen(false); setMobileSearchOpen(false); }} className="relative min-h-[44px] min-w-[44px] inline-flex items-center justify-center rounded-lg hover:bg-[#e7d9be] text-[#8a7456]"><Bell size={18} />{notifs > 0 ? <span className="absolute -top-0.5 -right-0.5 min-w-4 h-4 px-1 bg-red-500 rounded-full text-[9px] font-bold flex items-center justify-center text-white">{notifs > 99 ? '99+' : notifs}</span> : null}</button>
            <button type="button" aria-label="Ouvrir l’assistant ERP" onClick={onOpenAssistant} className="min-h-[44px] min-w-[44px] inline-flex items-center justify-center rounded-lg hover:bg-[#e7d9be] text-[#8a7456] hover:text-emerald-500"><Bot size={18} /></button>
            <button type="button" aria-label="Ouvrir les paramètres" onClick={() => { setSettingsOpen((value) => !value); setNotificationsOpen(false); setMobileSearchOpen(false); }} className="min-h-[44px] min-w-[44px] inline-flex items-center justify-center rounded-lg hover:bg-[#e7d9be] text-[#8a7456]"><Settings size={18} /></button>
          </div>
          {mobileSearchOpen ? <div className="fixed inset-0 z-50 bg-black/30 sm:hidden"><div className="absolute inset-x-3 top-3 rounded-3xl border border-[#d6c3a0] bg-white p-4 shadow-2xl"><div className="flex items-center justify-between gap-3 mb-3"><div><p className="text-sm font-black text-[#2f2415]">Recherche globale</p><p className="text-xs text-[#8a7456]">Retrouver une vente, un lot, un animal, une tâche…</p></div><button type="button" onClick={() => setMobileSearchOpen(false)} className="min-h-[44px] min-w-[44px] inline-flex items-center justify-center rounded-xl text-[#8a7456] hover:bg-[#fff8e8]"><X size={18} /></button></div><VoiceSearch value={globalSearch} onChange={setGlobalSearch} placeholder="Rechercher dans Horizon Farm..." /><div className="mt-3 max-h-[65vh] overflow-y-auto space-y-2">{globalSearch.trim() && results.length > 0 ? results.map((result) => <button key={`${result.moduleKey}-${result.id}`} type="button" onClick={() => { navigate(result.moduleKey); setGlobalSearch(''); }} className="w-full text-left rounded-2xl border border-[#eadcc2] bg-[#fffdf8] px-3 py-3"><div className="text-sm font-black text-[#2f2415]">{result.title}</div><div className="mt-1 text-xs text-[#8a7456]">{result.moduleKey} · {result.subtitle}</div></button>) : <div className="rounded-2xl border border-[#eadcc2] bg-[#fffdf8] px-4 py-8 text-center text-sm text-[#8a7456]">{globalSearch.trim() ? 'Aucun résultat trouvé.' : 'Tape un mot-clé ou utilise la dictée vocale.'}</div>}</div></div></div> : null}
          {notificationsOpen ? <div className="absolute right-2 md:right-16 top-14 z-50 w-[min(94vw,420px)] bg-white border border-[#d6c3a0] rounded-2xl shadow-2xl p-4"><div className="flex items-center justify-between mb-3"><div><p className="text-sm font-bold text-[#2f2415]">Centre de notifications</p><p className="text-xs text-[#8a7456]">{alerts.length} alerte(s) métier active(s)</p></div><button type="button" onClick={() => setNotificationsOpen(false)} className="text-[#8a7456] hover:text-[#2f2415]"><X size={16} /></button></div><div className="space-y-2 max-h-[60vh] overflow-y-auto">{alerts.length > 0 ? alerts.map((alert) => <button key={alert.id} type="button" onClick={() => navigate(alert.moduleKey)} className={`w-full text-left rounded-xl border p-3 transition-all ${alert.severity === 'danger' ? 'bg-red-500/10 border-red-500/20' : 'bg-amber-500/10 border-amber-500/20'}`}><div className="flex gap-2"><AlertTriangle size={15} className={alert.severity === 'danger' ? 'text-red-500 shrink-0 mt-0.5' : 'text-amber-500 shrink-0 mt-0.5'} /><div><p className="text-xs font-bold text-[#2f2415]">{alert.type}</p><p className="text-xs text-[#7d6a4a] mt-0.5">{alert.text}</p></div></div></button>) : <div className="rounded-xl bg-emerald-500/10 border border-emerald-500/20 p-3 flex gap-2"><CheckCircle size={16} className="text-emerald-500 shrink-0 mt-0.5" /><div><p className="text-xs font-bold text-[#2f2415]">Tout est calme</p><p className="text-xs text-[#7d6a4a]">Aucune alerte importante détectée.</p></div></div>}</div></div> : null}
    ...
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-30 bg-[#fffdf8]/95 backdrop-blur border-t border-[#e7d9be] px-2 pt-2 pb-[calc(env(safe-area-inset-bottom)+0.5rem)] shadow-2xl"><div className="grid grid-cols-5 gap-1">{mobileNavItems.map((item) => { const Icon = item.icon; const isActive = active === item.id; return <button key={item.id} type="button" onClick={() => navigate(item.id)} className={`min-w-0 rounded-xl px-1 py-2 flex flex-col items-center gap-1 ${isActive ? 'bg-[#c9a96a] text-[#2f2415]' : 'text-[#8a7456]'}`}><Icon size={18} /><span className="text-[10px] font-semibold truncate w-full text-center">{item.label.replace('Impact Business', 'Décisions').replace('Impact & Valeur ERP', 'Décisions').replace('Impact & Valeur', 'Décisions')}</span></button>; })}<button type="button" onClick={onOpenAssistant} className="min-w-0 rounded-xl px-1 py-2 flex flex-col items-center gap-1 text-emerald-600 bg-emerald-500/10"><Bot size={18} /><span className="text-[10px] font-semibold truncate w-full text-center">Assistant</span></button></div></nav>
  </div>;
}
