...
const NAV_GROUPS = [
  ...
  },
  {
    ...
    icon: Leaf,
    target: 'animaux',
    matches: ['animaux', 'avicole', 'cultures', 'sante'],
    hint: 'Animaux · avicole · cultures',
  },
  {
    id: 'commerce',
    label: 'Commerce',
    icon: ShoppingCart,
    target: 'ventes',
    matches: ['ventes', 'clients', 'fournisseurs'],
    hint: 'Ventes · clients · fournisseurs',
  },
  {
    id: 'argent',
    label: 'Argent',
    icon: Wallet,
    target: 'finances',
    matches: ['finances', 'comptabilite', 'documents', 'investissements'],
    hint: 'Cash · dépenses · preuves',
  },
  {
    id: 'actions',
    label: 'Actions',
  ...
  },
  {
    ...
    icon: BarChart3,
    target: 'rapports',
    matches: ['rapports', 'impact_business', 'centre_ia'],
    hint: 'Analyse simple',
  },
  {
    id: 'ferme',
    ...
    icon: Package,
    target: 'stock',
    matches: ['stock', 'equipements', 'smartfarm', 'sync', 'sync_activity', 'audit_logs', 'gestion_systeme', 'rh', 'tracabilite'],
    hint: 'Stock · équipements · système',
  },
];
...
export default function AppLayout({
  ...
  const alerts = useMemo(() => {
    ...
    const financesRisque = (dataMap.finances || [])
      ...
      .map((item) => ({
        id: `finance-${item.id}`,
        type: item.type === 'entree' ? 'À encaisser' : 'Paiement à vérifier',
        text: `${item.libelle || item.title || 'Transaction'}: ${item.montant || 0} FCFA`,
        moduleKey: 'finances',
        severity: 'amber',
      }));
  ...
  return (
    <div
      className="h-screen bg-[#f7f3ea] text-[#23301f] overflow-hidden"
    ...
    >
      ...
      <div className="flex h-full overflow-hidden">
        <aside
          ...
          } fixed md:relative inset-y-0 left-0 z-40 w-[84vw] max-w-80 shrink-0 bg-[#103c2d] text-white flex flex-col transition-all duration-300 overflow-hidden shadow-2xl md:shadow-none`}
        >
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-3 rounded-2xl transition-all text-left relative ${
                    isActive
                      ? 'bg-white text-[#103c2d] shadow-sm'
                ...
                >
                  ...
                  {item.hasAlert ? (
                    <span className={`w-2 h-2 rounded-full bg-amber-400 shrink-0 ${sidebarOpen ? '' : 'absolute top-2 right-2'}`} />
                  ) : null}
                </button>
              );
            })}
          ...
          <div className="p-3 border-t border-white/10 space-y-2">
            ...
            {sidebarOpen ? (
              <button
                type="button"
                onClick={onSignOut}
                className="w-full flex items-center gap-2 px-3 py-2 rounded-xl bg-white/8 hover:bg-white/12 transition-colors text-left"
              >
                <div className="w-7 h-7 rounded-full bg-emerald-300/25 flex items-center justify-center text-emerald-100 font-black text-xs">
        ...
        <main className="flex-1 flex flex-col overflow-hidden min-w-0">
          <header className="h-16 bg-white border-b border-[#e7d9be] flex items-center px-3 md:px-6 gap-2 md:gap-4 shrink-0 relative">
            <button
              ...
              className="md:hidden p-2 rounded-xl hover:bg-[#f2eadb] text-[#526457]"
              title="Menu"
            >
              <Menu size={20} />
            </button>
            ...
            <div className="hidden md:block flex-1 relative max-w-xl ml-2">
              <VoiceSearch
                ...
                placeholder="Chercher animal, client, facture, stock..."
              />
            ...
            <div className="ml-auto flex items-center gap-1 md:gap-2">
              <div className={`hidden lg:flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-black ${
                demoMode ? 'border-amber-200 bg-amber-50 text-amber-700' : 'border-emerald-200 bg-emerald-50 text-emerald-700'
              }`}>
                {demoMode ? 'Démo' : 'ERP réel'} · {uiSettings.complexity === 'expert' ? 'Expert' : 'Simple'}
              </div>

              <div className="hidden xl:flex items-center gap-1.5 text-xs bg-[#f7f3ea] border border-[#e7d9be] rounded-full px-3 py-1.5 text-[#6f6046]">
                {weatherLoading ? 'Météo...' : `${meteo.temp}°C · ${meteo.condition || 'météo'}`}
              </div>
              ...
              <button
                ...
                className="relative p-2 rounded-xl hover:bg-[#f2eadb] transition-colors text-[#526457]"
                title="Notifications"
              >
                <Bell size={18} />
              ...
              <button
                ...
                className="p-2 rounded-xl hover:bg-[#f2eadb] transition-colors text-[#526457] hover:text-emerald-700"
                title="Assistant ERP"
              >
                <Bot size={18} />
              </button>
              ...
              <button
                ...
                className="p-2 rounded-xl hover:bg-[#f2eadb] transition-colors text-[#526457]"
                title="Paramètres"
              >
                <Settings size={18} />
              </button>
            ...
            {notificationsOpen ? (
              <div className="absolute right-2 md:right-16 top-16 z-50 w-[min(94vw,420px)] bg-white border border-[#d6c3a0] rounded-3xl shadow-2xl p-4">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    <p className="text-sm font-black text-[#2f2415]">À traiter</p>
                    <p className="text-xs text-[#8a7456]">{alerts.length} priorité(s) affichée(s)</p>
                  </div>
                  ...
                  {alerts.length > 0 ? (
                    alerts.map((alert) => (
                      <button
                        ...
                        className={`w-full text-left rounded-2xl border p-3 transition-all ${alert.severity === 'danger' ? 'bg-red-50 border-red-200' : 'bg-amber-50 border-amber-200'}`}
                      >
                        <div className="flex gap-2">
                          <AlertTriangle size={15} className={alert.severity === 'danger' ? 'text-red-500 shrink-0 mt-0.5' : 'text-amber-500 shrink-0 mt-0.5'} />
                          <div>
                            <p className="text-xs font-black text-[#2f2415]">{alert.type}</p>
                    ...
                    <div className="rounded-2xl bg-emerald-50 border border-emerald-200 p-3 flex gap-2">
                      ...
                      <div>
                        <p className="text-xs font-black text-[#2f2415]">Tout est calme</p>
                        <p className="text-xs text-[#7d6a4a]">Aucune priorité importante détectée.</p>
                      </div>
                    </div>
                  )}
                </div>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-30 bg-white/95 backdrop-blur border-t border-[#e7d9be] px-2 pt-2 pb-[calc(env(safe-area-inset-bottom)+0.5rem)] shadow-2xl">
        <div className="grid grid-cols-5 gap-1">
          {mobileNavItems.map((item) => {
            ...
            return (
              <button
                ...
                type="button"
                onClick={() => navigate(item.target)}
                className={`min-w-0 rounded-2xl px-1 py-2 flex flex-col items-center gap-1 ${isActive ? 'bg-[#103c2d] text-white' : 'text-[#667566]'}`}
              >
                <item.icon size={18} />
          ...
          <button
            type="button"
            onClick={onOpenAssistant}
            className="min-w-0 rounded-2xl px-1 py-2 flex flex-col items-center gap-1 text-emerald-700 bg-emerald-50"
          >
            <Bot size={18} />
            <span className="text-[10px] font-bold truncate w-full text-center">Aide</span>
          </button>
        </div>
      </nav>
    </div>
