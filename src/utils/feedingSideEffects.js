...
export function buildFeedingFinanceRow({ log = {}, amount = 0, date = '' } = {}) {
  ...
  return {
    ...
    status: 'comptabilise',
    cash_effect: false,
    source_type: 'consommation_stock',
    event_key: log.event_key ? `${log.event_key}:cost` : undefined,
    idempotency_key: log.event_key ? `${log.event_key}:cost` : undefined,
    farm_id: log.farm_id || undefined,
    recorded_by: log.recorded_by || 'system',
