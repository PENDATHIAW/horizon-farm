...
const closedStatuses = ['convertie', 'vendue', 'fermee', 'fermée', 'annulee', 'annulée', 'inactive', 'cloture', 'clôturé'];
...
function animalReadyForSale(animal = {}) {
  ...
  return clean(animal.id) && !['vendu', 'decede', 'décédé', 'sorti', 'annule'].includes(status)
...
function cultureReadyForSale(culture = {}) {
  ...
  return clean(culture.id) && qty > 0 && !['vendue', 'perdu', 'annulee', 'annulée'].includes(status)
...
function stockReadyForSale(stock = {}) {
  ...
  return clean(stock.id) && qty > 0 && !['reserve', 'réservé', 'bloque', 'bloqué', 'perime', 'périmé'].includes(status)
...
function animalOpportunity(animal = {}) {
  ...
  return {
    id: `DERIVED-ANIMAL-${animal.id}`,
    opportunity_key: `animaux:${animal.id}`,
    source_module: 'animaux',
    source_type: 'animal',
    source_id: animal.id,
    related_id: animal.id,
    title: `Animal prêt à vendre: ${animal.name || animal.nom || animal.id}`,
    product_name: `${animal.name || animal.nom || animal.id} · ${animal.type || 'Animal'}`,
    quantity: 1,
    unit: 'tete',
    unit_price: unitPrice,
    estimated_amount: unitPrice,
    status: 'ouverte',
    statut: 'ouverte',
    is_derived: true,
    created_from: 'animaux',
    notes: 'Source prête à convertir détectée depuis le module Animaux.',
  };
}
...
function cultureOpportunity(culture = {}) {
  ...
  return {
    id: `DERIVED-CULTURE-${culture.id}`,
    opportunity_key: `cultures:${culture.id}`,
    source_module: 'cultures',
    source_type: 'culture',
    source_id: culture.id,
    related_id: culture.id,
    title: `Récolte prête à vendre: ${culture.nom || culture.name || culture.id}`,
    product_name: culture.nom || culture.name || culture.id,
    quantity: qty,
    unit: culture.unite || culture.unit || 'kg',
    unit_price: unitPrice,
    estimated_amount: Math.max(0, qty * unitPrice),
    status: 'ouverte',
    statut: 'ouverte',
    ...
    notes: 'Source prête à convertir détectée depuis le module Cultures.',
  };
}
...
function stockOpportunity(stock = {}) {
  ...
  return {
    id: `DERIVED-STOCK-${stock.id}`,
    opportunity_key: `stock:${stock.id}`,
    source_module: 'stock',
    source_type: 'stock',
    source_id: stock.id,
    related_id: stock.id,
    title: `Stock prêt à vendre: ${stock.produit || stock.name || stock.id}`,
    product_name: stock.produit || stock.name || stock.id,
    quantity: qty,
    unit: stock.unite || stock.unit || 'unite',
    unit_price: unitPrice,
    estimated_amount: Math.max(0, qty * unitPrice),
    status: 'ouverte',
    statut: 'ouverte',
    is_derived: true,
    created_from: 'stock',
    notes: 'Source prête à convertir détectée depuis le module Stock.',
  };
}

export function deriveSalesOpportunities({ opportunities = [], lots = [], animaux = [], cultures = [], stocks = [] } = {}) {
  const persisted = arr(opportunities).filter(isOpenSalesOpportunity);
  const derived = [
