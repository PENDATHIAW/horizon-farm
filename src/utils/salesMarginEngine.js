...
export function calculateSalesMargin(input = {}, context = {}) {
  ...
  const saleAmount = totalOf(input) || quantity * unitPrice;
  ...
  if (sourceModule.includes('stock')) {
    ...
    if (stock) {
      ...
      return marginResult({ saleAmount, directCost, costSource: directCost > 0 ? 'cout_stock' : 'cout_stock_indisponible', sourceLabel: stock.produit || stock.id });
    }
  }
  ...
  return marginResult({ saleAmount, directCost: explicitCost, costSource: explicitCost > 0 ? 'cout_saisi' : 'cout_indisponible', sourceLabel: input.product_name || input.produit || input.title || input.id });
}
...
export function summarizeSalesMargins(rows = [], context = {}) {
  const details = arr(rows).map((row) => enrichWithSalesMargin(row, context));
  const ca = details.reduce((sum, row) => sum + totalOf(row), 0);
  const directCost = details.reduce((sum, row) => sum + toNumber(row.cout_revient), 0);
  const margin = ca - directCost;
  return { details, ca, directCost, margin, marginRate: ca > 0 ? Number(((margin / ca) * 100).toFixed(1)) : 0 };
}
