...
export function AppProvider({ children }) {
  ...
  const createAnimalFollowUpTaskAndAlert = useCallback(async (animal) => { const nextDate = animal?.date_prochaine_verification || animal?.next_action_date || animal?.prochaine_visite; if (!animal?.id || !nextDate) return; try { const taskId = `TSK-${animal.id}-${String(nextDate).replace(/-/g, '')}`; const alertDate = new Date(nextDate); alertDate.setDate(alertDate.getDate() - 3); const issueKey = `animaux:suivi_sanitaire:${animal.id}:${String(nextDate).replace(/-/g, '')}`; await tachesService.create({ id: taskId, title: `Verification sanitaire ${animal.id} - ${animal.name || 'animal'}`, module_lie: 'animaux', assigned_to: animal.veterinaire_id || 'Equipe ferme', due_date: alertDate.toISOString().slice(0, 10), priority: 'haute', status: 'a_faire', checklist: 'Verifier etat sante; noter poids; mettre a jour fiche; contacter veterinaire si besoin', issue_key: issueKey, source_module: 'animaux', source_record_id: animal.id, related_module: 'sante', related_record_id: animal.id, origin_type: 'automatic' }); await supabase.from('alertes_center').insert({ id: `ALERT-${taskId}`, title: `Rappel sanitaire ${animal.id}`, message: `Controle sanitaire prevu le ${nextDate}. Alerte creee 3 jours avant.`, module_source: 'animaux', entity_type: 'animal', entity_id: animal.id, severity: 'warning', status: 'nouvelle', action_recommandee: 'Verifier la fiche animal et confirmer la visite veterinaire.', send_whatsapp: false, issue_key: issueKey, source_module: 'animaux', source_record_id: animal.id, related_module: 'sante', related_record_id: animal.id, origin_type: 'automatic' }); await refreshModule('taches'); } catch (error) { console.warn('Rappel sanitaire non cree', error.message); } }, [refreshModule]);
  const emitBusinessEvents = useCallback((events = []) => { const validEvents = events.filter((event) => event?.event_type && event?.title); if (validEvents.length === 0) return; markLocalWrite('business_events'); void Promise.allSettled(validEvents.map((event) => createBusinessEvent(event))).then(() => refreshModule('business_events')).catch((error) => { console.warn('Evenements metier non enregistres', error.message); }); }, [markLocalWrite, refreshModule]);
