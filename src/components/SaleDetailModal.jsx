...
export default function SaleDetailModal({
  ...
  const paidFromPayments = payments.reduce((sum, payment) => sum + Number(payment.montant || 0), 0);
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/50 p-3">
      <div className="flex max-h-[94vh] w-full max-w-5xl flex-col overflow-hidden rounded-3xl border border-[#e8d5b0] bg-[#fdf8f0] shadow-2xl">
        <div className="flex items-start justify-between gap-3 border-b border-[#e8d5b0] px-5 py-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-[#8a7456]">Fiche vente complete</p>
            <h2 className="text-xl font-black text-[#2f2415]">CMD-{String(order.id || '').slice(-6)}</h2>
            <div className="mt-2 flex flex-wrap gap-2">
              <Pill tone={order.statut_commande === 'annule' ? 'red' : order.statut_commande === 'livre' ? 'emerald' : 'sky'}>{statusLabel(order.statut_commande || 'brouillon')}</Pill>
              <Pill tone={order.statut_paiement === 'paye' ? 'emerald' : order.statut_paiement === 'partiel' ? 'amber' : 'red'}>{statusLabel(order.statut_paiement || 'non_paye')}</Pill>
              <Pill tone={order.statut_livraison === 'livre' ? 'emerald' : 'amber'}>{statusLabel(order.statut_livraison || 'a_livrer')}</Pill>
            </div>
          </div>
        ...
        <div className="overflow-y-auto p-5">
          <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
            ...
            <Section title="Lignes vendues" icon={Package}>
              <div className="space-y-3">
                {items.length === 0 ? (
                  <p className="rounded-xl bg-amber-50 p-3 text-sm text-amber-700">Aucune ligne detaillee trouvee. La commande existe, mais les produits ne sont pas encore detailles dans sales_order_items.</p>
                ) : items.map((item) => (
            ...
            <Section title="Livraison" icon={Truck}>
              <div className="space-y-2">
                {deliveries.length === 0 ? <p className="text-sm text-[#8a7456]">Aucune livraison liee pour le moment.</p> : deliveries.map((delivery) => (
          ...
          <Section title="Impacts inter-modules" icon={CheckCircle}>
            <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
              <ImpactBadge label="Source vendue" state={impactApplied ? 'ok' : order.statut_commande === 'brouillon' ? 'neutral' : 'warning'} hint={impactApplied ? 'Stock, lot, animal ou culture impacte.' : 'Impact non detecte ou commande encore brouillon.'} />
              <ImpactBadge label="Finance" state={paid > 0 && paidTransactions.length > 0 ? 'ok' : paid > 0 ? 'warning' : 'neutral'} hint={paid > 0 ? (paidTransactions.length ? 'Recette visible dans Finances.' : 'Paiement saisi mais transaction finance non trouvee.') : 'Non applicable tant que rien n est encaisse.'} />
              <ImpactBadge label="Creance client" state={remaining > 0 ? 'ok' : 'neutral'} hint={remaining > 0 ? (receivableTransactions.length ? 'Creance visible dans Finances.' : 'Reste a payer suivi sur la commande.') : 'Non applicable: aucun reste a payer.'} />
              <ImpactBadge label="Comptabilite" state={hasAccounting ? 'ok' : 'neutral'} hint={hasAccounting ? 'Ecriture comptable liee.' : 'Comptabilite non configuree pour cette vente.'} />
              <ImpactBadge label="Tracabilite" state={linkedEvents.length ? 'ok' : 'warning'} hint={linkedEvents.length ? `${linkedEvents.length} evenement(s) business_events lies.` : 'Aucun evenement lie detecte.'} />
              <ImpactBadge label="Documents" state={invoices.length || linkedDocuments.length ? 'ok' : 'neutral'} hint={invoices.length || linkedDocuments.length ? 'Facture/document lie disponible.' : 'Aucun document lie pour le moment.'} />
            </div>
          </Section>
          ...
          <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
            <Section title="Paiements et finances" icon={CreditCard}>
              <div className="space-y-3">
                {payments.length === 0 ? <p className="text-sm text-[#8a7456]">Aucun paiement encaisse dans l onglet Paiements.</p> : payments.map((payment) => (
                  <div key={payment.id} className="rounded-xl border border-[#e8d5b0] bg-[#fffdf8] p-3">
                    <InfoRow label="Date" value={fmtDate(payment.date_paiement)} />
                    <InfoRow label="Montant" value={fmtCurrency(Number(payment.montant || 0))} />
                    <InfoRow label="Moyen" value={payment.moyen_paiement || '-'} />
                  </div>
                ))}
                {transactions.length ? (
                  <div className="rounded-xl bg-emerald-50 p-3 text-sm text-emerald-700">
                    Argent / creance visibles dans Finances: {transactions.map((tx) => tx.id).join(', ')}
                  </div>
                ) : null}
              </div>
            </Section>
              ...
              <div className="space-y-3">
                ...
                {invoices.map((invoice) => (
                  <div key={invoice.id} className="rounded-xl border border-[#e8d5b0] bg-[#fffdf8] p-3">
                    ...
                    <InfoRow label="Statut" value={statusLabel(invoice.statut)} />
                    <InfoRow label="Montant" value={fmtCurrency(Number(invoice.montant_total || 0))} />
                  </div>
                ))}
                {linkedEvents.length ? (
                  <div className="space-y-2">
                ...
                ) : <p className="text-sm text-[#8a7456]">Aucune trace business_events liee.</p>}
              </div>
            </Section>
          </div>
        </div>
        ...
        <div className="flex flex-wrap justify-end gap-2 border-t border-[#e8d5b0] px-5 py-4">
          ...
          <button onClick={onInvoice} className="rounded-xl bg-sky-600 px-4 py-2 text-sm font-semibold text-white hover:bg-sky-700">Generer facture</button>
