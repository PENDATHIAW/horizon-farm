...
export default function AssistantPanel({ open, onClose, dataMap, onNavigate }) {
  ...
  const scheduleSilenceProcessing = (text) => {
    ...
    silenceTimerRef.current = window.setTimeout(() => processCommand(cleaned, { fromSilence: true }), 1600);
  };
  ...
  const resetConversation = () => {
    ...
    setMessages([{ role: 'assistant', text: 'Nouvelle conversation ouverte. Parle puis marque une pause : Horizon traitera automatiquement.' }]);
    setQuery('');
  };
  ...
  const toggleTerrainMode = () => {
    if (!voice.supported) {
      ...
      setLocalOpen(true);
      setMessages((prev) => [...prev, { role: 'assistant', text: 'La reconnaissance vocale Web n’est pas disponible ici. Il faudra utiliser le bouton micro, ou une vraie couche native plus tard.' }]);
      return;
    }
    setTerrainMode((prev) => {
      const next = !prev;
  ...
  return <>
    ...
    <button type="button" onClick={toggleTerrainMode} className={`fixed right-4 bottom-20 z-[66] rounded-full border px-3 py-2 text-xs font-black shadow-xl ${terrainMode ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-[#d6c3a0] bg-white text-[#7d6a4a]'}`}>
      <Ear size={13} className="inline mr-1" /> {terrainMode ? (voice.listening ? 'Horizon écoute' : 'Micro bloqué') : 'Activer Horizon' }
    </button>
