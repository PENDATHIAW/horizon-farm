...
const hasWakeWord = (value = '') => {
  ...
  return text.includes('hey horizon') || text.includes('he horizon') || text.includes('horizon');
};

const stripWakeWord = (value = '') =>
  normalize(value)
    .replace(/\b(hey|he|eh|e)\s+horizon\b/g, '')
    .replace(/\bhorizon\b/g, '')
    .trim();
...
const buildRefreshKeys = (result = {}, draft = {}) => {
  ...
  modules.forEach((module) => {
    (REFRESH_KEYS_BY_MODULE[module] || [module]).forEach((key) => keys.add(key));
  });
...
export default function AssistantPanel({ open, onClose, dataMap, onNavigate }) {
  ...
  const [messages, setMessages] = useState([
    {
      role: 'assistant',
      text: 'Assistant prêt. Active Terrain ON : je traiterai ta phrase après une pause.',
    },
  ]);
  ...
  const buildAssistantTextFromDraft = (nextDraft) => {
    ...
    if (missing.length) {
      return `J’ai préparé un brouillon. Il reste ${missing.length} champ(s) à compléter. Modules concernés : ${impacted}.`;
    }

    if (nextDraft.next_required_form) {
      return `J’ai préparé le brouillon, mais un formulaire lié est requis : ${nextDraft.next_required_form.title}.`;
    }
  ...
  const validateDraft = async () => {
    ...
    try {
      ...
      if (!response.ok || !result.ok) {
        throw new Error(
          result.message ||
          result.execution?.results?.find?.((item) => item.error)?.error ||
  ...
  const cancelDraft = (reason = 'Brouillon annulé.') => {
    setDraft(null);
    setQuery('');
  ...
  const resetConversation = () => {
    ...
    setMessages([
      {
        role: 'assistant',
        text: 'Nouvelle conversation ouverte. Parle puis marque une pause : Horizon traitera automatiquement.',
      },
    ]);
    setQuery('');
  };
  ...
  const loadExternalDraft = (nextDraft, sourceLabel = 'Centre IA') => {
    ...
    const text = `${sourceLabel} a préparé un brouillon. Vérifie les champs, complète si besoin, puis dis “valide” ou clique Valider.`;
  ...
  const processCommand = (rawText, { fromSilence = false } = {}) => {
    ...
    const nextDraft = draft
      ? updateHorizonDraft(draft, cleaned, dataMap)
      : interpretHorizonCommand(cleaned, dataMap);
    ...
    setMessages((prev) => [
      ...
      { role: 'assistant', text: draftText || fallback.answer },
    ]);
    ...
    if (draftText) {
      ...
      if (fallback.moduleKey) onNavigate?.(fallback.moduleKey);
      speech.speak(fallback.answer);
    }
  ...
  const voice = useVoiceRecognition({
    ...
    onInterim: (text) => {
      ...
      if (hasWakeWord(text) && wakeState === 'idle') wakeHorizon();
      if (terrainMode && wakeState === 'idle') setWakeState('listening');

      setLocalOpen(true);
      setQuery(stripWakeWord(text) || text);
  ...
  useEffect(() => {
    const handler = (event) => {
      loadExternalDraft(event.detail?.draft, event.detail?.sourceLabel || 'Centre IA');
    };

    window.addEventListener('horizon-open-draft', handler);

    return () => window.removeEventListener('horizon-open-draft', handler);
  }, [speech, onNavigate]);
  ...
  const toggleTerrainMode = () => {
    if (!voice.supported) {
      toast.error('Reconnaissance vocale non supportée par ce navigateur/app');
      setLocalOpen(true);
      setMessages((prev) => [
        ...prev,
        {
          role: 'assistant',
          text: 'La reconnaissance vocale Web n’est pas disponible ici. Il faudra utiliser le bouton micro, ou une vraie couche native plus tard.',
        },
      ]);
      return;
    }
    ...
    setTerrainMode((prev) => {
      ...
      if (next) {
        setLocalOpen(true);
        setWakeState('listening');
        toast.success('Terrain ON. Autorise le micro, parle, puis marque une pause.');
        window.setTimeout(() => voice.start(), 250);
      } else {
        voice.stop();
        setWakeState('idle');
  ...
  const toggleVoiceReplies = () => {
    if (!speech.supported) {
      toast.error('La réponse vocale n’est pas disponible ici');
      return;
    }
    ...
    setDraft((current) =>
      current
        ? {
            ...
            missing_fields: (current.missing_fields || []).filter((field) => field !== key),
          }
        : current
    );
  };
  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed right-4 bottom-20 z-[66] rounded-full border px-3 py-2 text-xs font-black shadow-xl ${
          terrainMode
            ? 'border-emerald-200 bg-emerald-50 text-emerald-700'
            : 'border-[#d6c3a0] bg-white text-[#7d6a4a]'
        }`}
      >
        <Ear size={13} className="inline mr-1" />
        {terrainMode ? (voice.listening ? 'Horizon écoute' : 'Micro bloqué') : 'Activer Horizon'}
      </button>

      {panelOpen ? (
        <aside className="fixed right-4 bottom-4 z-50 w-[min(460px,calc(100vw-2rem))] max-h-[88vh] bg-[#ffffff]/95 backdrop-blur border border-[#d6c3a0] rounded-3xl shadow-2xl overflow-hidden">
          <div className="px-4 py-3 border-b border-[#d6c3a0] flex items-center gap-3 bg-[#fffdf8]">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <p className="font-black text-[#2f2415]">Horizon</p>

                <span className="inline-flex items-center gap-1 rounded-full bg-[#2f2415] px-2 py-0.5 text-[10px] font-black text-white">
                  <Zap size={10} /> IA
                </span>
                ...
                {terrainMode ? (
                  ...
                  >
                    <Ear size={10} />
                    {voice.listening ? 'écoute' : 'micro en attente'}
                  </span>
                ) : null}
              </div>

              <p className="text-xs text-[#8a7456]">
                Dis “valide”, “annule” ou complète le brouillon par la voix.
              </p>
            </div>
            ...
            <button
              ...
              className="p-2 text-[#8a7456] hover:text-[#2f2415]"
          ...
          <div className="px-4 py-3 border-b border-[#eadcc2] bg-white space-y-3">
            <div className="rounded-2xl border border-[#eadcc2] bg-[#fffdf8] p-3 space-y-3">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-xs font-black text-[#2f2415]">Mode Terrain</p>
                  <p className="text-[11px] text-[#8a7456]">
                    Le wake word est un bonus. Terrain ON traite aussi sans “Hey Horizon”.
                  </p>
                </div>
                ...
                <button
                  ...
                  className={`rounded-xl border px-3 py-1.5 text-xs font-black ${
                    terrainMode
                      ? 'border-emerald-200 bg-emerald-50 text-emerald-700'
                      : 'border-[#d6c3a0] bg-white text-[#8a7456]'
                  }`}
                >
                  {terrainMode ? 'ON' : 'Activer'}
                </button>
              </div>
              ...
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-[11px] text-[#8a7456]">Lecture audio des réponses.</p>
                </div>
                ...
                >
                  {speech.enabled ? (
                    ...
                    <VolumeX size={14} className="inline" />
                  )}
                  {' '}
              ...
              {terrainMode ? (
                ...
                >
                  🎤{' '}
                  {voice.listening
                    ? 'J’écoute. Parle puis marque une pause.'
                    : `Micro non actif : ${voice.error || 'permission/compatibilité à vérifier.'}`}
                </div>
              ) : null}
            </div>
          </div>
          ...
          <div className="max-h-[52vh] overflow-y-auto p-4 space-y-3">
              ...
              <div
                ...
                className={`rounded-2xl px-3 py-2 text-sm leading-relaxed ${
                  message.role === 'assistant'
                    ? 'bg-[#fffdf8] border border-[#eadcc2] text-[#7d6a4a]'
            ...
            {results.length > 0 && !draft ? (
              <div className="space-y-2">
                <p className="text-[11px] uppercase tracking-widest text-[#8a7456] font-bold">
                  Résultats trouvés
                </p>
                ...
                {results.map((result) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      toast.success(`Ouverture ${moduleLabel(result.moduleKey)}`);
                    }}
                    className="w-full text-left bg-[#fffdf8] border border-[#d6c3a0] rounded-xl p-2 hover:border-emerald-500 transition-colors"
                  >
                    <div className="text-sm font-semibold text-[#2f2415]">
                      {result.title}
                    </div>
                    <div className="text-xs text-[#8a7456]">
                      {moduleLabel(result.moduleKey)} · {result.subtitle}
                    </div>
                  </button>
                ))}
              </div>
          ...
          <div className="p-3 border-t border-[#d6c3a0] flex gap-2 bg-[#fffdf8]">
            <input
              ...
              onKeyDown={(event) => {
                if (event.key === 'Enter') processCommand(query);
              }}
              className="flex-1 bg-white border border-[#d6c3a0] rounded-xl px-3 py-2 text-sm text-[#2f2415] outline-none focus:border-emerald-500"
              placeholder={voice.listening ? voice.transcript || 'J’écoute...' : 'Écris ou active Horizon...'}
            />
