...
const valueLabel = (value) => {
  if (value === null || value === undefined || value === '') return 'À renseigner';
  if (value === 'paid') return 'Payé';
  if (value === 'credit') return 'À crédit';
      ...
      {missing.length ? (
        <div className="rounded-2xl border border-red-200 bg-red-50 p-3">
          ...
          <div className="flex flex-wrap gap-2">
            {missing.map((field) => <span key={field} className="rounded-full bg-white border border-red-200 px-2 py-1 text-[11px] font-bold text-red-700">{labels[field] || field}</span>)}
          </div>
        </div>
      ) : null}
