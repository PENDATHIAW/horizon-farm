...
export default function SettingsPanel({ open, onClose, user, displayUser, online, meteo = {}, weatherSource, sidebarOpen, setSidebarOpen, setActive, onSignOut }) {
  ...
  const toggleDemo = (value) => { setDemoMode(value); setDemoEnabled(value); toast.success(value ? 'Mode démo activé' : 'Mode réel activé'); };
  ...
  return <div className="absolute right-3 top-14 z-50 w-[min(94vw,460px)] rounded-2xl border border-[#d6c3a0] bg-white p-4 shadow-2xl">
    ...
    <div className="max-h-[72vh] space-y-3 overflow-y-auto pr-1">
      ...
      <SettingRow icon={Database} title="Données affichées" description={demoEnabled ? 'Données d’exemple visibles.' : 'Données réelles uniquement.'}><Toggle checked={demoEnabled} onChange={toggleDemo} /></SettingRow>
      <SettingRow icon={Moon} title="Thème" description="Changer l’apparence générale."><Select value={settings.theme} onChange={(value) => updateSetting('theme', value)} options={[{ value: 'system', label: 'Système' }, { value: 'light', label: 'Clair' }, { value: 'dark', label: 'Sombre' }]} /></SettingRow>
