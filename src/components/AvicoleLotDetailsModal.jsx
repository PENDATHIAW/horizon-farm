...
function isSaleLikeEvent(event = {}) {
  ...
  return /(vente|vendu|sale|sold|paiement|payment|encaisse|encaiss|revenu|produit|commande|client|chiffre d|ca\b|oeuf|œuf|poulet)/.test(text);
}
...
function linkedFinanceOut(lot = {}, transactions = []) {
  return arr(transactions).filter((tx) => matchLot(tx, lot) && ['sortie', 'expense', 'out', 'charge', 'depense', 'dépense'].includes(clean(tx.type))).reduce((sum, tx) => sum + money(tx), 0);
}
function consolidatedLotFinance({ lot, metrics, layer, active, businessEvents = [], salesOrders = [], payments = [], transactions = [] }) {
  ...
  const saleEvents = events.filter((event) => isSaleLikeEvent(event) || (estimated > 0 && Math.abs(money(event) - estimated) < 1));
...
export default function AvicoleLotDetailsModal({ open, onClose, lot, productionLogs = [], alimentationLogs = [], opportunities = [], salesOrders = [], payments = [], transactions = [], businessEvents = [], onCreateOpportunity, onUpdateOpportunity, onRefreshOpportunities, onCreateBusinessEvent, onRefreshBusinessEvents, onNavigate }) {
  ...
  return <BaseModal open={open} onClose={onClose} title={`Fiche ${layer ? 'pondeuses' : 'poulets de chair'} · ${lot.name || lot.id}`}>
    <div className="space-y-4">
      ...
      <Section title="Situation du lot" note="Lecture opérationnelle. Les coûts et alertes sont calculés depuis les journaux liés, pas saisis ici."><Field label="Effectif initial" value={fmtNumber(lot.initial_count || lot.effectif_initial || 0)} /><Field label="Effectif actif" value={fmtNumber(active)} /><Field label="Morts" value={fmtNumber(deadCount(lot))} /><Field label="Malades / à surveiller" value={fmtNumber(sickCount(lot))} /><Field label="Date entrée" value={lot.date_debut || lot.entry_date || '-'} /><Field label="Phase" value={lot.phase || '-'} /></Section>
      {layer ? <><PondeuseProductionPanel profile={ponteProfile} /><Section title="Objectif ponte vivant" note="Objectif calculé selon âge du lot, effectif actif, historique des ramassages, casses et baisse éventuelle de ponte."><Field label="Objectif initial" value={`${livingTarget.objectiveInitial || 0}%`} /><Field label="Objectif âge" value={`${livingTarget.ageExpectedPct || 0}%`} /><Field label="Objectif vivant" value={`${livingTarget.livingObjectivePct || 0}%`} /><Field label="Taux réel récent" value={`${livingTarget.realLayingPct || 0}%`} /><Field label="Œufs attendus / jour" value={fmtNumber(livingTarget.expectedEggsDay || 0)} /><Field label="Œufs réels / jour" value={fmtNumber(livingTarget.recentDailyEggs || 0)} /><Field label="Écart / jour" value={`${livingTarget.gapEggsDay >= 0 ? '+' : ''}${fmtNumber(livingTarget.gapEggsDay || 0)} œufs`} /><Field label="Action IA" value={livingTarget.action} /></Section></> : <><WeightProjectionPanel title="Objectif poids vivant & vente" projection={growthProjection} /><Section title="Objectif poids vivant chair" note="L’objectif se recalcule après chaque pesée selon le gain moyen réel du lot."><Field label="Poids moyen actuel" value={livingTarget.currentWeight ? `${livingTarget.currentWeight} kg` : 'À renseigner via suivi'} /><Field label="Objectif initial" value={`${livingTarget.defaultTargetWeight || 0} kg`} /><Field label="Objectif vivant" value={`${livingTarget.livingTarget || 0} kg`} /><Field label="Projection J45" value={`${livingTarget.projectedWeight || 0} kg`} /><Field label="Gain réel / jour" value={`${livingTarget.realGainPerDay || 0} kg/j`} /><Field label="Prochaine pesée" value={livingTarget.nextWeighingDate || '-'} /><Field label="Statut" value={livingTarget.status?.replaceAll('_', ' ') || '-'} /><Field label="Action IA" value={livingTarget.action} /></Section></>}
