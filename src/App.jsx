...
const MODULES = {
  ...
  fournisseurs: lazy(() => import('./modules/FournisseursReadable')),
...
export default function App() {
  ...
  const refreshAll = async () => {
    await Promise.allSettled(['animaux','avicole','sante','veterinaires','finances','investissements','business_plans','bp_investment_lines','bp_recurring_costs','bp_revenue_projections','bp_funding_sources','bp_links','bp_risks','stock','clients','fournisseurs','tracabilite','cultures','ventes','documents','taches','rapports','equipements','audit_logs','alimentation_logs','production_oeufs_logs','sensor_devices','camera_devices','business_events','alertes_center','whatsapp_templates','whatsapp_logs','sales_orders','sales_order_items','deliveries','invoices','payments','sales_opportunities'].map((m) => refreshModule(m)));
  };

  const refreshSalesWorkflow = async () => {
    await Promise.allSettled([salesOrdersCrud.refresh(), invoicesCrud.refresh(), paymentsCrud.refresh(), financesCrud.refresh(), clientsCrud.refresh(), stockCrud.refresh(), animauxCrud.refresh(), avicoleCrud.refresh(), culturesCrud.refresh(), documentsCrud.refresh(), businessEventsCrud.refresh(), alertesCenterCrud.refresh()]);
  };

  const decisionDataMap = { animaux: animauxCrud.rows, avicole: avicoleCrud.rows, lots: avicoleCrud.rows, cultures: culturesCrud.rows, stock: stockCrud.rows, stocks: stockCrud.rows, clients: clientsCrud.rows, fournisseurs: fournisseursCrud.rows, investissements: investissementsCrud.rows, business_plans: businessPlansCrud.rows, sales_orders: salesOrdersCrud.rows, salesOrders: salesOrdersCrud.rows, payments: paymentsCrud.rows, finances: financesCrud.rows, transactions: financesCrud.rows, production_oeufs_logs: productionOeufsLogsCrud.rows, productionLogs: productionOeufsLogsCrud.rows, alimentation_logs: alimentationLogsCrud.rows, alimentationLogs: alimentationLogsCrud.rows, market_prices: dataMap.market_prices || [], market_calendar_events: dataMap.market_calendar_events || [], smartfarm_events: dataMap.smartfarm_events || [], meteo: liveMeteo };

  const navItems = useMemo(() => [
    { id: 'dashboard', label: 'Accueil', icon: LayoutDashboard },
    { id: 'assistant_erp', label: 'Assistant ERP', icon: Bot },
    { id: 'centre_ia', label: 'Centre décisionnel', icon: BrainCircuit, hasAlert: stocksCritiques > 0 || lotsAlerte > 0 || financesAlerte > 0 || culturesRisque > 0 },
    { id: 'objectifs_croissance', label: 'Objectifs & Croissance', icon: Goal, hasAlert: financesAlerte > 0 || lotsAlerte > 0 || culturesRisque > 0 },
    { id: 'animaux', label: 'Animaux', icon: Beef },
    { id: 'avicole', label: 'Avicole', icon: Bird },
    { id: 'sante', label: 'Santé & Vaccins', icon: HeartPulse, hasAlert: vaccinsRetard > 0 },
    { id: 'finances', label: 'Finances', icon: DollarSign },
    { id: 'comptabilite', label: 'Comptabilité', icon: Calculator },
    { id: 'investissements', label: 'Investissements', icon: PiggyBank },
    { id: 'impact_business', label: 'Impact & Valeur', icon: Scale, hasAlert: vaccinsRetard > 0 || stocksCritiques > 0 || animauxMalades > 0 || lotsAlerte > 0 },
    { id: 'stock', label: 'Stock', icon: Warehouse, hasAlert: stocksCritiques > 0 },
    { id: 'clients', label: 'Clients', icon: UserRound },
    { id: 'ventes', label: 'Ventes', icon: ShoppingCart },
    { id: 'fournisseurs', label: 'Fournisseurs', icon: Handshake },
    { id: 'tracabilite', label: 'Traçabilité', icon: GitBranch },
    { id: 'alertes', label: 'Alertes', icon: Bell, hasAlert: (dataMap.alertes_center || []).filter((a) => a.status === 'nouvelle').length > 0 },
    { id: 'cultures', label: 'Cultures', icon: Sprout, hasAlert: culturesRisque > 0 },
    { id: 'documents', label: 'Documents', icon: FolderOpen },
    { id: 'taches', label: 'Tâches', icon: ClipboardList },
    { id: 'rh', label: 'RH & Équipe', icon: UserCog },
    { id: 'rapports', label: 'Rapports', icon: BarChart3 },
    { id: 'equipements', label: 'Équipements', icon: Wrench },
    { id: 'smartfarm', label: 'Smart Farm', icon: Tractor },
    { id: 'sync_activity', label: 'Activité & Sync ERP', icon: Wifi, hasAlert: !online },
    { id: 'gestion_systeme', label: 'Gestion du système', icon: Settings },
  ...
  const syncActivityProps = { onRefreshAll: refreshAll, onFlushOffline: flushOfflineQueue, online, lastOnlineAt, dataMap, auditLogs: auditLogsCrud.rows, auditLoading: auditLogsCrud.loading, onRefreshAuditLogs: auditLogsCrud.refresh, onNavigate: setActive, onCreateTask: tachesCrud.create, onRefreshTasks: tachesCrud.refresh, onCreateAlert: alertesCenterCrud.create, onRefreshAlertes: alertesCenterCrud.refresh, onUpdateSalesOrder: salesOrdersCrud.update, onRefreshSalesOrders: salesOrdersCrud.refresh, onUpdateOpportunity: salesOpportunitiesCrud.update, onRefreshOpportunities: salesOpportunitiesCrud.refresh, onCreateDocument: documentsCrud.create, onRefreshDocuments: documentsCrud.refresh, onCreateBusinessEvent: businessEventsCrud.create, onRefreshBusinessEvents: businessEventsCrud.refresh };
  ...
  const moduleProps = {
    ...
    centre_ia: { lots: avicoleCrud.rows, productionLogs: productionOeufsLogsCrud.rows, alimentationLogs: alimentationLogsCrud.rows, stocks: stockCrud.rows, marketPrices: dataMap.market_prices || [], marketCalendarEvents: dataMap.market_calendar_events || [], salesOrders: salesOrdersCrud.rows, payments: paymentsCrud.rows, transactions: financesCrud.rows, smartfarmEvents: dataMap.smartfarm_events || [], sensors: sensorDevicesCrud.rows, cameras: cameraDevicesCrud.rows, meteo: liveMeteo, dataMap: decisionDataMap, onNavigate: setActive },
    objectifs_croissance: { dataMap: decisionDataMap, onNavigate: setActive, onCreateBusinessPlan: businessPlansCrud.create, onRefreshBusinessPlans: businessPlansCrud.refresh },
    ...
    finances: { rows: financesCrud.rows, loading: financesCrud.loading, onCreate: financesCrud.create, onUpdate: financesCrud.update, onDelete: financesCrud.remove, onRefresh: financesCrud.refresh, animaux: animauxCrud.rows, lots: avicoleCrud.rows, cultures: culturesCrud.rows, stocks: stockCrud.rows, investissements: investissementsCrud.rows, clients: clientsCrud.rows, fournisseurs: fournisseursCrud.rows, alimentationLogs: alimentationLogsCrud.rows, businessPlans: businessPlansCrud.rows, salesOrders: salesOrdersCrud.rows, payments: paymentsCrud.rows, onCreateBusinessEvent: businessEventsCrud.create, onRefreshBusinessEvents: businessEventsCrud.refresh, onNavigate: setActive },
    ...
    clients: { rows: clientsCrud.rows, loading: clientsCrud.loading, salesOrders: salesOrdersCrud.rows, payments: paymentsCrud.rows, transactions: financesCrud.rows, onCreate: clientsCrud.create, onUpdate: clientsCrud.update, onDelete: clientsCrud.remove, onRefresh: clientsCrud.refresh, onNavigate: setActive },
    fournisseurs: { rows: fournisseursCrud.rows, stocks: stockCrud.rows, tasks: tachesCrud.rows, transactions: financesCrud.rows, finances: financesCrud.rows, documents: documentsCrud.rows, loading: fournisseursCrud.loading, onCreate: fournisseursCrud.create, onUpdate: fournisseursCrud.update, onDelete: fournisseursCrud.remove, onRefresh: fournisseursCrud.refresh, onUpdateStock: stockCrud.update, onRefreshStock: stockCrud.refresh, onCreateTask: tachesCrud.create, onRefreshTasks: tachesCrud.refresh, onCreateAlert: alertesCenterCrud.create, onRefreshAlertes: alertesCenterCrud.refresh, onCreateBusinessEvent: businessEventsCrud.create, onRefreshBusinessEvents: businessEventsCrud.refresh, onNavigate: setActive },
    ...
    cultures: { rows: culturesCrud.rows, transactions: financesCrud.rows, opportunities: salesOpportunitiesCrud.rows, loading: culturesCrud.loading, onCreate: culturesCrud.create, onUpdate: culturesCrud.update, onDelete: culturesCrud.remove, onRefresh: culturesCrud.refresh, onCreateOpportunity: salesOpportunitiesCrud.create, onUpdateOpportunity: salesOpportunitiesCrud.update, onRefreshOpportunities: salesOpportunitiesCrud.refresh, onCreateBusinessEvent: businessEventsCrud.create, onRefreshBusinessEvents: businessEventsCrud.refresh, onNavigate: setActive },
    ventes: { rows: salesOrdersCrud.rows, orderItems: salesOrderItemsCrud.rows, deliveriesList: deliveriesCrud.rows, invoicesList: invoicesCrud.rows, paymentsList: paymentsCrud.rows, opportunities: salesOpportunitiesCrud.rows, animaux: animauxCrud.rows, lots: avicoleCrud.rows, cultures: culturesCrud.rows, stocks: stockCrud.rows, clients: clientsCrud.rows, transactions: financesCrud.rows, businessEvents: businessEventsCrud.rows, documents: documentsCrud.rows, alertes: alertesCenterCrud.rows, loading: salesOrdersCrud.loading, onCreate: salesOrdersCrud.create, onUpdate: salesOrdersCrud.update, onDelete: salesOrdersCrud.remove, onRefresh: salesOrdersCrud.refresh, onRefreshWorkflow: refreshSalesWorkflow, onRefreshOpportunities: salesOpportunitiesCrud.refresh, onCreateItem: salesOrderItemsCrud.create, onUpdateItem: salesOrderItemsCrud.update, onDeleteItem: salesOrderItemsCrud.remove, onCreateDelivery: deliveriesCrud.create, onUpdateDelivery: deliveriesCrud.update, onDeleteDelivery: deliveriesCrud.remove, onRefreshDeliveries: deliveriesCrud.refresh, onCreateInvoice: invoicesCrud.create, onUpdateInvoice: invoicesCrud.update, onDeleteInvoice: invoicesCrud.remove, onRefreshInvoices: invoicesCrud.refresh, onCreatePayment: paymentsCrud.create, onUpdatePayment: paymentsCrud.update, onDeletePayment: paymentsCrud.remove, onRefreshPayments: paymentsCrud.refresh, onCreateOpportunity: salesOpportunitiesCrud.create, onUpdateOpportunity: salesOpportunitiesCrud.update, onDeleteOpportunity: salesOpportunitiesCrud.remove, onUpdateAnimal: animauxCrud.update, onRefreshAnimals: animauxCrud.refresh, onUpdateLot: avicoleCrud.update, onRefreshLots: avicoleCrud.refresh, onUpdateCulture: culturesCrud.update, onRefreshCultures: culturesCrud.refresh, onUpdateStock: stockCrud.update, onRefreshStocks: stockCrud.refresh, onCreateFinanceTransaction: financesCrud.create, onRefreshFinances: financesCrud.refresh, onCreateTrace: tracabiliteCrud.create, onCreateBusinessEvent: businessEventsCrud.create, onRefreshBusinessEvents: businessEventsCrud.refresh, onCreateDocument: documentsCrud.create, onRefreshDocuments: documentsCrud.refresh, onCreateAlert: alertesCenterCrud.create, onRefreshAlertes: alertesCenterCrud.refresh, onUpdateClient: clientsCrud.update, onNavigate: setActive },
    ...
    taches: { rows: tachesCrud.rows, alertes: alertesCenterCrud.rows, animaux: animauxCrud.rows, lots: avicoleCrud.rows, stocks: stockCrud.rows, sensorDevices: sensorDevicesCrud.rows, loading: tachesCrud.loading, onCreate: tachesCrud.create, onUpdate: tachesCrud.update, onDelete: tachesCrud.remove, onRefresh: tachesCrud.refresh, onUpdateAlert: alertesCenterCrud.update, onRefreshAlertes: alertesCenterCrud.refresh, onCreateBusinessEvent: businessEventsCrud.create, onRefreshBusinessEvents: businessEventsCrud.refresh },
    ...
    gestion_systeme: {}, sync: syncActivityProps, sync_activity: syncActivityProps,
  };
