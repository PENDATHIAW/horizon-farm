...
export default function App() {
  ...
  const [syncActivityTab, setSyncActivityTab] = useState('Vérifications');
  ...
  const navigateModule = useCallback((moduleId, options = {}) => {
    ...
    if (resolved === 'financements') {
      if (tab) setFinancementsTab(tab);
      trackNavOpen('financements');
      setActiveState('financements');
      return;
    }
    ...
    if (resolved === 'sync_activity' || resolved === 'audit_logs' || resolved === 'sync') {
      if (tab) setSyncActivityTab(tab);
      trackNavOpen('sync_activity');
      setActiveState('sync_activity');
      return;
    }
    if (resolved === 'centre_ia' || resolved === 'objectifs_croissance') {
      if (tab) {
        if (resolved === 'centre_ia') setCentreTab(resolveCentreTab(tab));
        else setObjectifsTab(tab || defaultTabForLegacyModule(moduleId) || 'Suivi du Business Plan');
      }
      if (options?.productionQuestion) {
        window.setTimeout(() => {
          window.dispatchEvent(new CustomEvent('horizon-production-question', {
  ...
  return {
    ...
    finance_pilotage: {
      ...
      onTabChange: (nextTab) => setFinanceTab(nextTab),
      transactions: rows(c.finances),
      transactionsAll: rows(c.finances),
      finances: rows(c.finances),
      documents: rows(c.documents),
    ...
    avicole: { ...base('avicole'), transactions: rows(c.finances), fournisseurs: rows(c.fournisseurs), tasks: rows(c.taches), alertes: rows(c.alertes_center), documents: rows(c.documents), bpFundingSources: rows(c.bp_funding_sources), alimentationLogs: rows(c.alimentation_logs), productionLogs: rows(c.production_oeufs_logs), onCreateProduction: c.production_oeufs_logs.create, onUpdateProduction: c.production_oeufs_logs.update, onDeleteProduction: c.production_oeufs_logs.remove, onRefreshProduction: c.production_oeufs_logs.refresh, onCreateTask: c.taches.create, onRefreshTasks: c.taches.refresh, onCreateAlert: c.alertes_center.create, onRefreshAlertes: c.alertes_center.refresh, onCreateFinanceTransaction: c.finances.create, onRefreshFinances: c.finances.refresh, onCreateDocument: c.documents.create, onRefreshDocuments: c.documents.refresh, opportunities: rows(c.sales_opportunities), onCreateOpportunity: c.sales_opportunities.create, onUpdateOpportunity: c.sales_opportunities.update, onRefreshOpportunities: c.sales_opportunities.refresh, existingTasks: rows(c.taches), existingAlerts: rows(c.alertes_center), ...shared },
    ...
    financements: { initialTab: financementsTab, onTabChange: (nextTab) => setFinancementsTab(nextTab), dataMap: decisionDataMapRaw, crud: c, meteo: liveMeteo, documents: rows(c.documents), animaux: rows(c.animaux), lots: rows(c.avicole), cultures: rows(c.cultures), stocks: rows(c.stock), clients: rows(c.clients), fournisseurs: rows(c.fournisseurs), salesOrders: rows(c.sales_orders), salesOrdersAll: rows(c.sales_orders), payments: rows(c.payments), paymentsAll: rows(c.payments), transactions: rows(c.finances), transactionsAll: rows(c.finances), businessPlans: rows(c.business_plans), investissements: rows(c.investissements), businessEvents: rows(c.business_events), alertes: rows(c.alertes_center), rapports: rows(c.rapports), auditLogs: rows(c.audit_logs), onNavigate: setActive, onCreateDocument: c.documents.create, onRefreshDocuments: c.documents.refresh, ...shared },
    ...
    audit_logs: { ...syncActivityProps, initialTab: syncActivityTab, onTabChange: (nextTab) => setSyncActivityTab(nextTab) },
    smartfarm: {
      ...
      sensors: rows(c.sensor_devices),
      cameras: rows(c.camera_devices),
      tasks: rows(c.taches),
      alertes: rows(c.alertes_center),
      dataMap,
      sensorLoading: c.sensor_devices.loading,
      cameraLoading: c.camera_devices.loading,
      onCreateSensor: c.sensor_devices.create,
      onUpdateSensor: c.sensor_devices.update,
      onDeleteSensor: c.sensor_devices.remove,
      onRefreshSensors: c.sensor_devices.refresh,
      onCreateCamera: c.camera_devices.create,
      onUpdateCamera: c.camera_devices.update,
      onDeleteCamera: c.camera_devices.remove,
      onRefreshCameras: c.camera_devices.refresh,
      onCreateSmartfarmEvent: c.smartfarm_events.create,
      onUpdateSmartfarmEvent: c.smartfarm_events.update,
      onRefreshSmartfarmEvents: c.smartfarm_events.refresh,
      onCreateTask: c.taches.create,
      onRefreshTasks: c.taches.refresh,
      onCreateAlert: c.alertes_center.create,
      onRefreshAlertes: c.alertes_center.refresh,
    ...
    gestion_systeme: {
      ...
      onTabChange: (nextTab) => setGestionSystemeTab(nextTab),
  ...
  }, [c, user, liveMeteo, decisionDataMapRaw, crudFingerprint, centreTab, objectifsTab, commercialTab, elevageTab, agriFeedsTab, culturesTab, achatsStockTab, achatsStockContext, financeTab, activiteSuiviTab, documentsRapportsTab, rhTab, gestionSystemeTab, financementsTab, smartfarmTab, syncActivityTab, farmsPanelAction, effectiveAccessibleFarms, refreshAccessibleFarms, online, lastOnlineAt, dataMap, refreshAll, refreshSalesWorkflowFn, navigateModule, setActive, flushOfflineQueue, handleManageFarms, farmComparisonData, openAssistantWithQuery, base, weatherLoading]);
