...
export default function App() {
  ...
  const moduleProps = {
    ...
    elevage: { animaux: rows(c.animaux), lots: rows(c.avicole), sante: rows(c.sante), stocks: rows(c.stock), alimentationLogs: rows(c.alimentation_logs), businessEvents: rows(c.business_events), productionLogs: rows(c.production_oeufs_logs), onNavigate: setActive },
    commercial: { clients: rows(c.clients), salesOrders: rows(c.sales_orders), payments: rows(c.payments), invoices: rows(c.invoices), deliveries: rows(c.deliveries), opportunities: rows(c.sales_opportunities), animals: rows(c.animaux), lots: rows(c.avicole), cultures: rows(c.cultures), stocks: rows(c.stock), businessEvents: rows(c.business_events), onNavigate: setActive },
    achats_stock: { stocks: rows(c.stock), fournisseurs: rows(c.fournisseurs), suppliers: rows(c.fournisseurs), transactions: rows(c.finances), finances: rows(c.finances), alimentationLogs: rows(c.alimentation_logs), businessEvents: rows(c.business_events), alertes: rows(c.alertes_center), documents: rows(c.documents) },
    finance_pilotage: { transactions: rows(c.finances), finances: rows(c.finances), documents: rows(c.documents), investissements: rows(c.investissements), businessPlans: rows(c.business_plans), bpRecurringCosts: rows(c.bp_recurring_costs), bpRevenueProjections: rows(c.bp_revenue_projections), bpRisks: rows(c.bp_risks), payments: rows(c.payments), salesOrders: rows(c.sales_orders), fournisseurs: rows(c.fournisseurs), clients: rows(c.clients) },
    activite_suivi: { alertes: rows(c.alertes_center), taches: rows(c.taches), tasks: rows(c.taches), tracabilite: rows(c.tracabilite), businessEvents: rows(c.business_events), auditLogs: rows(c.audit_logs), onNavigate: setActive, onCreateTask: c.taches.create, onUpdateTask: c.taches.update, onRefreshTasks: c.taches.refresh, onCreateAlert: c.alertes_center.create, onUpdateAlert: c.alertes_center.update, onRefreshAlertes: c.alertes_center.refresh, onCreateBusinessEvent: c.business_events.create, onRefreshBusinessEvents: c.business_events.refresh },
    documents_rapports: { documents: rows(c.documents), rapports: rows(c.rapports), reports: rows(c.rapports), transactions: rows(c.finances), finances: rows(c.finances), salesOrders: rows(c.sales_orders), payments: rows(c.payments), animaux: rows(c.animaux), lots: rows(c.avicole), cultures: rows(c.cultures), stocks: rows(c.stock), clients: rows(c.clients), fournisseurs: rows(c.fournisseurs), businessPlans: rows(c.business_plans), investissements: rows(c.investissements), businessEvents: rows(c.business_events), onNavigate: setActive },
