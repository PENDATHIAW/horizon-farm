...
export default function App() {
  ...
  useEffect(() => {
    if (!user) return;
    const raw = sessionStorage.getItem('horizon_post_login_module');
    if (!raw) return;
    sessionStorage.removeItem('horizon_post_login_module');
    try {
      const parsed = JSON.parse(raw);
      if (parsed?.moduleId) navigateModule(parsed.moduleId, parsed.options || {});
    } catch {
      navigateModule(raw);
    }
  }, [user, navigateModule]);
