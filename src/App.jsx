...
export default function App() {
  ...
  return {
    ...
    comptabilite: { transactions: rows(c.finances), finances: rows(c.finances), salesOrders: rows(c.sales_orders), payments: rows(c.payments), clients: rows(c.clients), fournisseurs: rows(c.fournisseurs), stocks: rows(c.stock), animaux: rows(c.animaux), lots: rows(c.avicole), cultures: rows(c.cultures), sante: rows(c.sante), alimentationLogs: rows(c.alimentation_logs), businessEvents: rows(c.business_events), investissements: rows(c.investissements), equipements: rows(c.equipements), documents: rows(c.documents), onCreateFinanceTransaction: c.finances.create, onRefreshFinances: c.finances.refresh, onNavigate: setActive },
    ...
    investisseurs_forums: { crud: c, dataMap: decisionDataMapRaw, meteo: liveMeteo, periodLabel: formatPeriodScopeLabel(periodScope), animaux: rows(c.animaux), lots: rows(c.avicole), avicole: rows(c.avicole), cultures: rows(c.cultures), stocks: rows(c.stock), stock: rows(c.stock), clients: rows(c.clients), fournisseurs: rows(c.fournisseurs), salesOrders: rows(c.sales_orders), salesOrdersAll: rows(c.sales_orders), payments: rows(c.payments), paymentsAll: rows(c.payments), transactions: rows(c.finances), transactionsAll: rows(c.finances), documents: rows(c.documents), businessPlans: rows(c.business_plans), investissements: rows(c.investissements), alertes: rows(c.alertes_center), taches: rows(c.taches), sante: rows(c.sante), onCreateDocument: c.documents.create, onRefreshDocuments: c.documents.refresh, onNavigate: setActive, ...shared },
  ...
  return <AppLayout navItems={navItems} active={active} onNavigate={setActive} sidebarOpen={sidebarOpen} setSidebarOpen={setSidebarOpen} user={user} signOut={signOut} online={online} notifs={notifs} weather={liveMeteo} weatherLoading={weatherLoading} weatherSource={weatherSource} onOpenAssistant={() => setAssistantOpen(true)} periodScope={periodScope} onPeriodScopeChange={handlePeriodScopeChange}>
    <ErrorBoundary moduleName={active} resetKey={active} onBackToDashboard={() => setActive('dashboard')}><Suspense fallback={<div className="rounded-3xl border border-[#d6c3a0] bg-white p-6 text-[#8a7456]">Chargement du module...</div>}><ActiveModule {...activeModuleProps} periodLabel={periodLabel} /></Suspense></ErrorBoundary>
