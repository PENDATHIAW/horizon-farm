...
export default function Animaux({ rows = [], alimentationLogs = [], vaccins = [], loading, onCreate, onUpdate, onDelete, onRefresh }) {
  ...
  const filtered = useMemo(() => activityRows.filter((a) => { const passStatus = statusFilter === 'tous' || (a.status || 'actif') === statusFilter; const passHealth = healthFilter === 'tous' || (a.health_status || 'sain') === healthFilter; const q = localSearch.trim().toLowerCase(); const passSearch = !q || String(a.name || '').toLowerCase().includes(q) || String(a.id || '').toLowerCase().includes(q); return passStatus && passHealth && passSearch; }), [activityRows, statusFilter, healthFilter, localSearch]);
  ...
  const applyQuickFilter = (kind) => { setHealthFilter('tous'); if (kind === 'actifs') setStatusFilter('actif'); if (kind === 'prets') setStatusFilter('pret_a_la_vente'); if (kind === 'malades') { setStatusFilter('tous'); setHealthFilter('malade'); } if (kind === 'vendus') setStatusFilter('vendu'); if (kind === 'pertes') setStatusFilter('mort'); };
