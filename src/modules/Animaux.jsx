...
export default function Animaux({ rows = [], alimentationLogs = [], vaccins = [], loading, onCreate, onUpdate, onDelete, onRefresh }) {
  ...
  const initialAnimal = useMemo(() => {
    ...
    return { id, tag: id, type: 'Bovin', status: 'actif', health_status: 'sain', mode_acquisition: 'achat', date_achat: today, date_entree_ferme: today, date_poids_entree: today, date_derniere_pesee: today, sexe: 'F', en_gestation: false, statut_reproduction: 'inconnu', marge_cible_pct: 25, sale_price: 0 };
  }, [rows]);
  ...
  const preparePayload = (payload) => {
    ...
    return {
      ...
      pret_vente_recommande: readiness.recommended || Boolean(payload.pret_vente_recommande),
      raison_pret_vente: readiness.reason,
      status: readiness.recommended && (payload.status || 'actif') === 'actif' ? 'pret_a_la_vente' : payload.status || 'actif',
    };
  };
  ...
  const kpis = {
    ...
    prets: rows.filter((a) => a.status === 'pret_a_la_vente' || a.pret_vente_recommande || getAnimalSaleReadiness({ animal: a, metrics: metricsFor(a) }).recommended).length,
    malades: rows.filter((a) => a.health_status === 'malade').length,
    vendus: rows.filter((a) => a.status === 'vendu').length,
    pertes: rows.filter((a) => ['mort', 'vole'].includes(a.status)).length,
  };
  ...
  const exportRows = () => { exportToCsv({ rows: filtered, columns: ['id', 'tag', 'name', 'type', 'sexe', 'poids', 'poids_objectif', 'prix_vente_estime_auto', 'prix_minimum_acceptable', 'sale_readiness_status', 'status'], fileName: 'animaux.csv' }); exportToExcel({ rows: filtered, fileName: 'animaux.xlsx', sheetName: 'Animaux' }); exportToPdf({ rows: filtered, columns: ['id', 'tag', 'name', 'type', 'sexe', 'poids', 'status'], fileName: 'animaux.pdf', title: 'Liste des animaux' }); toast.success('Exports CSV/Excel/PDF generes'); };
  ...
  const columns = [
    ...
    { key: 'marge', label: 'Marge reelle', render: (a) => { const { margin } = metricsFor(a); if (margin === null) return <span className="text-[#b39b78]">Non vendu</span>; return <span className={margin >= 0 ? 'text-emerald-400 font-semibold' : 'text-red-400 font-semibold'}>{fmtCurrency(margin)}</span>; } },
  ...
  return (
    <div className="space-y-6">
      <SectionHeader title="Gestion des Animaux" sub="Croissance, objectif de vente, sante et rentabilite" actions={<><Btn icon={RefreshCw} variant="outline" small onClick={onRefresh}>Actualiser</Btn><Btn icon={Download} variant="outline" small onClick={exportRows}>Exporter</Btn><Btn icon={Plus} small onClick={() => setModal('create')}>Ajouter animal</Btn></>} />
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4"><KpiCard icon={CheckCircle} label="Actifs" value={kpis.actifs} color="bg-emerald-500/20 text-emerald-400" /><KpiCard icon={Tag} label="Prets vente" value={kpis.prets} color="bg-amber-500/20 text-amber-400" /><KpiCard icon={AlertTriangle} label="Malades" value={kpis.malades} color="bg-red-500/20 text-red-400" /><KpiCard icon={Tag} label="Vendus" value={kpis.vendus} color="bg-sky-500/20 text-sky-400" /><KpiCard icon={XCircle} label="Pertes" value={kpis.pertes} color="bg-zinc-700/30 text-zinc-300" /></div>
