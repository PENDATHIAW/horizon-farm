...
const paid = (row = {}) => toNumber(row.montant_paye ?? row.paid_amount ?? row.amount_paid ?? row.paid);
const remaining = (row = {}) => Math.max(0, toNumber(row.reste_a_payer ?? row.remaining_amount ?? (amount(row) - paid(row))));
...
function buildAnswers(dataMap = {}) {
  ...
  const sales = arr(dataMap.sales_orders || dataMap.salesOrders || dataMap.ventes);
  ...
  const unpaid = sales.filter((sale) => remaining(sale) > 0);
  ...
  return [
    ...
    { key: 'sales', icon: ShoppingCart, module: 'ventes', q: 'Qui me doit de l’argent ?', a: unpaid.length ? `${unpaid.length} vente(s) avec reste à payer, total estimé ${fmtCurrency(unpaid.reduce((s, row) => s + remaining(row), 0))}.` : 'Aucune créance visible sur les ventes.', value: unpaid.length, danger: unpaid.length > 0 },
...
export default function AssistantERPQuickAnswers({ dataMap = {}, onNavigate }) {
  ...
  return <section className="rounded-3xl border border-[#d6c3a0] bg-white p-5 shadow-sm space-y-4"><div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-3"><div><p className="inline-flex items-center gap-2 rounded-full border border-[#eadcc2] bg-[#fffdf8] px-3 py-1 text-xs font-black text-[#8a7456]"><Bot size={14} /> Questions rapides</p><h3 className="mt-3 text-xl font-black text-[#2f2415]">Demande métier sans jargon</h3><p className="mt-1 text-sm text-[#8a7456]">Réponses directes à partir des données ERP déjà saisies.</p></div>{selected ? <button type="button" onClick={() => onNavigate?.(selected.module)} className="rounded-xl bg-[#2f2415] px-4 py-2 text-sm font-black text-white">Ouvrir {selected.module}</button> : null}</div><div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">{answers.map((item) => <AnswerCard key={item.key} item={item} active={selected?.key === item.key} onClick={() => setSelectedKey(item.key)} />)}</div>{selected ? <div className={`rounded-2xl border p-4 ${selected.danger ? 'border-amber-200 bg-amber-50 text-amber-900' : 'border-emerald-200 bg-emerald-50 text-emerald-800'}`}><p className="text-xs uppercase tracking-widest font-black opacity-80">Réponse</p><p className="mt-1 text-lg font-black">{selected.q}</p><p className="mt-2 text-sm leading-relaxed">{selected.a}</p></div> : null}</section>;
}
