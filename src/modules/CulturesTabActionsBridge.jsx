...
function Modal({ title, values, setValues, fields, onClose, onSubmit, saving }) {
  ...
  return <div className="fixed inset-0 z-[80] bg-earth/30 p-4 flex items-center justify-center"><div className="w-full max-w-xl rounded-2xl bg-card border border-line shadow-float overflow-hidden"><div className="p-6 border-b border-line flex items-start justify-between gap-3"><div><p className="text-xs uppercase tracking-normal text-slate">Fiche cultures</p><h3 className="text-xl font-semibold text-earth">{title}</h3></div><button type="button" onClick={onClose} className="text-slate font-semibold">×</button></div><div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-3 max-h-[70vh] overflow-y-auto">{fields.map((field) => <label key={field.key} className={`space-y-1 ${field.full ? 'md:col-span-2' : ''}`}><span className="text-xs text-slate">{field.label}</span>{field.type === 'select' ? <select className="w-full rounded-lg border border-line bg-white px-3 py-2 text-sm" value={values[field.key] || ''} onChange={(e) => set(field.key, e.target.value)}><option value="">Choisir</option>{(field.options || []).map((item) => <option key={item.value || item} value={item.value || item}>{item.label || item}</option>)}</select> : field.type === 'textarea' ? <textarea rows={3} className="w-full rounded-lg border border-line bg-white px-3 py-2 text-sm" value={values[field.key] || ''} onChange={(e) => set(field.key, e.target.value)} /> : <input type={field.type || 'text'} className="w-full rounded-lg border border-line bg-white px-3 py-2 text-sm" value={values[field.key] || ''} onChange={(e) => set(field.key, e.target.value)} />}</label>)}</div><div className="p-4 border-t border-line flex justify-end gap-2"><button type="button" className="px-4 py-2 rounded-xl border border-line" onClick={onClose}>Annuler</button><button type="button" disabled={saving} className="px-4 py-2 rounded-xl bg-horizon text-white font-semibold disabled:opacity-60" onClick={onSubmit}>{saving ? 'Enregistrement...' : 'Enregistrer'}</button></div></div></div>;
}
function ActionCard({ icon: Icon, title, desc, onClick }) { return <button type="button" onClick={onClick} className="rounded-xl border border-line bg-card p-3 text-left hover:border-horizon"><Icon size={16} className="text-horizon-dark" /><p className="font-semibold text-earth mt-2">{title}</p><p className="text-xs text-slate mt-1">{desc}</p></button>; }
