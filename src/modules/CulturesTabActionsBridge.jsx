...
import { buildCultureInputUsageWorkflow, buildCultureLossWorkflow, cultureAvailableQty, cultureOpportunityKey, findCultureOpportunity } from '../utils/cultureWorkflows';
...
export default function CulturesTabActionsBridge({ tab, rows = [], stocks = [], opportunities = [], onCreate, onUpdate, onDelete, onRefresh, onCreateOpportunity, onUpdateOpportunity, onRefreshOpportunities, onUpdateStock, onRefreshStock, onCreateBusinessEvent, onRefreshBusinessEvents }) {
  ...
  const saveHarvest = async () => {
    ...
    try { setSaving(true); await onUpdate?.(culture.id, { quantite_recoltee: toNumber(culture.quantite_recoltee) + qty, quantite_disponible: toNumber(culture.quantite_disponible) + qty, unite_recolte: unit, prix_vente_unitaire: price, prix_vente_kg: price, prix_vente_estime: price || culture.prix_vente_estime, revenu_estime: toNumber(culture.revenu_estime) || revenue, revenu_reel: toNumber(culture.revenu_reel) || revenue, statut: 'recolte', date_recolte: today(), last_harvest_at: now(), notes_recolte: values.notes || culture.notes_recolte || '' }); await Promise.allSettled([onRefresh?.(), onRefreshBusinessEvents?.(), onRefreshOpportunities?.(), onRefreshStock?.()]); toast.success('Récolte enregistrée — stock et opportunité synchronisés'); setModal(''); } catch (error) { toast.error(error.message || 'Récolte impossible'); } finally { setSaving(false); }
  };
  ...
  const createOpportunity = async (row) => {
    ...
    try { setSaving(true); const key = cultureOpportunityKey(row); const existing = findCultureOpportunity(opportunities, row); const unitPrice = unitPriceOf(row); const payload = { opportunity_key: key, dedupe_key: key, source_module: 'cultures', created_from: 'cultures', source_type: 'recolte_culture', entity_type: 'culture', source_id: row.id, entity_id: row.id, culture_id: row.id, title: `Récolte vendable: ${cultureName(row)}`, product_name: `${cultureName(row)} · ${parcelName(row)}`, quantity: qty, quantite: qty, unit: unitOf(row), unite: unitOf(row), unit_price: unitPrice, prix_unitaire: unitPrice, estimated_amount: qty * unitPrice, montant_estime: qty * unitPrice, status: 'ouverte', statut: 'ouverte', notes: `Récolte disponible ${fmtNumber(qty)} ${unitOf(row)}`, updated_at: now() }; if (existing?.id && onUpdateOpportunity) await onUpdateOpportunity(existing.id, payload); else await onCreateOpportunity?.({ id: makeId('OPP'), ...payload, created_at: now(), side_effects_managed: true }); await onUpdate?.(row.id, { vendable: true, pret_a_la_vente: true, ready_for_sale: true, sale_ready: true, sale_ready_confirmed_at: row.sale_ready_confirmed_at || now(), last_sale_opportunity_at: now() }); await Promise.allSettled([onRefresh?.(), onRefreshOpportunities?.(), onRefreshBusinessEvents?.()]); toast.success(existing ? 'Opportunité mise à jour' : 'Opportunité créée'); } catch (error) { toast.error(error.message || 'Opportunité impossible'); } finally { setSaving(false); }
  };
