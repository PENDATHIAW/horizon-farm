...
export default function FinancingFinancialStatementGuide({ data = {}, onNavigate }) {
  ...
  return <section className="rounded-3xl border border-line bg-white p-6 shadow-card space-y-4">
    <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-xs uppercase tracking-normal text-slate font-semibold flex items-center gap-2"><Landmark size={15} /> Aide avant dossier financeur</p><h3 className="text-xl font-semibold text-earth mt-1">Prévisionnels à vérifier avant export</h3><p className="text-sm text-slate mt-1">Ce panneau sert seulement à relire les chiffres clés. Le vrai PDF se génère plus bas dans “Modifier le brouillon”.</p></div><button type="button" onClick={() => onNavigate?.('objectifs_croissance')} className="rounded-xl bg-earth px-4 py-2 text-sm font-semibold text-white">Voir BP</button></div>
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-4"><div className="rounded-2xl border border-line bg-card p-4"><p className="font-semibold text-earth flex items-center gap-2"><Calculator size={16} /> Compte de résultat prévisionnel 3 ans</p><div className="mt-3 overflow-x-auto"><table className="min-w-full text-xs"><thead><tr className="bg-earth text-white"><th className="p-2 text-left">Année</th><th className="p-2 text-right">CA</th><th className="p-2 text-right">Charges</th><th className="p-2 text-right">Marge</th><th className="p-2 text-right">Résultat net</th></tr></thead><tbody>{years.map((row) => <tr key={row.year} className="border-b border-line"><td className="p-2 font-semibold">A{row.year}</td><td className="p-2 text-right">{money(row.revenue)}</td><td className="p-2 text-right">{money(row.charges)}</td><td className="p-2 text-right">{money(row.margin)}</td><td className="p-2 text-right">{money(row.net)}</td></tr>)}</tbody></table></div></div><div className="rounded-2xl border border-line bg-card p-4"><p className="font-semibold text-earth flex items-center gap-2"><Table2 size={16} /> Trésorerie mensuelle année 1</p><div className="mt-3 overflow-x-auto"><table className="min-w-full text-xs"><thead><tr className="bg-earth text-white"><th className="p-2 text-left">Mois</th><th className="p-2 text-right">Entrées</th><th className="p-2 text-right">Sorties</th><th className="p-2 text-right">Solde</th></tr></thead><tbody>{months.map((row) => <tr key={row.month} className="border-b border-line"><td className="p-2 font-semibold">M{row.month}</td><td className="p-2 text-right">{money(row.inflow)}</td><td className="p-2 text-right">{money(row.outflow)}</td><td className="p-2 text-right">{money(row.balance)}</td></tr>)}</tbody></table></div></div></div>
    <div className="rounded-2xl border border-positive bg-positive-bg p-4 text-sm text-positive"><b>À reprendre dans le brouillon :</b> montant demandé clair, plan de remboursement 24–36 mois, différé éventuel, hypothèses de prix, devis fournisseurs, emplois créés, site/localisation, photos et captures ERP. L’ERP sert de preuve de pilotage : ventes, dépenses, justificatifs, stock, alertes, tâches et reporting financeur.</div>
  </section>;
}
