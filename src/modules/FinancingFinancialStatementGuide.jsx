...
export default function FinancingFinancialStatementGuide({ data = {}, onNavigate }) {
  ...
  return <section className="rounded-3xl border border-[#d6c3a0] bg-white p-5 shadow-sm space-y-4">
    <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-xs uppercase tracking-widest text-[#8a7456] font-black flex items-center gap-2"><Landmark size={15} /> Dossier banque / partenaire</p><h3 className="text-xl font-black text-[#2f2415] mt-1">États financiers à joindre</h3><p className="text-sm text-[#8a7456] mt-1">Pour un projet nouveau, on joint un prévisionnel solide plutôt que des états historiques inexistants.</p></div><button type="button" onClick={() => onNavigate?.('objectifs_croissance')} className="rounded-xl bg-[#2f2415] px-4 py-2 text-sm font-black text-white">Voir BP</button></div>
