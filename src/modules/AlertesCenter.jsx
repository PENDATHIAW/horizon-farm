...
const MODULE_BADGE = { animaux: 'bg-vigilance-bg text-horizon-dark', avicole: 'bg-vigilance-bg text-horizon-dark', cultures: 'bg-positive-bg text-positive', stock: 'bg-neutral-bg text-neutral', finances: 'bg-neutral-bg text-neutral', clients: 'bg-neutral-bg text-neutral', smartfarm: 'bg-neutral-bg text-neutral', equipements: 'bg-vigilance-bg text-horizon-dark', autre: 'bg-neutral-bg text-neutral' };
...
function Filter({ label, values, active, setActive }) {
  return <div className="flex flex-wrap gap-2"><span className="text-xs text-slate self-center">{label}:</span>{values.map((value) => <button key={value} type="button" onClick={() => setActive(value)} className={`px-3 py-2 rounded-lg text-xs capitalize font-medium transition-all ${active === value ? 'bg-earth text-white' : 'bg-card border border-line text-slate hover:border-horizon'}`}>{value === 'tous' ? 'Tous' : label === 'Urgence' ? severityLabel(value) : label === 'Espace' ? moduleLabel(value) : statusLabel(value)}</button>)}</div>;
}
...
export default function AlertesCenter({ alertes = [], taches = [], businessEvents = [], salesOrders = [], payments = [], transactions = [], animaux = [], lots = [], stocks = [], cultures = [], clients = [], fournisseurs = [], equipements = [], sensorDevices = [], loading, onCreate, onUpdate, onDelete, onRefresh, onSendWhatsApp, onCreateTask, onRefreshTasks, onNavigate }) {
  ...
  }, [autoAlerts, autoTaskEnabled, onCreate, onCreateTask, onUpdate, onRefresh, onRefreshTasks, persistAutoIfNeeded, taches]);
  ...
  const openCommentModal = (alerte) => {
    setCommentTarget(alerte);
    setCommentDraft('');
  };
  ...
  return <div className="space-y-6">
    <div className="rounded-xl bg-vigilance-bg border border-vigilance px-4 py-3 flex flex-wrap items-center gap-2 text-sm text-horizon-dark font-medium"><span>ℹ️</span><span>WhatsApp prépare le message. Tu gardes la main avant l’envoi.</span>{autoNotifyCount > 0 ? <button type="button" onClick={sendAllUrgent} className="ml-auto rounded-lg bg-vigilance px-3 py-1 text-white text-xs">Préparer {autoNotifyCount} urgence(s)</button> : null}</div>
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4"><KpiCard icon={Bell} label="Alertes ouvertes" value={allAlerts.length} color="bg-neutral text-neutral" /><KpiCard icon={Bell} label="Nouvelles" value={nouvellesCount} color={nouvellesCount > 0 ? 'bg-vigilance text-horizon-dark' : 'bg-neutral-bg text-neutral'} /><KpiCard icon={AlertTriangle} label="Urgentes" value={critiquesCount} color={critiquesCount > 0 ? 'bg-urgent text-urgent' : 'bg-neutral-bg text-neutral'} /><KpiCard icon={Zap} label="WhatsApp prêt" value={autoNotifyCount} color={autoNotifyCount > 0 ? 'bg-urgent text-urgent' : 'bg-neutral-bg text-neutral'} /></div>
    ...
    <div className="bg-white border border-line rounded-2xl p-4 space-y-3"><div className="relative"><Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Rechercher..." className="w-full pl-8 pr-3 py-2 text-sm border border-line rounded-xl bg-card text-earth focus:outline-none focus:border-horizon" /></div><Filter label="Urgence" values={['tous', 'info', 'warning', 'critique', 'urgence']} active={severityFilter} setActive={setSeverityFilter} /><Filter label="Statut" values={['tous', 'nouvelle', 'lue', 'traitee']} active={statusFilter} setActive={setStatusFilter} />{modules.length > 0 ? <Filter label="Espace" values={['tous', ...modules]} active={moduleFilter} setActive={setModuleFilter} /> : null}</div>
    {loading && <div className="bg-white border border-line rounded-2xl p-8 text-center text-slate">Chargement...</div>}
    {!loading && filtered.length === 0 && <div className="bg-white border border-line rounded-2xl p-12 text-center"><CheckCircle size={56} className="mx-auto mb-4 text-positive" /><p className="text-lg font-semibold text-earth">Aucune alerte active</p><p className="text-sm text-slate mt-1">Tout est sous contrôle.</p></div>}
    <div className="space-y-3">{filtered.map((alerte) => { const whatsappLabel = whatsappStatusLabel(alerte); return <div key={alerte.id} className={`bg-white border border-line rounded-2xl p-4 flex gap-4 border-l-4 ${SEVERITY_BORDER[alerte.severity] || 'border-l-sky-400'}`}><div className="shrink-0 text-2xl mt-1">{SEVERITY_EMOJI[alerte.severity] || 'ℹ️'}</div><div className="flex-1 min-w-0"><div className="flex items-start gap-2 flex-wrap mb-1"><button type="button" onClick={() => openLinkedModule(alerte)} className="font-semibold text-earth flex-1 min-w-0 text-left hover:text-positive"><ExternalLink size={14} className="inline" /> {alerte.title || 'Alerte'}</button>{alerte.module_source && <span className={`text-xs px-2 py-1 rounded-full font-semibold shrink-0 ${MODULE_BADGE[alerte.module_source] || 'bg-neutral-bg text-neutral'}`}>{moduleLabel(alerte.module_source)}</span>}{alerte.isAuto && <span className="text-xs px-2 py-1 rounded-full bg-line text-slate shrink-0">Détectée</span>}<span className={`text-xs px-2 py-1 rounded-full shrink-0 ${alerte.status === 'traitee' ? 'bg-positive-bg text-positive' : alerte.status === 'lue' ? 'bg-neutral-bg text-neutral' : 'bg-vigilance-bg text-horizon-dark'}`}>{statusLabel(alerte.status || 'nouvelle')}</span></div>{alerte.message && <p className="text-sm text-slate mb-1">{alerte.message}</p>}{alerte.action_recommandee && <p className="text-xs text-positive font-semibold">→ Action : {alerte.action_recommandee}</p>}<div className="flex items-center gap-3 mt-2 flex-wrap">{alerte.entity_id && <span className="text-xs text-slate">Concerné : {alerte.entity_id}</span>}{alerte.amount != null && <span className="text-xs font-semibold text-earth">{fmtCurrency(alerte.amount)}</span>}<span className="text-xs text-slate">Responsable : {recipientLabel(alerte.responsable || config.defaultRecipient, config)}</span>{whatsappLabel && <span className={`text-xs ${alerte.whatsapp_status === 'echec' ? 'text-urgent' : 'text-positive'}`}>{whatsappLabel}</span>}{alerte.task_intent === 'a_creer' && <span className="text-xs text-positive">Tâche demandée</span>}{alerte.created_at && <span className="text-xs text-slate">{new Date(alerte.created_at).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short', year: 'numeric' })}</span>}</div></div><div className="flex flex-col gap-2 shrink-0 items-end"><button type="button" onClick={() => openLinkedModule(alerte)} className="text-xs px-3 py-1 rounded-lg border border-line text-slate hover:bg-line transition-colors whitespace-nowrap">Voir source</button>{onCreateTask ? <button type="button" onClick={() => handleCreateTaskFromAlert(alerte)} className="text-xs px-3 py-1 rounded-lg border border-positive text-positive hover:bg-positive-bg transition-colors whitespace-nowrap">Créer tâche</button> : null}{(alerte.status || 'nouvelle') === 'nouvelle' && <button type="button" onClick={() => handleMarkRead(alerte)} className="text-xs px-3 py-1 rounded-lg border border-line text-slate hover:bg-line transition-colors whitespace-nowrap">Marquer lu</button>}{(alerte.status || 'nouvelle') !== 'traitee' && (alerte.status || 'nouvelle') !== 'resolue' && <button type="button" onClick={() => handleTraiter(alerte)} className="text-xs px-3 py-1 rounded-lg border border-positive text-positive hover:bg-positive-bg transition-colors whitespace-nowrap">Traiter</button>}{(alerte.status || 'nouvelle') !== 'resolue' && <button type="button" onClick={() => handleResoudre(alerte)} className="text-xs px-3 py-1 rounded-lg border border-positive text-positive hover:bg-positive-bg transition-colors whitespace-nowrap">Résoudre</button>}{(alerte.status || 'nouvelle') !== 'ignoree' && <button type="button" onClick={() => handleIgnorer(alerte)} className="text-xs px-3 py-1 rounded-lg border border-line text-neutral hover:bg-neutral-bg transition-colors whitespace-nowrap">Ignorer</button>}<button type="button" onClick={() => openCommentModal(alerte)} className="text-xs px-3 py-1 rounded-lg border border-line text-slate hover:bg-line transition-colors whitespace-nowrap">Commentaire</button><button type="button" disabled={sendingId === alerte.id} onClick={() => handleSendWhatsApp(alerte)} className="text-xs px-3 py-1 rounded-lg border border-line text-neutral hover:bg-neutral-bg transition-colors whitespace-nowrap disabled:opacity-60"><MessageCircle size={12} className="inline" /> {sendingId === alerte.id ? 'Préparation...' : 'WhatsApp'}</button>{!alerte.isAuto ? <ActionIconButton icon={X} color="red" title="Supprimer" onClick={() => { setSelected(alerte); setModal('delete'); }} /> : null}</div></div>; })}</div>
    ...
    {modal === 'config' ? <div className="fixed inset-0 bg-earth/30 z-50 flex items-center justify-center p-4"><div className="bg-white rounded-2xl border border-line p-6 w-full max-w-lg space-y-4"><div className="flex items-center justify-between"><h3 className="font-semibold text-earth">Configuration alertes</h3><button type="button" onClick={() => setModal(null)}><X size={18} /></button></div><label className="block text-sm"><span className="text-slate">Nom propriétaire</span><input className="mt-1 w-full rounded-lg border border-line bg-card px-3 py-2" value={config.ownerName} onChange={(e) => setConfig((prev) => ({ ...prev, ownerName: e.target.value }))} /></label><label className="block text-sm"><span className="text-slate">WhatsApp propriétaire</span><input className="mt-1 w-full rounded-lg border border-line bg-card px-3 py-2" placeholder="Ex: 221785890153" value={config.ownerWhatsapp} onChange={(e) => setConfig((prev) => ({ ...prev, ownerWhatsapp: e.target.value }))} /></label><label className="flex items-center gap-2 text-sm text-earth"><input type="checkbox" checked={config.autoWhatsApp} onChange={(e) => setConfig((prev) => ({ ...prev, autoWhatsApp: e.target.checked }))} /> Préparer WhatsApp pour critiques/urgences</label><label className="flex items-center gap-2 text-sm text-earth"><input type="checkbox" checked={config.notifyOwnerOnUrgency} onChange={(e) => setConfig((prev) => ({ ...prev, notifyOwnerOnUrgency: e.target.checked }))} /> Toujours me notifier sur urgence/critique</label><label className="block text-sm"><span className="text-slate">Destinataire par défaut</span><select className="mt-1 w-full rounded-lg border border-line bg-card px-3 py-2" value={config.defaultRecipient} onChange={(e) => setConfig((prev) => ({ ...prev, defaultRecipient: e.target.value }))}>{responsibleOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label><p className="text-xs text-slate">Les urgences peuvent préparer un message WhatsApp pour toi ou l’équipe. L’ERP garde le statut et le destinataire.</p><Btn onClick={saveAlertConfig}>Sauvegarder</Btn></div></div> : null}
    <QuickInputModal
      open={Boolean(commentTarget)}
      title="Commentaire sur l'alerte"
      description={commentTarget?.title || ''}
      label="Commentaire"
      type="textarea"
      value={commentDraft}
      onChange={setCommentDraft}
      submitLabel="Ajouter"
      onClose={() => { setCommentTarget(null); setCommentDraft(''); }}
      onSubmit={() => {
        if (!commentTarget) return;
        handleAddComment(commentTarget, commentDraft);
