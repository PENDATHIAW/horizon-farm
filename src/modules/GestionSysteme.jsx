import { CheckCircle2, Edit, Eye, EyeOff, LockKeyhole, Plus, Save, Shield, ShieldCheck, Trash2, UserCog, Users } from 'lucide-react';
...
export default function GestionSysteme() {
  ...
  const save = async () => {
    ...
    try {
      ...
      toast.error(error.message || 'Profil Supabase non synchronisé');
    }
  ...
  const remove = (target) => {
    ...
    toast.success('Utilisateur retiré côté interface');
  };

  return <div className="space-y-6">
    <SectionHeader title="Gestion du système" sub="Utilisateurs, visiteurs, rôles, habilitations, sécurité Supabase et accès modules" actions={<><Btn variant="outline" small onClick={() => setShowMatrix((v) => !v)}>{showMatrix ? 'Masquer matrice' : 'Voir matrice'}</Btn><Btn icon={Plus} small onClick={() => setSelected(newUser())}>Créer utilisateur</Btn></>} />

    <div className="rounded-3xl border border-[#d6c3a0] bg-white p-5 shadow-sm">
      <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
        <div>
          <p className="inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs font-black text-emerald-700"><ShieldCheck size={14} /> Sécurité ERP activée</p>
          ...
          <p className="mt-2 text-xs text-[#8a7456]">Les mots de passe sont gérés par Supabase Auth. Ils ne sont jamais visibles ni stockés dans la table profils.</p>
        </div>
    ...
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
      <div className="xl:col-span-2 rounded-3xl border border-[#d6c3a0] bg-white p-5 shadow-sm space-y-3">
        <h3 className="font-black text-[#2f2415]">Utilisateurs & visiteurs</h3>
      ...
      <div className="rounded-3xl border border-[#d6c3a0] bg-white p-5 shadow-sm space-y-3">
        {selected ? <><h3 className="font-black text-[#2f2415]">{users.some((u) => u.id === selected.id) ? 'Modifier accès' : 'Créer / inviter'}</h3><Field label="Nom" value={selected.nom} onChange={(v) => setS('nom', v)} /><Field label="Email" value={selected.email} onChange={(v) => setS('email', v)} /><Field label="Équipe / périmètre" value={selected.equipe} onChange={(v) => setS('equipe', v)} />
          ...
          <label className="block text-sm"><span className="text-[#8a7456]">Statut</span><select value={selected.statut} onChange={(e) => setS('statut', e.target.value)} className="mt-1 w-full rounded-xl border border-[#d6c3a0] bg-[#fffdf8] px-3 py-2"><option value="active">Actif</option><option value="pending">En attente</option><option value="suspended">Suspendu</option><option value="disabled">Désactivé</option></select></label>
    ...
    <div className="rounded-3xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800 flex gap-2"><ShieldCheck size={18} className="shrink-0" /> Les habilitations sont reliées à Supabase Auth via la table profiles et la fondation RLS ajoutée dans les migrations. Les droits d’accès doivent être appliqués en base avec les migrations fournies.</div>
  </div>;
}
