...
function HeyHorizonTaskCard({ draft, onCreateTask, onCreateBusinessEvent, onRefreshTasks, onRefreshBusinessEvents, onClose }) {
  ...
  return <section className="rounded-3xl border border-positive bg-positive-bg p-6 shadow-card space-y-4">
    <div className="flex items-start justify-between gap-3"><div><p className="text-xs uppercase tracking-normal text-positive font-semibold flex items-center gap-2"><ClipboardList size={15} /> Fiche préparée par Hey Horizon</p><h3 className="mt-1 text-xl font-semibold text-earth">Tâche à créer</h3><p className="mt-1 text-sm text-positive">Complète si besoin, puis valide. La tâche et l’événement métier seront créés.</p></div><button type="button" onClick={onClose} className="rounded-full border border-positive bg-white p-2 text-positive"><X size={16} /></button></div>
    <div className="grid grid-cols-1 md:grid-cols-3 gap-3"><label className="space-y-1 md:col-span-2"><span className="text-xs font-semibold text-positive">Titre</span><input value={title} onChange={(e) => setTitle(e.target.value)} className="w-full min-h-[44px] rounded-xl border border-positive bg-white px-3 py-2 text-sm" /></label><label className="space-y-1"><span className="text-xs font-semibold text-positive">Échéance</span><input type="date" value={dueDate} onChange={(e) => setDueDate(e.target.value)} className="w-full min-h-[44px] rounded-xl border border-positive bg-white px-3 py-2 text-sm" /></label><label className="space-y-1"><span className="text-xs font-semibold text-positive">Priorité</span><select value={priority} onChange={(e) => setPriority(e.target.value)} className="w-full min-h-[44px] rounded-xl border border-positive bg-white px-3 py-2 text-sm"><option value="normale">Normale</option><option value="haute">Haute</option><option value="critique">Critique</option></select></label><label className="space-y-1"><span className="text-xs font-semibold text-positive">Module lié</span><input value={moduleLie} onChange={(e) => setModuleLie(e.target.value)} className="w-full min-h-[44px] rounded-xl border border-positive bg-white px-3 py-2 text-sm" /></label><label className="space-y-1"><span className="text-xs font-semibold text-positive">Checklist</span><input value={checklist} onChange={(e) => setChecklist(e.target.value)} className="w-full min-h-[44px] rounded-xl border border-positive bg-white px-3 py-2 text-sm" /></label><label className="space-y-1 md:col-span-3"><span className="text-xs font-semibold text-positive">Note</span><input value={note} onChange={(e) => setNote(e.target.value)} className="w-full min-h-[44px] rounded-xl border border-positive bg-white px-3 py-2 text-sm" /></label></div>
    <div className="rounded-xl border border-positive bg-white p-3 text-sm text-positive"><CheckCircle2 size={14} className="inline" /> À la validation : tâche créée, historique alimenté, module Tâches rafraîchi.</div>
    <div className="flex justify-end"><button type="button" onClick={submit} disabled={saving} className="rounded-xl bg-earth px-6 py-2 text-sm font-semibold text-white disabled:opacity-60">{saving ? 'Création...' : 'Créer la tâche'}</button></div>
  </section>;
}
