import { BarChart2, CheckCircle2, Edit, Eye, FileText, PackagePlus, Plus, Receipt, Save, Trash2, X } from 'lucide-react';
...
const MONTHLY = [
  ['Location champ prêt à exploiter', 'location'],
  ['Location bâtiment / poulailler', 'location'],
...
function activePlan(bps) { return safeArray(bps).find(isHorizon) || safeArray(bps)[0]; }
function nav(id) {
...
function buildRows(projections, investment) {
  ...
  return safeArray(projections).slice().sort((a, b) => toNumber(a.mois_index) - toNumber(b.mois_index)).map((p) => {
    const ca = toNumber(p.ca_estime);
    const charges = toNumber(p.charges_estimees);
    const marge = ca - charges;
    cumul += marge;
    return { mois: p.mois_index, ca, charges, marge, cumul, pct: Math.min(100, Math.max(0, ((investment + cumul) / Math.max(1, investment)) * 100)) };
  });
}
function buildMetrics({ plan, lines, costs, projections, transactions }) {
  const investment = lines.reduce((a, r) => a + toNumber(r.total), 0);
  const effective = lines.filter((r) => st(r) === 'effectif').reduce((a, r) => a + toNumber(r.total), 0);
  const remaining = Math.max(0, investment - effective);
  const monthly = costs.reduce((a, r) => a + toNumber(r.montant_mensuel), 0);
  const cycleCharges = projections.reduce((a, r) => a + toNumber(r.charges_estimees), 0) || monthly * toNumber(plan?.duree_cycle_mois || 12);
  const revenue = projections.reduce((a, r) => a + toNumber(r.ca_estime), 0);
  const net = revenue - cycleCharges - investment;
  const roi = investment > 0 ? (net / investment) * 100 : 0;
  const rows = buildRows(projections, investment);
  const payback = rows.find((r) => r.cumul >= 0)?.mois || null;
  const missingForecast = !revenue && !cycleCharges;
  const score = missingForecast ? 0 : Math.max(0, Math.min(100, Math.round((roi > 0 ? 35 : 0) + (payback ? 25 : 0) + (revenue > cycleCharges ? 20 : 0) + (projections.length ? 20 : 0))));
  const verdict = missingForecast ? 'Prévisions à compléter' : score >= 75 ? 'Pertinent' : score >= 50 ? 'À sécuriser' : 'À revoir';
  const cashOut = safeArray(transactions).filter((r) => String(r.type).toLowerCase() === 'sortie' && st(r) !== 'annule').reduce((a, r) => a + toNumber(r.montant), 0);
  return { investment, effective, remaining, monthly, cycleCharges, revenue, net, roi, rows, payback, score, verdict, missingForecast, cashOut };
}
function Card({ label, value, tone, sub }) {
  ...
  return <div className="rounded-xl border border-[#eadcc2] bg-[#fffdf8] p-3"><p className="text-xs text-[#8a7456]">{label}</p><p className={`font-black mt-1 ${cls}`}>{value}</p>{sub && <p className="text-xs text-[#8a7456] mt-1">{sub}</p>}</div>;
}
...
function AmortizationModal({ open, onClose, rows, metrics }) {
  ...
  return <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
    <div className="bg-white rounded-2xl border border-[#d6c3a0] w-full max-w-5xl max-h-[85vh] overflow-hidden flex flex-col">
      <div className="p-4 border-b border-[#eadcc2] flex justify-between items-start gap-3"><div><h3 className="font-black text-xl text-[#2f2415]">Plan d’amortissement</h3><p className="text-sm text-[#8a7456]">Le solde part de l’investissement prévu, puis se réduit avec la marge mensuelle.</p></div><button type="button" onClick={onClose} className="p-2 rounded-xl hover:bg-[#fff3d8]"><X size={18} /></button></div>
      {metrics.missingForecast ? <div className="p-5 text-sm text-[#7d6a4a]">Les prévisions de CA et/ou charges ne sont pas encore renseignées. Tant que les projections restent à 0, l’amortissement restera à 0% et le solde restera négatif.</div> : null}
...
function Summary({ plan, metrics }) {
  const [modal, setModal] = useState(false);
  if (!plan) return <div className="rounded-2xl border border-[#d6c3a0] bg-white p-5"><h2 className="font-black text-xl text-[#2f2415]">Investissements</h2><p className="text-sm text-[#8a7456]">Crée un Business Plan pour piloter rentabilité et amortissement.</p></div>;
  return <div className="rounded-2xl border border-[#d6c3a0] bg-white p-5 space-y-4">
    <div className="flex flex-col xl:flex-row xl:items-start justify-between gap-4"><div><p className="text-xs uppercase tracking-widest text-[#8a7456]">Résumé du Business Plan</p><h2 className="text-2xl font-black text-[#2f2415] mt-1">{plan.nom}</h2><p className="text-sm text-[#7d6a4a] mt-1">Décider: combien investir, combien encaisser, quand récupérer l’argent.</p></div><div className="flex flex-wrap gap-2"><Btn icon={BarChart2} onClick={() => setModal(true)}>Plan d’amortissement</Btn>{['finances','comptabilite','ventes','stock','avicole','animaux','cultures','documents'].map((x) => <Btn key={x} small variant="outline" onClick={() => nav(x)}>{x[0].toUpperCase() + x.slice(1)}</Btn>)}</div></div>
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
      <Card label="Investissement prévu" value={fmtCurrency(metrics.investment)} />
      <Card label="Investi effectif" value={fmtCurrency(metrics.effective)} sub={`Reste: ${fmtCurrency(metrics.remaining)}`} />
      <Card label="Charges du cycle" value={fmtCurrency(metrics.cycleCharges)} sub={`Mensuel: ${fmtCurrency(metrics.monthly)}`} />
      <Card label="CA prévu fin cycle" value={fmtCurrency(metrics.revenue)} />
      <Card label="Gain/perte fin cycle" value={fmtCurrency(metrics.net)} tone={metrics.net >= 0 ? 'good' : 'bad'} />
      <Card label="ROI" value={metrics.missingForecast ? 'À compléter' : fmtPercent(metrics.roi)} tone={metrics.roi >= 0 && !metrics.missingForecast ? 'good' : 'bad'} />
      <Card label="Payback" value={metrics.payback ? `Mois ${metrics.payback}` : 'Non atteint'} />
      <Card label="Décision" value={`${metrics.verdict} · ${metrics.score}%`} tone={metrics.score >= 75 ? 'good' : metrics.score >= 50 ? 'warn' : 'bad'} />
    </div>
    <AmortizationModal open={modal} onClose={() => setModal(false)} rows={metrics.rows} metrics={metrics} />
  </div>;
}

function Editor({ plan, lines, onUpdateBpInvestmentLine, onCreateBpInvestmentLine, onDeleteBpInvestmentLine, onCreateBpRecurringCost, onCreateFinanceTransaction, onRefreshBusinessPlans, onRefreshFinances }) {
  ...
  const save = async () => { setBusy('save'); try { await Promise.all(draft.map((l) => onUpdateBpInvestmentLine?.(l.id, { designation: clean(l.designation), categorie: l.categorie, quantite: toNumber(l.quantite), unite: l.unite, prix_unitaire: toNumber(l.prix_unitaire), total: lineTotal(l), statut: l.statut || 'prevu', preuve_url: l.preuve_url || '', transaction_id: l.transaction_id || '' }))); await onRefreshBusinessPlans?.(); setEditing(false); toast.success('BP mis à jour'); } catch (e) { toast.error(e.message || 'Mise à jour impossible'); } finally { setBusy(''); } };
  const addLine = async (i) => { setBusy(i.designation); try { await onCreateBpInvestmentLine?.({ id: makeId('BPLI'), business_plan_id: plan.id, ...i, total: lineTotal(i), statut: 'prevu' }); await onRefreshBusinessPlans?.(); toast.success('Dépense ajoutée'); } catch (e) { toast.error(e.message || 'Ajout impossible'); } finally { setBusy(''); } };
  const addCost = async (i) => { setBusy(i.designation); try { await onCreateBpRecurringCost?.({ id: makeId('BPCOST'), business_plan_id: plan.id, ...i, frequence: 'mensuelle' }); await onRefreshBusinessPlans?.(); toast.success('Charge mensuelle ajoutée'); } catch (e) { toast.error(e.message || 'Ajout impossible'); } finally { setBusy(''); } };
  const markEffective = async (l) => { setBusy(l.id); try { let transactionId = l.transaction_id || ''; if (!transactionId && onCreateFinanceTransaction) { transactionId = makeId('FIN'); await onCreateFinanceTransaction({ id: transactionId, type: 'sortie', montant: toNumber(l.total), categorie: 'investissement', activite: 'Investissements', libelle: clean(l.designation), statut: 'paye', date: new Date().toISOString().slice(0,10), bp_line_id: l.id, business_plan_id: plan.id }); await onRefreshFinances?.(); } await onUpdateBpInvestmentLine?.(l.id, { statut: 'effectif', effective_at: new Date().toISOString(), transaction_id: transactionId }); await onRefreshBusinessPlans?.(); toast.success('Dépense passée en effectif'); } catch (e) { toast.error(e.message || 'Passage en effectif impossible'); } finally { setBusy(''); } };
  return <div className="rounded-2xl border border-[#d6c3a0] bg-white p-5 space-y-5"><div className="flex justify-between gap-3 flex-wrap"><div><h3 className="font-black text-[#2f2415]">Dépenses du BP</h3><p className="text-sm text-[#8a7456]">Le statut “prévu” veut dire budgeté. L’action “Passer effectif” veut dire réellement acheté/payé.</p></div><div className="flex gap-2">{editing ? <Btn icon={Save} onClick={save} disabled={busy === 'save'}>Enregistrer</Btn> : <Btn icon={Edit} onClick={() => setEditing(true)}>Modifier</Btn>}{editing && <Btn variant="outline" onClick={() => setEditing(false)}>Annuler</Btn>}</div></div><div className="overflow-x-auto rounded-xl border border-[#eadcc2]"><table className="w-full text-sm"><thead><tr className="bg-[#fffdf8] text-left text-xs uppercase text-[#8a7456]"><th className="px-3 py-2">Dépense</th><th className="px-3 py-2">Catégorie</th><th className="px-3 py-2">Qté</th><th className="px-3 py-2">Unité</th><th className="px-3 py-2">Prix</th><th className="px-3 py-2">Total</th><th className="px-3 py-2">Statut</th><th className="px-3 py-2">Preuve</th><th className="px-3 py-2">Transaction</th><th className="px-3 py-2">Action</th></tr></thead><tbody>{rows.map((l) => <tr key={l.id} className="border-t border-[#eadcc2]"><Cell edit={editing} value={clean(l.designation)} onChange={(v) => update(l.id, { designation: v })} wide /><Cell edit={editing} value={l.categorie || ''} onChange={(v) => update(l.id, { categorie: v })} /><Cell edit={editing} type="number" value={l.quantite ?? 0} onChange={(v) => update(l.id, { quantite: v })} /><Cell edit={editing} value={l.unite || ''} onChange={(v) => update(l.id, { unite: v })} /><Cell edit={editing} type="number" value={l.prix_unitaire ?? 0} onChange={(v) => update(l.id, { prix_unitaire: v })} display={fmtCurrency(l.prix_unitaire)} /><td className="px-3 py-2 font-black">{fmtCurrency(editing ? lineTotal(l) : l.total)}</td><td className="px-3 py-2">{editing ? <select className="rounded-lg border border-[#d6c3a0] px-2 py-1" value={l.statut || 'prevu'} onChange={(e) => update(l.id, { statut: e.target.value })}><option value="prevu">prévu</option><option value="effectif">effectif</option><option value="annule">annulé</option></select> : (l.statut || 'prévu')}</td><Cell edit={editing} value={l.preuve_url || ''} onChange={(v) => update(l.id, { preuve_url: v })} placeholder="preuve" /><Cell edit={editing} value={l.transaction_id || ''} onChange={(v) => update(l.id, { transaction_id: v })} placeholder="transaction" /><td className="px-3 py-2">{!editing && st(l) !== 'effectif' ? <button type="button" disabled={busy === l.id} className="text-xs font-bold text-emerald-600" onClick={() => markEffective(l)}><CheckCircle2 size={12} className="inline" /> Passer effectif</button> : <span className="text-xs text-[#8a7456]">Déjà effectif</span>}{editing && <button type="button" className="text-xs text-red-500 ml-2" onClick={() => onDeleteBpInvestmentLine?.(l.id)}><Trash2 size={12} className="inline" /> Suppr.</button>}</td></tr>)}</tbody></table></div><div className="grid grid-cols-1 xl:grid-cols-2 gap-4"><Quick title="Ajouter dépense ponctuelle" icon={PackagePlus} items={ONE_TIME} onClick={addLine} busy={busy} /><Quick title="Ajouter charge mensuelle" icon={BarChart2} items={MONTHLY} onClick={addCost} busy={busy} /></div></div>;
}
...
function Quick({ title, icon: Icon, items, onClick, busy }) { return <div><div className="flex items-center gap-2 mb-2"><Icon size={16} className="text-[#9a6b12]" /><p className="font-bold text-[#2f2415]">{title}</p></div><div className="grid grid-cols-1 md:grid-cols-2 gap-2">{items.map((i) => <button type="button" disabled={Boolean(busy)} key={i.designation} className="rounded-xl border border-[#eadcc2] bg-[#fffdf8] p-3 text-left hover:border-[#b6975f] disabled:opacity-60" onClick={() => onClick(i)}><p className="font-bold text-sm text-[#2f2415]"><Plus size={13} className="inline" /> {i.designation}</p><p className="text-xs text-[#8a7456]">{i.categorie}</p></button>)}</div></div>; }
...
export default function InvestissementsV7(props) {
  ...
  return <div className="space-y-6"><Summary plan={plan} metrics={m} /><Editor plan={plan} lines={lines} {...props} /><div className="rounded-2xl border border-[#d6c3a0] bg-white p-5"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2"><FileText size={18} className="text-[#9a6b12]" /><div><h3 className="font-black text-[#2f2415]">Gestion complète des BP</h3><p className="text-xs text-[#8a7456]">À ouvrir seulement pour créer, dupliquer ou consulter les anciennes fiches.</p></div></div><Btn icon={showFull ? Eye : FileText} variant="outline" onClick={() => setShowFull(!showFull)}>{showFull ? 'Masquer' : 'Ouvrir'}</Btn></div>{showFull && <div className="mt-4"><BaseInvestissements {...props} /></div>}</div></div>;
}
