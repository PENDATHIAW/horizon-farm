...
export default function AnnualCommercialCalendarPanel() {
  return (
    <div className="rounded-3xl border border-[#d6c3a0] bg-white p-5 shadow-sm space-y-4">
      <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
        <div>
          ...
          <h3 className="text-xl font-black text-[#2f2415] mt-1">Vendre toute l’année, maximiser les pics</h3>
          <p className="text-sm text-[#8a7456] mt-1">Le Centre décisionnel distingue vente permanente et périodes optimales pour choisir quoi pousser, quand investir, et quels clients cibler.</p>
        </div>
        <div className="rounded-2xl bg-[#fffdf8] border border-[#eadcc2] p-3 text-xs text-[#7d6a4a] max-w-md">
          <b className="text-[#2f2415]">Principe :</b> bouchers, foirails, Touba, Berndé, restaurants, boutiques et consommateurs permettent de vendre toute l’année. Les fêtes et saisons servent à maximiser prix, volume et vitesse de vente.
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
        {HORIZON_COMMERCIAL_MONTHS.map((month) => (
          <div key={month.month} className="rounded-2xl border border-[#eadcc2] bg-[#fffdf8] p-4 min-w-0">
            ...
            <div className="mt-2 space-y-1">
              {month.actions?.slice(0, 2).map((action) => <p key={action} className="text-[11px] text-[#7d6a4a] leading-snug">• {action}</p>)}
            </div>

            {month.risks?.length ? <p className="mt-2 text-[11px] text-amber-700 flex items-start gap-1"><ShieldAlert size={12} className="mt-0.5 shrink-0" /> {month.risks.slice(0, 2).join(' · ')}</p> : null}
          </div>
        ))}
      </div>
      ...
      <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4">
        ...
        <div className="mt-2 grid grid-cols-1 md:grid-cols-2 gap-2">
          {HORIZON_TRANSVERSAL_STRATEGIES.slice(0, 6).map((strategy) => <p key={strategy} className="text-xs text-emerald-800">• {strategy}</p>)}
        </div>
      </div>
    </div>
  );
