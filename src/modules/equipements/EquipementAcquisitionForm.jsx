...
export default function EquipementAcquisitionForm({ suppliers = [], onSubmit }) {
  ...
  return (
    <form onSubmit={submit} className="rounded-2xl border border-[#d6c3a0] bg-white p-5 shadow-sm">
      <div className="mb-4"><h2 className="font-black text-[#2f2415]">Nouvelle acquisition</h2><p className="mt-1 text-sm text-[#8a7456]">Crée une fiche équipement, une dépense validée et une preuve liée.</p></div>
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        <label className="text-sm font-bold text-[#6f6048]">Équipement<input required value={form.name} onChange={(event) => set('name', event.target.value)} className="mt-1 w-full rounded-lg border border-[#d6c3a0] px-3 py-2 font-normal" /></label>
