...
export const ACTIVITE_SECTION = 'rounded-3xl border border-line bg-white p-6 shadow-card sm:p-6 min-w-0';
...
export function ActiviteKpi({ label, value, tone = 'neutral', onClick }) {
...
export function ActiviteSection({ title, subtitle, children, className = '' }) {
  return (
    <section className={`${ACTIVITE_SECTION} space-y-4 ${className}`}>
      {title ? (
...
export function ActiviteActionCard({ title, text, onClick, icon: Icon }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
export function ActiviteTodoRow({ title, detail, actionLabel, onAction, onOpen, busy }) {
  return (
