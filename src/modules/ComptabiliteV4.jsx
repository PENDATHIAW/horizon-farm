import { AlertTriangle, Archive, Beef, Bird, BookOpen, CheckCircle, CreditCard, Download, Eye, FileText, Landmark, Package, Plus, RefreshCw, Scale, Sprout, Stethoscope, TrendingUp, Truck, Wallet, Wrench } from 'lucide-react';
...
function navigate(onNavigate, moduleId) { if (onNavigate) onNavigate(moduleId); }
function badge(statusValue) {
  if (statusValue === 'valide') return 'bg-emerald-50 text-emerald-700 border-emerald-200';
  if (statusValue === 'annule') return 'bg-red-50 text-red-600 border-red-200';
  return 'bg-amber-50 text-amber-700 border-amber-200';
}
function StatusBadge({ value }) { return <span className={`text-xs px-2 py-1 rounded-full border ${badge(value)}`}>{value || 'brouillon'}</span>; }
...
function ConnectionPanel({ snapshot, transactions, clients, fournisseurs, stocks, animaux, lots, cultures, sante, investissements, equipements, onNavigate }) {
  ...
  const cards = [
    ...
    { icon: Receipt, title: 'Ventes / Clients', value: fmtCurrency(snapshot.receivables), detail: `${safeArray(clients).length} client(s), créances à justifier ou relancer.`, moduleId: 'ventes', active: snapshot.receivables > 0 },
    ...
    { icon: Stethoscope, title: 'Santé', value: fmtCurrency(healthCosts), detail: `${safeArray(sante).length} soin(s)/vaccin(s) liés aux charges.`, moduleId: 'sante', active: healthCosts > 0 },
  ...
  return <div className="bg-white border border-[#d6c3a0] rounded-2xl p-5"><div className="mb-4"><h3 className="font-black text-[#2f2415]">Connexions comptables</h3><p className="text-sm text-[#8a7456]">La comptabilité contrôle les flux venus des modules métier. Les montants partagés sont alignés sur Finances.</p></div><div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">{cards.map((card) => <LinkCard key={card.title} {...card} onNavigate={onNavigate} />)}</div></div>;
}

export default function ComptabiliteV4({ transactions = [], finances = [], salesOrders = [], payments = [], clients = [], fournisseurs = [], stocks = [], animaux = [], lots = [], cultures = [], sante = [], investissements = [], equipements = [], onRefreshFinances, onNavigate }) {
  ...
  const validateEntry = async (entryId) => {
    setSaving(true);
    try { await comptabiliteService.validateEntry(entryId); await load(); toast.success('Écriture validée'); } catch (error) { toast.error(error.message || 'Validation impossible'); } finally { setSaving(false); }
  };
  ...
  return <div className="space-y-6">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-4"><KpiCard icon={TrendingUp} label="CA à contrôler" value={fmtCurrency(snapshot.revenue)} sub="aligné sur Finances" color="bg-emerald-500/20 text-emerald-500" /><KpiCard icon={Wallet} label="Cash net" value={fmtCurrency(snapshot.cashNet)} sub="cash encaissé - charges payées" color={snapshot.cashNet >= 0 ? 'bg-emerald-500/20 text-emerald-500' : 'bg-red-500/20 text-red-500'} /><KpiCard icon={AlertTriangle} label="À préparer" value={fmtNumber(draftableTransactions.length)} sub="transactions finances" color="bg-amber-500/20 text-amber-500" /><KpiCard icon={Landmark} label="Créances" value={fmtCurrency(snapshot.receivables)} sub="aligné sur Finances" color="bg-amber-500/20 text-amber-500" /><KpiCard icon={CreditCard} label="Dettes" value={fmtCurrency(snapshot.debts)} sub={`${dettes.length} ligne(s) à payer`} color="bg-red-500/20 text-red-500" /></div>
