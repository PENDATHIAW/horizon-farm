...
import { buildAnimalProposedSaleDisplay, ERP_REVENUE_LABEL, PROPOSED_PRICE_MARGIN_LABEL, SALE_PRICE_HELP_ANIMAL } from '../utils/salePricePresentation.js';
...
function statusBadge(status) {
  const map = { vendu: 'bg-neutral-bg text-neutral border-line', pret: 'bg-positive-bg text-positive border-positive', presque: 'bg-vigilance-bg text-horizon-dark border-vigilance', retard: 'bg-urgent-bg text-urgent border-urgent', normal: 'bg-neutral-bg text-neutral border-line' };
  const label = { vendu: 'Vendu', pret: 'Prêt vente', presque: 'Presque prêt', retard: 'En retard', normal: 'Normal' }[status] || status;
  return <span className={`inline-flex rounded-full border px-2 py-1 text-xs font-semibold ${map[status] || map.normal}`}>{label}</span>;
}
...
function MiniMetric({ label, value, danger = false }) { return <div className={`rounded-2xl border p-4 ${danger ? 'border-urgent bg-urgent-bg' : 'border-line bg-white'}`}><p className="text-xs uppercase tracking-normal text-slate">{label}</p><p className={`mt-2 text-xl font-semibold ${danger ? 'text-urgent' : 'text-earth'}`}>{value}</p></div>; }
function ProgressBar({ value }) { const pct = Math.max(0, Math.min(100, Number(value || 0))); return <div className="min-w-[120px]"><div className="h-2 rounded-full bg-line overflow-hidden"><div className="h-full rounded-full bg-earth" style={{ width: `${pct}%` }} /></div><p className="mt-1 text-xs font-semibold text-earth">{Math.round(value || 0)}%</p></div>; }
...
function AnimalDetailModal({ open, onClose, animal, alimentationLogs = [], vaccins = [], businessEvents = [], salesOrders = [], payments = [], transactions = [], marketPrices = [] }) {
  ...
  useEffect(() => { if (open) queueMicrotask(() => setTab('identite')); }, [open, animal?.id]);
  ...
  return <BaseModal open={open} onClose={onClose} title={`Fiche ${animal.type || animal.espece || 'animal'} - ${physicalIdOf(animal)}`} size="5xl"><div className="space-y-6">
    ...
    {tab === 'identite' ? (
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 rounded-2xl border border-line bg-white p-4">
          <p className="font-semibold text-earth mb-3">Identité complète</p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">{identityRows.map(([label, value]) => <div key={label} className="rounded-xl border border-line bg-card p-3"><p className="text-xs text-slate">{label}</p><p className="font-semibold text-earth mt-1">{fallbackText(value)}</p></div>)}</div>
        </div>
    ...
    {tab === 'croissance' ? (
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="rounded-2xl border border-line bg-card p-4"><p className="font-semibold text-earth mb-2">Notes terrain</p><p className="text-sm text-slate whitespace-pre-wrap">{fallbackText(animal.notes || animal.note || animal.commentaire)}</p></div>
      </div>
    ) : null}

    {tab === 'finances' ? (
      <div className="rounded-2xl border border-urgent bg-urgent-bg p-4"><p className="font-semibold text-urgent mb-1">{COST_UNIFIED_LABEL}, prix proposé et marges</p><p className="text-sm text-urgent mb-3">{SALE_PRICE_HELP_ANIMAL} · {MARGIN_GROSS_DEFINITION_SHORT} pour le revenu fiche enregistré.</p>{costs.warnings.length ? <div className="mb-3 rounded-xl border border-vigilance bg-vigilance-bg p-3 text-sm text-horizon-dark"><AlertTriangle size={15} className="inline" /> {costs.warnings.join(' ')}</div> : null}{proposed.ficheDivergeNote ? <div className="mb-3 rounded-xl border border-vigilance bg-vigilance-bg p-3 text-sm text-horizon-dark">{proposed.ficheDivergeNote}</div> : null}<div className="grid grid-cols-2 lg:grid-cols-4 gap-2">{[['Prix achat', costs.achat], ['Alimentation/coût lié', costs.alimentation], ['Soins/vaccins liés', costs.sante], ['Autres frais', costs.autres], ['Événements de charge', costs.evenements], ['Finance liée', costs.finance], [COST_UNIFIED_LABEL, costs.total], ['Prix proposé (moteur)', proposed.proposedPrice], ['Plancher acceptable', proposed.minimumPrice || salePricing.minimumPrice], [PROPOSED_PRICE_MARGIN_LABEL, proposed.marginOnProposed], [ERP_REVENUE_LABEL, costs.sale], [MARGIN_GROSS_LABEL, costs.marge], ['Payé', costs.paid], ['Reste à encaisser', costs.remaining], ['Commandes liées', costs.salesCount], ['Coût/kg', g.current > 0 ? costs.total / g.current : 0]].map(([label, value]) => <div key={label} className="rounded-xl bg-white border border-urgent p-3"><p className="text-xs text-slate">{label}</p><p className={`font-semibold mt-1 ${(label === MARGIN_GROSS_LABEL || label === PROPOSED_PRICE_MARGIN_LABEL) && value != null && value < 0 ? 'text-urgent' : 'text-earth'}`}>{label === 'Commandes liées' ? fmtNumber(value || 0) : (label === MARGIN_GROSS_LABEL || label === PROPOSED_PRICE_MARGIN_LABEL) && value == null ? '—' : fmtCurrency(value || 0)}</p>{label === ERP_REVENUE_LABEL ? <p className="mt-1 text-meta text-slate">{costs.saleSource}</p> : null}{label === 'Prix proposé (moteur)' && proposed.pricingBasis ? <p className="mt-1 text-meta text-slate">{proposed.pricingBasis}</p> : null}{label === PROPOSED_PRICE_MARGIN_LABEL ? <p className="mt-1 text-meta text-slate">{proposed.marginSource}</p> : null}</div>)}</div></div>
    ) : null}

    {tab === 'historique' ? (
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="rounded-2xl border border-line bg-card p-4"><p className="font-semibold text-earth mb-2">Documents / photos</p>{docs.length ? <div className="space-y-2">{docs.map((doc, index) => <div key={`${doc.url || doc.title}-${index}`} className="rounded-xl bg-white border border-line px-3 py-2"><p className="text-sm font-semibold text-earth">{fallbackText(doc.title || doc.nom || doc.type, 'Document animal')}</p><p className="text-xs text-slate break-all">{fallbackText(doc.url || doc.file_url || doc.lien, 'Lien non renseigné')}</p></div>)}</div> : <p className="text-sm text-horizon-dark">Aucun document ou photo renseigné.</p>}</div>
        <div className="rounded-2xl border border-line bg-card p-4"><p className="font-semibold text-earth mb-2">Historique de vie</p><div className="space-y-2">{historyRows.map((item, index) => <div key={`${item.date || 'date'}-${item.title}-${index}`} className="rounded-xl bg-white border border-line px-3 py-2"><p className="text-xs text-slate">{dateLabel(item.date)}</p><p className="font-semibold text-earth">{item.title}</p><p className="text-xs text-slate">{item.detail}</p></div>)}</div>{!historyRows.length ? <p className="text-sm text-horizon-dark">Aucun événement lié visible.</p> : null}</div>
      </div>
    ) : null}
  </div></BaseModal>;
}
export default function AnimauxSpeciesFocused({ species = 'Bovin', rows = [], alimentationLogs = [], vaccins = [], businessEvents = [], salesOrders = [], payments = [], transactions = [], marketPrices = [], onCreate, onUpdate, onDelete, onRefresh }) {
  ...
  return <div className="space-y-6"><SectionHeader title={`Gestion des ${speciesPlural(species)}`} sub="Fiches animaux : identité, pesées J+15/J-1, coûts, santé, Finance, vente liée et marge." actions={<><Btn icon={RefreshCw} variant="outline" small onClick={onRefresh}>Actualiser</Btn><Btn icon={Download} variant="outline" small onClick={exportRows}>Exporter</Btn><Btn icon={Plus} small onClick={() => setModal('create')}>Ajouter {species}</Btn></>} /><AnimalHealthBridge rows={normalizedRows} alimentationLogs={alimentationLogs} vaccins={vaccins} onUpdate={onUpdate} onRefresh={onRefresh} /><div className="grid grid-cols-2 lg:grid-cols-5 gap-4"><button onClick={() => setFilter('actifs')}><KpiCard icon={CheckCircle} label="Actifs" value={summary.active.length} color="bg-positive text-positive" /></button><button onClick={() => setFilter('prets')}><KpiCard icon={CheckCircle} label="Prêts vente" value={summary.ready.length} color="bg-vigilance text-horizon-dark" /></button><button onClick={() => setFilter('retard')}><KpiCard icon={AlertTriangle} label="Pesées en retard" value={summary.late.length} color="bg-urgent text-urgent" /></button><button onClick={() => setFilter('vendus')}><KpiCard icon={Lock} label="Vendus/verrouillés" value={summary.sold.length} color="bg-neutral text-neutral" /></button><button onClick={() => setFilter('surveillance')}><KpiCard icon={AlertTriangle} label="À surveiller" value={summary.sick.length} color="bg-urgent text-urgent" /></button></div><div className="grid grid-cols-2 lg:grid-cols-4 gap-4"><MiniMetric label={COST_UNIFIED_LABEL} value={fmtCurrency(summary.invested)} /><MiniMetric label={PRODUCTION_FINANCE_LABELS.revenue} value={fmtCurrency(summary.revenue)} /><MiniMetric label={MARGIN_GROSS_LABEL} value={summary.marginCount ? fmtCurrency(summary.margin) : '—'} danger={summary.marginCount > 0 && summary.margin < 0} /><MiniMetric label="Poids moyen" value={`${summary.avgWeight.toFixed(1)} kg`} /></div><div className="flex flex-wrap gap-2">{['tous', 'actifs', 'prets', 'retard', 'vendus', 'surveillance'].map((item) => <button key={item} type="button" onClick={() => setFilter(item)} className={`px-3 py-2 rounded-lg text-sm capitalize ${filter === item ? 'bg-earth text-white font-semibold' : 'bg-white border border-line text-slate'}`}>{item === 'prets' ? 'prêts vente' : item}</button>)}</div><div className="rounded-3xl border border-line bg-white overflow-hidden"><div className="px-6 py-4 border-b border-line"><p className="font-semibold text-earth">Liste {speciesPlural(species)}</p><p className="text-sm text-slate">Les infos importantes sont visibles ici. La courbe complète est dans la fiche.</p></div><div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-card text-xs uppercase tracking-normal text-slate"><tr><th className="px-4 py-3 text-left">Animal</th><th className="px-4 py-3 text-left">Poids</th><th className="px-4 py-3 text-left">Objectif</th><th className="px-4 py-3 text-left">Progression</th><th className="px-4 py-3 text-left">Pesée</th><th className="px-4 py-3 text-left">Santé</th><th className="px-4 py-3 text-left">Statut</th><th className="px-4 py-3 text-right">Prix proposé</th><th className="px-4 py-3 text-right">{COST_UNIFIED_LABEL}</th><th className="px-4 py-3 text-right">{PROPOSED_PRICE_MARGIN_LABEL}</th><th className="px-4 py-3 text-right">Actions</th></tr></thead><tbody>{filtered.map((row) => { const g = growthInfo(row); const costs = costBreakdown(row, { alimentationLogs, vaccins, businessEvents, salesOrders, payments, transactions }); const rowPricing = recommendAnimalSalePrice({ animal: row, alimentationLogs, vaccins, marketPrices }); const rowProposed = buildAnimalProposedSaleDisplay(rowPricing, costs); return <tr key={row.id} className="border-t border-line hover:bg-card"><td className="px-4 py-3"><div className="flex items-center gap-2"><QrCode size={14} className="text-positive" /><div><p className="font-semibold text-earth">{row.name || row.nom || physicalIdOf(row)}</p><p className="text-xs text-slate">{physicalIdOf(row)} · {species}</p></div></div></td><td className="px-4 py-3 font-semibold text-earth">{fmtNumber(g.current)} kg</td><td className="px-4 py-3 text-slate">{g.target ? `${fmtNumber(g.target)} kg` : 'À renseigner'}</td><td className="px-4 py-3"><ProgressBar value={g.progress} /></td><td className="px-4 py-3"><p className={g.weighingStatus === 'retard' ? 'font-semibold text-urgent' : 'text-slate'}>{g.nextWeighing || '—'}</p><p className="text-xs text-horizon-dark">Rappel {g.reminderDate || '—'}</p></td><td className="px-4 py-3 text-slate">{healthOf(row)}</td><td className="px-4 py-3">{isLocked(row) ? <span className="inline-flex items-center gap-1 text-neutral font-semibold"><Lock size={13} /> {statusOf(row)}</span> : statusBadge(g.status)}</td><td className="px-4 py-3 text-right"><p className="font-semibold text-positive">{rowProposed.proposedPrice > 0 ? fmtCurrency(rowProposed.proposedPrice) : "—"}</p><p className="text-meta text-slate">plancher {rowProposed.minimumPrice > 0 ? fmtCurrency(rowProposed.minimumPrice) : "—"}</p>{rowProposed.pricingBasis ? <p className="text-meta text-slate">{rowProposed.pricingBasis}</p> : null}</td><td className="px-4 py-3 text-right"><p className="font-semibold text-earth">{fmtCurrency(costs.total)}</p>{!costs.costComplete ? <p className="text-meta text-horizon-dark">Coût partiel</p> : null}{costs.finance > 0 ? <p className="text-meta text-slate">Finance {fmtCurrency(costs.finance)}</p> : null}</td><td className="px-4 py-3 text-right"><p className={`text-xs font-semibold ${rowProposed.marginOnProposed != null && rowProposed.marginOnProposed >= 0 ? 'text-positive' : rowProposed.marginOnProposed != null ? 'text-urgent' : 'text-slate'}`}>{rowProposed.marginOnProposed != null ? fmtCurrency(rowProposed.marginOnProposed) : '—'}</p><p className="text-meta text-slate">{rowProposed.marginSource}</p>{rowProposed.ficheDiverges ? <p className="text-meta text-horizon-dark">fiche {fmtCurrency(rowProposed.erpRevenue)}</p> : null}</td><td className="px-4 py-3"><div className="flex justify-end gap-1"><ActionIconButton icon={Eye} title="Voir" color="sky" onClick={() => { setSelected(row); setModal('details'); }} />{!isLocked(row) ? <ActionIconButton icon={Edit} title="Modifier" color="amber" onClick={() => { setSelected(row); setModal('edit'); }} /> : null}<ActionIconButton icon={Trash2} title="Supprimer" color="red" onClick={() => { setSelected(row); setModal('delete'); }} /></div></td></tr>; })}{!filtered.length ? <tr><td colSpan={11} className="px-4 py-8 text-center text-slate">Aucun animal pour ce filtre.</td></tr> : null}</tbody></table></div></div><AnimalDetailModal open={modal === 'details'} onClose={() => setModal(null)} animal={selected} alimentationLogs={alimentationLogs} vaccins={vaccins} businessEvents={businessEvents} salesOrders={salesOrders} payments={payments} transactions={transactions} marketPrices={marketPrices} /><CreateModal open={modal === 'create'} onClose={() => setModal(null)} onSubmit={submitCreate} fields={createFields} initialValues={initialValues} deriveValues={deriveCreateValues} loading={saving} title={`Ajouter ${species}`} submitLabel="Ajouter" /><EditModal open={modal === 'edit'} onClose={() => setModal(null)} onSubmit={submitEdit} fields={editFields} initialValues={selected ? { ...selected, poids_history_text: parseHistory(selected.poids_history || selected.weight_history || selected.historique_poids).map((p) => `${p.date} | ${p.poids}${p.note ? ` | ${p.note}` : ''}`).join('\n') } : {}} loading={saving} title={`Modifier ${species}`} submitLabel="Enregistrer" /><DeleteModal open={modal === 'delete'} onClose={() => setModal(null)} onConfirm={submitDelete} itemLabel={selected ? selected.name || selected.nom || selected.id : ''} loading={saving} /></div>;
}
