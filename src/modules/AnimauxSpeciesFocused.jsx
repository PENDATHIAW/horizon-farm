...
function AnimalDetailModal({ open, onClose, animal, alimentationLogs = [], vaccins = [], businessEvents = [], salesOrders = [], payments = [], transactions = [], marketPrices = [] }) {
  ...
  return <BaseModal open={open} onClose={onClose} title={`Fiche ${animal.type || animal.espece || 'animal'} - ${physicalIdOf(animal)}`} size="5xl"><div className="space-y-5">
    ...
    {tab === 'finances' ? (
      <div className="rounded-2xl border border-red-200 bg-red-50 p-4"><p className="font-black text-red-800 mb-1">{COST_UNIFIED_LABEL}, prix proposé et marges</p><p className="text-sm text-red-700 mb-3">{SALE_PRICE_HELP_ANIMAL} · {MARGIN_GROSS_DEFINITION_SHORT} pour le revenu fiche enregistré.</p>{costs.warnings.length ? <div className="mb-3 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800"><AlertTriangle size={15} className="inline" /> {costs.warnings.join(' ')}</div> : null}{proposed.ficheDivergeNote ? <div className="mb-3 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">{proposed.ficheDivergeNote}</div> : null}<div className="grid grid-cols-2 lg:grid-cols-4 gap-2">{[['Prix achat', costs.achat], ['Alimentation/coût lié', costs.alimentation], ['Soins/vaccins liés', costs.sante], ['Autres frais', costs.autres], ['Événements de charge', costs.evenements], ['Finance liée', costs.finance], [COST_UNIFIED_LABEL, costs.total], ['Prix proposé (moteur)', proposed.proposedPrice], ['Plancher acceptable', proposed.minimumPrice || salePricing.minimumPrice], [PROPOSED_PRICE_MARGIN_LABEL, proposed.marginOnProposed], [ERP_REVENUE_LABEL, costs.sale], [MARGIN_GROSS_LABEL, costs.marge], ['Payé', costs.paid], ['Reste à encaisser', costs.remaining], ['Commandes liées', costs.salesCount], ['Coût/kg', g.current > 0 ? costs.total / g.current : 0]].map(([label, value]) => <div key={label} className="rounded-xl bg-white border border-red-100 p-3"><p className="text-xs text-[#8a7456]">{label}</p><p className={`font-black mt-1 ${(label === MARGIN_GROSS_LABEL || label === PROPOSED_PRICE_MARGIN_LABEL) && value != null && value < 0 ? 'text-red-600' : 'text-[#2f2415]'}`}>{label === 'Commandes liées' ? fmtNumber(value || 0) : (label === MARGIN_GROSS_LABEL || label === PROPOSED_PRICE_MARGIN_LABEL) && value == null ? '—' : fmtCurrency(value || 0)}</p>{label === ERP_REVENUE_LABEL ? <p className="mt-1 text-[11px] text-[#8a7456]">{costs.saleSource}</p> : null}{label === 'Prix proposé (moteur)' && proposed.pricingBasis ? <p className="mt-1 text-[11px] text-[#8a7456]">{proposed.pricingBasis}</p> : null}{label === PROPOSED_PRICE_MARGIN_LABEL ? <p className="mt-1 text-[11px] text-[#8a7456]">{proposed.marginSource}</p> : null}</div>)}</div></div>
    ) : null}
