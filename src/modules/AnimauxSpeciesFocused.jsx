...
function growthInfo(row = {}) {
  ...
  return { history, current, target, progress, gain, gainDay, status, decision };
}
...
export default function AnimauxSpeciesFocused({ species = 'Bovin', rows = [], alimentationLogs = [], vaccins = [], loading, onCreate, onUpdate, onDelete, onRefresh }) {
  ...
  const summary = useMemo(() => {
    ...
    const invested = normalizedRows.reduce((sum, row) => sum + purchaseCost(row) + toNumber(row.alimentation) + toNumber(row.sante), 0);
