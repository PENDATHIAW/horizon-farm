...
import { coherenceRowTab } from './achatsStockMetrics.js';
...
const MODULE_LINKS = [
  { key: 'elevage', label: 'Élevage', tab: 'Alimentation', hint: 'Distributions aliment cheptel & lots' },
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                ...
                {finding.auto_action ? (
                  <Btn variant="outline" small disabled={busyId === finding.id} onClick={() => onApplyFinding?.(finding)}>
                    {finding.auto_action === 'create_task' ? 'Créer tâche' : finding.auto_action === 'create_alert' ? 'Créer alerte' : 'Appliquer'}
                  </Btn>
                ) : null}
              </div>
            </div>
