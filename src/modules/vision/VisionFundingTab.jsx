  ...
  return (
    <div className="space-y-5">
      ...
      <Section icon={Scale} title="Dossier financeur">
        ...
        <Row title="Chiffres clés" detail={`CA ${fmtCurrency(data.salesAmount)} · objectif ${fmtCurrency(data.growthGoal?.monthTarget || 0)} · trésorerie ${fmtCurrency(data.treasuryResult ?? data.balance)}`} value="Pilotage" onClick={() => onNavigate?.('objectifs_croissance', { tab: 'Performance' })} />
        <Row title="Preuves & justificatifs" detail={`${fmtNumber(data.documents.length)} doc(s) · ${fmtNumber(data.missingProof)} manquant(s)`} value={data.missingProof ? 'À compléter' : 'OK'} tone={data.missingProof ? 'warn' : 'good'} onClick={() => onNavigate?.('documents_rapports', { tab: 'Preuves' })} />
        <Row title="Risques maîtrisés" detail={`${fmtNumber(data.risks.filter((r) => r.tone !== 'bad').length)} risque(s) modéré(s)`} value={`${fmtNumber(data.risks.filter((r) => r.tone === 'bad').length)} critique(s)`} tone={data.risks.some((r) => r.tone === 'bad') ? 'warn' : 'good'} onClick={() => onNavigate?.('centre_decisionnel', { tab: 'Efficacité' })} />
        ...
        {data.documents.length ? data.documents.slice(0, 6).map((r) => (
          <Row key={r.id || label(r)} title={label(r)} detail={`${r.type || r.categorie || 'Document'} · ${dateOf(r)}`} value="Doc" onClick={() => onNavigate?.('documents_rapports', { tab: 'Bibliothèque' })} />
        )) : null}
