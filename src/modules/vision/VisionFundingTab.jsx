  ...
  return (
    <div className="space-y-5">
      ...
      <Section icon={Scale} title="Dossier financeur">
        ...
        <Row title="Risques maîtrisés" detail={`${fmtNumber(data.risks.filter((r) => r.tone !== 'bad').length)} risque(s) modéré(s)`} value={`${fmtNumber(data.risks.filter((r) => r.tone === 'bad').length)} critique(s)`} tone={data.risks.some((r) => r.tone === 'bad') ? 'warn' : 'good'} onClick={() => onNavigate?.('centre_ia', { tab: 'Efficacité' })} />
