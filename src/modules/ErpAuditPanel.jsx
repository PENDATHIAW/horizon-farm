import { AlertTriangle, CheckCircle2, ClipboardCheck, Download, RefreshCw, XCircle } from 'lucide-react';
...
function buildAudit(data) {
  ...
  const modules = ['Accueil','Centre décisionnel','Objectifs','Animaux','Avicole','Santé','Finances','Comptabilité','Investissements','Impact Business','Stock','Clients','Ventes','Fournisseurs','Traçabilité','Alertes','Cultures','Documents','Tâches','RH','Rapports','Équipements','Smart Farm','Sync'];
  const grouped = modules.map((module) => {
    ...
    return { module, severity, issues: list };
  });
...
function exportAudit(report) {
  const rows = report.grouped.flatMap((g) => g.issues.length ? g.issues.map((i) => ({ module: g.module, statut: labelSeverity(i.severity), probleme: i.title, detail: i.detail, action: i.action })) : [{ module: g.module, statut: 'OK', probleme: '', detail: '', action: '' }]);
  const csv = ['module;statut;probleme;detail;action', ...rows.map((r) => [r.module, r.statut, r.probleme, r.detail, r.action].map((v) => `"${String(v || '').replace(/"/g, '""')}"`).join(';'))].join('\n');
...
export default function ErpAuditPanel() {
  ...
  return <section className="rounded-3xl border border-[#d6c3a0] bg-white p-5 shadow-sm space-y-5">
    <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
      <div><p className="inline-flex items-center gap-2 rounded-full border border-[#eadcc2] bg-[#fffdf8] px-3 py-1 text-xs font-black text-[#8a7456]"><ClipboardCheck size={14} /> Audit testeur ERP</p><h2 className="mt-3 text-2xl font-black text-[#2f2415]">Simulation d’un test humain module par module</h2><p className="mt-1 text-sm text-[#8a7456]">Même interface pour données simulées ou réelles : l’audit vérifie cohérence, liens entre modules, ventes, coûts, marges, documents, alertes et tâches.</p></div>
      <div className="flex flex-wrap gap-2"><Btn icon={RefreshCw} variant="outline" small onClick={refreshAll}>Recharger audit</Btn><Btn icon={Download} variant="outline" small onClick={() => exportAudit(report)}>Exporter CSV</Btn></div>
    </div>
