...
export default function CultureOperationalHealthPanel({ rows = [], salesOrders = [], onNavigate }) {
  ...
  return <section className="rounded-3xl border border-[#d6c3a0] bg-white p-5 shadow-sm space-y-4">
    ...
    <div className="flex flex-wrap justify-end gap-2"><button type="button" onClick={() => onNavigate?.('achats_stock', { tab: 'Stock' })} className="rounded-xl border border-[#eadcc2] bg-[#fffdf8] px-3 py-2 text-sm font-bold text-[#2f2415]">Intrants stock</button><button type="button" onClick={() => onNavigate?.('commercial', { tab: 'Opportunités' })} className="rounded-xl border border-[#eadcc2] bg-[#fffdf8] px-3 py-2 text-sm font-bold text-[#2f2415]">Commercial</button><button type="button" onClick={() => onNavigate?.('cultures', { tab: 'Intrants & Météo' })} className="rounded-xl border border-[#eadcc2] bg-[#fffdf8] px-3 py-2 text-sm font-bold text-[#2f2415]">Météo</button></div>
  </section>;
}
