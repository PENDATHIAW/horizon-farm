...
function repairLabel(issue = {}) {
  if (issue.module === 'sales_orders' && String(issue.message || '').toLowerCase().includes('statut')) return 'Mettre à jour';
  if (issue.module === 'sales_opportunities') return 'Fermer';
...
function canRepair(issue = {}, props = {}) { const label = repairLabel(issue); if (label === 'Mettre à jour') return Boolean(props.onUpdateSalesOrder); if (label === 'Fermer') return Boolean(props.onUpdateOpportunity); if (label === 'Créer justificatif') return Boolean(props.onCreateDocument); if (label === 'Créer tâche') return Boolean(props.onCreateTask); return Boolean(props.onCreateAlert); }
...
async function repairIssue(issue, props) {
  ...
  if (label === 'Créer justificatif') {
    await props.onCreateDocument?.({ id: makeRepairId('DOC'), title: `Justificatif à compléter`, document_category: 'justificatif', module_source: issue.module || 'sync_activity', entity_type: issue.module || 'erp_issue', entity_id: issue.row_id || issue.linked_id || '', related_id: issue.linked_id || issue.row_id || '', notes: issue.message || 'Justificatif créé depuis les vérifications.', status: 'a_completer' });
    await props.onRefreshDocuments?.();
    await createRepairTrace(props, issue, 'Justificatif créé');
    return 'Justificatif créé';
  }
  if (label === 'Créer tâche') {
    await props.onCreateTask?.({ id: makeRepairId('TASK'), title: `À vérifier`, module_lie: issue.module || 'sync_activity', entity_type: issue.module || 'erp_issue', related_id: issue.row_id || issue.linked_id || '', due_date: today(), priority: issue.severity === 'critical' ? 'critique' : 'haute', status: 'a_faire', notes: issue.message || 'Tâche créée depuis les vérifications.', source_module: 'sync_activity', source_record_id: issue.row_id || issue.linked_id || issue.flow || '', action_key: issueKey(issue) });
