...
async function upsertMeatStock({ stockCrud, produit, categorie, quantityDelta, animal, event }) {
  ...
  if (existing) {
    ...
    await stockCrud.update?.(existing.id, { quantite: Number(nextQty.toFixed(2)), statut: nextQty <= 0 ? 'epuise' : 'ok', stock_status: nextQty <= 0 ? 'epuise' : 'ok', last_movement_type: delta > 0 ? 'entree_abattage_animal' : 'correction_abattage_animal', last_movement_qty: Number(delta.toFixed(2)), last_movement_at: new Date().toISOString(), linked_event_id: event.id });
    await stockCrud.refresh?.(); return;
  }
  if (delta < 0) return;
  await stockCrud.create?.({ id: makeId('STKVIANDE'), produit, categorie, activite_liee: 'animaux', quantite: Number(delta.toFixed(2)), unite: 'kg', seuil: 0, stock_max: 0, prixUnit: 0, statut: event.destination === 'vente_directe' ? 'reserve' : 'ok', stock_status: event.destination === 'vente_directe' ? 'reserve' : 'ok', source_module: 'animaux', source_record_id: animal.id, origine_label: `${animal.type || 'Animal'} ${animal.name || animal.tag || animal.id}`, linked_event_id: event.id, date_derniere_reception: event.date || today(), last_movement_type: 'entree_abattage_animal', last_movement_qty: Number(delta.toFixed(2)), last_movement_at: new Date().toISOString(), notes: `Stock viande issu de l’abattage animal` });
...
export default function AnimalSlaughterStockBridge({ rows = [], businessEvents = [], onUpdate, onRefresh, onCreateBusinessEvent, onRefreshBusinessEvents }) {
  ...
  const submit = async (e) => {
    ...
    const payload = { ...form, id: form.id || `ABAT-AN-${Date.now()}`, animal_id: animal.id, related_id: animal.id, animal_name: animal.name || animal.tag || animal.id, animal_type: animal.type, produit_stock: product.produit, categorie_stock: product.categorie, poids_carcasse: weight, poids_total_abattage: weight, type_evenement: 'abattage_animal', source_module: 'animaux_abattage', module_lie: 'animaux', title: `Abattage animal: ${animal.name || animal.id}`, message: `${product.produit}: ${weight.toFixed(2)} kg`, status: 'genere', date: form.date || today() };
