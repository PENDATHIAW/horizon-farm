...
function Field({ label, children }) { return <label className="text-xs font-semibold text-slate space-y-1"><span>{label}</span>{children}</label>; }
...
const buildInitialSlaughterForm = (animalId = '') => ({ id: `ABAT-AN-${Date.now()}`, animal_id: animalId, date: today(), age_class: '', poids_carcasse: '', destination: 'stock', frais_abattage: 0, frais_decoupe: 0, frais_emballage: 0, frais_transport: 0, autres_frais: 0, responsable: '', notes: '' });
...
export default function AnimalSlaughterStockBridge({ rows = [], alimentationLogs = [], vaccins = [], businessEvents = [], onUpdate, onRefresh, onCreateBusinessEvent, onRefreshBusinessEvents }) {
  ...
  const [form, setForm] = useState(() => buildInitialSlaughterForm(activeAnimals[0]?.id));
  ...
  const submit = async (e) => {
    ...
    setEditing(null); setForm(buildInitialSlaughterForm(activeAnimals[0]?.id));
  };
