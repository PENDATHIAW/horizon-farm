...
export default function SanteEvolution({ rows = [], onNavigate }) {
  ...
  return <div className="space-y-5">
    ...
    {impacts.length ? <div className="rounded-2xl border border-[#eadcc2] bg-[#fffdf8] p-4 text-sm text-[#7d6a4a]"><b className="text-[#2f2415]">Conséquences dominantes :</b> {impacts.map(([name, count]) => `${name} (${fmtNumber(count)})`).join(' · ')}</div> : null}
