...
export default function Sync({ onRefreshAll, onFlushOffline, online = true, lastOnlineAt, dataMap = {}, embedded = false }) {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4"><KpiCard icon={Wifi} label="Réseau" value={online ? 'Connecté' : 'Hors ligne'} sub={online ? 'À jour' : 'Local'} color={online ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'} /><KpiCard icon={Cloud} label="Dernier envoi" value={lastSync} sub="Dernier passage" color="bg-sky-500/20 text-sky-400" /><KpiCard icon={Database} label="En attente" value={pendingItems.length} sub="À envoyer" color={pendingItems.length ? 'bg-amber-500/20 text-amber-400' : 'bg-emerald-500/20 text-emerald-400'} /><KpiCard icon={History} label={embedded ? 'État envoi' : 'Journal visible'} value={embedded ? (online ? 'OK' : 'Local') : syncLogRows.length} sub={embedded ? 'journal séparé' : 'activité + contrôle + événements'} color="bg-purple-500/20 text-purple-400" /></div>
    {highRiskCount ? <div className="rounded-2xl border border-red-200 bg-red-50 p-4 text-sm text-red-700"><AlertTriangle size={16} className="inline" /> {highRiskCount} action(s) hors ligne à risque élevé. Vérifier avant envoi.</div> : null}
