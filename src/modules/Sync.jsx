...
export default function Sync({ onRefreshAll, onFlushOffline, online = true, lastOnlineAt, dataMap = {}, embedded = false }) {
  ...
  const actions = <><Btn icon={RefreshCw} variant="outline" small onClick={synchronize} disabled={syncing}>{syncing ? 'Sync...' : 'Synchroniser'}</Btn><Btn icon={Download} variant="outline" small onClick={downloadBackup}>Backup</Btn><Btn icon={Trash2} variant="outline" small onClick={clearLocalQueue}>Vider file offline</Btn></>;

  return <div className="space-y-6">
    {!embedded ? <SectionHeader title="Mode Offline & Synchronisation" sub="Données locales, sauvegarde, logs sync et contrôle des conflits. Ne supprime jamais les tables ERP." actions={actions} /> : <div className="flex flex-wrap justify-end gap-2">{actions}</div>}
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4"><KpiCard icon={Wifi} label="Réseau" value={online ? 'Connecté' : 'Hors ligne'} sub={online ? 'À jour' : 'Local'} color={online ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'} /><KpiCard icon={Cloud} label="Dernière sync" value={lastSync} sub="Dernier passage" color="bg-sky-500/20 text-sky-400" /><KpiCard icon={Database} label="En attente" value={pendingItems.length} sub="À synchroniser" color={pendingItems.length ? 'bg-amber-500/20 text-amber-400' : 'bg-emerald-500/20 text-emerald-400'} /><KpiCard icon={History} label={embedded ? 'État sync' : 'Logs visibles'} value={embedded ? (online ? 'OK' : 'Local') : syncLogRows.length} sub={embedded ? 'journal séparé' : 'sync + audit + événements'} color="bg-purple-500/20 text-purple-400" /></div>
    ...
    {!embedded ? <ModuleTimeline title="Timeline synchronisation" subtitle="Réseau, actions locales, audit logs, événements métier et conflits détectés." rows={timelineRows} onRefresh={synchronize} navigateLabel="Synchroniser" /> : null}
