...
function DiagnosticCard({ diagnostic, invoice }) {
  ...
  return (
    <div className="rounded-2xl border border-[#eadcc2] bg-[#fffdf8] p-4 space-y-3">
      <p className="font-black text-[#2f2415]">{rec.headline || 'Diagnostic économique'}</p>
      <p className="text-sm text-[#7d6a4a]">{rec.summary}</p>
      <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
        <div className="rounded-xl border border-[#eadcc2] bg-white p-3 text-xs">
          <span className="text-[#8a7456]">Fournisseur</span>
          <p className="font-black text-[#2f2415]">{invoice?.fournisseur || '—'}</p>
        </div>
        <div className="rounded-xl border border-[#eadcc2] bg-white p-3 text-xs">
          <span className="text-[#8a7456]">Total</span>
          <p className="font-black text-[#2f2415]">{fmtCurrency(invoice?.montant_total || 0)}</p>
        </div>
        <div className="rounded-xl border border-[#eadcc2] bg-white p-3 text-xs">
          <span className="text-[#8a7456]">Évolution prix</span>
          <p className={`font-black ${price.trend === 'hausse' ? 'text-amber-700' : price.trend === 'baisse' ? 'text-emerald-700' : 'text-[#2f2415]'}`}>
            {price.delta_pct != null ? `${price.delta_pct > 0 ? '+' : ''}${price.delta_pct} %` : 'N/A'}
          </p>
        </div>
        <div className="rounded-xl border border-[#eadcc2] bg-white p-3 text-xs">
          <span className="text-[#8a7456]">Stockable</span>
          <p className="font-black text-[#2f2415]">{invoice?.stockable ? 'Oui' : 'Non (charge)'}</p>
        </div>
      </div>
      {margin.applicable ? (
      ...
      {treasury.alerte_tresorerie ? (
        <p className="text-xs text-red-700 font-semibold">{treasury.alerte_tresorerie}</p>
      ) : null}
      {rec.bullets?.length ? (
        <ul className="text-xs text-[#8a7456] space-y-1">
...
export default function InvoiceOcrIntelligentPanel({
  ...
  return (
    <section className="rounded-3xl border border-[#d6c3a0] bg-white p-5 shadow-sm space-y-5">
      <div>
        <p className="text-xs uppercase tracking-[0.2em] text-[#9a6b12] font-black">OCR Intelligent Horizon</p>
      ...
      <div className="flex flex-wrap gap-2">
        {INVOICE_OCR_DEMO_SAMPLES.map((sample) => (
          <button
            ...
            className="rounded-xl border border-[#eadcc2] bg-[#fffdf8] px-3 py-2 text-xs font-black text-[#2f2415] hover:bg-[#dcfce7]"
          >
            {sample.label}
          </button>
        ))}
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <Field label="Importer facture (PDF / image)">
          ...
          <button type="button" onClick={() => fileRef.current?.click()} className="flex w-full items-center justify-center gap-2 rounded-xl border border-dashed border-[#d6c3a0] bg-[#fffdf8] px-4 py-6 text-sm font-black text-[#2f2415]">
            <Upload size={18} />
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <button
          ...
          onClick={onExecute}
          className="inline-flex items-center gap-2 rounded-xl bg-emerald-700 px-4 py-2 text-xs font-black text-white disabled:opacity-50"
        >
      ...
      {draft ? (
        <div className="rounded-2xl border border-emerald-200 bg-emerald-50/40 p-4 text-xs space-y-2">
          ...
          {safety?.needsConfirmation ? (
            <p className="text-amber-800">Confirmation requise avant exécution.</p>
          ) : null}
          {draft.warnings?.slice(0, 3).map((w) => (
            <p key={w} className="text-[#8a7456]">• {w}</p>
          ))}
        </div>
      ) : null}
    </section>
