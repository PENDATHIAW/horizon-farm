...
export default function Avicole({
  ...
  const summary = useMemo(() => {
    ...
    const avgWeight = activityRows.length
      ...
      : 0;
    const eggsToday = activityType === 'Pondeuse'
      ? pondeusesRows.reduce((sum, lot) => sum + toNumber(metricsFor(lot).eggMetrics?.todayEggs), 0)
      : 0;
    const brokenEggs = activityProductionLogs.reduce((sum, log) => sum + toNumber(log.oeufs_casses), 0);
    const trays = activityProductionLogs.reduce((sum, log) => sum + toNumber(log.plateaux), 0);
    const layingRate = totalTetes > 0 && activityType === 'Pondeuse' ? (eggsToday / totalTetes) * 100 : 0;
    return { totalTetes, revenu, feeding, mortality, sick, ready, avgWeight, eggsToday, brokenEggs, trays, layingRate };
  }, [activityRows, activityType, pondeusesRows, activityProductionLogs, metricsFor]);
...
function PondeuseKpis({ summary }) {
  return (
    <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
      ...
      <KpiCard icon={Activity} label="Oeufs du jour" value={fmtNumber(summary.eggsToday)} color="bg-emerald-500/20 text-emerald-400" />
      <KpiCard icon={TrendingUp} label="Taux ponte" value={`${summary.layingRate.toFixed(1)}%`} color="bg-sky-500/20 text-sky-400" />
      <KpiCard icon={AlertTriangle} label="Oeufs casses" value={fmtNumber(summary.brokenEggs)} color="bg-red-500/20 text-red-400" />
      <KpiCard icon={Activity} label="Alimentation" value={money(summary.feeding)} color="bg-emerald-500/20 text-emerald-400" />
    </div>
  );
}
...
function ProductionPanel({ logs, chartData, onAdd, onDetails, onEdit, onDelete }) {
  return (
      ...
      <div className="bg-white border border-[#d6c3a0] rounded-2xl p-5">
        <div className="flex items-center justify-between gap-3 mb-4">
          <div>
            ...
            <p className="text-xs text-[#8a7456]">Suivi reserve aux pondeuses.</p>
          </div>
