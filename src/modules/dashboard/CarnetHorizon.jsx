...
function DomainCard({ card }) {
  return (
    <article className="carnet-domain-card flex min-h-[168px] flex-col rounded-xl border border-[#e5dcc8] bg-gradient-to-b from-[#fffdf8] to-[#f8f3e8] p-3 shadow-[0_1px_0_rgba(47,36,21,0.05)]">
      <div className="flex items-center gap-1.5">
        <span className="text-base leading-none" aria-hidden="true">{card.icon}</span>
        <p className="text-[10px] font-black tracking-wide text-[#8a7456]">{card.title}</p>
      </div>
      ...
      {card.alerts?.length ? (
        <ul className="mt-2 space-y-0.5 border-t border-[#efe6d6] pt-2">
