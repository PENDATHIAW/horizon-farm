...
export default function StockLossFinanceBridge({ rows = [], onUpdate, onCreateFinanceTransaction, onRefreshFinances, onCreateBusinessEvent, onRefreshBusinessEvents, onRefresh }) {
  ...
  const declareLoss = async (row, qty) => {
    if (!row || qty <= 0) {
      ...
      {candidates.length ? (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          {candidates.map((row) => {
            ...
            return (
              <button key={row.id} type="button" onClick={() => openLossModal(row)} className="rounded-xl border border-amber-200 bg-white p-3 text-left hover:border-amber-400">
        ...
        <div className="rounded-xl border border-amber-200 bg-white p-3 text-sm text-amber-800">Aucun stock disponible pour déclarer une perte.</div>
      )}

      <QuickInputModal
        open={Boolean(lossTarget)}
        title="Déclarer une perte stock"
        description={lossTarget ? `${lossTarget.produit || lossTarget.nom || lossTarget.id} · disponible ${fmtNumber(lossTarget.quantite)} ${lossTarget.unite || ''}` : ''}
        ...
        onSubmit={() => declareLoss(lossTarget, toNumber(lossQty))}
        busy={lossSaving}
      />
    </section>
  );
