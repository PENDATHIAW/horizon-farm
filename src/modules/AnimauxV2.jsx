...
export default function AnimauxV2(props) {
  ...
  const dataMap = { sales_orders: salesOrders, salesOrders, payments, finances: transactions, transactions, animaux: props.rows || [] };
  ...
  return <div className="space-y-6 animaux-mobile-structured"><style>{`@media (max-width: 640px){.animaux-mobile-structured .rounded-2xl{border-radius:18px}.animaux-mobile-structured table{font-size:12px}.animaux-mobile-structured th,.animaux-mobile-structured td{padding-left:10px!important;padding-right:10px!important}.animaux-mobile-structured .text-2xl{font-size:1.35rem}.animaux-mobile-structured .grid{gap:.75rem}.animaux-mobile-structured .overflow-x-auto{max-width:100vw}}`}</style>
    ...
    <ModuleSection icon={PackageCheck} title={`${species}s : suivi quotidien`} subtitle="Fiches animaux, santé, disponibilité, poids, coûts, ventes et actions utiles."><AnimauxSpeciesFocused {...props} {...commonWorkflowProps} species={species} rows={speciesRows} onCreate={wrapCreate} onUpdate={wrapUpdate} /></ModuleSection>
    <ModuleSection icon={Scissors} title={`${species}s : abattage, transformation et stock`} subtitle="Sortie de l’animal, transformation éventuelle et création de stock vendable."><AnimalSlaughterStockBridge rows={speciesRows} alimentationLogs={props.alimentationLogs || []} vaccins={props.vaccins || []} businessEvents={businessEvents} onUpdate={props.onUpdate} onRefresh={props.onRefresh} onCreateBusinessEvent={props.onCreateBusinessEvent || businessEventsCrud.create} onRefreshBusinessEvents={props.onRefreshBusinessEvents || businessEventsCrud.refresh} /></ModuleSection>
    <ModuleSection icon={PackageCheck} title={`${species}s : frais liés à un animal`} subtitle="Transport, traitement spécial, emballage, main-d’œuvre ponctuelle ou autre frais directement rattaché à un animal."><DirectChargesBridge title={`Frais directs ${species.toLowerCase()}s`} subtitle="Ces frais améliorent le calcul du coût réel par animal." targetType="animaux" targets={speciesRows} businessEvents={businessEvents} onCreateBusinessEvent={props.onCreateBusinessEvent || businessEventsCrud.create} onUpdateBusinessEvent={props.onUpdateBusinessEvent || businessEventsCrud.update} onDeleteBusinessEvent={props.onDeleteBusinessEvent || businessEventsCrud.remove} onRefreshBusinessEvents={props.onRefreshBusinessEvents || businessEventsCrud.refresh} /></ModuleSection>
    ...
    <ModuleSection icon={BarChart3} title={`${species}s : évolution`} subtitle="Poids, croissance, alimentation, santé, ventes, marge et coût par animal."><AnimauxEvolution rows={speciesRows} alimentationLogs={props.alimentationLogs || []} vaccins={props.vaccins || []} businessEvents={businessEvents} opportunities={opportunities} salesOrders={salesOrders} payments={payments} transactions={transactions} onNavigate={props.onNavigate} /></ModuleSection>
  </div>;
}
