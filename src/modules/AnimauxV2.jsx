import { BarChart3, Beef, ChevronDown, ClipboardList, PackageCheck, Scale, X } from 'lucide-react';
...
function HeyHorizonAnimalCard({ draft, rows, species, onCreate, onUpdate, onCreateBusinessEvent, onRefresh, onRefreshBusinessEvents, onClose }) {
  ...
  return <section className="rounded-3xl border border-positive bg-positive-bg p-6 shadow-card space-y-4">
    <div className="flex items-start justify-between gap-3"><div><p className="text-xs uppercase tracking-normal text-positive font-semibold flex items-center gap-2"><Scale size={15} /> Fiche animal</p><h3 className="mt-1 text-xl font-semibold text-earth">{title}</h3></div><button type="button" onClick={onClose} className="rounded-full border border-positive bg-white p-2 text-positive"><X size={16} /></button></div>
    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
      {formType === 'animal_creation' ? <><label className="space-y-1"><span className="text-xs font-semibold text-positive">Espèce</span><select value={type} onChange={(e) => setType(e.target.value)} className="w-full min-h-[44px] rounded-xl border border-positive bg-white px-3 py-2 text-sm"><option>Bovin</option><option>Ovin</option><option>Caprin</option></select></label><label className="space-y-1"><span className="text-xs font-semibold text-positive">Nom / repère</span><input value={name} onChange={(e) => setName(e.target.value)} className="w-full min-h-[44px] rounded-xl border border-positive bg-white px-3 py-2 text-sm" /></label></> : <label className="space-y-1"><span className="text-xs font-semibold text-positive">Animal</span><select value={targetId} onChange={(e) => setTargetId(e.target.value)} className="w-full min-h-[44px] rounded-xl border border-positive bg-white px-3 py-2 text-sm"><option value={targetId}>{animal ? `${labelOf(animal)} · ${targetId}` : targetId || 'Choisir'}</option>{rows.filter((row) => String(row.id) !== String(targetId)).map((row) => <option key={row.id} value={row.id}>{labelOf(row)} · {row.id}</option>)}</select></label>}
      {formType === 'animal_loss' ? <label className="space-y-1"><span className="text-xs font-semibold text-positive">Statut</span><select value={status} onChange={(e) => setStatus(e.target.value)} className="w-full min-h-[44px] rounded-xl border border-positive bg-white px-3 py-2 text-sm"><option value="mort">Mort</option><option value="perdu">Perdu</option><option value="vole">Volé</option></select></label> : <label className="space-y-1"><span className="text-xs font-semibold text-positive">Poids kg</span><input type="number" value={weight} onChange={(e) => setWeight(e.target.value)} className="w-full min-h-[44px] rounded-xl border border-positive bg-white px-3 py-2 text-sm" /></label>}
      <label className="space-y-1"><span className="text-xs font-semibold text-positive">Date</span><input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full min-h-[44px] rounded-xl border border-positive bg-white px-3 py-2 text-sm" /></label>
      <label className="space-y-1 md:col-span-3"><span className="text-xs font-semibold text-positive">Note</span><input value={note} onChange={(e) => setNote(e.target.value)} className="w-full min-h-[44px] rounded-xl border border-positive bg-white px-3 py-2 text-sm" /></label>
    </div>
    <div className="flex justify-end"><button type="button" onClick={submit} disabled={saving} className="rounded-xl bg-earth px-6 py-2 text-sm font-semibold text-white disabled:opacity-60">{saving ? 'Validation...' : 'Valider'}</button></div>
  </section>;
}
...
export default function AnimauxV2(props) {
  ...
  return <div id="animaux-module-root" className="space-y-6 animaux-mobile-structured"><style>{`@media (max-width: 640px){.animaux-mobile-structured .rounded-2xl{border-radius:18px}.animaux-mobile-structured table{font-size:12px}.animaux-mobile-structured th,.animaux-mobile-structured td{padding-left:10px!important;padding-right:10px!important}.animaux-mobile-structured .text-2xl{font-size:1.35rem}.animaux-mobile-structured .grid{gap:.75rem}.animaux-mobile-structured .overflow-x-auto{max-width:100vw}}`}</style>
    ...
    <ModuleSection icon={Beef} title="Cheptel"><div className="grid grid-cols-1 md:grid-cols-3 gap-3">{ANIMAL_SPECIES_TABS.map((tab) => <button key={tab} type="button" onClick={() => setSpecies(tab)} className={`rounded-2xl border px-4 py-3 text-left transition-all ${species === tab ? 'bg-earth text-white border-earth' : 'bg-white text-slate border-line'}`}><p className="text-xs uppercase tracking-normal">Espèce</p><p className="font-semibold">{tab}s</p><p className="text-xs opacity-75">{counts[tab] || 0} animaux · {filterAnimalsBySpecies(props.rows || [], tab).filter(isOperationalAnimal).length} actifs</p></button>)}</div></ModuleSection>
    ...
    <div className="rounded-2xl border border-line bg-white p-4"><p className="text-xs uppercase tracking-normal text-horizon-dark font-semibold">Espèce active</p><p className="mt-1 text-xl font-semibold text-earth">{species}s</p><p className="mt-1 text-sm text-slate">{activeSpeciesRows.length} actif(s) · {historicalSpeciesRows.length} en historique</p></div>
    ...
    <CollapsibleSection icon={ClipboardList} title={`Cycle et historique · ${species}s`} defaultOpen={false}><LifecycleHistoryPanel mode="animaux" rows={speciesRows} salesOrders={salesOrders} deliveries={deliveries} businessEvents={businessEvents} sante={props.vaccins || props.sante || []} alimentationLogs={props.alimentationLogs || []} /></CollapsibleSection>
