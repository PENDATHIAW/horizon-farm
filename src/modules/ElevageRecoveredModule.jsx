...
const emitHorizonForm = (module, form_type, intent_label, draft_fields = {}) => window.dispatchEvent(new CustomEvent('horizon-open-form', { detail: { module, draft: { primary_module: module, form_type, intent_label, status: 'draft_ready', draft_fields } } }));
...
function Summary({ data, setTab }) {
  return <div className="space-y-5">
    <div className="grid grid-cols-2 gap-3 xl:grid-cols-6"><Stat label="Animaux actifs" value={fmtNumber(data.activeAnimals)} /><Stat label="Lots pondeuses" value={fmtNumber(data.pondeuses)} tone="good" /><Stat label="Lots chair" value={fmtNumber(data.chair)} /><Stat label="Ramassages œufs" value={fmtNumber(data.productionLogs.length)} tone="good" /><Stat label="Sorties aliment" value={fmtNumber(data.feedLogs.length)} /><Stat label="Soins en retard" value={fmtNumber(data.healthLate)} tone={data.healthLate ? 'warn' : 'good'} /></div>
