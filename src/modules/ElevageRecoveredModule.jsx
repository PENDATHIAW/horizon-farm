...
export default function ElevageRecoveredModule(props) {
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-3xl border border-line bg-white p-4 shadow-card sm:p-6"><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs uppercase tracking-normal text-horizon-dark font-semibold">Production</p><h1 className="mt-0.5 text-xl font-semibold text-earth sm:text-2xl">Élevage</h1><p className="mt-1 hidden text-sm text-slate sm:block">Lots, alimentation, production, santé, coûts et historique de l’élevage.</p>{props.periodLabel ? <div className="mt-2"><PeriodScopeBadge label={props.periodLabel} /></div> : null}</div><div className="rounded-2xl border border-line bg-card px-4 py-3 text-sm"><span className="text-slate">Santé module </span>{data.healthScore > 0 ? <b className={data.healthScore >= 75 ? 'text-positive' : 'text-horizon-dark'}>{data.healthScore}/100</b> : <b className="text-slate">à évaluer</b>}</div></div></section>
