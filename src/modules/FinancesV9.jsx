import { AlertTriangle, ArrowDown, ArrowUp, BookOpen, CreditCard, Download, Edit, Eye, FileText, Landmark, Package, Plus, Receipt, RefreshCw, Trash2, TrendingDown, TrendingUp, Wallet } from 'lucide-react';
...
const REVENUE_KEYS = GROUP_KEYS.filter((key) => GROUPS[key].type === 'entree');
const ACTIVITY_OPTIONS = GROUP_KEYS.map((key) => ({ value: key, label: GROUPS[key].label }));
...
export default function FinancesV9({ rows = [], loading, onCreate, onUpdate, onDelete, onRefresh, stocks = [], fournisseurs = [], alimentationLogs = [], businessPlans = [], salesOrders = [], payments = [] }) {
