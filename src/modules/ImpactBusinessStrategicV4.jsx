import { AlertTriangle, ArrowRight, Bird, Building2, CheckCircle2, DollarSign, Factory, FileCheck2, GitBranch, HeartPulse, Layers, Package, Receipt, Scale, ShieldCheck, Sprout, Target, TrendingUp, Truck, Users } from 'lucide-react';
...
function marginState({ revenue = 0, expenses = 0, estimated = false }) {
  ...
  if (rev <= 0 && exp <= 0) return { value: 0, display: 'À compléter', label: 'Aucun CA ni coût lié', confidence: 20, tone: 'amber', reliable: false };
  if (rev > 0 && exp <= 0) return { value: rev, display: 'Coûts à compléter', label: 'Le CA existe, mais les coûts ne sont pas encore liés', confidence: estimated ? 45 : 50, tone: 'amber', reliable: false };
  if (rev <= 0 && exp > 0) return { value: -exp, display: 'Ventes à lier', label: 'Des coûts existent, mais le CA n’est pas encore lié', confidence: 35, tone: 'amber', reliable: false };
...
function scoreProfit(marginInfo, revenue) {
  if (!marginInfo.reliable) return marginInfo.confidence;
  if (revenue <= 0) return 30;
  return clamp(50 + ((marginInfo.value / revenue) * 100));
}
function scoreCash(revenue, expenses, receivables) {
  const pressure = toNumber(expenses) + toNumber(receivables);
  if (pressure <= 0) return revenue > 0 ? 75 : 45;
  return clamp((toNumber(revenue) / pressure) * 60);
}
function scoreDemand(sales, revenue, repeatClients = 0) {
  return clamp((sales > 0 ? 50 + Math.min(30, sales * 5) : revenue > 0 ? 45 : 25) + Math.min(15, repeatClients * 2));
}
function scoreCapacity({ criticalStock = 0, lateTasks = 0, activePeople = 0, equipmentsOk = 0 }) {
  return clamp(62 + Math.min(12, activePeople * 2) + Math.min(10, equipmentsOk * 2) - criticalStock * 7 - lateTasks * 4);
}
function scoreRisk(lossRate, alerts) { return clamp(95 - toNumber(lossRate) * 4 - toNumber(alerts) * 10); }
function scoreRotation(days, margin) {
  if (!days) return 40;
  const cycles = 365 / Math.max(1, days);
  return clamp(35 + Math.min(35, cycles * 5) + Math.min(30, Math.max(0, margin * cycles / 12) / 10000));
}
function scoreData(points, expected) { return clamp((toNumber(points) / Math.max(1, toNumber(expected))) * 100); }
function recommendation(score, s) {
  if (s.data < 40) return ['Tester petit volume', 'Il manque encore des données pour engager gros.'];
  if (s.rentabilite < 45) return ['Compléter les chiffres', 'La marge n’est pas assez fiable pour décider fort.'];
  ...
  if (score >= 80) return ['À renforcer', 'Les indicateurs sont favorables.'];
...
function makePerspective(input) {
  const marginInfo = marginState(input);
  const s = {
    rentabilite: scoreProfit(marginInfo, input.revenue),
    cash: scoreCash(input.revenue, input.expenses, input.receivables || 0),
    rotation: scoreRotation(input.cycleDays, marginInfo.reliable ? marginInfo.value : 0),
  ...
  const score = clamp(s.rentabilite * 0.24 + s.cash * 0.15 + s.rotation * 0.13 + s.risque * 0.15 + s.demande * 0.15 + s.capacite * 0.10 + s.data * 0.08);
  const [label, reason] = recommendation(score, s);
  return { ...input, marginInfo, scores: s, score, recommendation: { label, reason } };
}
...
function computeImpact(props = {}) {
  ...
  return { decisions, links, gains, perspectives, growth };
}
...
function ScoreBar({ label, value }) { return <div><div className="flex justify-between text-xs text-[#8a7456] mb-1"><span>{label}</span><b>{value}/100</b></div><div className="h-2 rounded-full bg-[#eadcc2] overflow-hidden"><div className="h-full bg-[#2f2415]" style={{ width: `${clamp(value)}%` }} /></div></div></div>; }
...
function PerspectiveCard({ item, onNavigate }) { const Icon = item.icon; return <div className="rounded-2xl border border-[#d6c3a0] bg-white p-5 space-y-4"><div className="flex items-start justify-between gap-3"><div className="flex gap-3"><div className="w-11 h-11 rounded-xl bg-[#fff3d8] text-[#9a6b12] flex items-center justify-center"><Icon size={19} /></div><div><h3 className="font-black text-[#2f2415]">{item.title}</h3><p className="text-sm text-[#8a7456]">{item.recommendation.label}</p></div></div><div className="text-right"><p className="text-xs text-[#8a7456]">Score</p><p className="text-3xl font-black text-[#2f2415]">{item.score}</p></div></div><div className="rounded-xl bg-[#fffdf8] border border-[#eadcc2] p-3 text-sm text-[#7d6a4a]"><b className="text-[#2f2415]">Décision :</b> {item.recommendation.reason}</div><div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-sm"><div className="rounded-xl bg-[#fffdf8] border border-[#eadcc2] p-3"><span className="text-xs text-[#8a7456]">CA lié</span><b className="block text-[#2f2415]">{item.revenue > 0 ? money(item.revenue) : 'À lier'}</b></div><div className="rounded-xl bg-[#fffdf8] border border-[#eadcc2] p-3"><span className="text-xs text-[#8a7456]">Coûts liés</span><b className="block text-[#2f2415]">{item.expenses > 0 ? money(item.expenses) : 'À compléter'}</b></div><div className={`rounded-xl border p-3 ${item.marginInfo.tone === 'danger' ? 'bg-red-50 border-red-200 text-red-700' : item.marginInfo.tone === 'amber' ? 'bg-amber-50 border-amber-200 text-amber-700' : 'bg-emerald-50 border-emerald-200 text-emerald-700'}`}><span className="text-xs">Lecture marge</span><b className="block">{item.marginInfo.display}</b><span className="text-xs">{item.marginInfo.label}</span></div></div><div className="grid grid-cols-1 md:grid-cols-2 gap-2"><ScoreBar label="Rentabilité" value={item.scores.rentabilite} /><ScoreBar label="Cash" value={item.scores.cash} /><ScoreBar label="Rotation" value={item.scores.rotation} /><ScoreBar label="Risque maîtrisé" value={item.scores.risque} /><ScoreBar label="Demande" value={item.scores.demande} /><ScoreBar label="Capacité" value={item.scores.capacite} /><ScoreBar label="Fiabilité données" value={item.scores.data} /></div><div className="space-y-1">{item.details.map((detail) => <p key={detail} className="text-xs text-[#8a7456]">• {detail}</p>)}</div><div className="flex flex-wrap gap-2">{item.actions.map((action) => <ActionButton key={action.label} {...action} onNavigate={onNavigate} />)}</div></div>; }
