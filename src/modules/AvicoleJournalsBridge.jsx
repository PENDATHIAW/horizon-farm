...
function SlaughterJournal({ rows, businessEvents, stockCrud, onCreateBusinessEvent, onUpdateBusinessEvent, onDeleteBusinessEvent, onRefreshBusinessEvents, onUpdate, onRefresh }) {
  ...
  const chairIds = useMemo(() => new Set(chairLots.map((lot) => String(lot.id))), [chairLots]);
  const events = useMemo(() => { const merged = [...arr(businessEvents).filter(isSlaughter), ...arr(localEvents).filter(isSlaughter)]; return merged.filter((item) => linkedToLot(item, chairIds)).filter((item, index, all) => all.findIndex((candidate) => candidate.id === item.id) === index).sort((a, b) => String(b.date || b.created_at).localeCompare(String(a.date || a.created_at))); }, [businessEvents, localEvents, chairIds]);
  if (!chairLots.length) return null;
