...
function Field({ label, children }) {
  return <label className="text-xs font-bold text-[#8a7456] space-y-1"><span>{label}</span>{children}</label>;
}
...
function ActionButton({ children, onClick, icon: Icon, danger = false, type = 'button', disabled = false }) {
  return <button type={type} disabled={disabled} onClick={onClick} className={`inline-flex items-center gap-2 rounded-xl px-3 py-2 text-xs font-bold ${danger ? 'bg-red-50 text-red-700 border border-red-200' : 'bg-[#fffdf8] text-[#2f2415] border border-[#d6c3a0]'} disabled:opacity-50`}><Icon size={14} />{children}</button>;
}
...
function EggJournal({ rows, productionLogs, onCreateProduction, onUpdateProduction, onDeleteProduction, onRefreshProduction }) {
  ...
  const submit = async (e) => {
    ...
    toast.success(editing ? 'Ramassage modifié' : 'Ramassage enregistré');
    setEditing(null);
    setForm(initial);
  };
  ...
  return <section className="rounded-3xl border border-[#d6c3a0] bg-white p-5 shadow-sm space-y-4">
    ...
    <form onSubmit={submit} className="grid grid-cols-1 md:grid-cols-7 gap-2 rounded-2xl border border-[#eadcc2] bg-[#fffdf8] p-3">
      ...
      <Field label="Vendables"><Input readOnly value={Math.max(0, eggCount(form) - brokenEggs(form))} /></Field>
...
function SlaughterJournal({ rows, businessEvents, onCreateBusinessEvent, onUpdateBusinessEvent, onDeleteBusinessEvent, onRefreshBusinessEvents, onUpdate, onRefresh }) {
  ...
  const events = useMemo(() => arr(businessEvents).filter(isSlaughter).sort((a, b) => String(b.date || b.created_at).localeCompare(String(a.date || a.created_at))), [businessEvents]);
  ...
  const applyCountDelta = async (lot, delta) => {
    if (!lot || !delta) return;
    const next = Math.max(0, activeCount(lot) - delta);
    await onUpdate?.(lot.id, { current_count: next, effectif_actuel: next, abattus: toNumber(lot.abattus) + delta, status: next <= 0 ? 'cloture' : lot.status, phase: next <= 0 ? 'Clôturé' : lot.phase });
    await onRefresh?.();
  };
  ...
  const submit = async (e) => {
    ...
    const count = slaughterCount(form);
    const avg = avgSlaughterWeight(form);
    ...
    const previousCount = editing ? slaughterCount(editing) : 0;
    ...
    if (editing) await onUpdateBusinessEvent?.(editing.id, payload);
    else await onCreateBusinessEvent?.(payload);
    if (delta !== 0) await applyCountDelta(lot, delta);
    await onRefreshBusinessEvents?.();
    toast.success(editing ? 'Abattage modifié' : 'Abattage enregistré');
    setEditing(null);
    setForm(initial);
  };
  ...
  const remove = async (event) => { const lot = rows.find((item) => item.id === eventLotId(event)); await applyCountDelta(lot, -slaughterCount(event)); await onDeleteBusinessEvent?.(event.id); await onRefreshBusinessEvents?.(); toast.success('Abattage supprimé'); };

  return <section className="rounded-3xl border border-[#d6c3a0] bg-white p-5 shadow-sm space-y-4">
    <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><p className="flex items-center gap-2 text-lg font-black text-[#2f2415]"><Drumstick size={20} /> Journal d’abattage chair</p><p className="mt-1 text-sm text-[#8a7456]">Historise les abattages, diminue le lot chair et calcule le poids total.</p></div><ActionButton icon={RefreshCw} onClick={onRefreshBusinessEvents}>Actualiser</ActionButton></div>
    <form onSubmit={submit} className="grid grid-cols-1 md:grid-cols-8 gap-2 rounded-2xl border border-[#eadcc2] bg-[#fffdf8] p-3">
      <Field label="Lot chair"><Select value={form.lot_id || ''} onChange={(e) => update('lot_id', e.target.value)}><option value="">Choisir</option>{chairLots.map((lot) => <option key={lot.id} value={lot.id}>{lot.name || lot.id} · {fmtNumber(activeCount(lot))} actifs</option>)}</Select></Field>
      <Field label="Date"><Input type="date" value={form.date || ''} onChange={(e) => update('date', e.target.value)} /></Field>
      <Field label="Nombre abattu"><Input type="number" min="0" value={form.nombre_abattu || ''} onChange={(e) => update('nombre_abattu', e.target.value)} /></Field>
      <Field label={slaughterCount(form) <= 1 ? 'Poids du poulet (kg)' : 'Poids moyen (kg)'}><Input type="number" step="0.01" min="0" value={form.poids_moyen_abattage || ''} onChange={(e) => update('poids_moyen_abattage', e.target.value)} /></Field>
      <Field label="Poids total"><Input readOnly value={`${totalSlaughterWeight(form).toFixed(2)} kg`} /></Field>
    ...
    <div className="overflow-x-auto"><table className="min-w-full text-sm"><thead><tr className="text-left text-xs uppercase text-[#8a7456] border-b border-[#eadcc2]"><th className="py-2 pr-4">Date</th><th className="py-2 pr-4">Lot</th><th className="py-2 pr-4">Abattus</th><th className="py-2 pr-4">Poids moyen</th><th className="py-2 pr-4">Poids total</th><th className="py-2 pr-4">Destination</th><th className="py-2 pr-4">Actions</th></tr></thead><tbody>{events.map((event) => <tr key={event.id} className="border-b border-[#f0e5d0]"><td className="py-3 pr-4">{event.date || '—'}</td><td className="py-3 pr-4 font-bold">{event.lot_name || eventLotId(event)}</td><td className="py-3 pr-4">{fmtNumber(slaughterCount(event))}</td><td className="py-3 pr-4">{avgSlaughterWeight(event).toFixed(2)} kg</td><td className="py-3 pr-4">{totalSlaughterWeight(event).toFixed(2)} kg</td><td className="py-3 pr-4">{event.destination || 'stock'}</td><td className="py-3 pr-4"><div className="flex gap-1"><ActionButton icon={Edit} onClick={() => startEdit(event)}>Modifier</ActionButton>{onDeleteBusinessEvent ? <ActionButton icon={Trash2} danger onClick={() => remove(event)}>Supprimer</ActionButton> : null}</div></td></tr>)}{!events.length ? <tr><td colSpan="7" className="py-4 text-center text-[#8a7456]">Aucun abattage enregistré.</td></tr> : null}</tbody></table></div>
  </section>;
}

export default function AvicoleJournalsBridge(props) {
  return <div className="space-y-5"><EggJournal {...props} /><SlaughterJournal {...props} /></div>;
}
