...
function EggJournal({ rows, productionLogs, stockCrud, onCreateProduction, onUpdateProduction, onDeleteProduction, onRefreshProduction }) {
  ...
  const submit = async (e) => {
    ...
    await syncWithoutBlocking(() => syncEggStock({ log: payload, previousLog: editing }), 'Stock œufs/tablettes non synchronisé');
  ...
  const remove = async (log) => { await onDeleteProduction?.(log.id); await syncWithoutBlocking(() => syncEggStock({ log: {}, previousLog: log }), 'Stock œufs/tablettes non resynchronisé'); await syncWithoutBlocking(() => onRefreshProduction?.(), 'Rafraîchissement production indisponible'); toast.success(`Ramassage supprimé · stock corrigé (${tabletLabel(sellableEggs(log))})`); };

  return <section className="rounded-3xl border border-[#d6c3a0] bg-white p-5 shadow-sm space-y-4 sm:p-6 min-w-0"><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between min-w-0"><div className="min-w-0"><p className="flex items-center gap-2 text-lg font-black text-[#2f2415] break-words"><Egg size={20} className="shrink-0" /> Journal de ramassage des œufs</p><p className="mt-1 text-sm leading-relaxed text-[#8a7456] break-words">Visible uniquement pour les lots pondeuses actifs. Chaque ramassage alimente le stock en tablettes : 1 tablette = 30 œufs.</p></div><ActionButton icon={RefreshCw} onClick={onRefreshProduction}>Actualiser</ActionButton></div><form onSubmit={submit} className="hf-form-grid grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 rounded-2xl border border-[#eadcc2] bg-[#fffdf8] p-4"><Field label="Lot pondeuse"><Select value={form.lot_id || ''} onChange={(e) => update('lot_id', e.target.value)}><option value="">Choisir</option>{pondeuses.map((lot, index) => <option key={`${lot.id || 'lot'}-${index}`} value={lot.id}>{lot.name || lot.id}</option>)}</Select></Field><Field label="Date"><Input type="date" value={form.date || ''} onChange={(e) => update('date', e.target.value)} /></Field><Field label="Heure"><Input value={form.heure_ramassage || ''} onChange={(e) => update('heure_ramassage', e.target.value)} placeholder="ex: 08:30" /></Field><Field label="Œufs ramassés"><Input type="number" min="0" value={form.oeufs_produits || ''} onChange={(e) => update('oeufs_produits', e.target.value)} /></Field><Field label="Cassés"><Input type="number" min="0" value={form.oeufs_casses || 0} onChange={(e) => update('oeufs_casses', e.target.value)} /></Field><Field label="Vendables"><Input readOnly value={`${fmtNumber(sellableEggs(form))} œufs · ${tabletLabel(sellableEggs(form))}`} /></Field><div className="hf-form-actions flex flex-wrap items-end gap-2"><ActionButton type="submit" icon={editing ? Save : Plus}>{editing ? 'Modifier' : 'Ajouter'}</ActionButton>{editing ? <ActionButton icon={X} onClick={() => { setEditing(null); setForm(initial); }}>Annuler</ActionButton> : null}</div></form><div className="overflow-x-auto"><table className="min-w-full text-sm"><thead><tr className="text-left text-xs uppercase text-[#8a7456] border-b border-[#eadcc2]"><th className="py-2 pr-4">Date</th><th className="py-2 pr-4">Lot</th><th className="py-2 pr-4">Œufs</th><th className="py-2 pr-4">Cassés</th><th className="py-2 pr-4">Stock ajouté</th><th className="py-2 pr-4">Responsable</th><th className="py-2 pr-4">Actions</th></tr></thead><tbody>{logs.map((log, index) => { const sellable = sellableEggs(log); return <tr key={`${log.id || eventLotId(log) || 'ramassage'}-${index}`} className="border-b border-[#f0e5d0]"><td className="py-3 pr-4">{log.date || '—'}</td><td className="py-3 pr-4 font-bold">{log.lot_name || eventLotId(log)}</td><td className="py-3 pr-4"><b>{fmtNumber(eggCount(log))}</b><p className="text-[11px] text-[#8a7456]">{tabletLabel(eggCount(log))}</p></td><td className="py-3 pr-4">{fmtNumber(brokenEggs(log))}</td><td className="py-3 pr-4 font-bold text-emerald-700"><b>+{fmtNumber(sellable)} œufs</b><p className="text-[11px] text-emerald-700">{tabletLabel(sellable)}</p></td><td className="py-3 pr-4">{log.responsable_label || log.responsable || '—'}</td><td className="py-3 pr-4"><div className="flex gap-1"><ActionButton icon={Edit} onClick={() => startEdit(log)}>Modifier</ActionButton>{onDeleteProduction ? <ActionButton icon={Trash2} danger onClick={() => remove(log)}>Supprimer</ActionButton> : null}</div></td></tr>; })}{!logs.length ? <tr><td colSpan="7" className="py-4 text-center text-[#8a7456]">Aucun ramassage enregistré pour les lots pondeuses actifs.</td></tr> : null}</tbody></table></div></section>;
}
