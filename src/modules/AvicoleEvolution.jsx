...
const readyForSale = (lot = {}) => { const status = lower(lot.status || lot.statut || lot.phase); return ['pret_a_la_vente', 'pret_a_vendre_reforme', 'pret_vente', 'ready', 'vendu', 'vendu_partiellement'].some((word) => status.includes(word)) || Boolean(lot.pret_vente_recommande || lot.pret_vente_confirme); };
...
const logDate = (row = {}) => row.date || row.created_at || row.updated_at || row.event_date || row.date_operation;
...
function rowText(row = {}) { return lower(`${row.module || ''} ${row.source_module || ''} ${row.activity || ''} ${row.activite || ''} ${row.type || ''} ${row.nature || ''} ${row.category || ''} ${row.categorie || ''} ${row.produit || ''} ${row.product_name || ''} ${row.product_type || ''} ${row.title || ''} ${row.libelle || ''} ${row.description || ''} ${row.notes || ''} ${row.nom || ''}`); }
...
function isCostRow(row = {}) { const text = rowText(row); return /(sortie|charge|depense|dépense|frais|cout|coût|debit|débit|expense|achat|aliment|soin|vaccin|maintenance|emballage|transport|tablette|plateau)/.test(text); }
function matchesKeywords(row = {}, keywords = []) { const text = rowText(row); return keywords.some((keyword) => text.includes(keyword)); }
...
function ensure(map, key) { if (!map.has(key)) map.set(key, { key, mois: monthLabel(key), charges_aliments: 0, charges_soins: 0, charges_autres: 0, ca: 0, encaisse: 0, marge: 0, poids_moyen: 0, taux_mortalite: 0, effectif: 0, morts: 0, malades: 0, vendus: 0, prets: 0, oeufs: 0, vendables: 0, casses: 0, tablettes: 0, reliquat_oeufs: 0, taux_ponte: 0, taux_casse: 0, pondeuses: 0, dates: new Set(), sales_real: 0, cost_incomplete: false, cost_rows: 0 }); return map.get(key); }
function meaningfulLast(rows = [], keys = []) { return [...rows].reverse().find((row) => keys.some((key) => toNumber(row[key]) > 0)) || rows[rows.length - 1] || {}; }
...
function addRealSalesToMonthly({ map, salesOrders = [], payments = [], transactions = [], lotIds = new Set(), keywords = [] }) {
  ...
  realOrders.forEach((order) => { const amount = orderAmount(order); const bucket = ensure(map, monthKey(order.date || order.date_commande || order.created_at || order.updated_at)); bucket.ca += amount; bucket.sales_real += amount; });
  arr(payments).forEach((payment) => { const orderId = paymentOrderId(payment); const lotLinked = linkedToExistingLot(payment, lotIds); const textLinked = matchesKeywords(payment, keywords) || rowText(payment).includes('avicole'); if (!orderIds.has(orderId) && !lotLinked && !textLinked) return; const bucket = ensure(map, monthKey(payment.date || payment.created_at || payment.paid_at || payment.date_paiement)); bucket.encaisse += paymentAmount(payment); });
  arr(transactions).forEach((tx) => { if (!isIncome(tx) || !financeMatchesActivity(tx, lotIds, keywords)) return; const amount = transactionAmount(tx); if (amount <= 0) return; const bucket = ensure(map, monthKey(tx.date || tx.created_at || tx.updated_at || tx.date_operation || tx.date_paiement)); bucket.ca += amount; bucket.encaisse += amount; bucket.sales_real += amount; });
}
function addRealCostsToMonthly({ map, rows = [], lotIds = new Set(), keywords = [] }) {
  arr(rows).forEach((row) => {
    ...
    if (/aliment|provende|feed|ration|ma[iî]s|son|tourteau/.test(text)) bucket.charges_aliments += amount;
    else if (/sante|santé|soin|vaccin|traitement|veto|véto|vétérinaire|veterinaire|biosecurite|biosécurité/.test(text)) bucket.charges_soins += amount;
...
function buildChairMonthly({ rows = [], alimentationLogs = [], businessEvents = [], salesOrders = [], payments = [], transactions = [] }) {
  ...
  chair.forEach((lot) => { const key = monthKey(lot.date_sortie || lot.sale_date || lot.date_vente || lot.updated_at || lot.created_at || fallbackKey); const bucket = ensure(map, key); const effective = followedChairCount(lot); bucket.effectif += effective; bucket.morts += deadCount(lot); bucket.malades += sickCount(lot); bucket.vendus += soldOrExitedCount(lot); bucket.prets += readyForSale(lot) ? 1 : 0; bucket.poids_total = toNumber(bucket.poids_total) + avgWeight(lot); bucket.poids_count = toNumber(bucket.poids_count) + (avgWeight(lot) > 0 ? 1 : 0); const base = Math.max(1, lotInitial(lot)); bucket.taux_mortalite = ((bucket.morts / base) * 100); });
  addRealCostsToMonthly({ map, rows: alimentationLogs, lotIds: chairIds, keywords: ['chair', 'poulet', 'broiler', 'avicole', 'aliment'] });
  addRealCostsToMonthly({ map, rows: businessEvents, lotIds: chairIds, keywords: ['chair', 'poulet', 'broiler', 'avicole', 'sante', 'santé', 'charge'] });
  addRealCostsToMonthly({ map, rows: transactions, lotIds: chairIds, keywords: ['chair', 'poulet', 'broiler', 'avicole', 'aliment', 'sante', 'santé'] });
  addRealSalesToMonthly({ map, salesOrders, payments, transactions, lotIds: chairIds, keywords: ['chair', 'poulet', 'broiler', 'avicole'] });
  return [...map.values()].sort((a, b) => a.key.localeCompare(b.key)).map((row) => { const poids = row.poids_count ? row.poids_total / row.poids_count : 0; const charges = row.charges_aliments + row.charges_soins + row.charges_autres; const marge = row.ca - charges; return { ...row, poids_moyen: Number(poids.toFixed(2)), marge: Number(marge.toFixed(0)), cost_incomplete: charges <= 0 && row.effectif > 0, taux_marge: row.ca > 0 ? Number(((marge / row.ca) * 100).toFixed(1)) : 0, taux_mortalite: Number(row.taux_mortalite.toFixed(1)) }; });
}
function buildPonteMonthly({ rows = [], productionLogs = [], alimentationLogs = [], businessEvents = [], salesOrders = [], payments = [], transactions = [] }) {
  ...
  const map = new Map();
  arr(productionLogs).forEach((log) => { if (!matchesActivity(log, pondeuseIds, ['oeuf', 'œuf', 'ponte', 'pondeuse'])) return; const produced = eggs(log); const casse = broken(log); if (produced <= 0 && casse <= 0) return; const sellable = Math.max(0, produced - casse); const tablet = tabletsFromEggs(sellable); const bucket = ensure(map, monthKey(logDate(log))); bucket.oeufs += produced; bucket.casses += casse; bucket.vendables += sellable; bucket.tablettes += tablet.tablets; bucket.pondeuses = activePondeuses; if (log.date) bucket.dates.add(String(log.date)); });
  addRealCostsToMonthly({ map, rows: alimentationLogs, lotIds: pondeuseIds, keywords: ['pondeuse', 'ponte', 'oeuf', 'œuf', 'avicole', 'aliment'] });
  addRealCostsToMonthly({ map, rows: businessEvents, lotIds: pondeuseIds, keywords: ['pondeuse', 'ponte', 'oeuf', 'œuf', 'avicole', 'sante', 'santé', 'charge', 'emballage', 'tablette', 'plateau'] });
  addRealCostsToMonthly({ map, rows: transactions, lotIds: pondeuseIds, keywords: ['pondeuse', 'ponte', 'oeuf', 'œuf', 'avicole', 'aliment', 'sante', 'santé', 'emballage', 'tablette', 'plateau'] });
  addRealSalesToMonthly({ map, salesOrders, payments, transactions, lotIds: pondeuseIds, keywords: ['oeuf', 'œuf', 'ponte', 'pondeuse', 'tablette', 'plateau', 'avicole'] });
  return [...map.values()].sort((a, b) => a.key.localeCompare(b.key)).map((row) => { const days = Math.max(1, row.dates?.size || 1); const tauxPonte = activePondeuses > 0 ? (row.oeufs / (activePondeuses * days)) * 100 : 0; const tauxCasse = row.oeufs > 0 ? (row.casses / row.oeufs) * 100 : 0; const charges = row.charges_aliments + row.charges_soins + row.charges_autres; const marge = row.ca - charges; const tablet = tabletsFromEggs(row.vendables); return { ...row, tablettes: row.tablettes || tablet.tablets, reliquat_oeufs: tablet.remaining, pondeuses: activePondeuses, taux_ponte: Number(tauxPonte.toFixed(1)), taux_casse: Number(tauxCasse.toFixed(1)), cout_oeuf: row.vendables > 0 && charges > 0 ? charges / row.vendables : 0, cout_tablette: row.vendables > 0 && charges > 0 ? (charges / row.vendables) * EGGS_PER_TABLET : 0, cost_incomplete: row.vendables > 0 && charges <= 0, marge: Number(marge.toFixed(0)), taux_marge: row.ca > 0 ? Number(((marge / row.ca) * 100).toFixed(1)) : 0 }; });
}
...
function EmptyState() { return <div className="rounded-2xl border border-[#eadcc2] bg-[#fffdf8] p-4 text-sm text-[#8a7456]">Aucun lot avicole dans cette vue.</div>; }
...
export default function AvicoleEvolution({ rows = [], productionLogs = [], alimentationLogs = [], opportunities = [], businessEvents = [], salesOrders = [], payments = [], transactions = [], onNavigate }) {
  ...
  const linkedProductionLogs = arr(productionLogs).filter((log) => matchesActivity(log, activeLotIds, ['avicole', 'oeuf', 'œuf', 'ponte', 'pondeuse']));
  const linkedBusinessEvents = arr(businessEvents).filter((event) => matchesActivity(event, activeLotIds, ['avicole', 'sante', 'santé', 'charge', 'chair', 'poulet', 'ponte', 'pondeuse', 'oeuf', 'œuf']));
  const costs = activeRows.length ? summarizeAvicoleCosts({ rows: activeRows, alimentationLogs: linkedAlimentationLogs, productionLogs: linkedProductionLogs, slaughterEvents: linkedBusinessEvents, directCharges: linkedBusinessEvents, healthEvents: linkedBusinessEvents, defaultPricePerKg: 0 }) : { details: [], realFeedCost: 0, estimatedFeedCost: 0 };
  ...
  const ponte = showPonte ? buildPonteMonthly({ rows: activeRows, productionLogs: linkedProductionLogs, alimentationLogs: linkedAlimentationLogs, businessEvents: linkedBusinessEvents, salesOrders, payments, transactions }) : [];
  ...
  const totalRealCharges = [...chair, ...ponte].reduce((sum, row) => sum + row.charges_aliments + row.charges_soins + row.charges_autres, 0);
  ...
  const avgChairCostLive = average(costs.details.filter((item) => item.type === 'chair' && item.realFeedCost + item.healthCost + item.otherDirectCost + item.purchaseCost > 0), 'costPerLiveSubject');
  const avgChairCostKg = average(costs.details.filter((item) => item.type === 'chair'), 'costPerKg');
  const avgPonteCostEgg = average(ponte, 'cout_oeuf');
  const avgPonteCostTablet = average(ponte, 'cout_tablette');

  return <div className="space-y-5">
    <Header title={showPonte && !showChair ? 'Évolution Pondeuses' : showChair && !showPonte ? 'Évolution Chair' : 'Évolution Avicole'} subtitle={showPonte && !showChair ? 'Lecture pondeuses unique : coûts réels, ponte, tablettes, casse, CA et marge.' : showChair && !showPonte ? 'Lecture chair unique : coûts réels, ventes, marge, poids, morts, malades et vendus.' : 'Lecture consolidée chair et ponte.'} priority={priority} onNavigate={onNavigate} />
    {!activeRows.length ? <EmptyState /> : null}
    <div className="bg-white border border-[#d6c3a0] rounded-2xl p-4"><div className="grid grid-cols-2 lg:grid-cols-6 gap-3">{showChair ? <><SmallMetric label="Coût moyen sujet chair" value={costLabel(avgChairCostLive, !avgChairCostLive && chair.length)} hint="coûts réels uniquement" danger={!avgChairCostLive && chair.length} /><SmallMetric label="Coût / kg poulet" value={costLabel(avgChairCostKg, !avgChairCostKg && chair.length)} hint="après abattage réel" danger={!avgChairCostKg && chair.length} /></> : null}{showPonte ? <><SmallMetric label="Coût / œuf vendable" value={costLabel(avgPonteCostEgg, !avgPonteCostEgg && totalSellable > 0)} hint="coûts réels / œufs vendables" danger={!avgPonteCostEgg && totalSellable > 0} /><SmallMetric label="Coût / tablette" value={costLabel(avgPonteCostTablet, !avgPonteCostTablet && totalSellable > 0)} hint="1 tablette = 30 œufs" danger={!avgPonteCostTablet && totalSellable > 0} /></> : null}<SmallMetric label="Charges réelles" value={fmtCurrency(totalRealCharges)} hint="aliment + santé + autres" danger={costIncomplete} /><SmallMetric label="CA réel" value={fmtCurrency(totalSalesReal)} hint={totalSalesReal ? 'ventes/finances liées' : 'aucune vente liée'} /><SmallMetric label="Encaissé" value={fmtCurrency(totalEncaisse)} hint="paiements/finances liés" /></div>{costIncomplete ? <div className="mt-3 rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-sm font-bold text-red-700">Coût incomplet : des productions existent mais aucun coût réel lié n’a été trouvé. Compléter stock, achats, santé, charges, emballages ou transport.</div> : null}</div>

    {showChair ? <section className="space-y-4"><div className="bg-white border border-[#d6c3a0] rounded-2xl p-4"><p className="font-black text-[#2f2415]">Évolution Chair</p><p className="text-xs text-[#8a7456] mt-1">Poulets de chair : une seule lecture des coûts réels, ventes, marge, poids moyen, morts, malades et vendus.</p><div className="grid grid-cols-2 lg:grid-cols-6 gap-3 mt-4"><SmallMetric label="Effectif suivi" value={fmtNumber(lastChair.effectif || 0)} hint="actif ou vendu suivi" /><SmallMetric label="Poids moyen" value={`${Number(lastChair.poids_moyen || 0).toFixed(2)} kg`} /><SmallMetric label="Morts / malades" value={`${fmtNumber(lastChair.morts || 0)} / ${fmtNumber(lastChair.malades || 0)}`} danger={(lastChair.morts || lastChair.malades) > 0} /><SmallMetric label="Vendus / sorties" value={fmtNumber(lastChair.vendus || 0)} /><SmallMetric label="Coût sujet" value={costLabel(avgChairCostLive, !avgChairCostLive && chair.length)} danger={!avgChairCostLive && chair.length} /><SmallMetric label="Marge chair" value={fmtCurrency(chair.reduce((sum, row) => sum + row.marge, 0))} hint="CA réel - coûts réels" /></div></div><SmartEvolutionChart title="Chair — économie mensuelle" subtitle="Barres : charges réelles, CA réel, encaissé, marge. Courbe : poids moyen." months={labels(chair)} leftUnit="FCFA" rightUnit="kg" series={[{ name: 'Charges aliments', type: 'bar', unit: 'FCFA', data: values(chair, 'charges_aliments') }, { name: 'Charges santé/autres', type: 'bar', unit: 'FCFA', data: chair.map((row) => row.charges_soins + row.charges_autres) }, { name: 'CA ventes chair', type: 'bar', unit: 'FCFA', data: values(chair, 'ca') }, { name: 'Encaissé', type: 'bar', unit: 'FCFA', data: values(chair, 'encaisse') }, { name: 'Marge chair', type: 'bar', unit: 'FCFA', data: values(chair, 'marge') }, { name: 'Poids moyen', type: 'line', axis: 'right', unit: 'kg', data: values(chair, 'poids_moyen') }]} /><SmartEvolutionChart title="Chair — effectifs" subtitle="Effectif, morts, malades, vendus et taux de mortalité." months={labels(chair)} leftUnit="" rightUnit="%" series={[{ name: 'Effectif suivi', type: 'bar', data: values(chair, 'effectif') }, { name: 'Morts', type: 'bar', data: values(chair, 'morts') }, { name: 'Malades', type: 'bar', data: values(chair, 'malades') }, { name: 'Vendus / sorties', type: 'bar', data: values(chair, 'vendus') }, { name: 'Taux mortalité', type: 'line', axis: 'right', unit: '%', data: values(chair, 'taux_mortalite') }]} /></section> : null}

    {showPonte ? <section className="space-y-4"><div className="bg-white border border-[#d6c3a0] rounded-2xl p-4"><p className="font-black text-[#2f2415]">Évolution Ponte</p><p className="text-xs text-[#8a7456] mt-1">Pondeuses : coûts réels, CA tablettes d’œufs, encaissé, marge, production, vendables et taux de ponte.</p><div className="grid grid-cols-2 lg:grid-cols-6 gap-3 mt-4"><SmallMetric label="Pondeuses" value={fmtNumber(lastPonte.pondeuses || 0)} hint="actives" /><SmallMetric label="Œufs produits" value={fmtNumber(totalEggs)} hint={`${fmtNumber(tabletsFromEggs(totalEggs).tablets)} tablette(s) + ${fmtNumber(tabletsFromEggs(totalEggs).remaining)} œuf(s)`} /><SmallMetric label="Œufs vendables" value={fmtNumber(totalSellable)} hint={`${fmtNumber(totalTablets)} tablette(s) + ${fmtNumber(totalRemainingEggs)} œuf(s)`} /><SmallMetric label="Coût / œuf" value={costLabel(avgPonteCostEgg, !avgPonteCostEgg && totalSellable > 0)} danger={!avgPonteCostEgg && totalSellable > 0} /><SmallMetric label="Coût / tablette" value={costLabel(avgPonteCostTablet, !avgPonteCostTablet && totalSellable > 0)} danger={!avgPonteCostTablet && totalSellable > 0} /><SmallMetric label="Marge ponte" value={fmtCurrency(ponte.reduce((sum, row) => sum + row.marge, 0))} hint="CA réel - coûts réels" /></div></div><SmartEvolutionChart title="Ponte — économie mensuelle" subtitle="Barres : charges réelles, CA tablettes, encaissé, marge. Courbes : taux de ponte et casse." months={labels(ponte)} leftUnit="FCFA" rightUnit="%" series={[{ name: 'Charges aliments ponte', type: 'bar', unit: 'FCFA', data: values(ponte, 'charges_aliments') }, { name: 'Charges santé/autres', type: 'bar', unit: 'FCFA', data: ponte.map((row) => row.charges_soins + row.charges_autres) }, { name: 'CA tablettes', type: 'bar', unit: 'FCFA', data: values(ponte, 'ca') }, { name: 'Encaissé', type: 'bar', unit: 'FCFA', data: values(ponte, 'encaisse') }, { name: 'Marge ponte', type: 'bar', unit: 'FCFA', data: values(ponte, 'marge') }, { name: 'Taux ponte', type: 'line', axis: 'right', unit: '%', data: values(ponte, 'taux_ponte') }, { name: 'Taux casse', type: 'line', axis: 'right', unit: '%', data: values(ponte, 'taux_casse') }]} /><SmartEvolutionChart title="Ponte — production mensuelle" subtitle="Œufs produits, vendables, tablettes, casses et effectif pondeuses." months={labels(ponte)} leftUnit="" rightUnit="%" series={[{ name: 'Œufs produits', type: 'bar', data: values(ponte, 'oeufs') }, { name: 'Œufs vendables', type: 'bar', data: values(ponte, 'vendables') }, { name: 'Tablettes vendables', type: 'bar', data: values(ponte, 'tablettes') }, { name: 'Casses', type: 'bar', data: values(ponte, 'casses') }, { name: 'Pondeuses actives', type: 'line', data: values(ponte, 'pondeuses') }, { name: 'Taux ponte', type: 'line', axis: 'right', unit: '%', data: values(ponte, 'taux_ponte') }]} /></section> : null}

    <div className="bg-[#fffdf8] border border-[#d6c3a0] rounded-2xl p-4 text-sm text-[#7d6a4a] flex items-start gap-3"><TrendingUp size={18} className="text-[#9a6b12] mt-0.5" /><div><b className="text-[#2f2415]">Interprétation :</b> {!activeRows.length ? 'Aucun lot avicole dans cette vue.' : costIncomplete ? 'Des coûts réels manquent : pas d’estimation automatique, la marge reste à compléter.' : totalSalesReal > 0 ? 'Les graphes utilisent les commandes, paiements ou revenus Finance réellement liés aux lots avicoles.' : healthIssues > 0 ? `${fmtNumber(healthIssues)} point(s) santé/mortalité à traiter.` : showChair && readyLots > 0 ? `${fmtNumber(readyLots)} lot(s) chair à convertir en ventes.` : showPonte ? `Vue pondeuses : suivre pontes, casses, coûts réels, tablettes (${fmtNumber(totalTablets)} complètes) et marge.` : 'Compléter les coûts et ventes liées pour affiner les marges.'}</div></div>
    {priority ? <div className={`${costIncomplete || healthIssues ? 'bg-amber-50 border-amber-200 text-amber-800' : 'bg-emerald-50 border-emerald-200 text-emerald-800'} border rounded-2xl p-4 text-sm flex items-start justify-between gap-3`}><div className="flex items-start gap-2"><ShoppingBag size={18} className="mt-0.5" /><div><b>Action recommandée :</b> {priority.label}.</div></div><button type="button" onClick={() => onNavigate?.(priority.module)} className="shrink-0 rounded-xl bg-white/70 border border-current/10 px-3 py-1.5 text-xs font-bold">Ouvrir</button></div> : null}
  </div>;
}
