...
function isCostRow(row = {}) { const text = rowText(row); return /(sortie|charge|depense|frais|cout|debit|expense|achat|aliment|soin|vaccin|maintenance|emballage|transport|tablette|plateau)/.test(text); }
...
function buildPonteMonthly({ rows = [], productionLogs = [], alimentationLogs = [], businessEvents = [], salesOrders = [], payments = [], transactions = [] }) {
  ...
  addRealCostsToMonthly({ map, rows: businessEvents, lotIds: pondeuseIds, keywords: ['pondeuse', 'ponte', 'oeuf', 'avicole', 'sante', 'charge', 'emballage', 'tablette', 'plateau'] });
  addRealCostsToMonthly({ map, rows: transactions, lotIds: pondeuseIds, keywords: ['pondeuse', 'ponte', 'oeuf', 'avicole', 'aliment', 'sante', 'emballage', 'tablette', 'plateau'] });
  ...
  return [...map.values()].sort((a, b) => a.key.localeCompare(b.key)).map((row) => { const days = Math.max(1, row.dates?.size || 1); const charges = row.charges_aliments + row.charges_soins + row.charges_autres; const tablet = tabletsFromEggs(row.vendables); return { ...row, tablettes: row.tablettes || tablet.tablets, reliquat_oeufs: tablet.remaining, pondeuses: activePondeuses, taux_ponte: activePondeuses > 0 ? Number(((row.oeufs / (activePondeuses * days)) * 100).toFixed(1)) : 0, taux_casse: row.oeufs > 0 ? Number(((row.casses / row.oeufs) * 100).toFixed(1)) : 0, cout_oeuf: row.vendables > 0 && charges > 0 ? charges / row.vendables : 0, cout_tablette: row.vendables > 0 && charges > 0 ? (charges / row.vendables) * EGGS_PER_TABLET : 0, cost_incomplete: row.vendables > 0 && charges <= 0, marge: Number((row.ca - charges).toFixed(0)) }; });
}
...
export default function AvicoleEvolution({ rows = [], productionLogs = [], alimentationLogs = [], businessEvents = [], salesOrders = [], payments = [], transactions = [], onNavigate }) {
  ...
  const linkedBusinessEvents = arr(businessEvents).filter((event) => matchesActivity(event, activeLotIds, ['avicole', 'sante', 'charge', 'chair', 'poulet', 'ponte', 'pondeuse', 'oeuf']));
  ...
  const ponte = showPonte ? buildPonteMonthly({ rows: activeRows, productionLogs, alimentationLogs: linkedAlimentationLogs, businessEvents: linkedBusinessEvents, salesOrders, payments, transactions }) : [];
  ...
  return <div className="space-y-5">
    <div className="bg-white border border-[#d6c3a0] rounded-2xl p-4 space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3"><SmallMetric label="Charges réelles" value={fmtCurrency(totalRealCharges)} hint="aliment + santé + autres" danger={costIncomplete} /><SmallMetric label="CA réel" value={fmtCurrency(totalSalesReal)} hint={totalSalesReal ? 'ventes/finances liées' : 'aucune vente liée'} /><SmallMetric label="Encaissé" value={fmtCurrency(totalEncaisse)} hint="paiements/finances liés" /></div>
      {costIncomplete ? <div className="rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-sm font-bold text-red-700">Coût incomplet : des productions existent mais aucun coût réel lié n’a été trouvé. Compléter stock, achats, santé, charges, emballages ou transport.</div> : null}
    </div>
