...
function isCostRow(row = {}) { const text = rowText(row); return /(sortie|charge|depense|frais|cout|debit|expense|achat|aliment|soin|vaccin|maintenance|emballage|transport|tablette|plateau|alvéole|alveole|eau|electricite|électricité)/.test(text); }
...
function buildPonteMonthly({ rows = [], productionLogs = [], alimentationLogs = [], businessEvents = [], salesOrders = [], payments = [], transactions = [], pondeuseCosts = null }) {
  ...
  return [...map.values()].sort((a, b) => a.key.localeCompare(b.key)).map((row) => { const days = Math.max(1, row.dates?.size || 1); const tablet = tabletsFromEggs(row.vendables); const monthlyPackaging = row.charges_emballages || (tablet.tablets > 0 ? tablet.tablets * packagingPerTablet : 0); const monthlyProductionCost = row.vendables > 0 && costPerEggByRate > 0 ? row.vendables * costPerEggByRate : row.charges_aliments + row.charges_soins + row.charges_autres; const totalCost = monthlyProductionCost + monthlyPackaging; const costPerEgg = row.vendables > 0 && monthlyProductionCost > 0 ? monthlyProductionCost / row.vendables : 0; const costPerTablet = costPerEgg > 0 ? (costPerEgg * EGGS_PER_TABLET) + packagingPerTablet : 0; return { ...row, charges_aliments: monthlyProductionCost, charges_soins: 0, charges_autres: 0, charges_emballages: monthlyPackaging, tablettes: row.tablettes || tablet.tablets, reliquat_oeufs: tablet.remaining, pondeuses: activePondeuses, taux_ponte: activePondeuses > 0 ? Number(((row.oeufs / (activePondeuses * days)) * 100).toFixed(1)) : 0, taux_casse: row.oeufs > 0 ? Number(((row.casses / row.oeufs) * 100).toFixed(1)) : 0, cout_oeuf: costPerEgg, cout_tablette: costPerTablet, cost_incomplete: row.vendables > 0 && totalCost <= 0, marge: Number((row.ca - totalCost).toFixed(0)) }; });
}
...
export default function AvicoleEvolution({ rows = [], productionLogs = [], alimentationLogs = [], businessEvents = [], salesOrders = [], payments = [], transactions = [], onNavigate }) {
  ...
  const avgChairCostLive = average(costs.details.filter((item) => item.type === 'chair' && realProductionCostOfDetail(item) > 0), 'costPerLiveSubject');
  ...
  return <div className="space-y-5">
    ...
    <div className="bg-[#fffdf8] border border-[#d6c3a0] rounded-2xl p-4 text-sm text-[#7d6a4a] flex items-start gap-3"><TrendingUp size={18} className="text-[#9a6b12] mt-0.5" /><div><b className="text-[#2f2415]">Interprétation :</b> {costIncomplete ? 'Des coûts réels manquent : pas d’estimation automatique, la marge reste à compléter.' : totalSalesReal > 0 ? 'Les graphes utilisent les commandes, paiements ou revenus Finance réellement liés aux lots avicoles.' : healthIssues > 0 ? `${fmtNumber(healthIssues)} point(s) santé/mortalité à traiter.` : showChair && readyLots > 0 ? `${fmtNumber(readyLots)} lot(s) chair à convertir en ventes.` : showPonte ? 'Vue pondeuses : coût œuf basé sur coût journalier par poule / taux de ponte, tablette = œuf × 30 + emballage.' : 'Compléter les coûts et ventes liées pour affiner les marges.'}</div></div>
  </div>;
}
