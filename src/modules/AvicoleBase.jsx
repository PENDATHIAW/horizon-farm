...
import { buildAvicoleProposedSaleDisplay } from '../utils/salePricePresentation.js';
...
export default function AvicoleBase({ marketPrices = [], rows = [], alimentationLogs = [], productionLogs = [], opportunities = [], salesOrders = [], payments = [], transactions = [], businessEvents = [], loading, onCreate, onUpdate, onDelete, onRefresh, onCreateProduction, onCommitEggProduction, onRefreshProduction, onCreateOpportunity, onUpdateOpportunity, onRefreshOpportunities, onCreateBusinessEvent, onRefreshBusinessEvents, onNavigate, activity = 'pondeuse', lockActivity = false, embedInElevage = false }) {
  const [tab, setTab] = useState(activityToTab(activity));
  const [modal, setModal] = useState(null);
  const [selected, setSelected] = useState(null);
  const [saving, setSaving] = useState(false);

  useEffect(() => { queueMicrotask(() => setTab(activityToTab(activity))); }, [activity, lockActivity]);
  ...
  const initialEggEntry = useMemo(() => ({ id: '', lot_id: pondeusesDisponibles[0]?.id || '', date: today(), heure_ramassage: '', oeufs_produits: '', oeufs_casses: 0, responsable: responsibleOptions[0]?.value || 'TEAM-AVICOLE', notes: '' }), [pondeusesDisponibles, responsibleOptions]);

  const prepareLot = (payload, existing = {}) => {
    const base = { ...existing, ...payload };
    const initial = avicoleInitialCount(base);
    const current = initial > 0 ? avicoleCalculatedActiveCount(base) : avicoleRegisteredActiveCount(base);
    ...
    const defaults = applyAvicoleDecisionDefaults(prepared, existing, productionLogs);
  ...
  const submitEggEntry = async (payload) => { const lot = pondeusesDisponibles.find((item) => item.id === payload.lot_id); if (!lot) return toast.error('Choisir un lot pondeuse actif'); const eggCount = toNumber(payload.oeufs_produits); const brokenCount = Math.max(0, toNumber(payload.oeufs_casses)); if (eggCount <= 0) return toast.error('Saisir un nombre d’œufs supérieur à 0'); if (brokenCount > eggCount) return toast.error('Les casses ne peuvent pas dépasser les œufs ramassés'); const sellableEggs = Math.max(0, eggCount - brokenCount); const tablet = tabletsFromEggs(sellableEggs); try { setSaving(true); if (onCommitEggProduction) { await onCommitEggProduction({ ...payload, lot_id: lot.id, lot_name: lot.name || lot.id, date: payload.date || today(), oeufs_produits: eggCount, oeufs_casses: brokenCount, responsable_label: resolveResponsibleLabel(payload.responsable), notes: payload.notes, source_module: 'avicole' }); } else { await onCreateProduction?.({ ...payload, id: payload.id || `PROD-${Date.now()}`, lot_id: lot.id, lot_name: lot.name || lot.id, date: payload.date || today(), oeufs_produits: eggCount, oeufs_casses: brokenCount, oeufs_vendables: sellableEggs, tablettes: tablet.tablettes, tablettes_vendables: tablet.tablettes, plateaux: tablet.tablettes, oeufs_restants: tablet.oeufs_restants, oeufs_reliquat: tablet.oeufs_restants, oeufs_par_tablette: EGGS_PER_TABLET, unite_vente: 'tablette', responsable_label: resolveResponsibleLabel(payload.responsable), module_lie: 'avicole', related_id: lot.id, source_module: 'avicole', type_evenement: 'ramassage_oeufs' }); await onRefreshProduction?.(); } toast.success(`${tablet.tablettes} tablette(s) vendable(s) · ${tablet.oeufs_restants} œuf(s) restants`); setModal(null); } catch (e) { toast.error(e.message || 'Ajout ramassage impossible'); } finally { setSaving(false); } };
  ...
  const columns = [
    { key: 'name', label: 'Lot', sortable: true, render: (lot) => <div><p className="font-semibold text-earth">{lot.name || lot.id}</p><p className="text-xs text-slate">{lot.id}</p></div> },
    ...
    { key: 'effectif', label: 'Effectif', render: (lot) => <div><span className="font-semibold">{fmtNumber(activeCount(lot))}</span><p className="text-meta text-slate">Initial {fmtNumber(initialCount(lot))} - morts {fmtNumber(deadCount(lot))} - vendus/sortis {fmtNumber(soldCount(lot) + exitCount(lot))}</p>{sickCount(lot) > 0 ? <p className="text-meta font-semibold text-horizon-dark">{fmtNumber(sickCount(lot))} à surveiller, encore dans l’effectif</p> : null}{avicoleHasCountMismatch(lot) ? <p className="text-meta font-semibold text-urgent">Enregistré {fmtNumber(registeredCount(lot))}, recalculé {fmtNumber(calculatedCount(lot))}</p> : null}</div> },
    { key: 'achat', label: 'Achat sujets', render: (lot) => <div><b>{fmtCurrency(purchaseTotalOf(lot))}</b><p className="text-xs text-slate">{fmtCurrency(purchaseUnitOf(lot))}/sujet</p></div> },
    { key: 'morts', label: 'Morts / malades', render: (lot) => `${fmtNumber(deadCount(lot))} / ${fmtNumber(sickCount(lot))}` },
    { key: 'weight_avg', label: tab === 'Pondeuse' ? 'Ponte / tablettes' : 'Poids', render: (lot) => { const decision = buildAvicoleLotDecision(lot, productionLogs); if (decision.type === 'pondeuse') { const metrics = calculateLotMetrics({ lot, feedingLogs: alimentationLogs, productionLogs }); return <div><b>{decision.layingRate}% ponte</b><p className="text-xs text-slate">{fmtNumber(metrics.eggMetrics?.totalTablets || 0)} tablette(s) · {fmtNumber(metrics.eggMetrics?.totalRemainingEggs || 0)} œuf(s)</p><p className="text-xs text-slate">{fmtNumber(decision.avgEggsDay)} / {fmtNumber(decision.expectedEggsDay)} œufs/j</p></div>; } return latestWeight(lot) > 0 ? `${latestWeight(lot).toFixed(2)} / ${targetWeight(lot).toFixed(2)} kg` : `- / ${targetWeight(lot).toFixed(2)} kg`; } },
    { key: 'next', label: 'Suivi Horizon', render: (lot) => { const decision = buildAvicoleLotDecision(lot, productionLogs); if (decision.type === 'pondeuse') return <div><p className="font-semibold text-earth">Réforme: {decision.reformStart}</p><p className="text-xs text-slate">cible {decision.reformTarget}</p></div>; return <div><p className="font-semibold text-earth">Pesée: {decision.nextWeighingDate}</p><p className="text-xs text-slate">Rappel J-1: {decision.reminderWeighingDate || '-'}</p><p className="text-xs text-slate">attendu {decision.expectedWeight} kg</p></div>; } },
    { key: 'sale_price', label: 'Prix proposé', render: (lot) => { const sp = recommendAvicoleLotPrice({ lot, alimentationLogs, productionLogs, marketPrices }); const display = buildAvicoleProposedSaleDisplay(sp, lot); return <div><p className="font-semibold text-positive">{display.proposedTotal > 0 ? fmtCurrency(display.proposedTotal) : '-'}</p><p className="text-meta text-slate">{display.proposedUnit > 0 ? `${fmtCurrency(display.proposedUnit)}/sujet` : 'compléter coûts/poids'}</p>{display.pricingBasis ? <p className="text-meta text-slate">{display.pricingBasis}</p> : null}{display.marginOnProposed != null ? <p className="text-meta font-semibold text-positive">marge {fmtCurrency(display.marginOnProposed)}</p> : null}</div>; } },
    { key: 'readiness', label: 'Décision suggérée', render: (lot) => { const decision = buildAvicoleLotDecision(lot, productionLogs); return <Badge color={decisionColor(decision)}>{decision.decision}</Badge>; } },
  ...
  return <div className="space-y-6"><SectionHeader title="Gestion avicole" sub={`${tab === 'Pondeuse' ? 'Pondeuses' : 'Poulets de chair'} uniquement`} actions={<><Btn icon={RefreshCw} variant="outline" small onClick={onRefresh}>Actualiser</Btn><Btn icon={Download} variant="outline" small onClick={exportRows}>Exporter</Btn>{tab === 'Pondeuse' ? <Btn icon={Plus} small onClick={() => setModal('eggs')} disabled={!pondeusesDisponibles.length}>Ramassage œufs</Btn> : null}<Btn icon={Plus} small onClick={() => setModal('create')}>Ajouter {tab === 'Pondeuse' ? 'lot pondeuses' : 'lot chair'}</Btn></>} />
    <div className={embedInElevage ? ELEVAGE_KPI_GRID : 'grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-3 xl:grid-cols-6'}><KpiCard icon={Plus} label="Effectif actif" value={fmtNumber(totalEffectif)} color="bg-positive text-positive" /><KpiCard icon={Plus} label="Morts" value={fmtNumber(morts)} color="bg-urgent text-urgent" /><KpiCard icon={Plus} label="Malades" value={fmtNumber(malades)} color="bg-vigilance text-horizon-dark" /><KpiCard icon={Plus} label="Actions suggérées" value={actionsIa} color="bg-neutral text-neutral" /><KpiCard icon={Plus} label={tab === 'Pondeuse' ? 'Réforme 17+ mois' : chairKpiLabel} value={tab === 'Pondeuse' ? aReformer : chairKpiValue} color={tab === 'Pondeuse' ? 'bg-urgent text-urgent' : 'bg-positive text-positive'} /><KpiCard icon={Plus} label="Coût alim." value={fmtCurrency(coutAlim)} color="bg-neutral text-neutral" /></div>
    <div className="rounded-2xl border border-line bg-card p-3 text-xs text-slate">Lots clôturés: {inactiveLots.length} · Achat actif: {fmtCurrency(coutAchat)} · Règle effectif: initial - morts - vendus - pertes/sorties. Les malades restent dans l’effectif et sont affichés comme à surveiller.</div>
    <DataTable title={tab === 'Pondeuse' ? 'Lots pondeuses actifs' : 'Lots poulets de chair actifs'} rows={activeLots} columns={columns} loading={loading} initialSortKey="date_debut" searchPlaceholder="Rechercher un lot..." />
    {inactiveLots.length ? <DataTable title={`Lots clôturés ou à effectif nul (${inactiveLots.length})`} rows={inactiveLots} columns={columns} loading={loading} initialSortKey="date_debut" searchPlaceholder="Rechercher un lot clôturé..." /> : null}
    ...
    <AvicoleLotDetailsModal open={modal === 'details'} onClose={() => setModal(null)} lot={selected} productionLogs={productionLogs} alimentationLogs={alimentationLogs} opportunities={opportunities} salesOrders={salesOrders} payments={payments} transactions={transactions} businessEvents={businessEvents} onUpdate={onUpdate} onCreateOpportunity={onCreateOpportunity} onUpdateOpportunity={onUpdateOpportunity} onRefreshOpportunities={onRefreshOpportunities} onCreateBusinessEvent={onCreateBusinessEvent} onRefreshBusinessEvents={onRefreshBusinessEvents} onRefresh={onRefresh} onNavigate={onNavigate} />
  </div>;
}
