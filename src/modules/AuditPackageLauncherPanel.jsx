...
export default function AuditPackageLauncherPanel() {
  ...
  const run = async (lot, packageId, dryRun = false) => {
    ...
    try {
      ...
      if (dryRun) {
        ...
        setStatus(`Paquet appliqué pour ${lot}. ${files} fichier(s) envoyé(s) à GitHub. ${deploy}. Après build vert, rafraîchis et relance l’audit.`);
  ...
  return <section className="rounded-3xl border border-[#d6c3a0] bg-white p-5 shadow-sm space-y-4">
    <div>
      <p className="inline-flex items-center gap-2 rounded-full border border-[#eadcc2] bg-[#fffdf8] px-3 py-1 text-xs font-black text-[#8a7456]"><PackageCheck size={14} /> Paquets sécurisés par lot</p>
      <h2 className="mt-3 text-2xl font-black text-[#2f2415]">Appliquer une correction contrôlée par lot</h2>
      <p className="mt-1 text-sm text-[#8a7456]">Tous les lots reconnus par l’agent sont disponibles ici, même si une carte d’audit n’affiche pas encore son bouton.</p>
    </div>
    ...
    <div className="rounded-2xl border border-[#eadcc2] bg-[#fffdf8] p-4">
      ...
      <p className="mt-2 text-xs text-[#8a7456]">Le code n’est pas enregistré. Il autorise seulement l’action serveur.</p>
    </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
      {PACKAGES.map(([lot, packageId]) => <div key={packageId} className="rounded-2xl border border-[#eadcc2] bg-[#fffdf8] p-4">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          <Btn icon={CheckCircle2} small onClick={() => run(lot, packageId, false)} disabled={Boolean(busyPackage)}>{busyPackage === `${packageId}-apply` ? 'Application...' : 'Appliquer paquet sécurisé'}</Btn>
        </div>
      </div>)}
    </div>
  </section>;
