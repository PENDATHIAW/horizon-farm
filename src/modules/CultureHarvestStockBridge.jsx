...
export default function CultureHarvestStockBridge({ rows = [], businessEvents = [], onUpdate, onRefresh, onCreateBusinessEvent, onRefreshBusinessEvents }) {
  ...
  const submit = async (e) => {
    ...
    const journal = createImpactJournal(OPERATION_TYPES.RECOLTE, id);
    const tracked = instrumentHandlers({ onCreateBusinessEvent, onUpdate, onCreateStock: stockCrud.create }, journal);
    await tracked.onCreateBusinessEvent?.(event);
    if (form.destination !== 'perte') {
      const stockId = makeId('STKREC');
      await tracked.onCreateStock?.({
        id: stockId,
      ...
      markImpactCreated(journal, IMPACT_KEYS.STOCK_MOVEMENT, stockId);
    } else {
      markImpactNa(journal, IMPACT_KEYS.STOCK_UPDATED, 'Récolte enregistrée comme perte');
      markImpactNa(journal, IMPACT_KEYS.STOCK_MOVEMENT, 'Aucun stock créé (perte)');
    }
    await tracked.onUpdate?.(culture.id, { quantite_recoltee: form.destination === 'perte' ? toNumber(culture.quantite_recoltee) : toNumber(culture.quantite_recoltee) + qty, production_reelle: form.destination === 'perte' ? toNumber(culture.production_reelle) : toNumber(culture.production_reelle) + qty, pertes_recolte: form.destination === 'perte' ? toNumber(culture.pertes_recolte) + qty : toNumber(culture.pertes_recolte), date_derniere_recolte: form.date || today(), cout_recolte: toNumber(culture.cout_recolte) + extra, cout_revient_unitaire_recolte: Number(unitCost.toFixed(2)), statut: form.destination === 'perte' ? culture.statut : form.destination === 'vente_directe' ? culture.statut : 'recolte' });
    if (extra <= 0) markImpactNa(journal, IMPACT_KEYS.FINANCE, 'Frais de récolte nuls');
    else markImpactNa(journal, IMPACT_KEYS.FINANCE, 'Charge intégrée au coût de revient stock');
    markImpactNa(journal, IMPACT_KEYS.DOCUMENT, 'Aucun document requis');
    markImpactNa(journal, IMPACT_KEYS.TASK_ALERT, 'Aucune tâche ou alerte requise');
    await Promise.allSettled([stockCrud.refresh?.(), onRefresh?.(), onRefreshBusinessEvents?.()]);
    showWorkflowImpactToast(finalizeImpactJournal(journal, OPERATION_EXPECTATIONS[OPERATION_TYPES.RECOLTE]));
    setForm(initial);
  };
