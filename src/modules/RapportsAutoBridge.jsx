...
function activeCount(row = {}) { return Math.max(0, toNumber(row.current_count ?? row.effectif_actuel ?? row.initial_count ?? 0) - toNumber(row.mortality ?? row.morts) - toNumber(row.vendus) - toNumber(row.reformes) - toNumber(row.sorties)); }
function totalSales(orders = []) { return arr(orders).reduce((sum, order) => sum + money(order.montant_total ?? order.total ?? order.amount), 0); }
function totalPaid(payments = [], orders = []) { const paymentSum = arr(payments).reduce((sum, payment) => sum + money(payment.montant_paye ?? payment.montant ?? payment.amount), 0); const orderPaid = arr(orders).reduce((sum, order) => sum + money(order.montant_paye ?? order.paid_amount), 0); return Math.max(paymentSum, orderPaid); }
...
function reportContent(data = {}) {
  ...
  const recommendations = [receivables > 0 ? `Encaisser les montants ouverts (${fmtCurrency(receivables)}).` : 'Maintenir le suivi des encaissements.', stockCritical > 0 ? `Réapprovisionner ${stockCritical} produit(s) sous le seuil.` : 'Stock sous contrôle : continuer l’inventaire régulier.', sickAnimals > 0 || avicoleRisk > 0 ? 'Prioriser santé, biosécurité et fermer les alertes après action terrain.' : 'Santé animale stable : conserver les routines.', tasks > 0 ? `Clôturer ${tasks} tâche(s) terrain ouverte(s).` : 'Aucune tâche terrain bloquante détectée.', critical > 0 ? `Traiter ${critical} alerte(s) urgente(s) avec l’équipe.` : 'Aucune urgence active.'];
  return { sales, paid, receivables, stockValue, stockCritical, animals, sickAnimals, avicoleActive, avicoleRisk, healthCost, cultures: cultures.length, cultureRevenue, financeIn, financeOut, margin, tasks, critical, recommendations, summary: [`Résumé ferme - ${today()}`, `Ventes: ${fmtCurrency(sales)} dont encaissé ${fmtCurrency(paid)}. Reste à encaisser: ${fmtCurrency(receivables)}.`, `Finance: entrées ${fmtCurrency(financeIn)}, sorties ${fmtCurrency(financeOut)}, marge visible ${fmtCurrency(margin)}.`, `Stock: valeur estimée ${fmtCurrency(stockValue)}, ${stockCritical} produit(s) sous le seuil.`, `Animaux: ${fmtNumber(animals)} animal(aux), ${fmtNumber(sickAnimals)} malade(s).`, `Avicole: ${fmtNumber(avicoleActive)} sujet(s) actifs, ${fmtNumber(avicoleRisk)} lot(s) à surveiller.`, `Cultures: ${fmtNumber(cultures.length)} culture(s), revenu estimé/réel ${fmtCurrency(cultureRevenue)}.`, `Santé: coûts renseignés ${fmtCurrency(healthCost)}.`, `Suivi terrain: ${tasks} tâche(s) ouverte(s), ${critical} alerte(s) urgente(s).`, '', 'Actions recommandées:', ...recommendations.map((item) => `- ${item}`)].join('\n') };
}
...
export default function RapportsAutoBridge({ rows = [], data = {}, onCreate, onUpdate, onRefresh, onCreateDocument, onRefreshDocuments, onCreateBusinessEvent, onRefreshBusinessEvents, onCreateTask, onRefreshTasks, onCreateAuditLog, onRefreshAuditLogs }) {
  ...
  return <div className="rounded-2xl border border-line bg-white p-6 space-y-4"><div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3"><div><p className="text-xs uppercase tracking-normal text-slate">Rapports automatiques</p><h3 className="font-semibold text-earth">Synthèse prête à générer</h3><p className="text-sm text-slate mt-1">Les informations de ventes, finances, stock, santé, cultures, animaux, avicole, tâches et alertes sont rassemblées ici.</p></div><div className="grid grid-cols-2 gap-2 text-sm"><div className="rounded-xl border border-line bg-card px-3 py-2"><BarChart2 size={14} className="inline" /> {fmtCurrency(content.sales)}</div><div className="rounded-xl border border-line bg-card px-3 py-2"><FileText size={14} className="inline" /> {fmtCurrency(content.margin)}</div></div></div><div className="grid grid-cols-1 md:grid-cols-4 gap-2"><Metric label="Ventes" value={fmtCurrency(content.sales)} /><Metric label="Encaissements" value={fmtCurrency(content.paid)} /><Metric label="Stock sous seuil" value={content.stockCritical} /><Metric label="Marge estimée" value={fmtCurrency(content.margin)} /></div><div className="rounded-xl border border-line bg-card p-3 text-sm text-slate whitespace-pre-line">{content.summary}</div><div className="flex flex-wrap gap-2"><button type="button" disabled={saving === 'hebdo_erp'} className="rounded-xl bg-horizon px-4 py-2 text-sm font-semibold text-white disabled:opacity-60" onClick={() => generate('hebdo_erp', today())}>{saving === 'hebdo_erp' ? <RefreshCw size={14} className="inline animate-spin" /> : <CheckCircle2 size={14} className="inline" />} {weekly ? 'Mettre à jour hebdo' : 'Générer hebdo'}</button><button type="button" disabled={saving === 'mensuel_erp'} className="rounded-xl border border-line px-4 py-2 text-sm font-semibold text-earth disabled:opacity-60" onClick={() => generate('mensuel_erp', period)}>{saving === 'mensuel_erp' ? <RefreshCw size={14} className="inline animate-spin" /> : <FileText size={14} className="inline" />} {monthly ? 'Mettre à jour mensuel' : 'Générer mensuel'}</button><button type="button" disabled={saving === 'schedule-mensuel_erp'} className="rounded-xl border border-line px-4 py-2 text-sm font-semibold text-earth disabled:opacity-60" onClick={() => scheduleTask('mensuel_erp', period)}>Programmer tâche</button><button type="button" className="rounded-xl border border-line px-4 py-2 text-sm font-semibold text-earth" onClick={() => exportReport('pdf')}><Download size={14} className="inline" /> Exporter PDF</button><button type="button" className="rounded-xl border border-line px-4 py-2 text-sm font-semibold text-earth" onClick={() => exportReport('excel')}>Excel</button><button type="button" className="rounded-xl border border-line px-4 py-2 text-sm font-semibold text-earth" onClick={() => exportReport('csv')}>CSV</button></div></div>;
}
function Metric({ label, value }) { return <div className="rounded-xl border border-line bg-card p-3"><p className="text-xs text-slate">{label}</p><p className="font-semibold text-earth">{value}</p></div>; }
