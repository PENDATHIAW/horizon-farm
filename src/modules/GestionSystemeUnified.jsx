...
export default function GestionSystemeUnified({ equipements = [], transactions = [], documents = [], tasks = [], alertes = [], businessEvents = [], businessEventsAll = [], auditLogs = [], auditLogsAll = [], users = [], profiles = [], farm = {}, ferme = {}, online = true, lastOnlineAt, onRefreshAll, onFlushOffline, onClearAllData, dataMap: visionDataMap = {}, periodScope, periodFiltered = false, periodLabel = '', onNavigate, onCreateTask, onCreateAlert, onUpdateAlert, onCreateBusinessEvent, existingTasks = [], existingAlerts = [], initialTab = 'Vue admin', onTabChange, accessibleFarms = [], onFarmsChanged, companyId = null, farmsPanelAction = null, farmComparisonData = null, onManageFarms, syncProps = null }) {
  ...
  const exportDataSnapshot = useCallback(() => {
    try {
      const filtered = Object.fromEntries(
        Object.entries(dataMap || {}).filter(([, rows]) => Array.isArray(rows) && rows.length > 0),
      );
      const summary = Object.fromEntries(
        Object.entries(filtered).map(([key, rows]) => [key, rows.length]),
      );
      const payload = {
      ...
      anchor.href = url;
      anchor.download = `horizon-farm-snapshot-${new Date().toISOString().slice(0, 10)}.json`;
      anchor.click();
      URL.revokeObjectURL(url);
      toast.success('Snapshot ERP téléchargé');
    } catch (error) {
      toast.error(error.message || 'Export snapshot impossible');
    }
  }, [dataMap, farmInfo]);
  ...
  : tab === 'Utilisateurs' ? <div className="space-y-5"><Section icon={UserCog} title="Utilisateurs"><Row title="Utilisateur connecté" detail={user?.email || 'Compte courant'} value={role || 'Utilisateur'} tone={canManage ? 'good' : 'warn'} /><Row title="Profils connus" detail={`${count(users) || count(profiles)} profil(s) disponible(s) dans les données reçues.`} value="Suivi" /><Row title="Permissions" detail="Les actions sensibles restent contrôlées par les rôles système." value={canManage ? 'Modifiable' : 'Lecture'} tone={canManage ? 'good' : 'warn'} />{arr(users.length ? users : profiles).slice(0, 8).map((profile) => (<Row key={profile.id || profile.email} title={profile.nom || profile.name || profile.email || profile.id} detail={`${profile.email || 'email non renseigné'} · rôle : ${profile.role || profile.type || 'utilisateur'}`} value={profile.statut || profile.status || 'actif'} tone={String(profile.statut || profile.status || 'actif').toLowerCase() === 'actif' ? 'good' : 'warn'} />))}</Section><ActionNote>Création / suppression / changement de rôle : à effectuer dans la console Supabase (Auth) ; l’ERP affiche les profils reçus en lecture seule.</ActionNote></div>
  : tab === 'Paramètres' ? <div className="space-y-5"><Section icon={MapPin} title="Paramètres exploitation"><Row title="Nom ferme" detail={farmInfo.nom || farmInfo.name || 'À renseigner'} value="Profil" /><Row title="Localisation" detail={farmInfo.localisation || farmInfo.location || farmInfo.ville || farmInfo.city || 'Quartier, ville, pays à renseigner'} value="Adresse" /><Row title="Devise" detail={farmInfo.devise || farmInfo.currency || 'FCFA'} value="Finance" /><Row title="Unités" detail="Poids, quantités, surfaces et production doivent rester harmonisés." value="Standard" /></Section><div className="flex flex-wrap gap-2"><button type="button" onClick={() => setTab('Fermes')} className="rounded-xl border border-emerald-300 bg-emerald-50 px-4 py-3 text-sm font-black text-emerald-800">Modifier la ferme (Fermes) →</button><button type="button" onClick={() => onNavigate?.('dashboard')} className="rounded-xl border border-[#d6c3a0] bg-[#fffdf8] px-4 py-3 text-sm font-black text-[#2f2415]">Ouvrir Accueil</button></div></div>
  ...
  : tab === 'Sauvegardes' ? <div className="space-y-5"><Section icon={Wifi} title="Synchronisation"><Row title="Connexion" detail={lastOnlineAt ? `Dernière connexion : ${lastOnlineAt}` : 'État actuel de l’application'} value={online ? 'En ligne' : 'Hors ligne'} tone={online ? 'good' : 'warn'} /><Row title="Actions ouvertes" detail={`${arr(tasks).filter(open).length} tâche(s), ${arr(alertes).filter(open).length} alerte(s)`} value="À suivre" /><Row title="Événements récents" detail={`${count(eventRows)} événement(s) métier suivi(s).`} value="Historique" /></Section><div className="flex flex-wrap gap-2"><button type="button" onClick={() => onRefreshAll?.()} disabled={typeof onRefreshAll !== 'function'} className="rounded-xl border border-[#d6c3a0] bg-[#fffdf8] px-4 py-3 text-sm font-black text-[#2f2415] disabled:opacity-40">Actualiser les données</button><button type="button" onClick={() => onFlushOffline?.()} disabled={typeof onFlushOffline !== 'function'} className="rounded-xl border border-[#d6c3a0] bg-[#fffdf8] px-4 py-3 text-sm font-black text-[#2f2415] disabled:opacity-40">Synchroniser hors ligne</button><button type="button" onClick={exportDataSnapshot} className="rounded-xl border border-emerald-300 bg-emerald-50 px-4 py-3 text-sm font-black text-emerald-800">Télécharger snapshot JSON</button></div><ActionNote>La sauvegarde/restauration réelle doit être branchée uniquement côté administrateur. Le bouton « Télécharger snapshot JSON » exporte l’état actuel visible dans l’ERP.</ActionNote></div>
  : tab === 'Synchronisation' ? <SyncActivityCenter {...(syncProps || { online, lastOnlineAt, onRefreshAll, onFlushOffline, onNavigate })} embedded />
