  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 gap-3 xl:grid-cols-8">
        ...
        <Stat label="Santé ERP" value={`${snapshot.healthScore}/100`} tone={snapshot.healthScore >= 75 ? 'good' : 'warn'} />
...
export default function GestionSystemeUnified({ equipements = [], transactions = [], documents = [], tasks = [], alertes = [], businessEvents = [], auditLogs = [], users = [], profiles = [], farm = {}, ferme = {}, online = true, lastOnlineAt, onRefreshAll, onFlushOffline, onClearAllData, dataMap: visionDataMap = {}, onNavigate, onCreateTask, onCreateAlert, onUpdateAlert, onCreateBusinessEvent, existingTasks = [], existingAlerts = [] }) {
  ...
  const effectiveDataMap = Object.keys(visionDataMap).length ? visionDataMap : dataMap;
  ...
  const adminData = { auditLogCount: count(auditLogs) };
  ...
  : <div className="space-y-5"><VisionModuleAuditPanel dataMap={effectiveDataMap} onNavigate={onNavigate} />{coherenceRows.length ? <Section icon={Zap} title="Actions audit rapides">{coherenceRows.slice(0, 6).map((row) => <div key={row.id} className="flex flex-col gap-2 border-b border-[#eadcc2]/70 py-3 last:border-b-0 sm:flex-row sm:items-center sm:justify-between"><div><b className="text-[#2f2415]">{row.title}</b><p className="text-xs text-[#8a7456]">{row.detail}</p></div><div className="flex gap-2">{row.moduleId ? <button type="button" onClick={() => onNavigate?.(row.moduleId)} className="rounded-lg border border-[#d6c3a0] bg-white px-2 py-1 text-xs font-black">Ouvrir</button> : null}<button type="button" disabled={busyId === row.id} onClick={() => row.finding && applyFinding(row.finding)} className="rounded-lg bg-[#22c55e] px-2 py-1 text-xs font-black text-[#052e16] disabled:opacity-50">{busyId === row.id ? '…' : 'Créer tâche'}</button></div></div>)}</Section> : null}<Section icon={ClipboardList} title="Journal audit technique">{arr(auditLogs).length ? arr(auditLogs).slice(0, 12).map((log) => <Row key={log.id || labelOf(log)} title={labelOf(log)} detail={`${dateOf(log)} · ${log.module || log.module_source || log.actor || 'ERP'}`} value={log.action || log.event_type || 'Log'} />) : <ActionNote>Aucun log d'audit détaillé reçu. Les événements métier restent visibles dans Activité & Suivi.</ActionNote>}</Section></div>;
  return <div className="space-y-6"><section className="rounded-3xl border border-[#d6c3a0] bg-white p-5 shadow-sm"><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div><p className="text-xs uppercase tracking-[0.25em] text-[#9a6b12] font-black">Administration</p><h1 className="mt-1 text-2xl font-black text-[#2f2415]">Gestion du système</h1><p className="mt-1 text-sm text-[#8a7456]">Audit vision ERP, sécurité, synchronisation — pilotage administrateur.</p></div><div className="flex flex-wrap items-center gap-2"><div className="rounded-2xl border border-[#eadcc2] bg-[#fffdf8] px-4 py-3 text-sm"><span className="text-[#8a7456]">Santé </span><b className={snapshot.systemScore >= 75 ? 'text-emerald-700' : 'text-amber-700'}>{snapshot.systemScore}/100</b></div><div className={`rounded-2xl border px-4 py-3 text-sm font-black ${canManage ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-amber-200 bg-amber-50 text-amber-800'}`}>{canManage ? <ShieldCheck size={16} className="inline mr-1" /> : <ShieldAlert size={16} className="inline mr-1" />}{canManage ? 'Actions admin autorisées' : 'Lecture seule'}</div></div></div></section><Tabs active={tab} setActive={setTab} />{content}</div>;
}
