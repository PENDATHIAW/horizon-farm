...
function AdminOverview({ data, checks, snapshot, setTab, onApply, busyId, onNavigate, issuePanelProps = {} }) {
  return (
    <div className="space-y-5">
      ...
      <IssueProblemFichePanel {...issuePanelProps} />
...
export default function GestionSystemeUnified({ equipements = [], transactions = [], documents = [], tasks = [], alertes = [], salesOrders = [], payments = [], businessEvents = [], businessEventsAll = [], auditLogs = [], auditLogsAll = [], users = [], profiles = [], farm = {}, ferme = {}, online = true, lastOnlineAt, onRefreshAll, onFlushOffline, onClearAllData, dataMap: visionDataMap = {}, periodScope, periodFiltered = false, periodLabel = '', onNavigate, onCreateTask, onCreateAlert, onUpdateAlert, onCreateBusinessEvent, existingTasks = [], existingAlerts = [] }) {
  ...
  const issuePanelProps = { alertes, taches: tasks, businessEvents: eventRows, salesOrders, payments, onNavigate };
  const adminData = { auditLogCount: count(auditRows) };
  const content = tab === 'Vue admin' ? <AdminOverview data={adminData} checks={checks} snapshot={snapshot} setTab={setTab} onApply={applyFinding} busyId={busyId} onNavigate={onNavigate} issuePanelProps={issuePanelProps} />
