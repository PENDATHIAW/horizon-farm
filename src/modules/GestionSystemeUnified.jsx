...
function ResetDataSection({ canManage, onClearAllData, onRefreshAll }) {
...
export default function GestionSystemeUnified({ equipements = [], transactions = [], documents = [], tasks = [], alertes = [], businessEvents = [], auditLogs = [], users = [], profiles = [], farm = {}, ferme = {}, online = true, lastOnlineAt, onRefreshAll, onFlushOffline, onClearAllData }) {
  ...
  const { dataMap, deleteRecord, refreshModule } = useAppData();
  ...
  const clearAllData = useCallback(async () => {
    ...
    for (const [moduleKey, rows] of entries) {
      for (const row of rows) {
        ...
        if (rowId) await deleteRecord(moduleKey, rowId);
      }
    }
    await Promise.allSettled(entries.map(([moduleKey]) => refreshModule(moduleKey)));
    return true;
  }, [dataMap, deleteRecord, onClearAllData, refreshModule]);
  const checks = [
  ...
  : tab === 'Réinitialisation' ? <ResetDataSection canManage={canManage} onClearAllData={clearAllData} onRefreshAll={onRefreshAll} />
