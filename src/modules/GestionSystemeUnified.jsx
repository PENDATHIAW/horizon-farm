...
export default function GestionSystemeUnified({ equipements = [], transactions = [], documents = [], tasks = [], alertes = [], businessEvents = [], businessEventsAll = [], auditLogs = [], auditLogsAll = [], users = [], profiles = [], farm = {}, ferme = {}, online = true, lastOnlineAt, onRefreshAll, onFlushOffline, onClearAllData, dataMap: visionDataMap = {}, periodScope, periodFiltered = false, periodLabel = '', onNavigate, onCreateTask, onCreateAlert, onUpdateAlert, onCreateBusinessEvent, existingTasks = [], existingAlerts = [] }) {
  ...
  : <div className="space-y-5"><WorkflowQualityPanel dataMap={effectiveDataMap} onNavigate={onNavigate} /><VisionModuleAuditPanel dataMap={effectiveDataMap} onNavigate={onNavigate} />{coherenceRows.length ? <Section icon={Zap} title="Actions audit rapides">{coherenceRows.slice(0, 6).map((row) => <div key={row.id} className="flex flex-col gap-2 border-b border-[#eadcc2]/70 py-3 last:border-b-0 sm:flex-row sm:items-center sm:justify-between"><div><b className="text-[#2f2415]">{row.title}</b><p className="text-xs text-[#8a7456]">{row.detail}</p></div><div className="flex gap-2">{row.moduleId ? <button type="button" onClick={() => onNavigate?.(row.moduleId)} className="rounded-lg border border-[#d6c3a0] bg-white px-2 py-1 text-xs font-black">Ouvrir</button> : null}<button type="button" disabled={busyId === row.id} onClick={() => row.finding && applyFinding(row.finding)} className="rounded-lg bg-[#22c55e] px-2 py-1 text-xs font-black text-[#052e16] disabled:opacity-50">{busyId === row.id ? '…' : 'Créer tâche'}</button></div></div>)}</Section> : null}<Section icon={ClipboardList} title="Journal audit technique">{auditRows.length ? auditRows.slice(0, 12).map((log) => <Row key={log.id || labelOf(log)} title={labelOf(log)} detail={`${dateOf(log)} · ${log.module || log.module_source || log.actor || 'ERP'}`} value={log.action || log.event_type || 'Log'} />) : <ActionNote>Aucun log d'audit détaillé reçu. Les événements métier restent visibles dans Activité & Suivi.</ActionNote>}</Section></div>;
