...
const eventTargetId = (row = {}) => String(row.target_id || row.related_id || row.entity_id || row.source_record_id || row.culture_id || '');
const eventAmount = (row = {}) => toNumber(row.montant ?? row.amount ?? row.cout ?? row.cost ?? row.cout_total ?? row.total_cost ?? row.total);
const isCultureCharge = (row = {}) => {
  const text = String(`${row.type_evenement || ''} ${row.event_type || ''} ${row.type || ''} ${row.categorie || ''} ${row.category || ''} ${row.title || ''} ${row.libelle || ''}`).toLowerCase();
  const source = String(`${row.target_type || ''} ${row.module_lie || ''} ${row.source_module || ''}`).toLowerCase();
  return source.includes('culture') || text.includes('culture') || text.includes('engrais') || text.includes('semence') || text.includes('irrigation') || text.includes('phyto') || text.includes('carburant') || text.includes('charge_directe');
};
...
export default function CultureCostOverview({ rows = [], businessEvents = [] }) {
  const details = arr(rows).map((culture) => {
    const metrics = calculateCultureMetrics(culture);
    const chargesDirectes = directChargeFor(businessEvents, culture.id);
    const coutTotal = toNumber(culture.cout_total_reel) || metrics.costTotal + chargesDirectes;
    const qty = harvestQty(culture);
    const area = surface(culture);
    return { culture, metrics, chargesDirectes, coutTotal, qty, area, coutKg: qty > 0 ? coutTotal / qty : 0, coutSurface: area > 0 ? coutTotal / area : 0 };
  });
  ...
  }), { cout: 0, charges: 0, surface: 0, quantite: 0, marge: 0 });
  const recentCharges = arr(businessEvents).filter(isCultureCharge).slice(0, 6);
  ...
  return <section className="rounded-3xl border border-[#d6c3a0] bg-white p-5 shadow-sm space-y-4">
    <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
      <div>
        ...
        <p className="mt-1 text-sm text-[#8a7456]">Semences, engrais, irrigation, main-d’œuvre, intrants et charges directes par culture.</p>
      </div>
    ...
    <div className="overflow-x-auto rounded-2xl border border-[#eadcc2]">
      <table className="min-w-full text-sm">
        ...
        <tbody>{details.slice(0, 10).map((item) => <tr key={item.culture.id} className="border-b border-[#f0e5d0]"><td className="py-3 px-3 font-bold text-[#2f2415]">{cultureLabel(item.culture)}</td><td className="py-3 px-3">{fmtNumber(item.area)} m²</td><td className="py-3 px-3">{fmtNumber(item.qty)} kg</td><td className="py-3 px-3 font-black">{fmtCurrency(item.coutTotal)}</td><td className="py-3 px-3">{fmtCurrency(item.chargesDirectes)}</td><td className="py-3 px-3">{item.coutKg ? fmtCurrency(item.coutKg) : '—'}</td><td className="py-3 px-3">{item.coutSurface ? fmtCurrency(item.coutSurface) : '—'}</td></tr>)}</tbody>
      </table>
    </div>
    ...
    <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-800 flex items-start gap-2"><Wallet size={16} className="mt-0.5" /> Les charges ajoutées s’additionnent au coût réel de la culture sans remplacer l’historique.</div>
  </section>;
}
