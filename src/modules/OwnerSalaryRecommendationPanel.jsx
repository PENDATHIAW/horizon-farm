...
export default function OwnerSalaryRecommendationPanel({
  ...
  return <section className="rounded-3xl border border-line bg-white p-6 shadow-card space-y-4"><div className="flex flex-col lg:flex-row lg:items-start justify-between gap-3"><div><p className="flex items-center gap-2 text-lg font-semibold text-earth"><Wallet size={20} /> Rémunération propriétaire recommandée</p><p className="mt-1 text-sm text-slate">Proposition basée sur le résultat consolidé Finance, le cash disponible et une réserve de sécurité.</p></div><div className={`${recommendation.status === 'valider' ? 'bg-positive-bg text-positive border-positive' : recommendation.status === 'prudence' ? 'bg-vigilance-bg text-horizon-dark border-vigilance' : 'bg-urgent-bg text-urgent border-urgent'} rounded-2xl border px-4 py-3 text-sm font-semibold`}>{recommendation.status === 'valider' ? <CheckCircle2 size={15} className="inline" /> : negativeContext ? <AlertTriangle size={15} className="inline" /> : <ShieldCheck size={15} className="inline" />} {recommendation.reason}</div></div><div className="grid grid-cols-1 md:grid-cols-4 gap-3"><Mini icon={PiggyBank} label="Salaire recommandé" value={fmtCurrency(recommendation.recommended)} hint={recommendation.recommended > 0 ? 'à valider manuellement' : 'non recommandé maintenant'} danger={recommendation.recommended <= 0} /><Mini icon={ShieldCheck} label="Réserve gardée" value={fmtCurrency(recommendation.requiredReserve)} hint="35% du résultat positif" /><Mini icon={Wallet} label="Cash après réserve" value={fmtCurrency(recommendation.cashAvailableAfterReserve)} hint="avant nouveau salaire" danger={recommendation.cashAvailableAfterReserve <= 0} /><Mini icon={CheckCircle2} label="Salaire déjà validé" value={fmtCurrency(recommendation.alreadySalary)} hint="période courante" /></div><div className="grid grid-cols-1 md:grid-cols-2 gap-3"><Mini icon={TrendingResultIcon} label="Résultat exploitation consolidé" value={fmtCurrency(profit.operatingResult)} hint="après charges consolidées" danger={profit.operatingResult <= 0} /><Mini icon={Wallet} label="Cash après investissements" value={fmtCurrency(profit.cashResultAfterInvestments)} hint="base de décision rémunération" danger={profit.cashResultAfterInvestments <= 0} /></div><div className="rounded-2xl border border-line bg-card p-3 text-sm text-slate">La rémunération propriétaire utilise maintenant les mêmes sources que Finance : ventes, paiements, animaux, avicole, cultures, santé, alimentation, stock, fournisseurs, investissements et événements métier.</div><div className="flex flex-wrap gap-2"><Btn icon={Wallet} onClick={validateSalary} disabled={!canValidate}>Valider rémunération propriétaire</Btn>{!canValidate ? <span className="text-sm text-slate self-center">Aucun montant recommandé pour le moment.</span> : null}</div></section>;
}
