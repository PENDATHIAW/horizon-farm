...
const COSTS = {
  alimentation: 'Alimentation', stock: 'Stock / achats', sante: 'Santé', main_oeuvre: 'Main-d’œuvre', fournisseurs: 'Fournisseurs', transport_charge: 'Transport charge', energie: 'Énergie', maintenance: 'Maintenance', investissements: 'Investissements', equipements: 'Équipements', autres_charges: 'Autres charges',
};
...
function ensureFinanceFields() {
  ...
  if (moduleLie) moduleLie.options = ['animaux', 'avicole_oeufs', 'avicole_chair', 'avicole_reformes', 'cultures', 'produits_transformes', 'services_agricoles', 'location_materiel', 'transport_service', 'subventions', 'financements', 'alimentation', 'stock', 'sante', 'main_oeuvre', 'fournisseurs', 'transport_charge', 'energie', 'maintenance', 'investissements', 'equipements', 'autres_revenus', 'autres_charges'];
}

function textOf(row = {}) { return `${row.module_lie || ''} ${row.module_source || ''} ${row.categorie || ''} ${row.category || ''} ${row.type_produit || ''} ${row.product_type || ''} ${row.source_type || ''} ${row.libelle || ''} ${row.nom || ''}`.toLowerCase(); }
...
function openModule(moduleKey) { if (!moduleKey || typeof document === 'undefined') return; Array.from(document.querySelectorAll('nav button')).find((button) => button.textContent?.toLowerCase().includes(moduleKey.toLowerCase()))?.click(); }
...
function orderRemaining(order = {}) { const explicit = toNumber(order.reste_a_payer ?? order.remaining_amount, NaN); return Number.isFinite(explicit) ? Math.max(0, explicit) : Math.max(0, orderTotal(order) - orderPaid(order)); }
function makeActivity(id) { return { id, label: LABELS[id] || id, kind: REVENUE_KEYS.includes(id) ? 'revenue' : 'cost', products: 0, charges: 0, paid: 0, receivables: 0, sources: new Set() }; }

function computeAll({ rows, salesOrders, payments, fournisseurs, stocks, alimentationLogs }) {
  ...
  const map = new Map();
  const ensure = (key) => { if (!map.has(key)) map.set(key, makeActivity(key)); return map.get(key); };

  tx.forEach((row) => {
    const item = ensure(activityKey(row));
  ...
  const txRevenueTotal = tx.filter(isIn).reduce((s, r) => s + amount(r), 0);
  const orderTotalAll = orders.reduce((s, o) => s + orderTotal(o), 0);
  const shouldUseOrdersForActivities = orderTotalAll > txRevenueTotal + 1;
  if (shouldUseOrdersForActivities) {
    orders.forEach((order) => {
      const key = activityKey({ ...order, type: 'entree', categorie: order.categorie || order.category || order.source_type || 'vente' });
      const item = ensure(REVENUE_KEYS.includes(key) ? key : 'autres_revenus');
      item.products += orderTotal(order);
      item.paid += orderPaid(order);
      item.receivables += orderRemaining(order);
      item.sources.add('Ventes');
    });
  }
  const txPaidRevenue = tx.filter((r) => isIn(r) && isPaid(r)).reduce((s, r) => s + amount(r), 0);
  const paymentTotalAll = pay.reduce((s, p) => s + amount(p), 0);
  if (paymentTotalAll > txPaidRevenue + 1 && !shouldUseOrdersForActivities) {
    const item = ensure('autres_revenus');
    item.paid += paymentTotalAll - txPaidRevenue;
    item.products += paymentTotalAll - txPaidRevenue;
    item.sources.add('Paiements non classés');
  }
  ...
  safeArray(alimentationLogs).forEach((log) => {
    ...
    const item = ensure(t.includes('avicole') || t.includes('lot') || t.includes('poule') ? 'avicole_chair' : 'animaux');
    item.charges += cost;
    item.sources.add('Alimentation');
  });
  const supplierDebts = safeArray(fournisseurs).reduce((s, f) => s + toNumber(f.dettes), 0);
  if (supplierDebts > 0) { const item = ensure('fournisseurs'); item.charges += supplierDebts; item.sources.add('Fournisseurs'); }
  const stockValue = safeArray(stocks).reduce((s, it) => s + toNumber(it.quantite ?? it.quantity) * toNumber(it.prix_unitaire ?? it.unit_price ?? it.price), 0);
  if (stockValue > 0) { const item = ensure('stock'); item.stockValue = stockValue; item.sources.add('Stock'); }

  const activities = Array.from(map.values()).map((item) => {
  ...
  }).filter((item) => item.products || item.charges || item.paid || item.receivables || item.stockValue).sort((a, b) => b.margin - a.margin);

  const txIncomePaid = tx.filter((r) => isIn(r) && isPaid(r)).reduce((s, r) => s + amount(r), 0);
  ...
  const cashIn = Math.max(txIncomePaid, paymentTotalAll, orders.reduce((s, o) => s + orderPaid(o), 0));
  const receivables = orders.reduce((s, o) => s + orderRemaining(o), 0) + tx.filter((r) => isIn(r) && isPartialOrUnpaid(r)).reduce((s, r) => s + amount(r), 0);
  const revenue = Math.max(orderTotalAll, tx.filter(isIn).reduce((s, r) => s + amount(r), 0), cashIn + receivables);
  const debts = tx.filter((r) => isOut(r) && isPartialOrUnpaid(r)).reduce((s, r) => s + amount(r), 0) + supplierDebts;
  const margin = revenue - expenses;
  return { activities, summary: { revenue, cashIn, expenses, cash: cashIn - txExpensesPaid, receivables, debts, margin, marginRate: revenue > 0 ? (margin / revenue) * 100 : 0 }, zeroRows: safeArray(rows).filter((r) => !hasAmount(r)) };
}
...
function ActivityRow({ activity }) {
  ...
  const detail = activity.kind === 'cost'
    ? `Charges ${fmtCurrency(activity.charges)}${activity.stockValue ? ` · Valeur stock ${fmtCurrency(activity.stockValue)}` : ''} · Source ${activity.sourcesLabel}`
    : activity.pendingCosts
      ? `Charges ${fmtCurrency(activity.charges)} · Aucun produit enregistré · Source ${activity.sourcesLabel}`
...
export default function FinancesV4({ rows = [], loading, onCreate, onUpdate, onDelete, onRefresh, stocks = [], fournisseurs = [], alimentationLogs = [], businessPlans = [], salesOrders = [], payments = [] }) {
  ensureFinanceFields();
  ...
  const { activities, summary, zeroRows } = useMemo(() => computeAll({ rows, salesOrders, payments, fournisseurs, stocks, alimentationLogs }), [rows, salesOrders, payments, fournisseurs, stocks, alimentationLogs]);
  const revenueCoverage = REVENUE_KEYS.map((key) => { const item = activities.find((a) => a.id === key); return { key, label: REVENUES[key], amount: item?.products || 0, paid: item?.paid || 0, receivables: item?.receivables || 0, active: Boolean(item?.products || item?.paid || item?.receivables) }; });
  const activeRevenueCount = revenueCoverage.filter((i) => i.active).length;
  ...
  const chargesToWatch = activities.filter((a) => a.kind === 'cost').reduce((s, a) => s + a.charges, 0);
  ...
  const financeFormFields = useMemo(() => { const bpOptions = safeArray(businessPlans).map((bp) => ({ value: bp.id, label: bp.nom || bp.id })); return (MODULE_FORM_FIELDS.finances || []).map((field) => field.key === 'business_plan_id' && bpOptions.length ? { ...field, type: 'select', options: [{ value: '', label: '— Aucun —' }, ...bpOptions] } : field); }, [businessPlans]);
  ...
  const columns = [
    { key: 'date', label: 'Date', sortable: true }, { key: 'libelle', label: 'Libellé', sortable: true, render: (row) => <span className="font-semibold text-[#2f2415]">{row.libelle || '-'}</span> },
    ...
    { key: 'categorie', label: 'Catégorie', sortable: true, render: (row) => row.categorie || row.category || '-' }, { key: 'module_lie', label: 'Activité', sortable: true, render: (row) => LABELS[activityKey(row)] || '-' },
    ...
    { key: 'statut', label: 'Statut', sortable: true, render: (row) => <Badge status={row.statut || 'paye'} /> }, { key: 'paiement', label: 'Paiement', sortable: true },
  ...
  return <div className="space-y-6 pt-2">
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4"><InsightCard icon={FileText} title="Justificatifs à compléter" value={fmtNumber(txWithoutProof.length)} detail="Chaque produit/charge importante doit avoir reçu, facture ou preuve." /><InsightCard icon={AlertTriangle} title="Charges à suivre" value={fmtCurrency(chargesToWatch)} detail="Santé, stock, fournisseurs, équipements, alimentation et maintenance." /><InsightCard icon={BookOpen} title="Écritures ignorées" value={fmtNumber(zeroRows.length)} detail="Les lignes à 0 FCFA sont masquées pour éviter de fausser l’analyse." /></div>
    <div className="bg-white border border-[#d6c3a0] rounded-2xl p-5"><h3 className="font-black text-[#2f2415] mb-2">Sources de revenus agricoles couvertes</h3><p className="text-sm text-[#8a7456] mb-4">Cette section vérifie que l’argent vient des activités productives réelles, sans doubler les commandes déjà enregistrées en transactions.</p><div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">{revenueCoverage.map((item) => <RevenueChip key={item.key} item={item} />)}</div></div>
