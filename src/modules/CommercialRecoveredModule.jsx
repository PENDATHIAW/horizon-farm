...
function Summary({ data, setTab }) {
  return <div className="space-y-5"><div className="grid grid-cols-2 gap-3 xl:grid-cols-5"><Stat label="Ventes" value={fmtNumber(data.orders.length)} /><Stat label="Ouvertes" value={fmtNumber(data.openOrders.length)} tone={data.openOrders.length ? 'warn' : 'good'} /><Stat label="À encaisser" value={fmtCurrency(data.receivable)} tone={data.receivable ? 'warn' : 'good'} /><Stat label="Clients" value={fmtNumber(data.clients.length)} /><Stat label="Opportunités" value={fmtNumber(data.openOpportunities.length)} tone="good" /></div><section className="rounded-3xl border border-[#d6c3a0] bg-white p-5 shadow-sm"><h2 className="flex items-center gap-2 text-lg font-black text-[#2f2415]"><Handshake size={20} /> Parcours commercial récupéré</h2><p className="mt-2 text-sm leading-relaxed text-[#8a7456]">Le module commercial conserve maintenant le vrai workflow de l’ancien ERP : vente complète, lignes, livraison, facture, document, paiement, transaction finance, mise à jour du stock ou de la production, événement métier, relances et opportunités.</p><div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3"><button type="button" onClick={() => setTab('Ventes')} className="rounded-2xl border border-[#eadcc2] bg-[#fffdf8] p-4 text-left"><b className="text-[#2f2415]">Ventes</b><p className="mt-1 text-sm text-[#8a7456]">Créer, modifier, encaisser, livrer, facturer, clôturer.</p></button><button type="button" onClick={() => setTab('Clients')} className="rounded-2xl border border-[#eadcc2] bg-[#fffdf8] p-4 text-left"><b className="text-[#2f2415]">Clients</b><p className="mt-1 text-sm text-[#8a7456]">Fiches clients, achats, paiements, historique.</p></button><button type="button" onClick={() => setTab('Opportunités')} className="rounded-2xl border border-[#eadcc2] bg-[#fffdf8] p-4 text-left"><b className="text-[#2f2415]">Opportunités</b><p className="mt-1 text-sm text-[#8a7456]">Produits à vendre, clients à relancer, ventes proposées.</p></button></div></section></div>;
}
function Opportunities({ opportunities, setTab }) {
  return <section className="rounded-3xl border border-[#d6c3a0] bg-white p-5 shadow-sm"><h2 className="flex items-center gap-2 text-lg font-black text-[#2f2415]"><Lightbulb size={20} /> Opportunités commerciales</h2><div className="mt-4 divide-y divide-[#eadcc2]">{opportunities.length ? opportunities.slice(0, 20).map((row) => <button key={row.id || row.title || row.libelle} type="button" onClick={() => setTab('Ventes')} className="grid w-full grid-cols-1 gap-2 py-4 text-left md:grid-cols-[260px_1fr_auto]"><b className="text-[#2f2415]">{row.title || row.libelle || row.product_name || 'Opportunité'}</b><span className="text-sm text-[#8a7456]">{row.client_nom || row.customer_name || row.notes || row.source_module || 'Commercial'}</span><span className="rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs font-black text-emerald-700">{fmtCurrency(amount(row))}</span></button>) : <div className="rounded-2xl border border-[#eadcc2] bg-[#fffdf8] p-5 text-sm text-[#8a7456]">Aucune opportunité ouverte.</div>}</div></section>;
}
...
export default function CommercialRecoveredModule(props) {
  ...
  const data = useMemo(() => { const openOrders = orders.filter(isOpenOrder); const receivable = orders.reduce((sum, row) => sum + Math.max(0, amount(row) - paid(row) - payments.filter((p) => String(p.order_id || p.sale_id || p.source_record_id) === String(row.id)).reduce((s, p) => s + n(p.montant ?? p.amount ?? p.montant_paye), 0)), 0); return { orders, openOrders, receivable, clients, openOpportunities: opportunities.filter(isOpportunityOpen) }; }, [orders, payments, clients, opportunities]);
  ...
  return <div className="space-y-6"><section className="rounded-3xl border border-[#d6c3a0] bg-white p-5 shadow-sm"><p className="text-xs uppercase tracking-[0.25em] text-[#9a6b12] font-black">Gestion</p><h1 className="mt-1 text-2xl font-black text-[#2f2415]">Commercial</h1><p className="mt-1 text-sm text-[#8a7456]">Ventes, clients, opportunités et relances avec les vrais workflows de création et de suivi.</p></section><Tabs active={tab} onChange={setTab} />{tab === 'Résumé' ? <Summary data={data} setTab={setTab} /> : tab === 'Ventes' ? <VentesV3 {...salesProps} /> : tab === 'Clients' ? <ClientsReadable {...clientProps} /> : <Opportunities opportunities={data.openOpportunities} setTab={setTab} />}</div>;
}
