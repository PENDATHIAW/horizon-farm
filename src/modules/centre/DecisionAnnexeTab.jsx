import { BookOpen, Calculator, HelpCircle, Settings2 } from 'lucide-react';
import {
...
export default function DecisionAnnexeTab({
  ...
  const introSections = DECISION_METHODOLOGY_SECTIONS.filter((section) => {
    if (moduleId === 'centre_ia') return !['zootechnical', 'break_even'].includes(section.id);
    if (moduleId === 'objectifs_croissance') return section.id !== 'stock_audit';
    return true;
  });
    ...
    <div className="space-y-5">
      <section className="rounded-3xl border border-[#d6c3a0] bg-[#fffdf8] p-5">
        ...
        <p className="text-sm text-[#8a7456] mt-1">
          Chaque bloc ci-dessous reprend la formule exacte, les noms de variables utilisés dans le code, leurs sources ERP et les valeurs par défaut.
        </p>
      </section>
        ...
        <div className="mt-3 grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-2">
          {snapshot.pilotage.map((row) => (
            <div key={row.key} className="rounded-xl border border-[#eadcc2] bg-[#fffdf8] px-3 py-2">
              ...
              <p className="font-mono text-sm font-black text-[#2f2415]">{row.value}{row.unit !== '—' && row.unit !== 'sujets' ? row.unit : row.unit === 'sujets' ? '' : ''}{row.unit === 'sujets' || row.unit === 'j' || row.unit === '%' ? ` ${row.unit}` : ''}</p>
      ...
      <div className="space-y-4">
        {formulas.map((block) => (
          <section key={block.id} className="rounded-2xl border border-[#d6c3a0] bg-white p-4 shadow-sm">
            <p className="font-black text-[#2f2415] text-sm flex items-center gap-2">
