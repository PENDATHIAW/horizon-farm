  ...
  return (
    <div className="space-y-4">
      {greenpreneursAlerts.length ? (
        <section className="rounded-2xl border border-positive bg-positive-bg p-4 space-y-2">
          <p className="text-xs font-semibold uppercase tracking-normal text-positive">DER/FJ & économie circulaire</p>
          {greenpreneursAlerts.map((alert) => (
            <div key={alert.id} className="flex flex-col gap-2 rounded-xl border border-positive bg-white p-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <b className="text-sm text-earth">{alert.title}</b>
                <p className="text-xs text-slate">{alert.detail}</p>
              </div>
              {alert.navigate ? (
                <button
                  type="button"
                  onClick={() => onNavigate?.(alert.navigate.module, { tab: alert.navigate.tab })}
                  className="shrink-0 rounded-lg bg-earth px-3 py-2 text-meta font-semibold text-white"
                >
                  Ouvrir
                </button>
              ) : null}
