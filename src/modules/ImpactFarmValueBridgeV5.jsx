...
function DomainIntro({ title, visibility, decision, gain }) {
  return (
    <div className="rounded-2xl bg-[#fffdf8] border border-[#eadcc2] p-4 space-y-3">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-3">
        <InfoBlock title="Visibilité" text={visibility} />
        <InfoBlock title="À décider" text={decision} />
        <InfoBlock title="Gains / risques évités" text={gain} />
      </div>
    </div>
  );
}
...
function InfoBlock({ title, text }) {
  return (
    <div className="rounded-xl border border-[#eadcc2] bg-white p-3">
      <p className="text-xs uppercase tracking-widest text-[#8a7456]">{title}</p>
      <p className="text-sm text-[#2f2415] mt-1">{text}</p>
    </div>
  );
}
...
function DomainPanel({ domain, onNavigate }) {
  return (
    <div className="space-y-4">
      <DomainIntro title={domain.title} visibility={domain.visibility} decision={domain.decision} gain={domain.gain} />
...
function buildStats(props = {}) {
  ...
  const stockNoUnit = stocks.filter((row) => !unitOf(row)).length;
  ...
  const paidPayments = payments.filter((row) => !isUnpaid(row)).reduce((sum, row) => sum + amount(row), 0);
  ...
  return {
    ...
    finance: { revenue, expenses, margin, receivables, payments: payments.length, paidPayments, transactions: transactions.length },
