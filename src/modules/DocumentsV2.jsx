...
function DocumentsBridge(props) {
  ...
  const createDocFromTransaction = async (tx, bridgeProps) => {
    ...
    const result = await workflowSubmit(`document-link:${tx.id}`, async () => {
      ...
      const existingDoc = (bridgeProps.rows || []).some((row) => String(row.id) === String(docId));
      const doc = {
        id: docId,
  ...
  return <div className="rounded-2xl border border-[#d6c3a0] bg-white p-5 space-y-4"><div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3"><div><p className="text-xs uppercase tracking-widest text-[#8a7456]">Documents reliés</p><h3 className="font-black text-[#2f2415]">Preuves / factures à contrôler</h3><p className="text-sm text-[#8a7456] mt-1">Les montants importants doivent avoir une facture, un reçu ou une preuve. Une fiche “manquante” ne compte pas comme preuve valide.</p></div><div className="grid grid-cols-3 gap-2 text-sm"><Mini icon={FileText} label="Docs" value={docs.length} /><Mini icon={Receipt} label="Reliés" value={linked} /><Mini icon={AlertTriangle} label="À compléter" value={missing.length} /></div></div>{missing.length ? <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-2">{missing.map((tx) => <div key={tx.id} className="rounded-xl border border-[#eadcc2] bg-[#fffdf8] p-3"><p className="font-bold text-[#2f2415]">{tx.libelle || tx.id}</p><p className="text-xs text-[#8a7456] mt-1">{fmtCurrency(tx.montant)} · {tx.type}</p><button type="button" disabled={isBusy(`document-link:${tx.id}`)} className="mt-3 text-sm font-bold text-emerald-700 disabled:opacity-60" onClick={() => createDocFromTransaction(tx, bridgeProps)}><CheckCircle2 size={14} className="inline" /> {isBusy(`document-link:${tx.id}`) ? 'Création...' : 'Créer fiche preuve'}</button></div>)}</div> : <div className="rounded-xl border border-[#eadcc2] bg-[#fffdf8] p-3 text-sm text-[#8a7456]"><CheckCircle2 size={14} className="inline" /> Les preuves importantes sont bien suivies.</div>}</div>;
}
