...
import { getRhDirectory, RH_ROLES, RH_TEAMS, saveRhDirectory } from '../utils/rhDirectory';
...
const moduleOptions = [
  'dashboard', 'avicole', 'animaux', 'cultures', 'stock', 'sante', 'ventes', 'clients', 'fournisseurs', 'finances', 'documents', 'taches', 'equipements', 'smartfarm', 'rapports',
];
...
export default function RHEquipe({ onRefresh }) {
  ...
  const editPerson = (person) => {
    ...
    setForm({ ...person, modules_text: joinModules(person.modules) });
  };
  const set = (key, value) => setForm((prev) => ({ ...prev, [key]: value }));

  const savePerson = () => {
    if (!String(form.nom || '').trim()) return toast.error('Nom obligatoire');
    const payload = { ...form, modules: parseModules(form.modules_text ?? form.modules), updated_at: new Date().toISOString() };
    delete payload.modules_text;
    ...
    setForm({ ...payload, modules_text: joinModules(payload.modules) });
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {people.map((person) => (
              <div key={person.id} className="rounded-xl border border-[#eadcc2] bg-[#fffdf8] p-4">
                ...
                <p className="text-xs text-[#8a7456] mt-2">Modules : {joinModules(person.modules) || 'Tous / à préciser'}</p>
        ...
        <div className="bg-white border border-[#d6c3a0] rounded-2xl p-5 space-y-3">
          ...
          <label className="block text-sm"><span className="text-[#8a7456]">Rôle</span><select className="mt-1 w-full rounded-lg border border-[#d6c3a0] bg-[#fffdf8] px-3 py-2" value={form.role || ''} onChange={(e) => set('role', e.target.value)}>{RH_ROLES.map((role) => <option key={role} value={role}>{role}</option>)}</select></label>
          <Field label="Fonction" value={form.fonction || ''} onChange={(value) => set('fonction', value)} />
          ...
          <Field label="Modules autorisés" value={form.modules_text ?? joinModules(form.modules)} onChange={(value) => set('modules_text', value)} placeholder={moduleOptions.join(', ')} />
          <Field label="Rémunération" value={form.remuneration || ''} onChange={(value) => set('remuneration', value)} />
      ...
      <div className="bg-white border border-[#d6c3a0] rounded-2xl p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {teams.map((team) => <div key={team.id} className="rounded-xl border border-[#eadcc2] bg-[#fffdf8] p-3"><p className="font-bold text-[#2f2415]">{team.name}</p><p className="text-xs text-[#8a7456]">{team.type}</p><p className="text-xs text-[#8a7456] mt-1">Modules : {(team.modules || []).join(', ')}</p></div>)}
        </div>
      </div>
    </div>
  );
