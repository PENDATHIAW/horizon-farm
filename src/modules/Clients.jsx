...
function buildClientSalesSummary(client, salesOrders = [], payments = []) {
  ...
  const clientPayments = arr(payments).filter(validPayment).filter((payment) => orderIds.has(String(paymentOrderId(payment) || '')) || String(payment.client_id || '') === String(client.id || ''));
  const paidByOrder = clientPayments.reduce((acc, payment) => { const id = String(paymentOrderId(payment) || 'direct'); acc[id] = (acc[id] || 0) + paymentAmount(payment); return acc; }, {});
  const directClientPayments = clientPayments.filter((payment) => !paymentOrderId(payment)).reduce((sum, payment) => sum + paymentAmount(payment), 0);
...
export default function Clients({ rows = [], loading, salesOrders = [], payments = [], onCreate, onUpdate, onDelete, onRefresh, onNavigate }) {
  ...
  const metricsFor = (client) => {
    ...
    return { ...base, total: summary.totalAchete || base.total, averageBasketEstimate: summary.averageBasket || base.averageBasketEstimate, smartStatus: summary.resteAPayer > 0 ? 'a_relancer' : summary.totalAchete > 0 ? 'actif' : (base.smartStatus === 'a_relancer' ? 'prospect' : base.smartStatus), loyaltyScore: Math.max(base.loyaltyScore || 0, summary.totalAchete > 0 ? Math.min(100, 50 + summary.orders.length * 8) : 0) };
  };
  ...
  const submitDelete = async () => { if (!selected) return; try { setSaving(true); await onDelete(selected.id); toast.success('Client supprimé'); setModal(null); } catch (error) { toast.error(error.message || 'Erreur suppression client'); } finally { setSaving(false); } };
  const doExports = () => { const enrichedRows = filteredRows.map((client) => ({ ...client, ...metricsFor(client), ...salesSummaryFor(client) })); exportToCsv({ rows: enrichedRows, fileName: 'clients.csv' }); exportToExcel({ rows: enrichedRows, fileName: 'clients.xlsx', sheetName: 'Clients' }); exportToPdf({ rows: enrichedRows, title: 'Clients', fileName: 'clients.pdf' }); toast.success('Exports clients générés'); };
  ...
  const messageFor = (client) => { const summary = salesSummaryFor(client); if (summary.resteAPayer > 0) return `Bonjour ${clientName(client)}, sauf erreur, il reste ${fmtCurrency(summary.resteAPayer)} à régler sur vos commandes Horizon Farm. Merci.`; return `Bonjour ${clientName(client)}, souhaitez-vous renouveler votre commande Horizon Farm ?`; };
  ...
  const relanceCreance = async (client) => { const summary = salesSummaryFor(client); if (summary.resteAPayer <= 0) return toast.success('Aucune créance à relancer'); if (relanceAlreadyOpen(client)) return toast.success('Relance déjà en suivi pour ce client'); const message = messageFor(client); const key = clientReceivableKey(client); try { await logWhatsApp(client, message, 'relance_creance'); const taskId = makeId('TSK'); await alertesCrud.create?.({ id: makeId('ALT'), title: `Créance client: ${clientName(client)}`, message: `${fmtCurrency(summary.resteAPayer)} à encaisser`, module_source: 'clients', entity_type: 'client', entity_id: client.id, severity: 'warning', status: 'nouvelle', action_recommandee: 'Relancer le client et enregistrer le paiement.', alert_dedupe_key: key, linked_task_id: taskId }); await tachesCrud.create?.({ id: taskId, title: `Relancer ${clientName(client)}`, module_lie: 'clients', related_id: client.id, due_date: today(), priority: 'haute', status: 'a_faire', source_module: 'clients', task_dedupe_key: key, action_key: key, notes: message }); await Promise.allSettled([alertesCrud.refresh?.(), tachesCrud.refresh?.()]); window.open(toWhatsappLink(clientPhone(client), message), '_blank', 'noopener,noreferrer'); toast.success('Relance préparée'); } catch (error) { toast.error(error.message || 'Relance impossible'); } };
