import { AlertTriangle, CheckCircle, Download, Edit, Eye, Package, Plus, RefreshCw, Snowflake, Trash2, X } from 'lucide-react';
...
import { STOCK_CATEGORY_OPTIONS, dlcAlertLevel, requiresDlc, isFreshStockCategory } from '../utils/stockFreshProduct';
...
function stockFields(fournisseurs = []) {
  return [
    ...
    { key: 'categorie', label: 'Catégorie', type: 'select', required: true, options: STOCK_CATEGORY_OPTIONS },
    ...
    { key: 'numero_lot', label: 'N° lot', type: 'text' },
    { key: 'date_peremption', label: 'DLC / date péremption', type: 'date' },
...
function MovementModal({ row, type, defaultQty = 1, saving, onClose, onConfirm }) {
  ...
  const confirm = () => {
    ...
    if (type === 'perte' && !String(motif || '').trim()) return setError('Le motif de perte est obligatoire.');
...
export default function StocksV3({ rows = [], alimentationLogs = [], animaux = [], lots = [], fournisseurs = [], opportunities = [], taches = [], loading, onCreate, onUpdate, onDelete, onRefresh, onCreateAlimentation, onUpdateAlimentation, onDeleteAlimentation, onRefreshAlimentation, onCreateFinanceTransaction, onRefreshFinances, onCreateOpportunity, onUpdateOpportunity, onRefreshOpportunities, onCreateTask, onUpdateTask, onRefreshTasks, onCreateAlert, onRefreshAlertes, ...rest }) {
  ...
  const handleFreeze = async (row) => {
    try {
      ...
      await onRefresh?.();
      toast.success(`${productName(row)} transféré au congélateur`);
    } catch (error) {
  ...
  const openMovement = (row, type) => {
    if (type === 'sortie' && isFood(row)) {
      toast.error('Distribution aliment : utiliser l’onglet Élevage › Alimentation (ou le planificateur ci-dessous).');
      return;
    }
    setMovement({ row, type, defaultQty: type === 'entree' ? (stockMetrics(row).suggestedOrderQty || 1) : 1 });
  };
  const confirmMovement = async ({ qty, motif, withFinance }) => {
    ...
    try {
      ...
      if (type === 'entree') {
        ...
        if (isFood(row)) {
          toast.error('Utiliser Élevage › Alimentation pour distribuer l’aliment.');
        } else {
        ...
        await Promise.allSettled([onRefreshFinances?.()]);
  ...
  const submitCreateAlimentation = async (rawPayload) => { try { setSaving(true); const payload = normalizeAlimentationPayload(rawPayload); const stock = rows.find((r) => r.id === payload.stock_id); const qty = toNumber(payload.quantite); if (stock && qty > toNumber(stock.quantite)) throw new Error(`Stock insuffisant : ${fmtNumber(stock.quantite)} ${stock.unite || ''} disponible(s)`); const normalized = { ...payload, montant_total: toNumber(payload.montant_total) || (stock ? qty * unitPrice(stock) : 0), unite: payload.unite || stock?.unite || 'kg', duree_jours: payload.duree_jours || 1, source_module: 'stock', source_record_id: payload.stock_id || '' }; if (stock && qty > 0) { const preview = prepareFeedingWorkflow(normalized, { events: businessEventsCrud.rows }); await commitFeedingWorkflow(preview, { onCreateAlimentation, onUpdateStockMovement: () => moveStock(stock, 'sortie', qty, 'Utilisation alimentation'), onCreateBusinessEvent: rest.onCreateBusinessEvent || businessEventsCrud.create }); await businessEventsCrud.refresh?.(); } else { await onCreateAlimentation?.(normalized); } await onRefreshAlimentation?.(); await onRefresh?.(); toast.success('Utilisation alimentation enregistrée et liée'); setModal(null); } catch (error) { toast.error(error.message || 'Erreur alimentation'); } finally { setSaving(false); } };

  const columns = [
    { key: 'produit', label: 'Produit', sortable: true, render: (p) => <span className="text-[#2f2415] font-semibold">{p.produit}</span> }, { key: 'categorie', label: 'Catégorie', sortable: true, render: (p) => STOCK_CATEGORY_OPTIONS.find((c) => c.value === p.categorie)?.label || p.categorie || '—' }, { key: 'activite_liee', label: 'Activité liée', sortable: true, render: (p) => p.activite_liee || '—' }, { key: 'emplacement', label: 'Emplacement', render: (p) => p.emplacement || '—' }, { key: 'numero_lot', label: 'Lot', render: (p) => p.numero_lot || '—' }, { key: 'date_peremption', label: 'DLC', render: (p) => { const level = dlcAlertLevel(p); const cls = level === 'black' ? 'text-[#2f2415] font-black' : level === 'red' ? 'text-red-600 font-bold' : level === 'orange' ? 'text-amber-600 font-bold' : 'text-[#8a7456]'; return <span className={cls}>{p.date_peremption || (requiresDlc(p) ? '—' : 'n/a')}</span>; } }, { key: 'quantite', label: 'Quantité', sortable: true, render: (p) => <span className={stockMetrics(p).critical ? 'text-red-500 font-bold' : 'text-emerald-500 font-bold'}>{fmtNumber(stockMetrics(p).quantity)}</span> }, { key: 'unite', label: 'Unité', sortable: true }, { key: 'seuil', label: 'Seuil min.', sortable: true, render: (p) => fmtNumber(p.seuil) }, { key: 'prixUnit', label: 'Prix unit.', sortable: true, render: (p) => fmtCurrency(unitPrice(p)) }, { key: 'valeur', label: 'Valeur estimée', sortable: true, render: (p) => <span className="text-[#2f2415] font-semibold">{fmtCurrency(valueOf(p))}</span> }, { key: 'statut', label: 'Statut', sortable: true, render: (p) => <Badge status={statusOf(p)} /> }, { key: 'suggestion', label: 'À commander', render: (p) => stockMetrics(p).suggestedOrderQty > 0 ? <span className="text-amber-600 font-semibold">{fmtNumber(stockMetrics(p).suggestedOrderQty)} {p.unite || ''}</span> : <span className="text-emerald-600">OK</span> }, { key: 'actions', label: 'Actions', render: (p) => <div className="flex gap-1"><ActionIconButton icon={Eye} title="Voir" color="sky" onClick={() => { setSelected(p); setModal('details'); }} /><ActionIconButton icon={Edit} title="Modifier" color="amber" onClick={() => { setSelected(p); setModal('edit'); }} /><ActionIconButton icon={Plus} title="Réceptionner" color="emerald" onClick={() => openMovement(p, 'entree')} /><ActionIconButton icon={Package} title="Utiliser" color="amber" onClick={() => openMovement(p, 'sortie')} />{isFreshStockCategory(p.categorie) ? <ActionIconButton icon={Snowflake} title="Congeler" color="sky" onClick={() => handleFreeze(p)} /> : null} <ActionIconButton icon={AlertTriangle} title="Perte" color="red" onClick={() => openMovement(p, 'perte')} /><ActionIconButton icon={Trash2} title="Supprimer" color="red" onClick={() => { setSelected(p); setModal('delete'); }} /></div> },
  ];
  const alimColumns = [
    { key: 'date', label: 'Date', sortable: true }, { key: 'stock_id', label: 'Produit utilisé', render: (r) => rows.find((s) => s.id === r.stock_id)?.produit || r.stock_id || 'manuel' }, { key: 'categorie', label: 'Catégorie animale', sortable: true }, { key: 'cible_id', label: 'Concerné', render: (r) => r.cible_id || r.type_cible || '—' }, { key: 'quantite', label: 'Quantité', render: (r) => `${fmtNumber(r.quantite)} ${r.unite || ''}`.trim() }, { key: 'montant_total', label: 'Coût lié', render: (r) => fmtCurrency(r.montant_total) }, { key: 'duree_jours', label: 'Durée', render: (r) => `${r.duree_jours || 1} j` }, { key: 'actions', label: 'Actions', render: (r) => <div className="flex gap-1"><ActionIconButton icon={Eye} title="Voir" color="sky" onClick={() => { setSelectedAlim(r); setModal('detailsAlim'); }} /><ActionIconButton icon={Edit} title="Modifier" color="amber" onClick={() => { setSelectedAlim(r); setModal('editAlim'); }} /><ActionIconButton icon={Trash2} title="Supprimer" color="red" onClick={() => { setSelectedAlim(r); setModal('deleteAlim'); }} /></div> },
  ];
