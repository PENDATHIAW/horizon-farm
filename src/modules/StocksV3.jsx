...
export default function StocksV3({ rows = [], alimentationLogs = [], animaux = [], lots = [], fournisseurs = [], opportunities = [], taches = [], loading, onCreate, onUpdate, onDelete, onRefresh, onCreateAlimentation, onUpdateAlimentation, onDeleteAlimentation, onRefreshAlimentation, onCreateFinanceTransaction, onRefreshFinances, onCreateOpportunity, onUpdateOpportunity, onRefreshOpportunities, onCreateTask, onUpdateTask, onRefreshTasks, onCreateAlert, onRefreshAlertes, onOpenPurchaseReception, onUpdateSupplier, ...rest }) {
  ...
  const submitCreateAlimentation = async (rawPayload) => {
    const payloadPreview = normalizeAlimentationPayload(rawPayload);
    const feedKey = `feeding-stock:${payloadPreview.stock_id || 'new'}:${today()}`;
    await workflowSubmit(feedKey, async () => {
      ...
      if (stock && qty > toNumber(stock.quantite)) throw new Error(`Stock insuffisant : ${fmtNumber(stock.quantite)} ${stock.unite || ''} disponible(s)`);
      ...
      if (stock && qty > 0) {
        ...
        await commitFeedingWorkflow(preview, { onCreateAlimentation, onUpdateStockMovement: () => moveStock(stock, 'sortie', qty, 'Utilisation alimentation'), onCreateBusinessEvent: rest.onCreateBusinessEvent || businessEventsCrud.create });
        await businessEventsCrud.refresh?.();
      } else {
