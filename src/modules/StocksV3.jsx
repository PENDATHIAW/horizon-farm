...
function MovementModal({ row, type, defaultQty = 1, unitPriceEstimate = 0, saving, onClose, onConfirm }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-earth/30 p-3"><div className="w-full max-w-xl rounded-3xl border border-line bg-white shadow-float"><div className="flex items-start justify-between border-b border-line p-6"><div><p className="text-xs uppercase tracking-normal text-slate">Mouvement stock</p><h2 className="text-xl font-semibold text-earth">{title}</h2><p className="mt-1 text-sm text-slate">{productName(row)} · Disponible {fmtNumber(current)} {row?.unite || ''}</p></div><button type="button" onClick={onClose} aria-label="Fermer"><X size={18} /></button></div><div className="p-6 space-y-4">{error ? <div className="rounded-xl border border-urgent bg-urgent-bg p-3 text-sm text-urgent">{error}</div> : null}<div className="grid grid-cols-1 md:grid-cols-2 gap-3"><label className="space-y-1"><span className="text-xs font-semibold text-slate">Quantité</span><input type="number" value={qty} onChange={(e) => setQty(e.target.value)} className="w-full min-h-[44px] rounded-xl border border-line bg-card px-3 py-2 text-sm" /></label><label className="space-y-1"><span className="text-xs font-semibold text-slate">Motif</span><input value={motif} onChange={(e) => setMotif(e.target.value)} className="w-full min-h-[44px] rounded-xl border border-line bg-card px-3 py-2 text-sm" /></label></div>{type === 'entree' ? <label className="flex items-center gap-2 rounded-xl border border-line bg-card p-3 text-sm text-earth"><input type="checkbox" checked={withFinance} onChange={(e) => setWithFinance(e.target.checked)} /> Préparer aussi la dépense et la preuve/facture si le prix est renseigné</label> : null}<div className="rounded-2xl border border-positive bg-positive-bg p-4"><p className="text-xs uppercase tracking-normal font-semibold text-positive">Résumé avant validation</p><p className="mt-1 text-lg font-semibold text-positive">Nouvelle quantité : {fmtNumber(next)} {row?.unite || ''}</p><p className="text-sm text-positive">Valeur estimée du mouvement : {fmtCurrency(amount)}</p></div><div className="grid grid-cols-1 md:grid-cols-2 gap-2">{impacts.map((impact) => <div key={impact} className="rounded-xl border border-line bg-card p-3 text-sm text-slate"><CheckCircle size={14} className="inline text-positive" /> {impact}</div>)}</div><div className="flex justify-end gap-2"><button type="button" onClick={onClose} className="min-h-[44px] rounded-xl border border-line px-4 py-2 text-sm font-semibold text-slate">Annuler</button><button type="button" onClick={confirm} disabled={saving} className="min-h-[44px] rounded-xl bg-earth px-6 py-2 text-sm font-semibold text-white disabled:opacity-60">{saving ? 'Validation...' : 'Valider le mouvement'}</button></div></div></div></div>;
}
