  ...
  return (
    <div className="space-y-6">
      <SectionHeader
        ...
        sub="Suivre le cap commercial : objectif mensuel, CA réalisé, encaissement, reste à vendre et activités à pousser. Les décisions détaillées sont réservées au Centre décisionnel."
        actions={
      ...
      <div className="rounded-3xl border border-[#eadcc2] bg-[#fffdf8] p-5 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[#8a7456] mt-1">Si tu veux savoir quoi faire concrètement — investir, relancer, acheter, vendre ou sécuriser une deadline — ouvre le Centre décisionnel.</p>
        </div>
        <button type="button" onClick={() => onNavigate?.('centre_ia')} className="rounded-xl bg-[#2f2415] px-4 py-2 text-sm font-black text-white hover:bg-[#3d2f1d]">Ouvrir Centre décisionnel</button>
      </div>
    </div>
  );
}
