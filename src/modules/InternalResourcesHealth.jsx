...
export default function InternalResourcesHealth({ people, equipements = [], transactions = [], documents = [], tasks = [], onNavigate }) {
  ...
  return <section className="rounded-3xl border border-[#d6c3a0] bg-white p-5 shadow-sm space-y-4">
    ...
    <div className="flex flex-wrap justify-end gap-2"><Btn small variant="outline" onClick={() => onNavigate?.('equipe')}>Ouvrir équipe</Btn><Btn small variant="outline" onClick={() => onNavigate?.('equipements')}>Ouvrir équipements</Btn><Btn small variant="outline" onClick={() => onNavigate?.('documents')}>Ouvrir documents</Btn><Btn small variant="outline" onClick={() => onNavigate?.('finances')}>Ouvrir finances</Btn></div>
  </section>;
}
