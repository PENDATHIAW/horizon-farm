  ...
  if (!hasData) {
    return (
      <div className="rounded-3xl border border-line bg-card p-8 text-center">
        <Paperclip size={28} className="mx-auto text-horizon-dark" />
        <p className="mt-3 text-lg font-semibold text-earth">Aucune annexe commerciale ajoutée pour le moment.</p>
        <p className="mt-2 text-sm text-slate">Factures, bons de livraison, devis et preuves client apparaîtront ici après vos ventes.</p>
        {onNavigate ? (
          <button type="button" onClick={() => onNavigate('documents_rapports', { tab: 'Preuves' })} className="mt-4 rounded-xl bg-earth px-4 py-2 text-sm font-semibold text-white">
    ...
    <div className="space-y-4">
      ...
      {invoiceRows.length ? (
        <section className="rounded-2xl border border-line bg-white p-4">
          <p className="text-sm font-semibold text-earth flex items-center gap-2"><FileText size={15} /> Factures ({invoiceRows.length})</p>
          <div className="mt-3 divide-y divide-line/60">
            {invoiceRows.slice(0, 8).map((inv) => (
              <div key={inv.id} className="py-2 flex justify-between gap-2 text-sm">
                <span className="font-semibold text-earth">{inv.numero_facture || inv.id}</span>
                <span className="text-slate">{fmtCurrency(inv.montant_total ?? inv.amount)}</span>
              </div>
            ))}
          </div>
        </section>
      ...
      {deliveryRows.length ? (
        <section className="rounded-2xl border border-line bg-white p-4">
          <p className="text-sm font-semibold text-earth flex items-center gap-2"><Truck size={15} /> Bons de livraison ({deliveryRows.length})</p>
          <div className="mt-3 divide-y divide-line/60">
            {deliveryRows.slice(0, 8).map((d) => (
              <div key={d.id} className="py-2 flex justify-between gap-2 text-sm">
                <span className="font-semibold text-earth">{d.id} · {d.statut || d.status}</span>
                <span className="text-slate">{d.date_livraison || d.date || ''}</span>
              </div>
            ))}
          </div>
        </section>
      ...
      {commercialDocs.length ? (
        <section className="rounded-2xl border border-line bg-white p-4">
          <p className="text-sm font-semibold text-earth">Documents ({commercialDocs.length})</p>
          <div className="mt-3 space-y-2">
            {commercialDocs.slice(0, 10).map((doc) => (
              <div key={doc.id} className="rounded-xl border border-line bg-card px-3 py-2 text-sm">
                <p className="font-semibold text-earth">{doc.title || doc.nom || doc.id}</p>
                <p className="text-xs text-slate">{doc.document_category || doc.type || 'document'} · {doc.entity_id || doc.related_id || ''}</p>
              </div>
            ))}
          </div>
        </section>
      ...
      {quoteRows.length ? (
        <section className="rounded-2xl border border-line bg-neutral-bg p-4">
          <p className="text-sm font-semibold text-neutral">Devis archivés ({quoteRows.length})</p>
          <div className="mt-2 space-y-1 text-sm text-neutral">
