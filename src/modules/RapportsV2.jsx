...
function HorizonFinancingFileCard({ draft, data, onCreateDocument, onRefreshDocuments, onCreateBusinessEvent, onRefreshBusinessEvents, onClose }) {
  ...
  return <section className="rounded-3xl border border-positive bg-positive-bg p-6 shadow-card space-y-4">
    <div className="flex items-start justify-between gap-3"><div><p className="text-xs uppercase tracking-normal text-positive font-semibold flex items-center gap-2"><FileText size={15} /> Fiche préparée par Hey Horizon</p><h3 className="mt-1 text-xl font-semibold text-earth">Dossier Banque / Partenaire</h3><p className="mt-1 text-sm text-positive">Choisis le financeur et le BP. Le dossier est enrichi avec les données ERP disponibles.</p></div><button type="button" onClick={onClose} className="rounded-full border border-positive bg-white p-2 text-positive"><X size={16} /></button></div>
    <div className="grid grid-cols-1 md:grid-cols-3 gap-3"><label className="space-y-1"><span className="text-xs font-semibold text-positive">Financeur</span><select value={financier} onChange={(e) => setFinancier(e.target.value)} className="w-full min-h-[44px] rounded-xl border border-positive bg-white px-3 py-2 text-sm">{FINANCIERS.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label className="space-y-1"><span className="text-xs font-semibold text-positive">Business Plan</span><select value={bpId} onChange={(e) => setBpId(e.target.value)} className="w-full min-h-[44px] rounded-xl border border-positive bg-white px-3 py-2 text-sm"><option value="">Aucun / dossier général</option>{businessPlans.map((bp) => <option key={bp.id} value={bp.id}>{bpLabel(bp)}</option>)}</select></label><label className="space-y-1"><span className="text-xs font-semibold text-positive">Montant sollicité</span><input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} className="w-full min-h-[44px] rounded-xl border border-positive bg-white px-3 py-2 text-sm" /></label><label className="space-y-1 md:col-span-3"><span className="text-xs font-semibold text-positive">Objet du financement</span><input value={purpose} onChange={(e) => setPurpose(e.target.value)} className="w-full min-h-[44px] rounded-xl border border-positive bg-white px-3 py-2 text-sm" /></label></div>
    <div className="grid grid-cols-1 md:grid-cols-4 gap-2 text-sm text-positive"><label className="rounded-xl bg-white border border-positive p-3"><input type="checkbox" checked={includeCashflow} onChange={(e) => setIncludeCashflow(e.target.checked)} /> Trésorerie</label><label className="rounded-xl bg-white border border-positive p-3"><input type="checkbox" checked={includeRisk} onChange={(e) => setIncludeRisk(e.target.checked)} /> Risques</label><label className="rounded-xl bg-white border border-positive p-3"><input type="checkbox" checked={includeImpact} onChange={(e) => setIncludeImpact(e.target.checked)} /> Impact</label><label className="rounded-xl bg-white border border-positive p-3"><input type="checkbox" checked={includeEvidence} onChange={(e) => setIncludeEvidence(e.target.checked)} /> Preuves</label></div>
    <div className="rounded-xl border border-positive bg-white p-3 text-sm text-positive"><CheckCircle2 size={14} className="inline" /> Le document créé servira de base au PDF financeur : résumé, chiffres ERP, risques, impact et annexes.</div>
    <div className="flex justify-end"><button type="button" onClick={submit} disabled={saving} className="rounded-xl bg-earth px-6 py-2 text-sm font-semibold text-white disabled:opacity-60">{saving ? 'Préparation...' : 'Préparer le dossier'}</button></div>
  </section>;
}
