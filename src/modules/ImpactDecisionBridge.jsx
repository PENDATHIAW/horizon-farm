...
export default function ImpactDecisionBridge(props) {
  ...
  return <div className="rounded-3xl border border-[#d6c3a0] bg-white p-5 shadow-sm space-y-4">
    <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4"><div><p className="text-xs uppercase tracking-[0.25em] text-[#9a6b12] font-black flex items-center gap-2"><BrainCircuit size={16} aria-hidden="true" /> Impact du Centre décisionnel</p><h2 className="mt-1 text-xl font-black text-[#2f2415]">Ce que les analyses apportent concrètement à la ferme</h2><p className="mt-1 text-sm text-[#8a7456]">Cette zone relie les recommandations, les actions exécutées, les pertes, les alertes terrain et les résultats commerciaux.</p></div><Btn small onClick={() => props.onNavigate?.('centre_ia')}>Voir Centre décisionnel</Btn></div>
    ...
    <button type="button" onClick={() => setShowDetails((value) => !value)} aria-expanded={showDetails} className="min-h-[44px] rounded-2xl border border-[#eadcc2] bg-[#fffdf8] px-4 py-2.5 text-sm font-black text-[#2f2415] hover:bg-white transition-colors duration-200">{showDetails ? 'Masquer le détail des analyses' : 'Afficher le détail des analyses'}</button>
