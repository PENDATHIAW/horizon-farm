...
const source = (row = {}) => row.module_lie || row.source_module || row.module_source || row.module || '—';
  ...
  return <section className="rounded-3xl border border-[#d6c3a0] bg-white p-5 shadow-sm space-y-4">
    <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
      <div>
        ...
        <p className="mt-1 text-sm text-[#8a7456]">Repère les alertes critiques sans tâche, doublons, retards et tâches sans source claire.</p>
      </div>
      <div className={`${audit.issueCount ? 'bg-amber-50 text-amber-800 border-amber-200' : 'bg-emerald-50 text-emerald-800 border-emerald-200'} rounded-2xl border px-4 py-3 text-sm font-bold`}>
        {audit.issueCount ? `${audit.issueCount} point(s) à vérifier` : 'Tâches & alertes cohérentes'}
      </div>
    </div>

    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      <Card icon={BellRing} label="Alertes sans tâche" value={audit.criticalWithoutTask} danger={audit.criticalWithoutTask > 0} />
      <Card icon={ClipboardList} label="Doublons tâches" value={audit.duplicateTasks} danger={audit.duplicateTasks > 0} />
      <Card icon={Timer} label="Tâches en retard" value={audit.lateTasks} danger={audit.lateTasks > 0} />
      <Card icon={Link2} label="Sources manquantes" value={audit.orphanTasks} danger={audit.orphanTasks > 0} />
    </div>

    {risky.length ? <div className="overflow-x-auto rounded-2xl border border-[#eadcc2]">
      <table className="min-w-full text-sm">
        <thead><tr className="border-b border-[#eadcc2] bg-[#fffdf8] text-left text-xs uppercase text-[#8a7456]"><th className="py-2 px-3">Élément</th><th className="py-2 px-3">Source</th><th className="py-2 px-3">Problème</th><th className="py-2 px-3">Niveau</th></tr></thead>
        <tbody>{risky.map((issue, i) => {
          const item = issue.task || issue.alert || issue;
          return <tr key={`${issue.id}-${i}`} className="border-b border-[#f0e5d0]"><td className="py-3 px-3 font-bold text-[#2f2415]">{label(item)}<p className="text-xs text-[#8a7456]">{item.id || issue.id}</p></td><td className="py-3 px-3">{source(item)}</td><td className="py-3 px-3"><span className="rounded-full bg-amber-50 px-2 py-1 text-xs font-bold text-amber-700">{issue.type}</span></td><td className={`py-3 px-3 font-bold ${issue.severity === 'danger' ? 'text-red-600' : 'text-amber-700'}`}>{issue.severity === 'danger' ? 'Critique' : 'À vérifier'}</td></tr>;
        })}</tbody>
      </table>
    </div> : <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-800"><CheckCircle2 size={16} className="inline" /> Aucune incohérence critique détectée sur les tâches et alertes.</div>}
  </section>;
}
