...
export default function TaskAlertQualityControl({ tasks = [], alerts = [] }) {
  ...
  return <section className="rounded-3xl border border-line bg-white p-6 shadow-card space-y-4">
    <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
      <div>
        <p className="flex items-center gap-2 text-lg font-semibold text-earth"><ClipboardList size={20} /> Contrôle tâches & alertes</p>
        <p className="mt-1 text-sm text-slate">Vérifie les alertes sans tâche, les doublons, les retards et les actions à préciser.</p>
      </div>
      <div className={`${audit.issueCount ? 'bg-vigilance-bg text-horizon-dark border-vigilance' : 'bg-positive-bg text-positive border-positive'} rounded-2xl border px-4 py-3 text-sm font-semibold`}>
    ...
    {risky.length ? <div className="overflow-x-auto rounded-2xl border border-line">
      <table className="min-w-full text-sm">
        <thead><tr className="border-b border-line bg-card text-left text-xs uppercase text-slate"><th className="py-2 px-3">Élément</th><th className="py-2 px-3">Espace</th><th className="py-2 px-3">À regarder</th><th className="py-2 px-3">Niveau</th></tr></thead>
        <tbody>{risky.map((issue, i) => {
          const item = issue.task || issue.alert || issue;
          return <tr key={`${issue.id}-${i}`} className="border-b border-line"><td className="py-3 px-3 font-semibold text-earth">{label(item)}<p className="text-xs text-slate">{item.id || issue.id}</p></td><td className="py-3 px-3">{source(item)}</td><td className="py-3 px-3"><span className="rounded-full bg-vigilance-bg px-2 py-1 text-xs font-semibold text-horizon-dark">{issueLabel(issue.type)}</span></td><td className={`py-3 px-3 font-semibold ${issue.severity === 'danger' ? 'text-urgent' : 'text-horizon-dark'}`}>{issue.severity === 'danger' ? 'Urgent' : 'À vérifier'}</td></tr>;
        })}</tbody>
      </table>
    </div> : <div className="rounded-2xl border border-positive bg-positive-bg p-3 text-sm text-positive"><CheckCircle2 size={16} className="inline" /> Les tâches et alertes semblent bien suivies.</div>}
  </section>;
}
