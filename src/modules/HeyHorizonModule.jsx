...
const QUICK_COMMANDS = [
  ...
  { title: 'Stock bas', text: 'Lister les produits sous seuil et proposer quoi acheter', target: 'Achats & Stock' },
  ...
  { title: 'Priorités semaine', text: 'Préparer mes priorités de cette semaine', target: 'Pilotage & Stratégie' },
  { title: 'Relance client', text: 'Quels clients dois-je relancer aujourd’hui ?', target: 'Commercial' },
];
const MODULES = ['Commercial', 'Élevage', 'Achats & Stock', 'Finance & Pilotage', 'Activité & Suivi', 'Documents & Rapports', 'Pilotage & Stratégie'];
...
function inferIntent(text) {
  ...
  if (/vente|vendu|client|payer|payé|livrer|facture/.test(query)) return { module: 'Commercial', action: 'Préparer une vente complète', route: 'commercial' };
  if (/stock|acheter|achat|fournisseur|aliment|seuil|rupture/.test(query)) return { module: 'Achats & Stock', action: 'Préparer un mouvement stock ou un achat', route: 'achats_stock' };
  if (/animal|bovin|ovin|caprin|pondeuse|chair|vaccin|malade|mortalité|santé/.test(query)) return { module: 'Élevage', action: 'Préparer une action élevage', route: 'elevage' };
  if (/dépense|recette|finance|paiement|justificatif|preuve|compta|invest/.test(query)) return { module: 'Finance & Pilotage', action: 'Préparer une action financière', route: 'finance_pilotage' };
  if (/tâche|alerte|retard|suivi|historique|trace/.test(query)) return { module: 'Activité & Suivi', action: 'Préparer une action de suivi', route: 'activite_suivi' };
  if (/rapport|document|facture|reçu|export|preuve/.test(query)) return { module: 'Documents & Rapports', action: 'Préparer un document ou un rapport', route: 'documents_rapports' };
  return { module: 'Pilotage & Stratégie', action: 'Analyser et proposer une décision', route: 'pilotage_strategie' };
}
...
export default function HeyHorizonModule({ dataMap = {}, onOpenAssistant, onNavigate }) {
  ...
  const runDraft = (text = command) => {
    ...
    const impacts = [];
    if (intent.module === 'Commercial') impacts.push('Vente', 'Client', 'Paiement', 'Livraison', 'Stock');
    if (intent.module === 'Achats & Stock') impacts.push('Stock', 'Achat', 'Fournisseur', 'Finance');
    if (intent.module === 'Élevage') impacts.push('Fiche animal/lot', 'Santé', 'Tâche', 'Historique');
    if (intent.module === 'Finance & Pilotage') impacts.push('Trésorerie', 'Comptabilité', 'Preuve', 'Document');
    if (intent.module === 'Activité & Suivi') impacts.push('Tâche', 'Alerte', 'Historique');
    if (intent.module === 'Documents & Rapports') impacts.push('Document', 'Preuve', 'Rapport', 'Export');
    setCommand(text);
    setDraft({ text, ...intent, impacts });
  };
  return <div className="space-y-6"><section className="rounded-3xl border border-[#d6c3a0] bg-[#fffdf8] p-6 shadow-sm overflow-hidden relative"><div className="absolute -right-10 -top-10 h-36 w-36 rounded-full bg-emerald-200/50 blur-2xl" /><div className="relative flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><p className="flex items-center gap-2 text-xs uppercase tracking-[0.25em] text-[#9a6b12] font-black"><Bot size={16} /> Assistant ERP</p><h1 className="mt-2 text-3xl font-black text-[#2f2415]">Hey Horizon</h1><p className="mt-2 max-w-3xl text-sm leading-relaxed text-[#7d6a4a]">Un espace pour saisir vite, préparer une action, voir l’impact et ouvrir le bon module fusionné. L’assistant ne doit plus être seulement informatif.</p></div><button type="button" onClick={onOpenAssistant} className="rounded-2xl bg-[#2f2415] px-5 py-3 text-sm font-black text-white shadow-lg"><Mic size={17} className="inline mr-2" /> Ouvrir le panneau</button></div></section>
    ...
    {draft ? <Section icon={Wand2} title="Prévisualisation de l’action" action={<button type="button" onClick={() => onNavigate?.(draft.route)} className="rounded-xl bg-[#2f2415] px-3 py-2 text-xs font-black text-white">Ouvrir {draft.module}</button>}><div className="grid grid-cols-1 gap-3 lg:grid-cols-3"><div className="rounded-2xl border border-[#eadcc2] bg-[#fffdf8] p-4"><p className="text-xs text-[#8a7456]">Module</p><p className="mt-1 font-black text-[#2f2415]">{draft.module}</p></div><div className="rounded-2xl border border-[#eadcc2] bg-[#fffdf8] p-4"><p className="text-xs text-[#8a7456]">Action proposée</p><p className="mt-1 font-black text-[#2f2415]">{draft.action}</p></div><div className="rounded-2xl border border-[#eadcc2] bg-[#fffdf8] p-4"><p className="text-xs text-[#8a7456]">Impact</p><p className="mt-1 font-black text-[#2f2415]">{draft.impacts.join(' · ') || 'Analyse'}</p></div></div><div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">Avant validation réelle, l’ERP doit afficher les champs à confirmer puis créer/modifier les fiches concernées.</div></Section> : null}
