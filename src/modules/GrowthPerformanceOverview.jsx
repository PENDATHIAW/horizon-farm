...
export default function GrowthPerformanceOverview({ mode = 'avicole', rows = [], alimentationLogs = [], productionLogs = [], vaccins = [], businessEvents = [] }) {
  ...
  const avgProgress = items.length ? items.reduce((sum, item) => sum + toNumber(item.progress), 0) / items.length : 0;
  ...
  return <section className="rounded-3xl border border-[#d6c3a0] bg-white p-5 shadow-sm space-y-4">
    <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
      ...
      <div className={`rounded-2xl px-4 py-3 ${high ? 'bg-red-50 text-red-700' : medium ? 'bg-amber-50 text-amber-700' : 'bg-emerald-50 text-emerald-700'}`}>
        <p className="text-xs font-bold">Lecture rapide</p>
        <p className="text-sm font-black">{high ? `${high} priorité(s)` : medium ? `${medium} point(s) à surveiller` : 'Trajectoire maîtrisée'}</p>
      </div>
    </div>
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      <Card icon={Scale} label="Sujets / cultures" value={fmtNumber(items.length)} />
      <Card icon={TrendingUp} label="Progression moyenne" value={`${avgProgress.toFixed(1)}%`} hint="objectif connu uniquement" danger={avgProgress > 0 && avgProgress < 75} />
      <Card icon={Leaf} label="Coût total suivi" value={fmtCurrency(cost)} />
      <Card icon={AlertTriangle} label="Risques" value={`${high + medium}`} hint={`${high} élevé(s), ${medium} à surveiller`} danger={high + medium > 0} />
    </div>
    ...
    <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-800 flex items-start gap-2"><CheckCircle2 size={16} className="mt-0.5" /> Ce bloc rapproche croissance, objectifs et coûts pour repérer plus vite les sujets ou cultures qui consomment sans progresser suffisamment.</div>
  </section>;
}
