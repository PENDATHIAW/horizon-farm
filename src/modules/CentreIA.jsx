...
export default function CentreIA({ lots = [], productionLogs = [], alimentationLogs = [], stocks = [], marketPrices = [], marketCalendarEvents = [], salesOrders = [], payments = [], transactions = [], smartfarmEvents = [], sensors = [], cameras = [], meteo = null, dataMap = {}, onNavigate, onCreateTask, onRefreshTasks, onCreateBusinessEvent, onRefreshBusinessEvents }) {
  const centreDataMap = useMemo(() => ({ ...dataMap, lots, avicole: lots, productionLogs, production_oeufs_logs: productionLogs, alimentationLogs, alimentation_logs: alimentationLogs, stock: stocks, stocks, salesOrders, sales_orders: salesOrders, payments, transactions, finances: transactions, sensors, sensor_devices: sensors, cameras, camera_devices: cameras, smartfarmEvents, smartfarm_events: smartfarmEvents, market_prices: marketPrices, market_calendar_events: marketCalendarEvents, meteo }), [dataMap, lots, productionLogs, alimentationLogs, stocks, salesOrders, payments, transactions, sensors, cameras, smartfarmEvents, marketPrices, marketCalendarEvents, meteo]);
  ...
  return <div className="space-y-6"><SectionHeader title="Centre décisionnel" actions={<div className="flex flex-wrap gap-2"><Btn variant="outline" small onClick={() => onNavigate?.('investissements')}>Plan financier</Btn><Btn variant="outline" small onClick={() => onNavigate?.('objectifs_croissance')}>Objectifs</Btn><Btn variant="outline" small onClick={() => onNavigate?.('ventes')}>Ventes</Btn></div>} />
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4"><KpiCard icon={BrainCircuit} label="Score ferme" value={`${score}/100`} sub={score >= 75 ? 'Bon' : score >= 50 ? 'Moyen' : 'Risque'} color={score >= 75 ? 'bg-emerald-500/20 text-emerald-500' : score >= 50 ? 'bg-amber-500/20 text-amber-500' : 'bg-red-500/20 text-red-500'} /><KpiCard icon={TrendingUp} label="Objectif" value={`${goal.attainment ?? 0}%`} sub="Mensuel" color={(goal.attainment ?? 0) >= 90 ? 'bg-emerald-500/20 text-emerald-500' : 'bg-amber-500/20 text-amber-500'} onClick={() => onNavigate?.('objectifs_croissance')} /><KpiCard icon={LineChart} label="À encaisser" value={fmtCurrency(commercial.creances)} sub="Commercial" color={commercial.creances > 0 ? 'bg-amber-500/20 text-amber-500' : 'bg-emerald-500/20 text-emerald-500'} onClick={() => onNavigate?.('ventes')} /><KpiCard icon={ShieldAlert} label="Alertes" value={riskCount} sub="Prioritaires" color={riskCount > 0 ? 'bg-red-500/20 text-red-500' : 'bg-emerald-500/20 text-emerald-500'} /></div>
    <div className="rounded-3xl border border-[#d6c3a0] bg-[#2f2415] text-white p-5 shadow-sm space-y-4"><div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between"><div><p className="text-sm font-black text-[#f8e8b6] flex items-center gap-2"><Zap size={16} aria-hidden="true" /> Décisions</p><h2 className="text-xl font-black mt-1">Actions prioritaires</h2></div><Btn small variant="outline" onClick={() => onNavigate?.('objectifs_croissance')}>Objectifs</Btn></div><div className="grid grid-cols-1 xl:grid-cols-3 gap-3">{mainRecommendations.map((item) => <DecisionRecommendationCardCompact key={item.id} item={item} dataMap={centreDataMap} onNavigate={onNavigate} onCreateTask={onCreateTask} onRefreshTasks={onRefreshTasks} onCreateBusinessEvent={onCreateBusinessEvent} onRefreshBusinessEvents={onRefreshBusinessEvents} />)}{!mainRecommendations.length ? <div className="col-span-full rounded-2xl border border-white/15 bg-white/10 p-4 text-sm text-white/85">Aucune action prioritaire.</div> : null}</div></div>
    <DecisionSalesOpportunitiesPanel lots={lots} animaux={centreDataMap.animaux || []} productionLogs={productionLogs} cultures={centreDataMap.cultures || []} onNavigate={onNavigate} />
    ...
    <section className="rounded-3xl border border-[#d6c3a0] bg-white p-5 shadow-sm space-y-4"><div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between"><div><p className="font-black text-[#2f2415] flex items-center gap-2"><ShieldAlert size={18} className="text-red-500" aria-hidden="true" /> Alertes importantes</p></div><Btn small variant="outline" onClick={() => onNavigate?.('alertes')}>Alertes</Btn></div><div className="grid grid-cols-1 lg:grid-cols-3 gap-3">{mainRisks.map((insight) => <AlertCard key={insight.id} title={insight.title} text={insight.recommendation} tone={insight.priority === 'haute' || insight.severity === 'critique' ? 'red' : 'amber'} action="Ouvrir" onClick={() => openDraftFromInsight(insight)} />)}{!mainRisks.length ? <AlertCard title="Aucune alerte" text="Tout est à jour." tone="emerald" /> : null}</div></section>
    {automationRows.length ? <section className="rounded-3xl border border-[#d6c3a0] bg-white p-5 shadow-sm space-y-4"><div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between"><div><p className="font-black text-[#2f2415] flex items-center gap-2"><Sparkles size={18} className="text-purple-500" aria-hidden="true" /> Actions préparées</p></div><span className="rounded-full bg-purple-50 border border-purple-200 px-3 py-1 text-xs font-black text-purple-700">{plural(automations.total || 0, 'action', 'actions')}</span></div><div className="grid grid-cols-1 lg:grid-cols-3 gap-3">{automationRows.map((automation) => <div key={automation.id} className="rounded-2xl border border-[#eadcc2] bg-[#fffdf8] p-4"><p className="font-black text-[#2f2415] text-sm line-clamp-2">{automation.title}</p><p className="text-xs text-[#8a7456] mt-1 line-clamp-2">{automation.recommendation}</p><Btn onClick={() => openDraft(automation.draft, 'Centre décisionnel')} className="mt-3 w-full" small>Ouvrir</Btn></div>)}</div></section> : null}
  </div>;
}