/** Grilles responsive — module Finance & Pilotage */

export const FINANCE_STAT_GRID = 'grid min-w-0 grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4';
export const FINANCE_ACTION_GRID = 'grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4';
export const FINANCE_SECTION = 'rounded-3xl border border-[#d6c3a0] bg-white p-5 shadow-sm sm:p-6 min-w-0';
...
export function FinanceKpi({ label, value, tone = 'neutral', onClick }) {
...
export function FinanceSection({ title, subtitle, children, className = '' }) {
  return (
    <section className={`${FINANCE_SECTION} space-y-4 ${className}`}>
      {title ? (
...
export function FinanceActionCard({ title, text, onClick, icon: Icon }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
export function FinanceTodoRow({ title, detail, actionLabel, onAction, onOpen, busy }) {
  return (
