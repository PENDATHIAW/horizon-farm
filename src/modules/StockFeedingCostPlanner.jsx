...
export default function StockFeedingCostPlanner({ rows = [], animaux = [], lots = [], alimentationLogs = [], onOpenUseFood, simulateOnly = false }) {
  ...
  }, [alimentationLogs, selectedTarget?.id]);
  ...
  return <section className="rounded-3xl border border-[#d6c3a0] bg-white p-5 shadow-sm space-y-4">
    ...
    <div className={`${missingKg > 0 ? 'bg-amber-50 border-amber-200 text-amber-800' : 'bg-emerald-50 border-emerald-200 text-emerald-800'} border rounded-2xl p-4 text-sm flex flex-col md:flex-row md:items-start md:justify-between gap-3`}><div className="flex items-start gap-2"><Package size={18} className="mt-0.5" /><div><b>Action :</b> {missingKg > 0 ? `prévoir environ ${missingSacs.toFixed(1)} sac(s) supplémentaires.` : 'le stock couvre la période prévue.'}</div></div>{simulateOnly ? <span className="text-xs font-bold text-[#8a7456]">Simulation — distribution dans Élevage › Alimentation</span> : <button type="button" disabled={saving || !onOpenUseFood} onClick={applyPlan} className="shrink-0 rounded-xl bg-[#2f2415] text-white px-4 py-2 text-xs font-bold disabled:opacity-60"><CheckCircle2 size={14} className="inline" /> {saving ? 'Application...' : 'Appliquer au stock'}</button>}</div>

    <div className="rounded-2xl border border-[#eadcc2] bg-white p-4 space-y-3"><div className="flex flex-col md:flex-row md:items-center justify-between gap-2"><div><p className="flex items-center gap-2 font-black text-[#2f2415]"><History size={16} /> Historique alimentation cible</p><p className="text-xs text-[#8a7456]">Chaque application s’ajoute à l’historique, sans écraser les lignes précédentes.</p></div><div className="text-sm text-[#7d6a4a]"><b>{fmtCurrency(historyTotalCost)}</b> · {fmtNumber(historyTotalQty)} kg · {fmtCurrency(historyCostPerSubject)}/sujet</div></div>{targetHistory.length ? <div className="overflow-x-auto"><table className="min-w-full text-sm"><thead><tr className="border-b border-[#eadcc2] text-left text-xs uppercase text-[#8a7456]"><th className="py-2 pr-4">Date</th><th className="py-2 pr-4">Stock</th><th className="py-2 pr-4">Quantité</th><th className="py-2 pr-4">Coût</th><th className="py-2 pr-4">Notes</th></tr></thead><tbody>{targetHistory.slice(0, 8).map((log) => <tr key={log.id || `${log.date}-${log.stock_id}`} className="border-b border-[#f0e5d0]"><td className="py-3 pr-4">{log.date || '—'}</td><td className="py-3 pr-4">{log.produit || log.stock_id || '—'}</td><td className="py-3 pr-4">{fmtNumber(logQty(log))} {log.unite || 'kg'}</td><td className="py-3 pr-4 font-bold">{fmtCurrency(logCost(log))}</td><td className="py-3 pr-4 text-[#8a7456]">{log.notes || '—'}</td></tr>)}</tbody></table></div> : <div className="rounded-xl border border-[#eadcc2] bg-[#fffdf8] p-3 text-sm text-[#8a7456]">Aucune alimentation encore appliquée à cette cible.</div>}</div>
  </section>;
}
