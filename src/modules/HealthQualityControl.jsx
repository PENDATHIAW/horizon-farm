...
function Card({ icon: Icon, label, value, hint, danger = false }) {
  return <div className={`rounded-2xl border p-4 ${danger ? 'border-urgent bg-urgent-bg' : 'border-line bg-card'}`}><p className="flex items-center gap-2 text-xs uppercase tracking-normal text-slate"><Icon size={14} /> {label}</p><p className={`mt-2 text-xl font-semibold ${danger ? 'text-urgent' : 'text-earth'}`}>{value}</p>{hint ? <p className="mt-1 text-xs text-slate">{hint}</p> : null}</div>;
}
export default function HealthQualityControl({ rows = [], stocks = [], transactions = [], animaux = [], lots = [], onUpdate, onUpdateAnimal, onUpdateLot, onRefresh, onRefreshAnimals, onRefreshLots, onCreateFinanceTransaction, onRefreshFinances, onCreateDocument, onRefreshDocuments }) {
  ...
  const createHealthExpense = useCallback(async (item) => {
    const row = item.row;
  ...
  }, [onCreateDocument, onCreateFinanceTransaction, onRefresh, onRefreshDocuments, onRefreshFinances, onUpdate]);
  useEffect(() => {
  ...
  }, [audit.details, createHealthExpense, onCreateFinanceTransaction, onUpdate]);
  return <section className="rounded-3xl border border-line bg-white p-6 shadow-card space-y-4">
    <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3"><div><p className="flex items-center gap-2 text-lg font-semibold text-earth"><Stethoscope size={20} /> Contrôle santé</p><p className="mt-1 text-sm text-slate">Vérifie les soins, vaccins, coûts, produits utilisés, conséquences terrain et animaux ou lots concernés.</p></div><div className={`${audit.issueCount || priorityImpacts ? 'bg-vigilance-bg text-horizon-dark border-vigilance' : 'bg-positive-bg text-positive border-positive'} rounded-2xl border px-4 py-3 text-sm font-semibold`}>{audit.issueCount ? `${audit.issueCount} point(s) à regarder` : priorityImpacts ? `${priorityImpacts} conséquence(s) à suivre` : 'Santé bien suivie'}</div></div>
    ...
    {impacts.length ? <div className="rounded-2xl border border-line bg-card p-3 text-sm text-slate"><b className="text-earth">Conséquences dominantes :</b> {impacts.map(([name, count]) => `${name} (${fmtNumber(count)})`).join(' · ')}</div> : null}
    {audit.stockRisks.length ? <div className="rounded-2xl border border-urgent bg-urgent-bg p-3 text-sm text-urgent"><b>Stock santé faible :</b> {audit.stockRisks.map((s) => `${s.produit || s.nom || s.id} (${fmtNumber(s.quantite)} ${s.unite || ''})`).join(' · ')}</div> : null}
    {risky.length ? <div className="overflow-x-auto rounded-2xl border border-line"><table className="min-w-full text-sm"><thead><tr className="border-b border-line bg-card text-left text-xs uppercase text-slate"><th className="py-2 px-3">Intervention</th><th className="py-2 px-3">Concerné</th><th className="py-2 px-3">Date</th><th className="py-2 px-3">Coût</th><th className="py-2 px-3">Conséquence</th><th className="py-2 px-3">À regarder</th><th className="py-2 px-3">Action</th></tr></thead><tbody>{risky.map((item) => <tr key={item.row.id} className="border-b border-line"><td className="py-3 px-3 font-semibold text-earth">{label(item.row)}</td><td className="py-3 px-3">{readableTarget(item.row)}<p className="text-xs text-slate">{targetId(item.row) || 'à préciser'}</p></td><td className="py-3 px-3">{item.row.effectuee || item.row.prevue || item.row.date || '—'}</td><td className="py-3 px-3 font-semibold">{fmtCurrency(item.cost)}</td><td className="py-3 px-3"><span className={`rounded-full px-2 py-1 text-xs font-semibold ${impactPriority(item.row) ? 'bg-vigilance-bg text-horizon-dark' : 'bg-positive-bg text-positive'}`}>{impactLabel(item.row)}</span></td><td className="py-3 px-3"><div className="flex flex-wrap gap-1">{item.issues.map((issue) => <span key={issue} className="rounded-full bg-vigilance-bg px-2 py-1 text-xs font-semibold text-horizon-dark">{issueLabel(issue)}</span>)}</div></td><td className="py-3 px-3"><div className="flex flex-wrap gap-2">{item.issues.includes('Finance manquante') ? <span className="rounded-xl border border-positive bg-positive-bg px-3 py-2 text-xs font-semibold text-positive">Liaison automatique en cours</span> : null}{item.issues.includes('Cible encore malade') ? <button type="button" onClick={() => fixTargetStatus(item)} className="rounded-xl border border-line px-3 py-2 text-xs font-semibold text-earth"><CheckCircle2 size={14} className="inline" /> Mettre sous surveillance</button> : null}{!item.issues.includes('Finance manquante') && !item.issues.includes('Cible encore malade') ? '—' : null}</div></td></tr>)}</tbody></table></div> : <div className="rounded-2xl border border-positive bg-positive-bg p-3 text-sm text-positive"><CheckCircle2 size={16} className="inline" /> Rien d’urgent à corriger sur les soins et vaccins suivis.</div>}
  </section>;
}
