...
function DraftModal({ draft, setDraft, onClose, onExport, saving }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-3"><div className="w-full max-w-5xl max-h-[94vh] overflow-y-auto rounded-3xl border border-[#eadcc2] bg-white shadow-2xl"><div className="sticky top-0 z-10 flex items-start justify-between border-b border-[#eadcc2] bg-white p-5"><div><p className="text-xs uppercase tracking-widest text-[#8a7456]">Brouillon modifiable</p><h2 className="text-xl font-black text-[#2f2415]">Écris directement dans les sections</h2><p className="text-sm text-[#8a7456]">Chaque cadre ci-dessous est modifiable. Le PDF exporté reprendra exactement tes corrections. Si tu fermes sans exporter, les modifications du brouillon ne sont pas conservées.</p></div><button type="button" onClick={onClose} aria-label="Fermer"><X size={20} /></button></div><div className="mx-5 mt-5 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800"><b>Mode d’emploi :</b> remplace ou complète le texte dans les cadres, section par section, puis clique sur <b>Exporter le PDF avec mes modifications</b>.</div><div className="grid grid-cols-1 lg:grid-cols-2 gap-4 p-5">{Object.entries(labels).map(([key, label]) => <label key={key} className="space-y-1"><span className="text-xs font-black text-[#8a7456]">{label}</span><textarea rows={key === 'executive' ? 7 : 5} value={draft[key] || ''} onChange={(e) => setDraft((prev) => ({ ...prev, [key]: e.target.value }))} className="w-full rounded-xl border border-[#d6c3a0] bg-[#fffdf8] px-3 py-2 text-sm leading-relaxed" /></label>)}</div><div className="sticky bottom-0 flex justify-end gap-2 border-t border-[#eadcc2] bg-white p-4"><button type="button" onClick={onClose} className="rounded-xl border border-[#eadcc2] px-4 py-2 text-sm font-bold text-[#8a7456]">Fermer sans exporter</button><button type="button" disabled={saving} onClick={onExport} className="rounded-xl bg-[#2f2415] px-5 py-2 text-sm font-black text-white disabled:opacity-60"><Download size={15} className="inline" /> Exporter le PDF avec mes modifications</button></div></div></div>;
}
...
export default function FinancingDossierGenerator({ data = {}, onCreateDocument, onRefreshDocuments, onCreateBusinessEvent, onRefreshBusinessEvents }) {
  ...
  return <div className="space-y-4"><div className="rounded-2xl border border-[#eadcc2] bg-[#fffdf8] p-4"><p className="flex items-center gap-2 font-black text-[#2f2415]"><ShieldCheck size={18} /> Préparer un dossier banque / partenaire</p><p className="mt-1 text-sm text-[#8a7456]">Choisis le BP et le financeur, ouvre le brouillon, écris directement tes corrections dans les sections, puis exporte le PDF.</p></div><div className="grid grid-cols-1 md:grid-cols-3 gap-3"><label className="space-y-1"><span className="text-xs font-bold text-[#8a7456]">Business Plan</span><select value={planId || selectedPlan?.id || ''} onChange={(e) => setPlanId(e.target.value)} className="w-full min-h-[44px] rounded-xl border border-[#d6c3a0] bg-white px-3 py-2 text-sm">{plans.length ? plans.map((p) => <option key={p.id} value={p.id}>{p.nom || p.name || p.id}</option>) : <option value="">BP Horizon Farm officiel</option>}</select></label><label className="space-y-1"><span className="text-xs font-bold text-[#8a7456]">Financeur cible</span><select value={financeur} onChange={(e) => setFinanceur(e.target.value)} className="w-full min-h-[44px] rounded-xl border border-[#d6c3a0] bg-white px-3 py-2 text-sm">{Object.entries(FINANCEURS).map(([key, item]) => <option key={key} value={key}>{item.label}</option>)}</select></label>{financeur === 'AUTRE' ? <label className="space-y-1"><span className="text-xs font-bold text-[#8a7456]">Nom du financeur</span><input value={customFinanceur} onChange={(e) => setCustomFinanceur(e.target.value)} placeholder="Ex : banque, fonds, partenaire…" className="w-full min-h-[44px] rounded-xl border border-[#d6c3a0] bg-white px-3 py-2 text-sm" /></label> : <button type="button" onClick={openDraft} className="min-h-[44px] self-end rounded-xl bg-[#2f2415] px-4 py-2 text-sm font-black text-white"><Eye size={15} className="inline" /> Modifier le brouillon</button>}</div>{financeur === 'AUTRE' ? <div className="flex justify-end"><button type="button" onClick={openDraft} className="min-h-[44px] rounded-xl bg-[#2f2415] px-4 py-2 text-sm font-black text-white"><Eye size={15} className="inline" /> Modifier le brouillon</button></div> : null}<div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm"><Mini label="Investissement" value={fmtCurrency(summary.investment)} /><Mini label="CA annuel BP" value={fmtCurrency(summary.annualRevenue)} /><Mini label="Charges mensuelles" value={fmtCurrency(summary.monthlyCosts)} /><Mini label="Financeur" value={f.label} /></div>{draft ? <DraftModal draft={draft} setDraft={setDraft} onClose={() => setDraft(null)} onExport={exportDraft} saving={saving} /> : null}</div>;
}
