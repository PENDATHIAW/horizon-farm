...
const totalOfOrder = (order = {}) => numeric(order.montant_total ?? order.total ?? order.amount ?? order.total_amount);
const paidOfOrder = (order = {}) => numeric(order.montant_paye ?? order.paid_amount ?? order.amount_paid);
const paymentAmount = (payment = {}) => numeric(payment.montant_paye ?? payment.montant ?? payment.amount ?? payment.paid_amount);
...
function orderPayments(orderId, payments = []) { return asArray(payments).filter((payment) => String(paymentOrderId(payment) || '') === String(orderId || '') && !['annule', 'annulé', 'cancelled', 'rejete'].includes(normalize(payment.statut || payment.status))); }
...
function enrichPayload(moduleKey, text = '', payload = {}) {
  ...
  if (moduleKey === 'stock') { next.produit = next.produit || extractAfter(text, ['produit', 'stock']) || extractName(text, ['stock', 'quantite', 'quantité', 'unite']); next.categorie = next.categorie || extractAfter(text, ['categorie', 'catégorie']) || 'general'; next.quantite = next.quantite || extractNumber(text, ['quantite', 'quantité', 'qte']); next.unite = next.unite || extractAfter(text, ['unite', 'unité']) || 'unité'; next.prix_unitaire = next.prix_unitaire || extractNumber(text, ['prix', 'prix_unitaire']); }
  ...
  if (moduleKey === 'equipements') { next.name = next.name || next.nom || extractAfter(text, ['nom', 'equipement', 'équipement']) || extractName(text, ['equipement', 'équipement']); next.nom = next.nom || next.name; next.type = next.type || extractAfter(text, ['type']) || 'équipement'; next.status = next.status || (value.includes('panne') ? 'panne' : value.includes('maintenance') ? 'maintenance' : 'operationnel'); next.cout_reparation = next.cout_reparation || extractNumber(text, ['cout', 'coût', 'reparation', 'réparation']); }
  ...
  if (moduleKey === 'sante') { next.id = next.id || makeId('SAN'); next.nom = next.nom || extractAfter(text, ['nom', 'soin', 'intervention']) || extractName(text, ['soin', 'intervention']); next.type_intervention = next.type_intervention || (value.includes('vaccin') ? 'vaccination' : value.includes('urgence') ? 'urgence' : 'curatif'); next.target_mode = next.target_mode || 'global'; next.date = next.date || today(); next.cout = next.cout || extractNumber(text, ['cout', 'coût', 'montant']); next.severity = next.severity || (value.includes('urgence') || value.includes('critique') ? 'critique' : 'warning'); }
  if (moduleKey === 'ventes') { next.id = next.id || makeId('CMD'); next.client_id = next.client_id || extractAfter(text, ['client']); next.source_module = next.source_module || extractAfter(text, ['module', 'source']) || 'stock'; next.source_id = next.source_id || extractAfter(text, ['source_id', 'produit', 'animal', 'lot']); next.quantity = next.quantity || extractNumber(text, ['quantite', 'quantité', 'qte']) || 1; next.prix_unitaire = next.prix_unitaire || extractNumber(text, ['prix', 'prix_unitaire']); next.montant_total = next.montant_total || numeric(next.quantity) * numeric(next.prix_unitaire); next.product_name = next.product_name || next.produit || next.source_id || 'Vente'; }
  if (moduleKey === 'paiements') { next.order_id = next.order_id || extractAfter(text, ['commande', 'order', 'order_id']); next.client_id = next.client_id || extractAfter(text, ['client']); next.montant = next.montant || extractNumber(text, ['montant', 'payer', 'encaisser']); next.moyen_paiement = next.moyen_paiement || extractAfter(text, ['moyen', 'mode']) || 'cash'; next.date_paiement = next.date_paiement || today(); }
  return next;
}
