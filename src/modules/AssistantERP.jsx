...
export default function AssistantERP({ dataMap = {}, onNavigate }) {
  ...
  const createPaymentWorkflow = async (payload) => {
    ...
    const alreadyPaid = Math.max(paidOfOrder(order), orderPayments(order.id, cruds.paiements.rows).reduce((sum, payment) => sum + paymentAmount(payment), 0));
    const amount = numeric(payload.montant);
    const nextPaid = Math.min(total || alreadyPaid + amount, alreadyPaid + amount);
    ...
    const paymentId = payload.id || makeId('PAY');
    const txId = makeId('TRX');
    const payment = { id: paymentId, order_id: order.id, sale_id: order.id, source_record_id: order.id, client_id: payload.client_id || order.client_id || '', invoice_id: order.invoice_id || payload.invoice_id || '', date_paiement: payload.date_paiement || today(), date: payload.date_paiement || today(), montant: amount, montant_paye: amount, amount, moyen_paiement: payload.moyen_paiement || 'cash', mode_paiement: payload.moyen_paiement || 'cash', statut: 'paye', notes: payload.notes || 'Paiement créé depuis Assistant ERP', created_from: 'assistant_erp' };
    const finance = { id: txId, type: 'entree', libelle: `Encaissement ${order.product_name || order.libelle || order.id}`, montant: amount, date: payment.date, categorie: getFinanceCategoryFromSale(order), module_lie: 'ventes', related_id: order.id, activite: getFinanceActivityFromSale(order), client_id: order.client_id || payload.client_id || '', statut: 'paye', source_module: 'ventes', source_record_id: order.id, source_type: order.source_type || order.type_vente || order.product_type, source_id: order.source_id || order.product_id || order.entity_id, invoice_id: payment.invoice_id, payment_id: paymentId, moyen_paiement: payment.moyen_paiement, created_from: 'assistant_erp' };
    await cruds.paiements.create(payment);
    await cruds.finances.create(finance);
    await cruds.ventes.update(order.id, { montant_paye: nextPaid, reste_a_payer: remaining, statut_paiement: remaining <= 0 ? 'paye' : 'partiel', payment_status: remaining <= 0 ? 'paye' : 'partiel', statut_commande: 'confirme', order_status: 'confirme', last_payment_id: paymentId, last_payment_date: payment.date, last_transaction_id: txId });
    await Promise.allSettled([cruds.paiements.refresh?.(), cruds.finances.refresh?.(), cruds.ventes.refresh?.()]);
    return { label: `${order.id} · ${amount}`, route: 'ventes' };
  };
  ...
  const createDirect = async () => {
    ...
    try {
      ...
      else if (moduleKey === 'paiements') result = await createPaymentWorkflow(payload);
