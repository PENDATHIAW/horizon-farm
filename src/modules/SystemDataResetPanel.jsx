...
function collectSnapshot() {
  ...
  for (let i = 0; i < localStorage.length; i += 1) {
    ...
    if (!key) continue;
    if (isResettable(key)) rows.push({ key, value: localStorage.getItem(key) });
    if (isProtected(key)) protectedRows.push({ key });
  }
