...
function buildEntries({ transactions = [], salesOrders = [], payments = [] }) {
  ...
  arr(salesOrders).forEach((sale) => {
    ...
    if (paid > 0) entries.push({ id: `cash-${sale.id}`, source: `Vente ${sale.id}`, debit: 'Caisse / Banque', credit: 'Ventes', amount: paid, meaning: 'Encaissement client enregistré automatiquement.' });
    if (credit > 0) entries.push({ id: `receivable-${sale.id}`, source: `Vente ${sale.id}`, debit: 'Créances clients', credit: 'Ventes', amount: credit, meaning: 'Reste à payer suivi automatiquement comme créance.' });
  });
...
export default function AccountingAutoEntriesPreview({ transactions = [], salesOrders = [], payments = [] }) {
  ...
  return <section className="rounded-3xl border border-[#d6c3a0] bg-white p-5 shadow-sm space-y-4">
    <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-3"><div><p className="text-xs uppercase tracking-widest text-[#8a7456] font-black flex items-center gap-2"><BookOpenCheck size={15} /> Écritures générées automatiquement</p><h3 className="text-xl font-black text-[#2f2415] mt-1">Lecture débit / crédit simplifiée</h3><p className="text-sm text-[#8a7456] mt-1">Ces lignes sont déduites des ventes, paiements et dépenses. La saisie manuelle doit rester exceptionnelle.</p></div><div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-800">Débit = Crédit</div></div>
    <div className="grid grid-cols-2 md:grid-cols-4 gap-2"><Mini icon={Receipt} label="Écritures affichées" value={entries.length} /><Mini icon={CreditCard} label="Total débit" value={fmtCurrency(totalDebit)} /><Mini icon={Scale} label="Total crédit" value={fmtCurrency(totalCredit)} /><Mini icon={BookOpenCheck} label="Mode" value="Automatique" /></div>
    <div className="overflow-x-auto rounded-2xl border border-[#eadcc2] bg-[#fffdf8]"><table className="min-w-full text-sm"><thead className="bg-[#2f2415] text-white"><tr><th className="px-3 py-2 text-left">Source</th><th className="px-3 py-2 text-left">Débit</th><th className="px-3 py-2 text-left">Crédit</th><th className="px-3 py-2 text-right">Montant</th><th className="px-3 py-2 text-left">Sens métier</th></tr></thead><tbody>{entries.length ? entries.map((entry) => <tr key={entry.id} className="border-t border-[#eadcc2]"><td className="px-3 py-2 font-bold text-[#2f2415]">{entry.source}</td><td className="px-3 py-2 text-[#7d6a4a]">{entry.debit}</td><td className="px-3 py-2 text-[#7d6a4a]">{entry.credit}</td><td className="px-3 py-2 text-right font-black text-[#2f2415]">{fmtCurrency(entry.amount)}</td><td className="px-3 py-2 text-[#8a7456]">{entry.meaning}</td></tr>) : <tr><td colSpan="5" className="px-3 py-6 text-center text-[#8a7456]">Aucune écriture automatique à afficher pour le moment.</td></tr>}</tbody></table></div>
  </section>;
}
