...
function LayerHelpBanner() {
  return <div className="rounded-2xl border border-vigilance bg-vigilance-bg p-4 text-sm text-horizon-dark"><p className="flex items-center gap-2 font-semibold text-horizon-dark"><Info size={17} aria-hidden="true" /> Journal de ponte</p><p className="mt-2 leading-relaxed">La production d’œufs se saisit dans <b>Ramassage œufs / Journal de ponte</b>. Les tablettes sont calculées sur la base de <b>30 œufs = 1 tablette</b>.</p></div>;
}
function ActivityEntryCard({ icon: Icon, active, title, rows = [], productionLogs = [], action, onClick }) {
  ...
  return <button type="button" onClick={onClick} className={`rounded-3xl border p-6 text-left shadow-card transition-all ${active ? 'border-earth bg-earth text-white' : 'border-line bg-white hover:border-horizon hover:shadow-float'}`}>
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="flex items-start gap-3 min-w-0"><div className={`rounded-2xl p-3 ${active ? 'bg-white/15 text-horizon' : 'bg-vigilance-bg text-horizon-dark'}`}><Icon size={22} aria-hidden="true" /></div><div className="min-w-0"><p className={`text-xl font-semibold break-words ${active ? 'text-white' : 'text-earth'}`}>{title}</p></div></div><span className={`w-fit rounded-full px-3 py-1 text-xs font-semibold ${active ? 'bg-horizon text-earth' : 'bg-earth text-white'}`}>{action}</span></div>
    ...
    <div className={`mt-3 rounded-xl border p-3 text-xs leading-relaxed ${active ? 'border-white/15 bg-white/10 text-white/80' : urgent ? 'border-vigilance bg-vigilance-bg text-horizon-dark' : 'border-positive bg-positive-bg text-positive'}`}>{urgent ? `${urgent} action(s) prioritaire(s).` : 'Aucune urgence.'}</div>
  </button>;
}
function HeyHorizonAvicoleCard({ draft, rows, onUpdate, onCreateProduction, onCommitEggProduction, onRefreshProduction, onCreateBusinessEvent, onRefresh, onRefreshBusinessEvents, onClose, onCreateEggOpportunity }) {
  ...
  return <section className="rounded-3xl border border-line bg-white p-6 shadow-card space-y-4">
    <div className="flex items-start justify-between gap-3"><div><p className="text-xs uppercase tracking-normal text-slate font-semibold flex items-center gap-2"><CheckCircle2 size={15} /> Fiche avicole</p><h3 className="mt-1 text-xl font-semibold text-earth">{actionLabel}</h3></div><button type="button" onClick={onClose} className="rounded-full border border-line bg-white p-2 text-slate"><X size={16} /></button></div>
    <div className="grid grid-cols-1 md:grid-cols-3 gap-3"><label className="space-y-1"><span className="text-xs font-semibold text-slate">Lot</span><select value={lotId} onChange={(e) => setLotId(e.target.value)} className="w-full min-h-[44px] rounded-xl border border-line bg-white px-3 py-2 text-sm">{rows.map((item) => <option key={item.id} value={item.id}>{labelOf(item)} · {item.id} · {fmtNumber(currentOf(item))} actif(s)</option>)}</select></label><label className="space-y-1"><span className="text-xs font-semibold text-slate">{formType === 'egg_production' ? 'Œufs' : 'Quantité'}</span><input type="number" value={quantity} onChange={(e) => setQuantity(e.target.value)} className="w-full min-h-[44px] rounded-xl border border-line bg-white px-3 py-2 text-sm" /></label><label className="space-y-1"><span className="text-xs font-semibold text-slate">Date</span><input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full min-h-[44px] rounded-xl border border-line bg-white px-3 py-2 text-sm" /></label><label className="space-y-1 md:col-span-3"><span className="text-xs font-semibold text-slate">Note</span><input value={note} onChange={(e) => setNote(e.target.value)} className="w-full min-h-[44px] rounded-xl border border-line bg-white px-3 py-2 text-sm" /></label></div>
    <div className="rounded-xl border border-line bg-mist p-3 text-sm text-earth">{formType === 'egg_production' ? <>Tablettes : <b>{Math.floor(num(quantity) / 30)}</b></> : <>Effectif après action : <b>{fmtNumber(nextCount)}</b></>}</div>
