...
export default function VentesV4(props) {
  ...
  return <div className="space-y-5 ventes-mobile-structured">
    <div className="rounded-3xl border border-[#d6c3a0] bg-[#fffdf8] p-5 shadow-sm"><div className="flex flex-col md:flex-row md:items-center justify-between gap-4"><div><p className="text-xs uppercase tracking-widest text-[#8a7456] font-black">VENTES</p><h2 className="text-2xl font-black text-[#2f2415]">Enregistrer une vente</h2><p className="text-sm text-[#8a7456] mt-1">Produit, client, paiement, livraison et facture en une seule saisie.</p></div><button type="button" onClick={() => { setShowGuidedSale((v) => !v); setHorizonDraft(null); }} className="rounded-2xl bg-[#2f2415] px-5 py-3 text-sm font-black text-white"><CreditCard size={16} className="inline mr-1" /> Nouvelle vente</button></div></div>
    ...
    {showGuidedSale ? <Section icon={CreditCard} title="Nouvelle vente" subtitle="Saisir une vente complète."><VentesTerrainV3 {...props} /></Section> : null}
