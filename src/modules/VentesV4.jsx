...
function SaleActionModal({ sale, payments, props, onClose }) {
...
function SalesDesk({ props, payments }) {
  ...
  return <div className="space-y-4">
    {selected ? <SaleActionModal sale={selected} payments={payments} props={props} onClose={() => setSelected(null)} /> : null}
    ...
    <div className="space-y-2"><p className="text-sm font-black text-[#2f2415]">Ventes à traiter</p>{openSales.length ? <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">{openSales.map((sale) => { const remaining = remainingOf(sale, payments); const delivery = deliveryStatus(sale); return <article key={sale.id} className="rounded-2xl border border-[#d6c3a0] bg-white p-4 space-y-3"><div className="flex items-start justify-between gap-3"><div><p className="font-black text-[#2f2415]">{sale.product_name || sale.produit || sale.id}</p><p className="text-xs text-[#8a7456]">{sale.client_label || sale.client_name || 'Client'} · {sale.date || 'date non renseignée'}</p></div><div className="flex flex-wrap gap-1 justify-end">{statusBadge(remaining <= 0 ? 'Payé' : remaining < totalOf(sale) ? 'Partiel' : 'À encaisser', remaining <= 0 ? 'green' : 'amber')}{statusBadge(delivery === 'a_livrer' ? 'À livrer' : delivery === 'livre' ? 'Livré' : 'Récupéré', delivery === 'a_livrer' ? 'red' : 'green')}</div></div><div className="grid grid-cols-3 gap-2 text-sm"><div><span className="text-[#8a7456]">Total</span><b className="block text-[#2f2415]">{fmtCurrency(totalOf(sale))}</b></div><div><span className="text-[#8a7456]">Payé</span><b className="block text-[#2f2415]">{fmtCurrency(paidOf(sale, payments))}</b></div><div><span className="text-[#8a7456]">Reste</span><b className="block text-[#2f2415]">{fmtCurrency(remaining)}</b></div></div><div className="grid grid-cols-2 md:grid-cols-4 gap-2"><button type="button" onClick={() => setSelected(sale)} className="rounded-xl border border-[#d6c3a0] px-3 py-2 text-xs font-black text-[#7d6a4a]"><Edit3 size={13} className="inline" /> Modifier</button><button type="button" onClick={() => { setSelected(sale); }} className="rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-black text-emerald-700"><ReceiptText size={13} className="inline" /> Traiter</button><button type="button" onClick={() => setSelected(sale)} className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-black text-amber-700"><Truck size={13} className="inline" /> Livrer</button><button type="button" onClick={() => setSelected(sale)} className="rounded-xl border border-[#eadcc2] bg-[#fffdf8] px-3 py-2 text-xs font-black text-[#7d6a4a]"><FileText size={13} className="inline" /> Facture</button></div></article>; })}</div> : <div className="rounded-2xl border border-[#eadcc2] bg-[#fffdf8] p-4 text-sm text-[#8a7456]">Aucune vente ouverte.</div>}</div>
  </div>;
}
