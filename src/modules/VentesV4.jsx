...
import { isDelivered, isSaleClosed, linkedPaymentsForOrders, saleAmount, uniqueTodoCount } from './commercial/commercialMetrics.js';
...
function SaleTodoCard({ sale, linked, marginContext, embedded, needsPayHighlight, openSale, quickDeliver, deliveringId }) {
  const remaining = remainingForOrder(sale, linked);
  const total = saleAmount(sale);
  const margin = calculateSalesMargin(sale, marginContext);
  const needsPay = remaining > 0;
  const needsDelivery = !isDelivered(sale);
  return (
    <article key={sale.id} className={`rounded-2xl border bg-white p-4 space-y-3 ${needsPayHighlight && needsPay ? 'border-amber-300' : 'border-[#d6c3a0]'}`}>
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="font-black text-[#2f2415]">{sale.product_name || sale.produit || sale.id}</p>
          <p className="text-xs text-[#8a7456]">{sale.client_label || sale.client_name || 'Client'} · {sale.date || 'date non renseignée'}</p>
        </div>
        <div className="flex flex-wrap gap-1 justify-end">
          {needsPay ? statusBadge(remaining < total ? 'Partiel' : 'À encaisser', 'amber') : statusBadge('Payé', 'green')}
          {statusBadge(needsDelivery ? 'À livrer' : 'Livré', needsDelivery ? 'red' : 'green')}
        </div>
      </div>
      <div className={`grid gap-2 text-sm ${embedded ? 'grid-cols-2' : 'grid-cols-2 md:grid-cols-4'}`}>
        <div><span className="text-[#8a7456]">Total</span><b className="block text-[#2f2415]">{fmtCurrency(total)}</b></div>
        {needsPay ? <div><span className="text-[#8a7456]">Reste</span><b className="block text-amber-800">{fmtCurrency(remaining)}</b></div> : null}
        {!embedded ? <>
          <div className="hidden md:block"><span className="text-[#8a7456]">Coût</span><b className="block text-[#2f2415]">{margin.cout_a_completer ? 'À compléter' : fmtCurrency(margin.cout_revient)}</b></div>
          <div className="hidden md:block"><span className="text-[#8a7456]">Marge</span><b className={`block ${margin.cout_a_completer ? 'text-amber-700' : margin.marge_directe < 0 ? 'text-red-600' : 'text-emerald-700'}`}>{margin.cout_a_completer ? 'Non fiable' : fmtCurrency(margin.marge_directe)}</b></div>
        </> : null}
      </div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        <button type="button" onClick={() => openSale(sale, 'edit')} className="rounded-xl border border-[#d6c3a0] px-3 py-2 text-xs font-black text-[#7d6a4a]"><Edit3 size={13} className="inline" /> Modifier</button>
        {needsPay ? <button type="button" onClick={() => openSale(sale, 'pay')} className="rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-black text-emerald-700"><ReceiptText size={13} className="inline" /> Encaisser</button> : null}
        {needsDelivery ? <button type="button" onClick={() => quickDeliver(sale)} disabled={deliveringId === sale.id} className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-black text-amber-700"><Truck size={13} className="inline" /> Livrer</button> : null}
        <button type="button" onClick={() => openSale(sale, 'invoice')} className="rounded-xl border border-[#eadcc2] bg-[#fffdf8] px-3 py-2 text-xs font-black text-[#7d6a4a]"><FileText size={13} className="inline" /> Facture</button>
      </div>
    </article>
  );
}
...
function SalesDesk({ props, payments, onShowFollowup, embedded = false }) {
  ...
  const paymentQueue = useMemo(() => sales.filter((sale) => isOpenForPayment(sale, linked)), [sales, linked]);
  const fulfillmentQueue = useMemo(() => sales.filter((sale) => !isSaleClosed(sale, linked) && !isOpenForPayment(sale, linked)), [sales, linked]);
  const receivable = paymentQueue.reduce((sum, sale) => sum + remainingForOrder(sale, linked), 0);
  const toDeliver = fulfillmentQueue.filter((sale) => !isDelivered(sale)).length;
  const limit = embedded ? 12 : 6;
  const hasTodos = paymentQueue.length > 0 || fulfillmentQueue.length > 0;
  const cardProps = { linked, marginContext, embedded, openSale, quickDeliver, deliveringId };
  return <div className="space-y-4">
    ...
    {!embedded ? (
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="rounded-2xl border border-[#eadcc2] bg-[#fffdf8] p-4"><p className="text-xs text-[#8a7456]">Encaissements</p><b className="text-2xl text-[#2f2415]">{paymentQueue.length}</b></div>
    ...
    <div className="space-y-4">
      {!embedded ? <p className="text-sm font-black text-[#2f2415]">Actions rapides</p> : null}
      {!embedded ? <p className="text-xs text-[#8a7456]">Les encaissements soldés disparaissent de cette liste. Seules les livraisons ou factures restent visibles.</p> : null}
      {paymentQueue.length ? (
        <div className="space-y-2">
          {!embedded ? <p className="text-xs font-black uppercase tracking-wide text-amber-800">À encaisser ({paymentQueue.length})</p> : null}
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">
            {paymentQueue.slice(0, limit).map((sale) => <SaleTodoCard key={sale.id} sale={sale} {...cardProps} needsPayHighlight />)}
          </div>
        </div>
      ) : null}
      {fulfillmentQueue.length ? (
        <div className="space-y-2">
          {!embedded ? <p className="text-xs font-black uppercase tracking-wide text-[#7d6a4a]">Livraisons & factures ({fulfillmentQueue.length})</p> : null}
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">
            {fulfillmentQueue.slice(0, limit).map((sale) => <SaleTodoCard key={sale.id} sale={sale} {...cardProps} />)}
          </div>
        </div>
      ) : null}
      {!hasTodos ? <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800">Aucune vente à traiter — tout est à jour.</div> : null}
    </div>
  </div>;
}
...
export default function VentesV4(props) {
  ...
  const sales = props.rows || [];
  const openCount = uniqueTodoCount({ orders: sales, payments });
