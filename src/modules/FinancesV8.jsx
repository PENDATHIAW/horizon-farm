...
const ACTIVITY_GROUPS = {
  animaux: { label: 'Animaux', type: 'entree', modules: ['Vente bovins/ovins/caprins', 'Achat animal', 'Alimentation animaux', 'Santé animaux', 'Transport animaux'] },
...
const ACTIVITY_KEYS = Object.keys(ACTIVITY_GROUPS);
const ACTIVITY_OPTIONS = ACTIVITY_KEYS.map((key) => ({ value: key, label: ACTIVITY_GROUPS[key].label }));
const MODULE_OPTIONS = (activity) => (ACTIVITY_GROUPS[activity]?.modules || ['Autre']).map((label) => ({ value: label, label }));
const LABELS = Object.fromEntries(ACTIVITY_KEYS.map((key) => [key, ACTIVITY_GROUPS[key].label]));
const REVENUE_KEYS = ACTIVITY_KEYS.filter((key) => ACTIVITY_GROUPS[key].type === 'entree');

function labelForActivity(key) { return LABELS[key] || key || '-'; }
function activityKey(row = {}) {
  const explicit = String(row.categorie || row.category || '').toLowerCase();
  if (ACTIVITY_GROUPS[explicit]) return explicit;
...
function buildFinanceFields(businessPlans) {
  ...
  return (MODULE_FORM_FIELDS.finances || []).map((field) => {
    ...
    if (field.key === 'module_lie') return { ...field, label: 'Module / dépense liée', type: 'select', options: (form) => MODULE_OPTIONS(form.categorie) };
    if (field.key === 'business_plan_id' && bpOptions.length) return { ...field, type: 'select', options: [{ value: '', label: '— Aucun —' }, ...bpOptions] };
    return field;
  });
}
function deriveFinanceValues(next, changedKey, previous) {
  let out = { ...next };
  if (changedKey === 'categorie') {
    out.type = ACTIVITY_GROUPS[out.categorie]?.type || out.type;
    const valid = MODULE_OPTIONS(out.categorie).some((option) => option.value === out.module_lie);
    if (!valid) out.module_lie = '';
  }
...
function computeFinance({ rows, salesOrders, payments, fournisseurs, stocks, alimentationLogs }) {
  ...
  const activities = Object.fromEntries(ACTIVITY_KEYS.map((id) => [id, { id, label: labelForActivity(id), kind: ACTIVITY_GROUPS[id].type === 'entree' ? 'revenue' : 'cost', products: 0, paid: 0, receivables: 0, charges: 0, stockValue: 0 }]));
  tx.forEach((row) => {
  ...
  safeArray(alimentationLogs).forEach((log) => {
    ...
    if (cost <= 0) return;
    const text = `${log.type_cible || ''} ${log.cible_id || ''} ${log.lot_id || ''}`.toLowerCase();
    activities[text.includes('avicole') || text.includes('lot') ? 'avicole_chair' : 'animaux'].charges += cost;
  });
  ...
  const rowsOut = Object.values(activities).map((activity) => ({ ...activity, total: activity.products || activity.charges || activity.paid || activity.receivables || activity.stockValue, margin: activity.products - activity.charges, roi: activity.charges > 0 ? ((activity.products - activity.charges) / activity.charges) * 100 : activity.products > 0 ? 100 : 0 })).filter((activity) => activity.total).sort((a, b) => Math.abs(b.margin) - Math.abs(a.margin));
  return { activities: rowsOut, summary: { revenue, cashIn, expenses, paidExpenses, cash: cashIn - paidExpenses, receivables, debts, margin, marginRate: revenue > 0 ? (margin / revenue) * 100 : 0 } };
}
...
export default function FinancesV8({ rows = [], loading, onCreate, onUpdate, onDelete, onRefresh, stocks = [], fournisseurs = [], alimentationLogs = [], businessPlans = [], salesOrders = [], payments = [] }) {
  ...
  const financeFormFields = useMemo(() => buildFinanceFields(businessPlans), [businessPlans]);
  ...
  const columns = [
    ...
    { key: 'categorie', label: 'Activité', sortable: true, render: (row) => labelForActivity(activityKey(row)) },
  ...
  return <div className="space-y-6 pt-2">
    ...
    <div className="flex flex-wrap gap-2">{['tous', 'entree', 'sortie'].map((f) => <FilterButton key={f} active={typeFilter === f} onClick={() => setTypeFilter(f)}>{f === 'entree' ? 'Produits' : f === 'sortie' ? 'Charges' : 'Toutes'}</FilterButton>)}{['tous', 'paye', 'partiel', 'impaye', 'annule'].map((f) => <FilterButton key={f} active={statusFilter === f} onClick={() => setStatusFilter(f)}>{f}</FilterButton>)}{['tous', ...activities.map((a) => a.id)].map((f) => <FilterButton key={f} active={activityFilter === f} onClick={() => setActivityFilter(f)}>{f === 'tous' ? 'Toutes activités' : labelForActivity(f)}</FilterButton>)}</div>
    ...
    <CreateModal open={modal === 'create'} onClose={() => setModal(null)} onSubmit={(payload) => save(() => onCreate?.({ ...payload, statut: payload.statut || 'paye' }), 'Transaction ajoutée')} fields={financeFormFields} deriveValues={deriveFinanceValues} initialValues={{ id: generateSequentialId('finances', rows), type: 'entree', date: today(), statut: 'paye', paiement: 'Wave' }} autoId={() => generateSequentialId('finances', rows)} uploadFolder="finances" loading={saving} title="Ajouter produit / charge" submitLabel="Ajouter" />
    <EditModal open={modal === 'edit'} onClose={() => setModal(null)} onSubmit={(payload) => selected && save(() => onUpdate?.(selected.id, payload), 'Transaction modifiée')} fields={financeFormFields} deriveValues={deriveFinanceValues} initialValues={selected || {}} uploadFolder="finances" loading={saving} title="Modifier transaction" submitLabel="Enregistrer" />
