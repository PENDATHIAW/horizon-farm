...
export default function SmartFarm({
  ...
  const configuredSensors = sensors.length ? sensors : [
    { id: 'SIM-SENS-01', name: 'Simulation météo', type: 'temperature', zone: 'Station ferme', location: meteo?.locationLabel || 'Sénégal', status: 'simulation', battery_level: 100 },
  ];
  const configuredCameras = cameras.length ? cameras : [
    { id: 'SIM-CAM-01', name: 'Caméra simulation entrée', zone: 'entrée principale', type: 'simulation', status: 'simulation' },
  ];
  ...
  const verifySensors = async () => {
    ...
    try {
      ...
      toast.success(online ? 'Smart Farm synchronisé' : 'Mode hors ligne: données locales conservées');
    } finally {
      setChecking(false);
    }
  };
  ...
  const saveSensor = async (payload) => {
    try {
      ...
      toast.success(selectedSensor ? 'Capteur modifié' : 'Capteur ajouté');
      setModal(null);
    } catch (error) {
  ...
  const saveCamera = async (payload) => {
    try {
      ...
      toast.success(selectedCamera ? 'Caméra modifiée' : 'Caméra ajoutée');
      setModal(null);
    } catch (error) {
      toast.error(error.message || 'Erreur caméra');
    } finally {
      setSaving(false);
    }
  };
  ...
  const deleteCamera = async () => {
    ...
    try {
      ...
      toast.error(error.message || 'Erreur suppression caméra');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <SectionHeader
        title="Smart Farm"
        sub="Capteurs, caméras, météo et actions terrain"
        actions={
          <>
            ...
            <Btn icon={Camera} variant="outline" small onClick={() => { setSelectedCamera(null); setModal('createCamera'); }}>Ajouter caméra</Btn>
          </>
        }
      />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        <div className="bg-[#ffffff] border border-[#d6c3a0] rounded-2xl p-5">
          <p className="font-semibold text-[#2f2415] mb-4 flex items-center gap-2"><Radio size={16} className="text-emerald-400" />Mesures terrain</p>
        ...
        <div className="bg-[#ffffff] border border-[#d6c3a0] rounded-2xl p-5">
          <p className="font-semibold text-[#2f2415] mb-4 flex items-center gap-2"><Activity size={16} className="text-sky-400" />Zones suivies</p>
          <div className="flex flex-wrap gap-2">
            {zoneExamples.map((zone) => <span key={zone} className="rounded-full border border-[#eadcc2] bg-[#fffdf8] px-3 py-1 text-xs text-[#7d6a4a]">{zone}</span>)}
          </div>
          <div className="mt-4 rounded-xl bg-emerald-50 border border-emerald-200 p-3 text-sm text-emerald-700">Les alertes météo, capteurs et caméras peuvent créer une tâche terrain.</div>
        </div>

        <div className="bg-[#ffffff] border border-[#d6c3a0] rounded-2xl p-5">
          <p className="font-semibold text-[#2f2415] mb-4 flex items-center gap-2"><AlertTriangle size={16} className="text-amber-400" />Recommandations</p>
          <div className="space-y-2">
            {(meteo?.recommendations || ['Configurer un capteur pour remplacer progressivement la simulation.']).slice(0, 5).map((rec) => (
