  ...
  return (
    <section className="rounded-3xl border border-line bg-white p-6 shadow-card space-y-4">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        ...
        <button type="button" onClick={() => onNavigate?.('centre_ia')} className="min-h-[44px] rounded-xl bg-earth px-4 py-2 text-sm font-semibold text-white hover:bg-earth">Voir décisions</button>
      </div>
      ...
      <div className="rounded-2xl border border-line bg-card p-4 space-y-3">
        <div>
          <p className="text-xs uppercase tracking-normal text-slate font-semibold">Santé financière BP officiel</p>
          <p className="text-sm text-slate mt-1">Lecture rapide des données financières de l’onglet plan financier.</p>
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-2">
          <div className="rounded-xl border border-positive bg-positive-bg p-3 text-sm text-positive"><b>Résultat A1 :</b> {fmtCurrency(official.forecast.resultByYear[0])} · <b>CAF A1 :</b> {fmtCurrency(official.forecast.cashFlowCapacityByYear[0])}</div>
          <div className="rounded-xl border border-vigilance bg-vigilance-bg p-3 text-sm text-horizon-dark"><AlertTriangle size={14} className="inline" /> Charges poussins chair corrigées à 1 024 000 FCFA/mois selon ta stratégie validée.</div>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
        {lines.map((line) => (
          <div key={line.activity} className="rounded-2xl border border-line bg-card p-4">
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className="font-semibold text-earth">{line.label}</p>
                <p className="text-xs text-slate mt-1">Annuel : {fmtCurrency(line.annualRevenue)}</p>
              </div>
              <span className={`rounded-full px-2 py-1 text-xs font-semibold border ${Number(line.attainment || 0) >= 80 ? 'bg-positive-bg border-positive text-positive' : 'bg-vigilance-bg border-vigilance text-horizon-dark'}`}>{line.attainment || 0}%</span>
            </div>
            <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-slate">
              <div className="rounded-xl bg-white border border-line p-2"><b className="block text-earth">{fmtCurrency(line.target)}</b>Prévu mois</div>
              <div className="rounded-xl bg-white border border-line p-2"><b className="block text-earth">{fmtCurrency(line.actual)}</b>Réel mois</div>
            </div>
          </div>
        ))}
      </div>
...
function MiniValue({ label, value, sub }) {
  return <div className="rounded-xl border border-line bg-white p-3"><p className="text-xs text-slate">{label}</p><p className="font-semibold text-earth mt-1">{value}</p>{sub ? <p className="text-meta text-slate mt-1">{sub}</p> : null}</div>;
}
