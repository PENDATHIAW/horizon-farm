...
export default function CulturesIrrigationQuickForm({ rows = [], context, handlers, onSuccess }) {
  ...
  return (
    <section className="space-y-4 rounded-lg border border-[#d6c3a0] bg-white p-5 shadow-sm" data-testid="daily-irrigation-panel">
      <h3 className="flex items-center gap-2 text-lg font-black text-[#2f2415]"><Droplets size={20} /> {t('dailyEntries.irrigation.title')}</h3>
      <form onSubmit={submit} className="space-y-3" data-testid="daily-irrigation-form">
        ...
        <div className="flex justify-end"><button type="submit" disabled={busy} className="min-h-[42px] rounded-lg bg-[#2f2415] px-5 text-sm font-black text-white disabled:opacity-50" data-testid="daily-irrigation-submit">{busy ? t('dailyEntries.common.saving') : t('dailyEntries.common.save')}</button></div>
      </form>
    </section>
  );
}
