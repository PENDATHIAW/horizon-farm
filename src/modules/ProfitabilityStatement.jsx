...
import { autoEnrichCharges, auditChargeAssignment } from '../services/chargeAssignmentQualityService';
...
function Line({ label, value, strong = false, danger = false, muted = false }) {
  return <div className={`flex items-center justify-between gap-3 rounded-xl border px-4 py-3 ${strong ? 'border-earth bg-earth text-white' : danger ? 'border-vigilance bg-vigilance-bg text-horizon-dark' : muted ? 'border-line bg-card text-slate' : 'border-line bg-white text-earth'}`}><span className={strong ? 'font-semibold' : 'font-semibold'}>{label}</span><span className={strong ? 'font-semibold' : 'font-semibold'}>{value}</span></div>;
}
...
export default function ProfitabilityStatement({ transactions = [], salesOrders = [], payments = [], animaux = [], lots = [], cultures = [], stocks = [], sante = [], alimentationLogs = [], fournisseurs = [], investissements = [], equipements = [], businessEvents = [], compact = false }) {
  ...
  return <section className="rounded-3xl border border-line bg-white p-6 shadow-card space-y-4">
    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3"><div><p className="flex items-center gap-2 text-lg font-semibold text-earth"><Calculator size={20} /> {title}</p><p className="mt-1 text-sm text-slate">Les charges, pertes non-cash incluses, sont regroupées par activité. Le résultat d’exploitation est séparé du cash net et des prélèvements.</p></div><div className="flex flex-wrap gap-2"><div className={`${quality.reliability >= 80 ? 'bg-positive-bg text-positive border-positive' : 'bg-vigilance-bg text-horizon-dark border-vigilance'} rounded-2xl border px-4 py-3 text-sm font-semibold`}>Clarté {quality.reliability}%</div><div className={`${profit.operatingResult >= 0 ? 'bg-positive-bg text-positive border-positive' : 'bg-urgent-bg text-urgent border-urgent'} rounded-2xl border px-4 py-3 text-sm font-semibold`}>{profit.operatingResult >= 0 ? <TrendingUp size={15} className="inline" /> : <TrendingDown size={15} className="inline" />} Exploitation {fmtCurrency(profit.operatingResult)}</div></div></div>
    ...
    <div className="rounded-2xl border border-line bg-card p-3 text-sm text-slate">Cette section utilise maintenant les mêmes sources consolidées que la synthèse Finance : écritures Finance + animaux + avicole + cultures + santé + alimentation + stock + fournisseurs + investissements + équipements + événements métier.</div>
    {unresolvedCount || unallocatedCount ? <div className="rounded-2xl border border-vigilance bg-vigilance-bg p-3 text-sm text-horizon-dark flex gap-2"><AlertTriangle size={16} className="shrink-0 mt-1" /> {quality.summary.auto || 0} charge(s) ont été rangée(s). {unresolvedCount} charge(s) restent à vérifier, {unallocatedCount} charge(s) alimentation/santé doivent encore être mieux rattachées.</div> : <div className="rounded-2xl border border-positive bg-positive-bg p-3 text-sm text-positive flex gap-2"><CheckCircle2 size={16} className="shrink-0 mt-1" /> Les charges sont bien rangées. Le risque de compter deux fois une même dépense est limité.</div>}
  </section>;
}
