...
const monthLabel = (key) => {
  ...
  return ['Jan', 'Fév', 'Mar', 'Avr', 'Mai', 'Juin', 'Juil', 'Août', 'Sep', 'Oct', 'Nov', 'Déc'][Number(month || 1) - 1] || key;
};
const monthKeyOf = (date) => String(date || new Date().toISOString()).slice(0, 7);
const lastMonths = (count = 6) => {
  const base = new Date();
  return Array.from({ length: count }).map((_, index) => {
    const d = new Date(base.getFullYear(), base.getMonth() - (count - 1 - index), 1);
...
export default function Dashboard({
  ...
  const tasksLate = taches.filter((t) => t.status === 'retard').length;
  const tasksDone = taches.filter((t) => ['termine', 'terminé', 'done'].includes(String(t.status || t.statut || '').toLowerCase())).length;
  ...
  const taskRows = [{ label: 'Ouvertes', value: tasksOpen }, { label: 'Retard', value: tasksLate }, { label: 'Terminées', value: tasksDone }, { label: 'Alertes', value: alertesCritiques }];
  const activityRows = [{ label: 'Animaux', value: animaux.length }, { label: 'Lots', value: lotsData.length }, { label: 'Cultures', value: cultures.length }, { label: 'Clients', value: clients.length }, { label: 'Stock crit.', value: stocksCritiques }, { label: 'Opportunités', value: opportunitiesOpen }];

  return (
    <div className="space-y-4">
      <SectionHeader title="Dashboard" sub="Vue dirigeant: chiffres, alertes, graphiques et priorités" actions={<Btn icon={RefreshCw} variant="outline" small onClick={handleRefresh} disabled={refreshing}>{refreshing ? 'Actualisation...' : 'Actualiser'}</Btn>} />
      <div className="bg-[#2f2415] text-white border border-[#c9a96a]/40 rounded-3xl p-5 shadow-xl">
        <div className="flex flex-col gap-4 mb-5"><div><p className="text-xs uppercase tracking-[0.25em] text-[#c9a96a] font-bold">Dashboard dirigeant</p><h2 className="text-xl font-black mt-1">Essentiel de l’exploitation</h2></div><div className="flex flex-wrap gap-2">{quickActions.map((action) => <button key={action.module} type="button" onClick={() => onNavigate?.(action.module)} className="text-xs rounded-full bg-white/10 hover:bg-white/15 border border-white/10 px-3 py-1.5 text-[#f4e6c8]">{action.label}</button>)}</div></div>
