...
const buildFinanceChartData = (transactions = []) => {
  ...
  const months = Array.from({ length: 6 }, (_, index) => {
    ...
    return { key, mois: date.toLocaleDateString('fr-FR', { month: 'short' }), recettes: 0, depenses: 0 };
  });
...
export default function Dashboard({ lotsData = [], animaux = [], vaccins = [], stocks = [], clients = [], cultures = [], transactions = [], alimentationLogs = [], productionLogs = [], meteo, onRefresh }) {
  ...
  const valeurPotentielleOpportunites = animauxPretsVente.reduce((sum, animal) => sum + Number(animal.prix_vente_reel || animal.sale_price || animal.prix_vente_estime || 0), 0) + lotsPretsVente.reduce((sum, lot) => sum + lotMetrics(lot).grossRevenue, 0) + plateauxDisponibles * 2000 + kgCulturesDisponibles * 150;
  ...
  const handleRefresh = async () => {
    try { setRefreshing(true); await onRefresh?.(); toast.success('Dashboard actualise'); }
    catch (error) { toast.error(error.message || 'Actualisation impossible'); }
    finally { setRefreshing(false); }
  };

  return (
    <div className="space-y-6">
      <SectionHeader title="Tableau de bord" sub="Vue globale et temps reel de l'exploitation - Horizon Farm" actions={<Btn icon={RefreshCw} variant="outline" small onClick={handleRefresh} disabled={refreshing}>{refreshing ? 'Actualisation...' : 'Actualiser'}</Btn>} />
      ...
      <div><p className="text-xs font-semibold text-[#b39b78] uppercase tracking-widest mb-3">Production & Elevage</p><div className="grid grid-cols-2 lg:grid-cols-4 gap-4"><KpiCard icon={Beef} label="Animaux actifs" value={fmtNumber(animauxActifs)} sub="bovins + ovins + caprins" color="bg-amber-500/20 text-amber-400" /><KpiCard icon={Bird} label="Lots avicoles" value={lotsData.length} sub={`${fmtNumber(totalTetesLots)} tetes calculees`} color="bg-emerald-500/20 text-emerald-400" /><KpiCard icon={Activity} label="Production/jour (oeufs)" value={fmtNumber(productionOeufsJour)} sub="Depuis journal production" color="bg-sky-500/20 text-sky-400" trend={5} /><KpiCard icon={Weight} label="Cultures actives" value={cultures.filter((c) => !['termine', 'perdu'].includes(c.statut)).length} sub="maraichage" color="bg-purple-500/20 text-purple-400" trend={3} /></div></div>
      <div><p className="text-xs font-semibold text-[#b39b78] uppercase tracking-widest mb-3">Opportunites commerciales</p><div className="grid grid-cols-2 lg:grid-cols-5 gap-4"><KpiCard icon={Beef} label="Animaux prets" value={animauxPretsVente.length} sub="Opportunite, pas alerte" color="bg-emerald-500/20 text-emerald-400" /><KpiCard icon={Bird} label="Lots chair / reforme" value={lotsPretsVente.length} color="bg-sky-500/20 text-sky-400" /><KpiCard icon={Bird} label="Plateaux disponibles" value={fmtNumber(plateauxDisponibles)} color="bg-amber-500/20 text-amber-400" /><KpiCard icon={Weight} label="Cultures disponibles" value={`${fmtNumber(kgCulturesDisponibles)} kg`} color="bg-lime-500/20 text-lime-500" /><KpiCard icon={TrendingUp} label="Valeur potentielle" value={fmtCurrency(valeurPotentielleOpportunites)} sub={`${clientsARelancer.length} clients a relancer`} color="bg-purple-500/20 text-purple-400" /></div></div>
      <div><p className="text-xs font-semibold text-[#b39b78] uppercase tracking-widest mb-3">Sante & Stock</p><div className="grid grid-cols-2 lg:grid-cols-4 gap-4"><KpiCard icon={Heart} label="Animaux malades" value={malades} color="bg-red-500/20 text-red-400" /><KpiCard icon={Syringe} label="Vaccins en retard" value={vaccinsRetard} color="bg-red-500/20 text-red-400" /><KpiCard icon={AlertTriangle} label="Stocks critiques" value={stocksCritiques} color="bg-amber-500/20 text-amber-400" /><KpiCard icon={ShieldCheck} label="Taux mortalite moyen" value={`${mortalityRate.toFixed(1)}%`} sub="Objectif < 5%" color={mortalityRate < 5 ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'} /></div></div>
      <div><p className="text-xs font-semibold text-[#b39b78] uppercase tracking-widest mb-3">Alimentation calculee</p><div className="grid grid-cols-2 lg:grid-cols-5 gap-4"><KpiCard icon={Beef} label="Alim. bovins" value={fmtCurrency(feedingKpis.bovin)} color="bg-amber-500/20 text-amber-400" /><KpiCard icon={Beef} label="Alim. ovins" value={fmtCurrency(feedingKpis.ovin)} color="bg-lime-500/20 text-lime-500" /><KpiCard icon={Beef} label="Alim. caprins" value={fmtCurrency(feedingKpis.caprin)} color="bg-orange-500/20 text-orange-400" /><KpiCard icon={Bird} label="Alim. avicole" value={fmtCurrency(feedingKpis.avicole)} color="bg-sky-500/20 text-sky-400" /><KpiCard icon={Weight} label="Cout moyen/tete" value={fmtCurrency(feedingKpis.averagePerHead)} color="bg-emerald-500/20 text-emerald-400" /></div></div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4"><div className="lg:col-span-2 bg-[#ffffff] border border-[#d6c3a0] rounded-2xl p-5"><p className="font-semibold text-[#2f2415] mb-4">Evolution financiere 6 mois</p><ResponsiveContainer width="100%" height={220}><AreaChart data={financeChartData}><defs><linearGradient id="recG" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#22c55e" stopOpacity={0.3} /><stop offset="95%" stopColor="#22c55e" stopOpacity={0} /></linearGradient><linearGradient id="depG" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#ef4444" stopOpacity={0.3} /><stop offset="95%" stopColor="#ef4444" stopOpacity={0} /></linearGradient></defs><CartesianGrid strokeDasharray="3 3" stroke="#d6c3a0" /><XAxis dataKey="mois" stroke="#8a7456" fontSize={12} /><YAxis stroke="#8a7456" fontSize={10} tickFormatter={(v) => `${(v / 1000000).toFixed(1)}M`} /><Tooltip formatter={(v) => fmtCurrency(v)} contentStyle={{ background: '#ffffff', border: '1px solid #b6975f', borderRadius: 8 }} /><Area type="monotone" dataKey="recettes" stroke="#22c55e" fill="url(#recG)" strokeWidth={2} name="Recettes" /><Area type="monotone" dataKey="depenses" stroke="#ef4444" fill="url(#depG)" strokeWidth={2} name="Depenses" /></AreaChart></ResponsiveContainer></div>
      <div className="bg-[#ffffff] border border-[#d6c3a0] rounded-2xl p-5"><p className="font-semibold text-[#2f2415] mb-4">Alertes intelligentes</p><div className="space-y-3">{alertes.map((alert, i) => { const colors = { danger: 'border-red-500/30 bg-red-500/10 text-red-400', amber: 'border-amber-500/30 bg-amber-500/10 text-amber-400', info: 'border-sky-500/30 bg-sky-500/10 text-sky-400', success: 'border-emerald-500/30 bg-emerald-500/10 text-emerald-400' }; return <div key={`${alert.msg}-${i}`} className={`p-2.5 rounded-lg border text-xs ${colors[alert.type]}`}>{alert.msg}</div>; })}</div></div></div>
      <div className="bg-gradient-to-r from-[#ffffff] to-[#f4ebdb] border border-[#d6c3a0] rounded-2xl p-5"><p className="font-semibold text-[#2f2415] mb-4 flex items-center gap-2"><Zap size={16} className="text-amber-400" />Recommandations intelligentes</p><div className="grid grid-cols-1 md:grid-cols-2 gap-3">{recommandations.map((rec) => <div key={rec.msg} className="flex items-start gap-3 p-3 bg-[#fffdf8] rounded-xl border border-[#d6c3a0]"><span className="text-lg">{rec.icon}</span><p className="text-sm text-[#7d6a4a]">{rec.msg}</p></div>)}</div></div>
    </div>
  );
}
