...
export default function Dashboard({
  ...
  const cashDisponible = transactions.reduce((sum, t) => {
    ...
    return sum + (t.type === 'sortie' ? -amount : amount);
  }, 0);
  ...
  const totalTetesLots = lotsData.reduce((sum, lot) => sum + lotMetrics(lot).currentCount, 0);
  const alertesCount = malades + vaccinsRetard + stocksCritiques + culturesRisque + (cashDisponible < 0 ? 1 : 0) + (creancesClient > 0 ? 1 : 0);

  const topAlerts = [
    cashDisponible < 0 && { type: 'danger', title: 'Tresorerie negative', text: `${fmtCurrency(cashDisponible)} disponibles`, module: 'finances' },
    creancesClient > 0 && { type: 'amber', title: 'Creances clients', text: `${fmtCurrency(creancesClient)} a encaisser`, module: 'clients' },
    ...
    malades > 0 && { type: 'danger', title: 'Animaux malades', text: `${malades} animal(aux) a suivre`, module: 'animaux' },
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex flex-col gap-4 mb-5">
          ...
          <div className="flex flex-wrap gap-2">
            {quickActions.map((action) => (
              <button key={action.module} type="button" onClick={() => onNavigate?.(action.module)} className="text-xs rounded-full bg-white/10 hover:bg-white/15 border border-white/10 px-3 py-1.5 text-[#f4e6c8]">
                {action.label}
              </button>
            ))}
          </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        <div className="bg-gradient-to-r from-sky-900/80 via-sky-800/60 to-[#2f2415] border border-sky-700/30 rounded-3xl p-5 text-white shadow-xl xl:col-span-2">
          <div className="flex flex-col md:flex-row md:items-center gap-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 min-w-[260px]">
              ...
              <WeatherPill icon={Droplets} label="Hum." value={`${meteo?.humidite ?? '-'}%`} sub="air" />
              <WeatherPill icon={CloudRain} label="Pluie" value={meteo?.pluie ? 'Oui' : 'Non'} sub={`${meteo?.precipitationProbability ?? 0}%`} />
              <WeatherPill icon={Wind} label="Vent" value={`${meteo?.windSpeed ?? 0}`} sub="km/h" />
            </div>
          </div>
