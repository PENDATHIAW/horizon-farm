...
function marginScore(margin, revenue) {
  if (revenue <= 0) return 30;
  return clamp(50 + ((margin / revenue) * 100));
}
function cashScore(revenue, expenses, receivables) {
  if (expenses <= 0 && revenue > 0) return 80;
  const pressure = expenses + receivables;
  if (pressure <= 0) return 55;
  return clamp((revenue / pressure) * 60);
}
function demandScore(salesCount, revenue, unsold = 0, repeatClients = 0) {
  const base = salesCount > 0 ? 50 + Math.min(28, salesCount * 4) + Math.min(12, repeatClients * 3) : revenue > 0 ? 45 : 25;
  return clamp(base - Math.min(25, unsold * 4));
}
function capacityScore({ stockOk = true, tasksLate = 0, criticalStock = 0, activePeople = 0, equipmentsOk = 0, base = 62 }) {
  return clamp(base + Math.min(12, activePeople * 2) + Math.min(10, equipmentsOk * 2) - (stockOk ? 0 : 18) - (criticalStock * 7) - (tasksLate * 4));
}
function dataScore(points, expected) {
  if (!expected) return 30;
  return clamp((points / expected) * 100);
}
function rotationScore(cycleDays, revenue, margin) {
  if (!cycleDays) return revenue > 0 ? 55 : 35;
  const cyclesPerYear = 365 / Math.max(1, cycleDays);
  const monthlyMargin = (margin * cyclesPerYear) / 12;
  return clamp(35 + Math.min(35, cyclesPerYear * 5) + Math.min(30, monthlyMargin / 10000));
}
function riskControlScore(lossRate, alertCount = 0) {
  return clamp(95 - (lossRate * 4) - (alertCount * 10));
}
function recommendation(score, scores) {
  if (scores.data < 40) return { label: 'Tester petit volume', reason: 'Les données ne sont pas encore assez solides pour engager gros.' };
  if (scores.cash < 35) return { label: 'Reprendre avec financement sécurisé', reason: 'Le besoin de cash est trop fort pour augmenter sans filet.' };
  ...
  if (score >= 65) return { label: 'À reprendre avec contrôle', reason: 'Potentiel réel, avec quelques points à surveiller.' };
...
function buildPerspective({ id, title, module, icon, revenue, expenses, margin, cycleDays, lossRate, salesCount, unsold, stockOk, criticalStock, tasksLate, activePeople, equipmentsOk, repeatClients, dataPoints, expectedPoints, capacityBase, details = [], actions = [] }) {
  const scores = {
    ...
    demande: demandScore(salesCount, revenue, unsold, repeatClients),
    capacite: capacityScore({ stockOk, criticalStock, tasksLate, activePeople, equipmentsOk, base: capacityBase || 62 }),
...
function computeStrategic(props = {}) {
  const animalRows = arr(props.animaux);
  const lotRows = arr(props.lots);
  const prodRows = arr(props.productionLogs);
  const healthRows = arr(props.sante);
  const stockRows = arr(props.stocks);
  const txRows = arr(props.transactions);
  const orderRows = arr(props.salesOrders);
  const paymentRows = arr(props.payments);
  const alertRows = arr(props.alertes);
  const taskRows = arr(props.taches);
  const docRows = arr(props.documents);
  const eventRows = arr(props.businessEvents);
  const whatsappRows = arr(props.whatsappLogs);
  const realCultures = arr(props.cultures).filter((row) => !['parcelle', 'campagne', 'performance'].includes(lower(row.record_type || row.type_fiche)));
  const equipments = arr(props.equipements);
  ...
  const valuedStockRows = stockRows.filter((row) => stockUnitPrice(row) > 0);
  ...
  const doneTasks = taskRows.filter((row) => ['termine', 'terminé', 'traitee', 'traitée', 'done'].includes(statusOf(row)));
  ...
  const equipmentsOk = equipments.filter((row) => !['panne', 'hors_service'].includes(statusOf(row))).length;
  ...
  const recurringClients = clients.filter((client) => toNumber(client.totalAchats ?? client.totalachats) > 0).length;
  const reliableSuppliers = fournisseurs.filter((f) => toNumber(f.note) >= 4 || f.verified || f.favorite).length;
  const actionsAuto = alertRows.length + taskRows.length + docRows.length + whatsappRows.length + eventRows.length;
  const avoidedEntries = eventRows.reduce((sum, event) => sum + toNumber(event.saisies_evitees), 0) || Math.round(actionsAuto * 1.5);
  const proofScore = dataScore(docRows.length + txRows.length + orderRows.length + valuedStockRows.length + prodRows.length, Math.max(10, stockRows.length + orderRows.length + prodRows.length + 5));
  ...
  const perspectives = [
    buildPerspective({ id: 'cultures', title: tomateRows.length ? 'Reprendre tomates ?' : 'Reprendre une campagne cultures ?', module: 'cultures', icon: Sprout, revenue: cultureRevenue, expenses: cultureCost, margin: cultureRevenue - cultureCost, cycleDays: 90, lossRate: cultureLossRate, salesCount: cultureSales, unsold: 0, stockOk: criticalStocks.length === 0, criticalStock: criticalStocks.filter((r) => ['semence', 'engrais', 'phyto', 'traitement'].some((t) => lower(r.categorie || r.produit).includes(t))).length, tasksLate: lateTasks.length, activePeople, equipmentsOk, repeatClients: recurringClients, dataPoints: cultureScope.length + cultureSales + (cultureCost > 0 ? 1 : 0) + (cultureRevenue > 0 ? 1 : 0) + docRows.filter((d) => lower(d.module_source).includes('culture')).length, expectedPoints: 7, details: [`Marge estimée : ${money(cultureRevenue - cultureCost)}`, `Pertes récolte : ${fmtNumber(cultureLoss)} unité(s)`, `Ventes liées : ${cultureSales}`, `Stock intrants critique : ${criticalStocks.filter((r) => ['semence', 'engrais', 'phyto', 'traitement'].some((t) => lower(r.categorie || r.produit).includes(t))).length}`], actions: [{ label: 'Voir cultures', module: 'cultures' }, { label: 'Voir stock intrants', module: 'stock' }, { label: 'Voir ventes', module: 'ventes' }] }),
    buildPerspective({ id: 'chair', title: 'Relancer poulets de chair ?', module: 'avicole', icon: Bird, revenue: chairRevenue, expenses: chairCost, margin: chairRevenue - chairCost, cycleDays: 45, lossRate: chairLossRate, salesCount: orderRows.filter((r) => hasText(r, ['chair', 'poulet'])).length, unsold: 0, stockOk: criticalStocks.length === 0, criticalStock: criticalStocks.filter((r) => ['aliment', 'poussin'].some((t) => lower(r.categorie || r.produit).includes(t))).length, tasksLate: lateTasks.length, activePeople, equipmentsOk, repeatClients: recurringClients, dataPoints: chairLots.length + (chairCost > 0 ? 1 : 0) + (chairRevenue > 0 ? 1 : 0) + docRows.filter((d) => lower(d.module_source).includes('avicole')).length, expectedPoints: 6, details: [`Mortalité : ${fmtNumber(chairLoss)} sujet(s)`, `Marge estimée : ${money(chairRevenue - chairCost)}`, 'Cycle court : rotation rapide', `Fournisseurs fiables : ${reliableSuppliers}`], actions: [{ label: 'Voir lots', module: 'avicole' }, { label: 'Voir alimentation', module: 'stock' }, { label: 'Voir ventes', module: 'ventes' }] }),
    buildPerspective({ id: 'ponte', title: 'Augmenter les pondeuses ?', module: 'avicole', icon: Scale, revenue: eggRevenue, expenses: eggCost, margin: eggRevenue - eggCost, cycleDays: 30, lossRate: eggLossRate, salesCount: orderRows.filter((r) => hasText(r, ['oeuf', 'œuf', 'plateau'])).length, unsold: 0, stockOk: criticalStocks.length === 0, criticalStock: criticalStocks.filter((r) => lower(r.categorie || r.produit).includes('aliment')).length, tasksLate: lateTasks.length, activePeople, equipmentsOk, repeatClients: recurringClients, dataPoints: layerLots.length + prodRows.length + (eggRevenue > 0 ? 1 : 0) + (eggCost > 0 ? 1 : 0) + docRows.filter((d) => lower(d.module_source).includes('avicole')).length, expectedPoints: 7, details: [`Œufs produits : ${fmtNumber(eggs)}`, `Casses : ${fmtNumber(broken)} (${eggLossRate.toFixed(1)}%)`, `Marge estimée : ${money(eggRevenue - eggCost)}`], actions: [{ label: 'Voir ponte', module: 'avicole' }, { label: 'Voir ventes œufs', module: 'ventes' }, { label: 'Voir aliment', module: 'stock' }] }),
    buildPerspective({ id: 'animaux', title: 'Renforcer bétail / animaux ?', module: 'animaux', icon: HeartPulse, revenue: animalRevenue, expenses: animalCost, margin: animalRevenue - animalCost, cycleDays: 180, lossRate: animalRiskRate, salesCount: orderRows.filter((r) => hasText(r, ['animal', 'bovin', 'ovin', 'caprin'])).length, unsold: 0, stockOk: true, criticalStock: criticalStocks.filter((r) => lower(r.categorie || r.produit).includes('aliment')).length, tasksLate: lateTasks.length, activePeople, equipmentsOk, repeatClients: recurringClients, dataPoints: animalRows.length + (animalRevenue > 0 ? 1 : 0) + (animalCost > 0 ? 1 : 0) + healthRows.length + docRows.filter((d) => lower(d.module_source).includes('animaux')).length, expectedPoints: 8, details: [`Animaux suivis : ${fmtNumber(animalRows.length)}`, `Santé à risque : ${fmtNumber(sickAnimals.length)}`, `Marge estimée : ${money(animalRevenue - animalCost)}`], actions: [{ label: 'Voir animaux', module: 'animaux' }, { label: 'Voir santé', module: 'sante' }, { label: 'Voir ventes', module: 'ventes' }] }),
    buildPerspective({ id: 'stock_revente', title: 'Tester revente / distribution ?', module: 'stock', icon: Truck, revenue: stockSalesRevenue, expenses: stockCost * 0.35, margin: stockSalesRevenue - (stockCost * 0.35), cycleDays: 20, lossRate: criticalStocks.length * 3, salesCount: orderRows.filter((r) => hasText(r, ['stock', 'revente', 'produit'])).length, unsold: 0, stockOk: criticalStocks.length === 0, criticalStock: criticalStocks.length, tasksLate: lateTasks.length, activePeople, equipmentsOk, repeatClients: recurringClients, dataPoints: stockRows.length + (stockSalesRevenue > 0 ? 1 : 0) + valuedStockRows.length + reliableSuppliers, expectedPoints: 9, details: [`Stock valorisé : ${money(stockValue)}`, `Produits critiques : ${fmtNumber(criticalStocks.length)}`, `Fournisseurs fiables : ${fmtNumber(reliableSuppliers)}`], actions: [{ label: 'Voir stock', module: 'stock' }, { label: 'Voir fournisseurs', module: 'fournisseurs' }, { label: 'Voir ventes', module: 'ventes' }] }),
  ].filter((item) => item.dataPoints > 0 || item.revenue > 0 || item.expenses > 0);

  const bankability = clamp(proofScore * 0.35 + (docRows.length ? 20 : 0) + (stockValue > 0 ? 20 : 0) + (revenue > 0 ? 25 : 0));
  ...
  const erpAchievements = [
    { title: 'Décisions éclairées', value: fmtNumber(perspectives.length), detail: 'activités comparées avec scores', module: 'impact_business', icon: Target, tone: 'good' },
    { title: 'Valeur stock prouvée', value: money(stockValue), detail: `${fmtNumber(valuedStockRows.length)} produit(s) valorisé(s)`, module: 'stock', icon: Package, tone: stockValue > 0 ? 'good' : 'amber' },
    { title: 'Actions structurées', value: fmtNumber(actionsAuto), detail: `${fmtNumber(taskRows.length)} tâches · ${fmtNumber(alertRows.length)} alertes · ${fmtNumber(eventRows.length)} traces`, module: 'taches', icon: ClipboardCheck, tone: 'good' },
    { title: 'Saisies évitées', value: fmtNumber(avoidedEntries), detail: 'actions liées entre modules', module: 'audit', icon: CheckCircle2, tone: 'good' },
    { title: 'Dossier financement', value: `${bankability}/100`, detail: `${fmtNumber(docRows.length)} document(s), stock et revenus suivis`, module: 'rapports', icon: Building2, tone: bankability >= 60 ? 'good' : 'amber' },
    { title: 'Risques détectés', value: fmtNumber(openCriticalAlerts.length + criticalStocks.length + sickAnimals.length), detail: 'stocks, santé, alertes et retards visibles', module: 'alertes', icon: AlertTriangle, tone: openCriticalAlerts.length ? 'danger' : 'amber' },
  ];
  return { revenue, expenses, margin: revenue - expenses, receivables, stockValue, criticalStocks, openCriticalAlerts, lateTasks, sickAnimals, lateHealth, doneTasks, actionsAuto, avoidedEntries, proofScore, perspectives, erpAchievements, growth: { bankability, reinvestment, scalability, verticalization, proofScore } };
}
