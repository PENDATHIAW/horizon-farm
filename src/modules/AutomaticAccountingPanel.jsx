...
export default function AutomaticAccountingPanel({ transactions = [], documents = [], onNavigate }) {
  ...
  return <section className="rounded-3xl border border-[#d6c3a0] bg-white p-5 shadow-sm space-y-4">
    <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-3"><div><p className="text-xs uppercase tracking-widest text-[#8a7456] font-black flex items-center gap-2"><Scale size={15} /> Écritures automatiques</p><h3 className="text-xl font-black text-[#2f2415] mt-1">La comptabilité suit les actions métier</h3><p className="text-sm text-[#8a7456] mt-1">Ventes, paiements, créances, achats et dépenses alimentent automatiquement les mouvements. La saisie manuelle sert seulement aux régularisations exceptionnelles.</p></div><div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-800"><CheckCircle2 size={15} className="inline" /> Automatisation prioritaire</div></div>
    <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-2 text-sm"><Mini icon={Receipt} label="Ventes comptabilisées" value={k.salesEntries.length} hint={fmtCurrency(k.revenue)} /><Mini icon={FileText} label="Dépenses suivies" value={k.expenses.length} hint={fmtCurrency(k.charges)} /><Mini icon={ShieldCheck} label="Créances suivies" value={k.credits.length} danger={k.credits.length > 0} /><Mini icon={FileText} label="Preuves à joindre" value={k.proofMissing.length} danger={k.proofMissing.length > 0} /><Mini icon={Scale} label="Résultat simplifié" value={fmtCurrency(k.revenue - k.charges)} danger={k.revenue - k.charges < 0} /><Mini icon={CheckCircle2} label="Saisie manuelle" value="Exception" hint="correction uniquement" /></div>
    <div className="rounded-2xl border border-[#eadcc2] bg-[#fffdf8] p-4 text-sm text-[#7d6a4a]"><b className="text-[#2f2415]">Règle produit :</b> tu n’as pas besoin d’être comptable pour vendre ou dépenser. L’ERP crée les mouvements depuis les modules métier ; Comptabilité sert à contrôler les preuves, créances, dettes et régularisations.</div>
