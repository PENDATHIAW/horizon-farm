...
const animalName = (animal = {}) => animal.name || animal.nom || animal.tag || animal.boucle_numero || animal.id || 'Animal';
const animalCode = (animal = {}) => animal.boucle_numero || animal.qr_code || animal.tag || animal.id;
const healthStatus = (animal = {}) => clean(animal.health_status || animal.sante || animal.status_sante);
...
const amountOf = (row = {}) => toNumber(row.montant_total ?? row.cout_total ?? row.total_cost ?? row.cout ?? row.coût ?? row.amount ?? row.montant ?? row.cost ?? 0);

function matchAnimal(row = {}, animal = {}) {
  const id = String(animal.id || '');
  const code = String(animalCode(animal) || '');
  const directValues = [row.cible_id, row.animal_id, row.related_id, row.source_id, row.source_record_id, row.entity_id, row.target_id, row.boucle_numero, row.qr_code, row.tag].map((v) => String(v || ''));
  if (directValues.some((value) => value && (value === id || value === code))) return true;
  const text = clean(`${row.animal || ''} ${row.libelle || ''} ${row.title || ''} ${row.description || ''} ${row.notes || ''} ${row.nom || ''}`);
  return Boolean(id && text.includes(clean(id))) || Boolean(code && text.includes(clean(code)));
}

function feedingCostFor(animal, alimentationLogs = []) {
  return arr(alimentationLogs)
    .filter((log) => matchAnimal(log, animal))
    .reduce((sum, log) => sum + amountOf(log), 0);
}

function healthCostFor(animal, vaccins = []) {
  return arr(vaccins)
    .filter((row) => matchAnimal(row, animal))
...
export default function AnimalHealthBridge({ rows = [], alimentationLogs = [], vaccins = [], onUpdate, onRefresh }) {
  ...
  const activeRows = arr(rows).filter((animal) => !['vendu', 'mort', 'vole', 'volé', 'reforme', 'réforme'].includes(clean(animal.status || animal.statut)));
  ...
  const createHealthFollowUp = async (animal) => {
    try {
      ...
      await santeCrud.create?.({
        ...
        animal_id: animal.id,
        boucle_numero: animal.boucle_numero || animal.qr_code || animal.tag || '',
        target_scope: 'animal',
        target_count: 1,
        total_count: 1,
