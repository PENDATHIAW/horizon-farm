...
function Mini({ icon: Icon, label, value, danger = false }) {
  return <div className={`rounded-xl border px-3 py-2 ${danger ? 'border-vigilance bg-vigilance-bg' : 'border-line bg-white'}`}><Icon size={14} className={danger ? 'text-horizon-dark' : 'text-horizon-dark'} /><b className="block text-earth">{value}</b><span className="text-xs text-slate">{label}</span></div>;
}

function List({ title, rows = [], empty, render }) {
  return <div className="rounded-2xl border border-line bg-card p-4"><p className="font-semibold text-earth">{title}</p><div className="mt-3 space-y-2 text-sm">{rows.length ? rows.slice(0, 4).map((item, idx) => <div key={`${title}-${idx}`} className="rounded-xl border border-line bg-white px-3 py-2">{render(item)}</div>) : <div className="rounded-xl border border-positive bg-positive-bg px-3 py-2 text-positive">{empty}</div>}</div></div>;
}
...
export default function SalesWorkflowHealth(props) {
  ...
  if (!issueCount) return <section className="rounded-2xl border border-positive bg-positive-bg p-4 text-sm text-positive"><CheckCircle2 size={15} className="inline" /> Ventes récentes cohérentes. Les choix facture, paiement et livraison se font directement pendant la saisie de la vente.</section>;
  return <section className="rounded-3xl border border-line bg-white p-6 shadow-card space-y-4">
    <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-3"><div><p className="text-xs uppercase tracking-normal text-slate font-semibold flex items-center gap-2"><CheckCircle2 size={15} /> Anciennes ventes à régulariser</p><h3 className="text-xl font-semibold text-earth mt-1">Correction des ventes incomplètes</h3><p className="text-sm text-slate mt-1">Les nouvelles ventes guidées gèrent déjà client, paiement, facture et livraison. Cette zone sert seulement aux ventes importées, anciennes ou saisies avant le parcours guidé.</p></div><div className="rounded-2xl border border-vigilance bg-vigilance-bg p-3 text-sm text-horizon-dark"><AlertTriangle size={15} className="inline" /> {issueCount} ancien(s) point(s) à corriger</div></div>
    ...
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-3"><List title="Anciens crédits" rows={q.credits} empty="Aucun ancien crédit à relancer ici." render={({ order, remaining, due }) => <><b className="text-earth">{product(order)}</b><p className="text-xs text-slate">Reste {fmtCurrency(remaining)}{due ? ` · échéance ${due}` : ''}</p></>} /><List title="Ventes incomplètes" rows={q.incomplete} empty="Toutes les ventes ont les informations essentielles." render={({ order, missing }) => <><b className="text-earth">{order.id || product(order)}</b><p className="text-xs text-slate">À compléter : {missing.join(', ')}</p></>} /><List title="Factures / livraisons anciennes" rows={[...q.invoicesToSend, ...q.lateDeliveries]} empty="Aucune ancienne facture ou livraison à corriger." render={(order) => <><b className="text-earth">{product(order)}</b><p className="text-xs text-slate">Cas ancien/importé à régulariser.</p></>} /></div>
  </section>;
}