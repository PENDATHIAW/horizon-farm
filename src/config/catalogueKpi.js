...
export const CATALOGUE_KPI = Object.freeze({
  ...
  tresorerie: kpi({ code: 'tresorerie', libelle: 'Trésorerie', unite: 'FCFA', periodeParDefaut: 'à date', proprietaire: 'finance_pilotage', sourceLabel: 'kpiEngine.finance', sourceDisponible: disponible('finances', 'transactions', 'payments'), calcul: (moteur) => nombre(moteur?.finance?.resultatAllTime ?? moteur?.finance?.grossMargin) }),
  ...
  alertes_urgentes: kpi({ code: 'alertes_urgentes', libelle: 'Alertes urgentes', unite: 'alertes', proprietaire: 'activite_suivi', sourceLabel: 'alertes_center', sensFavorable: 'down', sourceDisponible: disponible('alertes_center', 'alertes'), calcul: (_moteur, data) => source(data, 'alertes_center', 'alertes').filter((row) => ouvert(row) && critique(row)).length }),
  taches_ouvertes: kpi({ code: 'taches_ouvertes', libelle: 'Tâches ouvertes', unite: 'tâches', proprietaire: 'activite_suivi', sourceLabel: 'taches', sensFavorable: 'down', sourceDisponible: disponible('taches', 'tasks'), calcul: (_moteur, data) => source(data, 'taches', 'tasks').filter(ouvert).length }),
