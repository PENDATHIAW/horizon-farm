...
export function composeDecisionDataMap({ crud = {}, dataMap = {}, liveMeteo = null } = {}) {
  return {
    ...
    bp_investment_lines: rows(crud.bp_investment_lines),
    bpInvestmentLines: rows(crud.bp_investment_lines),
    bp_recurring_costs: rows(crud.bp_recurring_costs),
    bpRecurringCosts: rows(crud.bp_recurring_costs),
    bp_funding_sources: rows(crud.bp_funding_sources),
    bpFundingSources: rows(crud.bp_funding_sources),
    ...
    vaccins: rows(crud.sante),
    price_catalog: rows(crud.price_catalog) || dataMap.price_catalog || [],
...
export function composeReportData(crud = {}) {
  return {
    ...
    audit_logs: rows(crud.audit_logs),
