...
function classifyAnimalSpeciesFromText(raw = '') { if (raw.includes('bovin') || raw.includes('boeuf') || raw.includes('vache') || raw.includes('taureau') || raw.includes('veau') || raw.includes('bov')) return 'bovins'; if (raw.includes('ovin') || raw.includes('mouton') || raw.includes('belier') || raw.includes('brebis') || raw.includes(' ov')) return 'ovins'; if (raw.includes('caprin') || raw.includes('chevre') || raw.includes('bouc') || raw.includes('cap')) return 'caprins'; return ''; }
function findAnimalSpeciesById(sourceId = '', dataMap = {}) { if (!sourceId) return ''; const clean = normalize(sourceId); const direct = classifyAnimalSpeciesFromText(clean); if (direct) return direct; const animal = arr(dataMap.animaux).find((row) => String(row.id) === String(sourceId) || String(row.tag) === String(sourceId)); if (!animal) return ''; return classifyAnimalSpeciesFromText(normalize(`${animal.type || ''} ${animal.espece || ''} ${animal.name || ''} ${animal.id || ''} ${animal.tag || ''}`)); }
...
function defaultEventsForYear(year) { return [{ id: `tabaski-${year}`, label: 'Tabaski', date: makeDate(year, 5, 27), activities: ['ovins', 'bovins', 'caprins'], note: 'Date indicative à remplacer par le calendrier officiel/local.' }, { id: `gamou-${year}`, label: 'Gamou', date: makeDate(year, 9, 5), activities: ['ovins', 'bovins', 'caprins', 'poulets_chair', 'oeufs'], note: 'Fenêtre indicative : à confirmer.' }, { id: `magal-${year}`, label: 'Magal', date: makeDate(year, 8, 20), activities: ['ovins', 'bovins', 'caprins', 'poulets_chair', 'oeufs'], note: 'Fenêtre indicative : à confirmer.' }, { id: `fin-annee-${year}`, label: 'Fin d’année', date: makeDate(year, 12, 24), activities: ['poulets_chair', 'oeufs', 'bovins', 'ovins', 'caprins'], note: 'Commandes groupées, restauration, familles.' }, { id: `ramadan-${year}`, label: 'Ramadan', date: makeDate(year, 2, 17), activities: ['poulets_chair', 'oeufs'], note: 'Date indicative.' }, { id: `korite-${year}`, label: 'Korité', date: makeDate(year, 3, 20), activities: ['poulets_chair', 'oeufs'], note: 'Date indicative.' }]; }
...
export function buildMarketEvents(referenceDate = new Date(), dataMap = {}) {
  return safeRun(() => {
    ...
    const defaults = [ref.getFullYear(), ref.getFullYear() + 1, ref.getFullYear() + 2].flatMap(defaultEventsForYear).map((event) => ({ ...event, source: 'default' }));
    const minDate = addDays(ref, -15); const maxDate = addDays(ref, 540);
    return [...customEvents, ...defaults].filter((event) => event.date >= minDate && event.date <= maxDate).sort((a, b) => a.date - b.date);
  }, []);
}

function buildTimingWindow({ activity, event, leadTime, referenceDate }) { const targetDate = safeDate(event.date); const latestStart = addDays(targetDate, -leadTime); const earliestStart = addDays(latestStart, -Math.max(14, Math.round(leadTime * 0.35))); const remainingDays = daysBetween(referenceDate, targetDate); const daysBeforeDeadline = daysBetween(referenceDate, latestStart); let status = 'in_time'; let label = 'Dans les temps'; let shouldRecommendInvestment = true; if (daysBeforeDeadline < 0) { status = 'too_late'; label = 'Trop tard pour cette fenêtre'; shouldRecommendInvestment = false; } else if (daysBeforeDeadline <= 7) { status = 'urgent_deadline'; label = 'Deadline très proche'; } else if (daysBeforeDeadline <= 21) { status = 'prepare_now'; label = 'À préparer maintenant'; } return { activity, eventId: event.id, eventLabel: event.label, eventNote: event.note || '', targetDate: iso(targetDate), earliestStart: iso(earliestStart), latestStart: iso(latestStart), leadTime, remainingDays, daysBeforeDeadline, status, statusLabel: label, shouldRecommendInvestment, tooLateReason: shouldRecommendInvestment ? '' : `La fenêtre ${event.label} est trop proche : ${remainingDays} jour(s) restants pour ${leadTime} jour(s) nécessaires.` }; }
function selectRollingWindow(activity, events, leadTime, referenceDate) { const windows = arr(events).filter((event) => arr(event.activities).includes(activity)).map((event) => buildTimingWindow({ activity, event, leadTime, referenceDate })); return windows.find((window) => window.status !== 'too_late') || windows.find((window) => window.status === 'too_late') || null; }
...
export function estimateLeadTimes(dataMap = {}) {
  return safeRun(() => {
    ...
    const lots = arr(dataMap.avicole || dataMap.lots); const animaux = arr(dataMap.animaux); const cultures = arr(dataMap.cultures);
  ...
  }, { oeufs: 150, poulets_chair: 42, animaux: 90, bovins: 90, ovins: 90, caprins: 90, cultures: 90 });
}
...
function buildRecommendation({ activity, title, priority, recommendation, timingWindow, coverageRow, capacity }) { if (!timingWindow) return null; const coverageDecision = buildCoverageDecision(activity, coverageRow, timingWindow); const timing = `${timingWindow.statusLabel} · cible ${timingWindow.eventLabel} (${timingWindow.targetDate}) · mise en place ${timingWindow.earliestStart} → ${timingWindow.latestStart}`; return { id: `${activity}-${timingWindow.eventId}`, title, activity, priority: priorityWithCoverage(timingWindow.shouldRecommendInvestment ? priority : 'basse', coverageDecision.priorityModifier), timing, recommendation: `${recommendation} ${coverageDecision.action}`, event_label: timingWindow.eventLabel, event_note: timingWindow.eventNote, target_date: timingWindow.targetDate, earliest_start: timingWindow.earliestStart, latest_start: timingWindow.latestStart, lead_time_days: timingWindow.leadTime, timing_status: timingWindow.status, timing_status_label: timingWindow.statusLabel, should_recommend_investment: timingWindow.shouldRecommendInvestment && coverageRow?.coverageStatus !== 'couvert', demand_level: coverageRow?.demandLevel || 'inconnue', demand_index: coverageRow?.demandIndex || 0, demand_units: coverageRow?.estimatedUnits || 0, demand_revenue: coverageRow?.revenueTarget || 0, available_units: coverageRow?.availableUnits || 0, available_revenue: coverageRow?.availableRevenue || 0, coverage_rate: coverageRow?.coverageRate || 0, coverage_status: coverageRow?.coverageStatus || 'inconnu', gap_units: coverageRow?.gapUnits || 0, gap_revenue: coverageRow?.gapRevenue || 0, capacity }; }
...
function buildTechnicalRecommendation(alert = {}) { const activity = activityFromTechnicalAlert(alert); return { id: `technical-${alert.id || alert.decision_key || Math.random().toString(36).slice(2)}`, title: alert.title || 'Alerte technique', activity, priority: priorityFromSeverity(alert.severity), timing: alert.message || alert.description || '', recommendation: alert.action_recommandee || alert.recommendation || 'Vérifier et traiter cette alerte.', event_label: 'Technique', event_note: alert.message || '', target_date: alert.due_date || alert.event_date || '', earliest_start: '', latest_start: '', lead_time_days: 0, timing_status: 'technical_alert', timing_status_label: 'Alerte technique', should_recommend_investment: false, demand_level: 'technique', demand_index: 0, demand_units: 0, demand_revenue: 0, available_units: 0, available_revenue: 0, coverage_rate: 0, coverage_status: 'technique', gap_units: 0, gap_revenue: 0, capacity: {}, technical_alert: alert }; }
...
export function buildDecisionCenterPlan(dataMap = {}, options = {}) {
  return safeRun(() => {
    ...
    const leadTimes = estimateLeadTimes(dataMap) || { oeufs: 150, poulets_chair: 42, animaux: 90, bovins: 90, ovins: 90, caprins: 90, cultures: 90 };
  ...
  }, { goals: fallbackGoals(options), recommendations: [], leadTimes: { oeufs: 150, poulets_chair: 42, animaux: 90, bovins: 90, ovins: 90, caprins: 90, cultures: 90 }, capacity: { activeLayers: 0, eggsDay: 0, tabletsDay: 0, layingRate: 0 }, calendar: buildCommercialCalendar(options.date || new Date()), generated_at: new Date().toISOString() });
}
