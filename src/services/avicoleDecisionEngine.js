...
function activeCount(lot = {}) { return avicoleActiveCount(lot); }
function initialCount(lot = {}) { return avicoleInitialCount(lot); }
...
function mortalityRateOf(lot = {}) { const initial = initialCount(lot); return initial > 0 ? Math.round((num(lot.mortality) / initial) * 100) : 0; }
...
function isCommerciallyClosedLot(lot = {}) {
  ...
  const closedKeywords = ['vendu', 'abattu', 'reforme', 'réforme', 'cloture', 'clôture', 'termine', 'terminé', 'sorti', 'perdu', 'mort'];
  return closedKeywords.some((word) => status.includes(norm(word))) || (active <= 0 && initialCount(lot) > 0);
}
function isLotClosedByLoss(lot = {}) { const status = lotStatus(lot); return ['perdu', 'perdu_mortalite', 'cloture_perte'].some((s) => status.includes(s)) || (activeCount(lot) <= 0 && initialCount(lot) > 0 && status.includes('perte')); }
function closedDecisionFor(lot = {}, type = 'chair') {
  ...
  if (status.includes('reforme') || status.includes('réforme')) return 'Lot réformé : conserver l’historique, suivre les ventes/réformes, ne plus planifier de suivi actif.';
  ...
  if (activeCount(lot) <= 0 && initialCount(lot) > 0) return type === 'pondeuse' ? 'Lot sans effectif actif : conserver l’historique, vérifier la cause de sortie et désactiver le suivi.' : 'Lot sans effectif actif : conserver l’historique, vérifier la cause de sortie et désactiver les rappels de pesée.';
...
function definedNumber(value, fallback) { return value === undefined || value === null || value === '' ? fallback : num(value); }
