...
export function prepareBiosecurityWorkflow(payload = {}, context = {}) {
  ...
  return {
    ...
    records: {
      alert: {
        id: alertId,
        title: payload.title || 'Risque biosécurité à traiter',
        message: payload.message || payload.description || 'Action biosécurité recommandée par Horizon Farm.',
        module_source: payload.module_source || 'biosécurité',
        entity_type: payload.entity_type || 'sanitary_event',
        entity_id: entityId,
        severity: risk === 'urgence' || risk === 'critique' ? risk : 'warning',
        status: 'nouvelle',
        action_recommandee: payload.action_recommandee || 'Appliquer le protocole biosécurité et documenter le résultat.',
        workflow_id: null,
      },
      task: {
        id: taskId,
        title: payload.task_title || `Biosécurité — ${eventType}`,
        module_lie: payload.module_source || 'biosécurité',
        related_id: entityId,
        ...
        source_module: 'biosécurité',
        source_record_id: entityId,
        linked_alert_id: alertId,
      },
      stock_movement: stockQty > 0 && payload.stock_id ? {
      ...
      document: documentId ? {
        id: documentId,
        title: payload.document_title || `Preuve biosécurité ${entityId || ''}`.trim(),
        document_category: 'sanitaire',
        module_source: 'biosécurité',
        entity_type: payload.entity_type || 'sanitary_event',
        entity_id: entityId,
      ...
      trace: {
        ...
        entity_type: payload.entity_type || 'sanitary_event',
        entity_id: entityId,
        title: payload.title || 'Action biosécurité générée',
        description: payload.message || payload.description || 'Workflow biosécurité préparé.',
        event_date: today(),
        severity: risk === 'urgence' || risk === 'critique' ? risk : 'warning',
