...
const paymentStatus = (amount, paidAmount) => {
  const total = toNumber(amount);
  const paid = toNumber(paidAmount);
  if (total <= 0) return 'paye';
  if (paid <= 0) return 'non_paye';
  if (paid < total) return 'partiel';
  return 'paye';
};

const invoiceStatus = (amount, paidAmount) => ({
  non_paye: 'non_payee',
  partiel: 'partielle',
  paye: 'payee',
}[paymentStatus(amount, paidAmount)] || 'non_payee');

const getSourceId = (order = {}) =>
  order.source_id || order.animal_id || order.lot_id || order.culture_id || order.stock_id || order.related_id || '';

const getStockQuantitySold = (order = {}) =>
  toNumber(order.quantite ?? order.quantity ?? order.qty ?? order.nombre ?? order.units_sold ?? 0);

const findById = (rows, id) => arr(rows).find((row) => String(row.id) === String(id));

const buildSourceAssetPatch = ({ order, context, activity, amount, transactionId }) => {
  const sourceId = getSourceId(order);
  ...
  if (activity === 'animaux') {
    return {
      ...
      patch: {
        status: 'vendu',
        date_vente: order.date_vente || today(),
        client_id: order.client_id || '',
        prix_vente_reel: amount,
        sale_price: amount,
  ...
  if (activity === 'avicole') {
    return {
      id: sourceId,
      patch: {
        status: order.statut_lot_apres_vente || order.lot_status_after_sale || 'vendu',
        date_fin_reelle: order.date_vente || today(),
        prix_vente_reel: amount,
        client_id: order.client_id || '',
        linked_transaction_id: transactionId,
      },
  ...
  if (activity === 'cultures') {
    return {
      id: sourceId,
      patch: {
        statut: order.statut_culture_apres_vente || order.culture_status_after_sale || 'vendu',
        date_vente: order.date_vente || today(),
        prix_vente_reel: amount,
        client_id: order.client_id || '',
        linked_transaction_id: transactionId,
  ...
  if (activity === 'stock') {
    ...
    const quantitySold = getStockQuantitySold(order);
    if (!stock || quantitySold <= 0) return null;
    return {
      id: sourceId,
      patch: {
        quantite: Math.max(0, toNumber(stock.quantite) - quantitySold),
        derniere_sortie: today(),
...
const updateSourceAsset = async ({ activity, sourcePatch, handlers }) => {
  if (!sourcePatch?.id) return;
  if (activity === 'animaux') await handlers.onUpdateAnimal?.(sourcePatch.id, sourcePatch.patch);
  if (activity === 'avicole') await handlers.onUpdateLot?.(sourcePatch.id, sourcePatch.patch);
  if (activity === 'cultures') await handlers.onUpdateCulture?.(sourcePatch.id, sourcePatch.patch);
  if (activity === 'stock') await handlers.onUpdateStock?.(sourcePatch.id, sourcePatch.patch);
};
...
export function prepareSaleWorkflow(payload = {}, context = {}) {
  ...
  const sourcePatch = buildSourceAssetPatch({ order, context, activity, amount, transactionId });
  const actions = [
    ...
    sourcePatch ? { id: 'update_source_asset', module: activity, type: 'update', label: 'Mettre à jour stock/animal/lot/culture' } : null,
    { id: 'create_trace', module: 'tracabilite', type: 'create', label: 'Créer événement traçabilité' },
    due > 0 ? { id: 'create_alert', module: 'alertes', type: 'create', label: 'Créer alerte créance si solde restant' } : null,
  ].filter(Boolean);
  return {
    workflow_type: 'sale',
    ...
    fields: {
      ...
      paid: fieldAuto({ value: due, source: 'vente.solde_a_encaisser' }),
      already_paid: fieldAuto({ value: alreadyPaid, source: 'vente.montant_deja_paye' }),
      due: fieldAuto({ value: due, source: 'amount - already_paid' }),
      activity: fieldAuto({ value: activity, source: 'vente.source_type' }),
    },
    records: {
      order_patch: { statut_commande: order.statut_commande === 'brouillon' ? 'confirme' : (order.statut_commande || 'confirme'), statut_paiement: paymentStatus(amount, alreadyPaid), montant_paye: alreadyPaid, invoice_id: invoiceId, payment_id: invoiceId ? paymentId : null, transaction_id: transactionId, workflow_id: null, secured_at: now() },
      invoice: { id: invoiceId, order_id: order.id, client_id: order.client_id || '', date: today(), total_amount: amount, montant_total: amount, status: invoiceStatus(amount, alreadyPaid), source_module: 'ventes', source_record_id: order.id },
      payment: { id: paymentId, order_id: order.id, invoice_id: invoiceId, client_id: order.client_id || '', date: today(), montant: due, amount: due, statut: 'paye', moyen_paiement: order.moyen_paiement || '', source_module: 'ventes', source_record_id: order.id },
      ...
      client_patch: client ? { dernier_achat: today(), total_achats: toNumber(client.total_achats) + amount, creances: Math.max(0, toNumber(client.creances) + amount - alreadyPaid) } : null,
      source_asset_patch: sourcePatch,
      ...
      alert: { id: safeId('ALE', context.alerts), title: 'Créance client à suivre', message: `${order.product_name || order.id}: solde restant à encaisser`, module_source: 'ventes', entity_id: order.id, severity: 'warning', status: 'nouvelle' },
    },
    actions,
  };
}
...
export async function commitSaleWorkflow(preview, handlers = {}) {
  ...
  const paymentAmount = Math.max(0, toNumber(finalValue(p.fields.paid)));
  const alreadyPaid = toNumber(finalValue(p.fields.already_paid));
  const paidAmount = Math.min(amount, alreadyPaid + paymentAmount);
  const remainingAmount = Math.max(0, amount - paidAmount);
  const activity = finalValue(p.fields.activity);
  const records = p.records;
  const statutPaiement = paymentStatus(amount, paidAmount);
  records.order_patch = { ...records.order_patch, statut_paiement: statutPaiement, montant_paye: paidAmount, workflow_id: p.workflow_id };
  records.invoice = { ...records.invoice, total_amount: amount, montant_total: amount, status: invoiceStatus(amount, paidAmount) };
  records.payment = { ...records.payment, montant: paymentAmount, amount: paymentAmount };
  records.finance = { ...records.finance, montant: paymentAmount, activite: activity };
  records.client_patch = records.client_patch ? { ...records.client_patch, creances: remainingAmount } : null;
  records.alert = records.alert ? { ...records.alert, message: `${p.source_order?.product_name || p.source_order?.id}: ${remainingAmount}`, remaining_amount: remainingAmount } : null;

  await handlers.onCreateInvoice?.(records.invoice);
  if (paymentAmount > 0) await handlers.onCreatePayment?.(records.payment);
  if (paymentAmount > 0) await handlers.onCreateFinanceTransaction?.(records.finance);
  ...
  await updateSourceAsset({ activity, sourcePatch: records.source_asset_patch, handlers });
  await handlers.onCreateDocument?.(records.document);
  await handlers.onCreateBusinessEvent?.(records.trace);
  if (remainingAmount > 0 && records.alert) await handlers.onCreateAlert?.(records.alert);
